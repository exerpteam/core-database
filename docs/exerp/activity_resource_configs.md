...
| `booking_resource_group_id` | Identifier of the related booking resource group record. | `int4` | Yes | No | - | [booking_resource_groups](booking_resource_groups.md) via (`booking_resource_group_id` -> `id`) | `1001` |
| `parent_activity_key` | Foreign key field linking this record to `activity`. | `int4` | Yes | No | [activity](activity.md) via (`parent_activity_key` -> `id`) | - | `42` |
| `resource_group_selection` | Numeric field used for identifiers, counters, or coded values. | `int4` | No | No | - | - | `42` |
