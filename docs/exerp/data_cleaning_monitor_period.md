...
| `one_shot` | Boolean flag used in business rules and filtering logic. | `bool` | No | No | - | - |
...
| `monitoring_start_time` | Epoch timestamp for monitoring start. | `int8` | No | No | - | - |
| `monitoring_stop_time` | Epoch timestamp for monitoring stop. | `int8` | Yes | No | - | - |
| `start_data_clean_out_line_id` | Foreign key field linking this record to `data_cleaning_out_line`. | `int4` | Yes | No | [data_cleaning_out_line](data_cleaning_out_line.md) via (`start_data_clean_out_line_id` -> `id`) | - |
| `start_data_clean_in_line_id` | Foreign key field linking this record to `data_cleaning_in_line`. | `int4` | Yes | No | [data_cleaning_in_line](data_cleaning_in_line.md) via (`start_data_clean_in_line_id` -> `id`) | - |
| `stop_data_clean_out_line_id` | Foreign key field linking this record to `data_cleaning_out_line`. | `int4` | Yes | No | [data_cleaning_out_line](data_cleaning_out_line.md) via (`stop_data_clean_out_line_id` -> `id`) | - |
| `stop_data_clean_in_line_id` | Foreign key field linking this record to `data_cleaning_in_line`. | `int4` | Yes | No | [data_cleaning_in_line](data_cleaning_in_line.md) via (`stop_data_clean_in_line_id` -> `id`) | - |
