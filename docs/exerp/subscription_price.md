...
| `to_date` | Date for to. | `DATE` | Yes | No | - | - |
...
| `binding` | Boolean flag used in business rules and filtering logic. | `bool` | No | No | - | - |
...
| `notified` | Boolean flag used in business rules and filtering logic. | `bool` | No | No | - | - |
...
| `applied` | Boolean flag used in business rules and filtering logic. | `bool` | No | No | - | - |
| `cancelled` | Boolean flag used in business rules and filtering logic. | `bool` | No | No | - | - |
| `approved` | Boolean flag used in business rules and filtering logic. | `bool` | No | No | - | - |
| `pending` | Boolean flag used in business rules and filtering logic. | `bool` | No | No | - | - |
| `approved_employee_center` | Center part of the reference to related approved employee data. | `int4` | Yes | No | - | - |
| `approved_employee_id` | Identifier of the related approved employee record. | `int4` | Yes | No | - | - |
| `approved_entry_time` | Epoch timestamp for approved entry. | `int8` | Yes | No | - | - |
| `cancelled_employee_center` | Center part of the reference to related cancelled employee data. | `int4` | Yes | No | - | - |
| `cancelled_employee_id` | Identifier of the related cancelled employee record. | `int4` | Yes | No | - | - |
| `cancelled_entry_time` | Epoch timestamp for cancelled entry. | `int8` | Yes | No | - | - |
| `aggregated_change_date` | Date for aggregated change. | `DATE` | Yes | No | - | - |
| `template_id` | Identifier of the related template record. | `int4` | Yes | No | - | [templates](templates.md) via (`template_id` -> `id`) |
| `event_config_id` | Identifier of the related event config record. | `int4` | Yes | No | - | - |
