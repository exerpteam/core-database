...
| `terms` | Text field containing descriptive or reference information. | `text(2147483647)` | Yes | No | - | - |
...
| `documentation_required` | Boolean flag used in business rules and filtering logic. | `bool` | No | No | - | - |
| `employee_number_required` | Boolean flag used in business rules and filtering logic. | `bool` | No | No | - | - |
| `documentation_interval_unit` | Numeric field used for identifiers, counters, or coded values. | `int4` | No | No | - | - |
| `documentation_interval` | Numeric field used for identifiers, counters, or coded values. | `int4` | Yes | No | - | - |
| `target_employee_count` | Numeric field used for identifiers, counters, or coded values. | `int4` | Yes | No | - | - |
| `target_time` | Epoch timestamp for target. | `int8` | Yes | No | - | - |
| `contactcenter` | Foreign key field linking this record to `persons`. | `int4` | Yes | No | [persons](persons.md) via (`contactcenter`, `contactid` -> `center`, `id`) | - |
| `contactid` | Foreign key field linking this record to `persons`. | `int4` | Yes | No | [persons](persons.md) via (`contactcenter`, `contactid` -> `center`, `id`) | - |
| `own_privileges` | Table field used by operational and reporting workloads. | `bytea` | Yes | No | - | - |
| `REF` | Text field containing descriptive or reference information. | `text(2147483647)` | Yes | No | - | - |
| `stop_new_date` | Date for stop new. | `DATE` | Yes | No | - | - |
| `cash_subscription_stop_date` | Boolean flag used in business rules and filtering logic. | `bool` | No | No | - | - |
| `availability` | Text field containing descriptive or reference information. | `text(2147483647)` | Yes | No | - | - |
...
| `family_corporate_status` | Numeric field used for identifiers, counters, or coded values. | `int4` | No | No | - | - |
| `max_family_corporate` | Numeric field used for identifiers, counters, or coded values. | `int4` | Yes | No | - | - |
| `require_other_payer` | Boolean flag used in business rules and filtering logic. | `bool` | No | No | - | - |
| `last_member_update` | Numeric field used for identifiers, counters, or coded values. | `int8` | No | No | - | - |
| `creation_date` | Date for creation. | `DATE` | Yes | No | - | - |
| `start_date` | Date when the record becomes effective. | `DATE` | Yes | No | - | - |
| `activation_date` | Date for activation. | `DATE` | Yes | No | - | - |
