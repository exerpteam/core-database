...
| `ip_config_id` | Foreign key field linking this record to `installment_plan_configs`. | `int4` | No | No | [installment_plan_configs](installment_plan_configs.md) via (`ip_config_id` -> `id`) | - | `1001` |
...
| `installements_count` | Numeric field used for identifiers, counters, or coded values. | `int4` | No | No | - | - | `42` |
...
| `collect_agreement_center` | Center part of the reference to related collect agreement data. | `int4` | Yes | No | - | - | `101` |
| `collect_agreement_id` | Identifier of the related collect agreement record. | `int4` | Yes | No | - | - | `1001` |
| `collect_agreement_subid` | Sub-identifier for related collect agreement detail rows. | `int4` | Yes | No | - | - | `1` |
| `name` | Text field containing descriptive or reference information. | `VARCHAR(300)` | Yes | No | - | - | `Example Name` |
| `last_modified` | Epoch timestamp for the latest update on the row. | `int8` | Yes | No | - | - | `42` |
| `single_booking_date` | Date for single booking. | `DATE` | Yes | No | - | - | `2025-01-31` |
