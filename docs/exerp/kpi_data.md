...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `field` | Foreign key field linking this record to `kpi_fields`. | `int4` | No | Yes | [kpi_fields](kpi_fields.md) via (`field` -> `id`) | - | `42` |
| `center` | Primary key component that scopes the record to a center. | `int4` | No | Yes | [centers](centers.md) via (`center` -> `id`) | - | `101` |
| `for_date` | Date for for. | `DATE` | No | Yes | - | - | `2025-01-31` |
| `VALUE` | Numeric field used for identifiers, counters, or coded values. | `NUMERIC(0,0)` | No | No | - | - | `99.95` |
| `TIMESTAMP` | Numeric field used for identifiers, counters, or coded values. | `int8` | Yes | No | - | - | `1738281600000` |
| `kind` | Numeric field used for identifiers, counters, or coded values. | `int4` | No | No | - | - | `42` |
