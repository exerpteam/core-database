...
| --- | --- | --- | --- | --- | --- | --- |
| `field` | Primary key component used to uniquely identify this record. | `int4` | No | Yes | [kpi_fields](kpi_fields.md) via (`field` -> `id`) | - |
| `center` | Primary key component used to uniquely identify this record. | `int4` | No | Yes | [centers](centers.md) via (`center` -> `id`) | - |
| `for_date` | Primary key component used to uniquely identify this record. | `DATE` | No | Yes | - | - |
| `VALUE` | Operational field `VALUE` used in query filtering and reporting transformations. | `NUMERIC(0,0)` | No | No | - | - |
| `TIMESTAMP` | Operational field `TIMESTAMP` used in query filtering and reporting transformations. | `int8` | Yes | No | - | - |
| `kind` | Business attribute `kind` used by kpi data workflows and reporting. | `int4` | No | No | - | - |
