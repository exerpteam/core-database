...
| `client_id` | Foreign key field linking this record to `clients`. | `int4` | Yes | No | [clients](clients.md) via (`client_id` -> `id`) | - | `1001` |
| `reader_device_id` | Foreign key field linking this record to `devices`. | `int4` | Yes | No | [devices](devices.md) via (`reader_device_id` -> `id`) | - | `1001` |
| `reader_device_sub_id` | Identifier of the related reader device sub record. | `text(2147483647)` | Yes | No | - | - | `1001` |
| `action_center` | Center part of the reference to related action data. | `int4` | Yes | No | - | - | `101` |
| `action_id` | Identifier of the related action record. | `int4` | Yes | No | - | - | `1001` |
