...
| `journalentry_id` | Foreign key field linking this record to `journalentries`. | `int4` | No | No | [journalentries](journalentries.md) via (`journalentry_id` -> `id`) | - |
...
| `signature_center` | Foreign key field linking this record to `signatures`. | `int4` | Yes | No | [signatures](signatures.md) via (`signature_center`, `signature_id` -> `center`, `id`) | - |
| `signature_id` | Foreign key field linking this record to `signatures`. | `int4` | Yes | No | [signatures](signatures.md) via (`signature_center`, `signature_id` -> `center`, `id`) | - |
