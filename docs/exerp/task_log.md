...
| `task_id` | Foreign key field linking this record to `tasks`. | `int4` | Yes | No | [tasks](tasks.md) via (`task_id` -> `id`) | - |
| `employee_center` | Foreign key field linking this record to `persons`. | `int4` | Yes | No | [persons](persons.md) via (`employee_center`, `employee_id` -> `center`, `id`) | - |
| `employee_id` | Foreign key field linking this record to `persons`. | `int4` | Yes | No | [persons](persons.md) via (`employee_center`, `employee_id` -> `center`, `id`) | - |
| `event_time` | Epoch timestamp for event. | `int8` | No | No | - | - |
| `entry_time` | Epoch timestamp for entry. | `int8` | No | No | - | - |
| `task_action_id` | Foreign key field linking this record to `task_actions`. | `int4` | Yes | No | [task_actions](task_actions.md) via (`task_action_id` -> `id`) | - |
| `task_step_id` | Foreign key field linking this record to `task_steps`. | `int4` | Yes | No | [task_steps](task_steps.md) via (`task_step_id` -> `id`) | - |
| `task_status` | Text field containing descriptive or reference information. | `text(2147483647)` | Yes | No | - | - |
| `previous_task_log_id` | Identifier of the related previous task log record. | `int4` | Yes | No | - | - |
