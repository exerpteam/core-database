...
| `bundle_campaign` | Foreign key field linking this record to `bundle_campaign`. | `int4` | Yes | No | [bundle_campaign](bundle_campaign.md) via (`bundle_campaign` -> `id`) | - |
| `rebated` | Boolean flag used in business rules and filtering logic. | `bool` | No | No | - | - |
...
| `units` | Numeric field used for identifiers, counters, or coded values. | `int4` | Yes | No | - | - |
