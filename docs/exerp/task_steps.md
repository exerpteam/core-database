...
| `task_activity_id` | Identifier of the related task activity record. | `int4` | Yes | No | - | [task_activity](task_activity.md) via (`task_activity_id` -> `id`) | `1001` |
| `progress_id` | Foreign key field linking this record to `progress`. | `int4` | Yes | No | [progress](progress.md) via (`progress_id` -> `id`) | - | `1001` |
