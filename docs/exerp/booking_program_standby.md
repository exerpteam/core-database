...
| `created_by_employee_center` | Foreign key field linking this record to `persons`. | `int4` | Yes | No | [persons](persons.md) via (`created_by_employee_center`, `created_by_employee_id` -> `center`, `id`) | - | `101` |
| `created_by_employee_id` | Foreign key field linking this record to `persons`. | `int4` | Yes | No | [persons](persons.md) via (`created_by_employee_center`, `created_by_employee_id` -> `center`, `id`) | - | `1001` |
| `start_booking_center` | Foreign key field linking this record to `bookings`. | `int4` | No | No | [bookings](bookings.md) via (`start_booking_center`, `start_booking_id` -> `center`, `id`) | - | `101` |
| `start_booking_id` | Foreign key field linking this record to `bookings`. | `int4` | No | No | [bookings](bookings.md) via (`start_booking_center`, `start_booking_id` -> `center`, `id`) | - | `1001` |
| `creation_time` | Epoch timestamp when the row was created. | `int8` | No | No | - | - | `1738281600000` |
| `creation_interface_type` | Numeric field used for identifiers, counters, or coded values. | `int4` | Yes | No | - | - | `42` |
