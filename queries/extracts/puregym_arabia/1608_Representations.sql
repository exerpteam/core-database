...
WHERE
    ...
    ar.balance < 0
    -- 9 days after the collection date of the failed payment.
    AND pr.due_date = params.cutDate - 9
    AND ar.ar_type = 4
    AND ((ch.id in (201,401) and pr.xfr_info IN ('Not enough balance',
                        'Refused',
                        'Issuer Unavailable',
                        'Pin tries exceeded',
