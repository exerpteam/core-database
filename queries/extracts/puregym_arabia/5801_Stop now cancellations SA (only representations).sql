...
WHERE
    p.center IN ($$Scope$$)
    and ar.balance < 0
    AND pr.STATE NOT IN (1,2,3,4,8,18)
     
    AND  (    pr.request_type in (6) 
and ch.id in (1001, 201)            AND pr.req_date+1 between CAST(DATE_TRUNC('day', now()- INTERVAL '7' DAY) AS DATE) AND CAST(DATE_TRUNC('day', now()) AS DATE))