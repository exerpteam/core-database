...
-- https://clublead.atlassian.net/browse/ST-9190
SELECT
...
WHERE
    ...
    AND ( (
            pr.request_type = 1 
and ch.id in (201,401)            AND pr.req_date+1 between CAST(DATE_TRUNC('day', now()- INTERVAL '7' DAY) AS DATE) AND CAST(DATE_TRUNC('day', now()) AS DATE)
            AND pr.xfr_info IN ('Acquirer Fraud',
                                ...
                                'Invalid Card Number'))
        OR (
            pr.request_type = 6 and ch.id in (201,401)
           AND pr.req_date+1 between CAST(DATE_TRUNC('day', now()- INTERVAL '7' DAY) AS DATE) AND CAST(DATE_TRUNC('day', now()) AS DATE)
            AND pr.xfr_info IN ('Declined Non Generic',
                                ...
                                'Invalid Card Number')) 
or (    pr.request_type in (6) 
and ch.id in (1001)            AND pr.req_date+1 between CAST(DATE_TRUNC('day', now()- INTERVAL '7' DAY) AS DATE) AND CAST(DATE_TRUNC('day', now()) AS DATE)))