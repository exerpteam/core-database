...
SELECT
    ...
    CAST(convert_from(je.big_text, 'UTF-8') AS VARCHAR)                            AS "Note Details",
...
LEFT JOIN
    (
        SELECT
            PERSON_CENTER,
            PERSON_ID,
            TO_CHAR(longtodateC(MAX(chk.CHECKIN_TIME), chk.PERSON_CENTER), 'YYYY-MM-DD HH24:MI') AS Last_Checkin
        FROM
            CHECKINS chk
        GROUP BY
            PERSON_CENTER,
...
WHERE
    ...
    /* Indicia Web agency emp id need to change*/
    AND je.CREATORCENTER||'emp'||je.CREATORID NOT IN ('100emp202',
                                                      '100emp203')