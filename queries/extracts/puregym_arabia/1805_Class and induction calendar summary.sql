...
WITH
    ...
    (
        SELECT
            /*+ materialize */
            c.id,
            CAST (dateToLongC(TO_CHAR(CAST($$StartDate$$ AS DATE), 'YYYY-MM-dd HH24:MI'), c.id) AS BIGINT)                  AS PeriodStart,
            CAST((dateToLongC(TO_CHAR(CAST($$EndDate$$ AS DATE), 'YYYY-MM-dd HH24:MI'), c.id)+ 86400 * 1000)-1 AS BIGINT) AS PeriodEnd
        FROM
            centers c
    )
SELECT
    c.NAME                                                                                                                                AS "Center",
    TO_CHAR(longToDateC(bo.STARTTIME, bo.center), 'DAY')                                                                                  AS "Day",
    TO_CHAR(longToDateC(bo.STARTTIME, bo.center), 'YYYY-MM-DD')                                                                           AS "Date",
    TO_CHAR(longToDateC(bo.STARTTIME, bo.center), 'HH24:MI')                                                                              AS "Time",
    bo.NAME                                                                                                                               AS "Name",
    ...
    ROUND((showup_waiting.total_waiting/ (
        CASE
            WHEN bo.WAITING_LIST_CAPACITY = 0
            THEN 1
            ELSE bo.WAITING_LIST_CAPACITY
        END))*100, 1) AS "Waiting list ratio %",
