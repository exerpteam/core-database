WITH
    ...
    (
        SELECT
            *
        FROM
            generate_series((CAST(now() AS DATE) - INTERVAL '28 day' + INTERVAL '1 hour'), (CAST
    ...
    (
        SELECT DISTINCT
            c.id,
            CAST(dateToLongC(TO_CHAR((s.d1), 'YYYY-MM-DD HH24:MI:SS') , c.id) AS BIGINT) AS
            lower_bound,
            CAST(dateToLongC(TO_CHAR((s.d1 + INTERVAL '1 hour'), 'YYYY-MM-DD HH24:MI:SS'), c.id) AS
...
SELECT
    ...
    COUNT(DISTINCT cin.person_center||'p'||cin.person_id)    checkins ,
...
AND cin.checkin_center = slots.id
JOIN
    CENTERS c
ON
