...
SELECT
    ...
    TO_CHAR(LONGTODATEC(b.starttime,b.center),'YYYY-MM-DD HH24:MI') AS Start_Date ,
    TO_CHAR(LONGTODATEC(b.stoptime,b.center),'YYYY-MM-DD HH24:MI')  AS End_Date ,
    ...
    CASE
        WHEN a.id = 2 --Personal Training Starter Session
        THEN 'Starter'
        ELSE 'Paid'
    END AS paid_or_starter
FROM
    params
JOIN
    participations par
...
ON
    b.activity = a.id
AND a.activity_group_id = 2 --Personal training
JOIN
    privilege_usages pu
ON
...
AND su.state = 'ACTIVE' --updated 10-23 NAP
LEFT JOIN
    persons trainer
ON
