...
-- Automatically chooses end of current month for cutdate. Made to be scheduled for export
WITH
    ...
    (
        ...
        FROM
            (
                SELECT
                    CAST( (date_trunc('month', CURRENT_DATE) + interval '1 month' - interval
                    '1 day') AT TIME ZONE 'America/New_York' AS DATE ) AS cutdate,
                    CAST( extract( epoch FROM timezone( 'America/New_York', CAST( (date_trunc
                    ('month', CURRENT_DATE) + interval '1 month') AS TIMESTAMP ) ) ) AS bigint ) *
                    1000 AS eod_long ) pr
        LEFT JOIN
            REPORT_PERIODS rp
        ON
