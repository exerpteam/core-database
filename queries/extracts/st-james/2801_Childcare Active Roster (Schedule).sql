WITH
    ...
    (   SELECT
			getstartofday(TO_CHAR(TO_DATE(getCenterTime(c.id), 'YYYY-MM-DD'), 'YYYY-MM-DD'), c.id) AS from_time,
			getendofday(TO_CHAR(TO_DATE(getCenterTime(c.id), 'YYYY-MM-DD'), 'YYYY-MM-DD'), c.id) AS to_time,
            c.id,
        c.time_zone,
        c.name
        FROM 
