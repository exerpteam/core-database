...
WITH
    ...
    (   SELECT
            ...
            CAST(datetolongc(TO_CHAR(to_date($$CancelDateFrom$$,'YYYY-MM-DD'),'YYYY-MM-DD'),id) AS BIGINT) AS from_Date_ts,
            CAST(datetolongc(TO_CHAR(to_date($$CancelDateTo$$,'YYYY-MM-DD')+1,'YYYY-MM-DD'),id) AS BIGINT) AS to_Date_ts
        FROM
            centers c
        WHERE
            c.id IN ($$Scope$$)
...
SELECT
    ...
    CASE s.SUB_STATE WHEN 1 THEN 'NONE' 
...
WHERE
   ...
   (
      ...
      WHERE s2.start_date <= (date_trunc('month', longtodatec(je.creation_time,je.p_center)) + INTERVAL '1 month - 1 day')::date
