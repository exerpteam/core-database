...
FROM
    ( SELECT
        ...
        , FIRST_VALUE(t.description) OVER (
                                       PARTITION BY
                                           mpr.DEFINITION_KEY
                                       ORDER BY
                                           mpr.SCOPE_TYPE DESC) test
        ...
        , CASE
            ...
            THEN CAST((xpath('FREEZELIMIT/FREEZEAPPLYPERIOD/FREEZEDURATION/@LENGTH', XMLPARSE
                (DOCUMENT convert_from(st.FREEZELIMIT,'UTF8'))))[1] AS VARCHAR) || ' ' || CAST 
                ( (xpath('FREEZELIMIT/FREEZEAPPLYPERIOD/FREEZEDURATION/@UNIT', XMLPARSE(DOCUMENT
                convert_from(st.FREEZELIMIT,'UTF8'))))[1] AS VARCHAR)
            ELSE NULL
        END AS "Within a period of"
        , CASE
            WHEN st.FREEZELIMIT IS NOT NULL
            THEN CAST((xpath('FREEZELIMIT/@MINDURATION', XMLPARSE(DOCUMENT convert_from
                (st.FREEZELIMIT,'UTF8'))))[1] AS VARCHAR)
            ELSE NULL
        END AS "Min duration"
        , CASE
            WHEN st.FREEZELIMIT IS NOT NULL
            THEN CAST((xpath('FREEZELIMIT/@MINDURATION_UNIT', XMLPARSE(DOCUMENT convert_from
                (st.FREEZELIMIT,'UTF8'))))[1] AS VARCHAR)
            ELSE NULL
        END AS "Min duration Unit"
        , CASE
            WHEN st.FREEZELIMIT IS NOT NULL
            THEN CAST((xpath('FREEZELIMIT/@MAXDURATION', XMLPARSE(DOCUMENT convert_from
                (st.FREEZELIMIT,'UTF8'))))[1] AS VARCHAR)
            ELSE NULL
        END AS "Max duration"
        , CASE
            WHEN st.FREEZELIMIT IS NOT NULL
            THEN CAST((xpath('FREEZELIMIT/@MAXDURATION_UNIT', XMLPARSE(DOCUMENT convert_from
                (st.FREEZELIMIT,'UTF8'))))[1] AS VARCHAR)
            ELSE NULL
        ...
        , CASE st.AGE_RESTRICTION_TYPE
            ...
            THEN 'BETWEEN'
            ELSE 'UNDEFINED'
        END "age restriction type"
        , st.AGE_RESTRICTION_VALUE "Age"
        , st.age_restriction_min_value "Min Age"
        , st.age_restriction_max_value "Max Age"
