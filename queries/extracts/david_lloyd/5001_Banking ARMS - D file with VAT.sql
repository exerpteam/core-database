...
WITH
    ...
    ( SELECT
        ...
        c.id                                                               AS center
        , datetolongc((CURRENT_DATE-interval '1 hour')::DATE::VARCHAR,c.id) AS from_date_long
        , datetolongc((CURRENT_DATE-interval '1 hour'+interval '1 day')::                 DATE::VARCHAR,c.id) AS to_date_long
    FROM
        centers c
        where c.id = :center
    )
    ...
    ( SELECT
        ...
        , COALESCE(il.net_amount ,-1*cl.net_amount ) AS "Net Amount"
        , COALESCE(il.total_amount-il.net_amount ,-1*(cl.total_amount-cl.net_amount )) AS "VAT"
        ,"Tender"
        ,"Tender Amount"
        ,t.comment
    FROM
