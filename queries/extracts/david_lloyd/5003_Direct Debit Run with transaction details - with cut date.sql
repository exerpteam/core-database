...
, 
 res AS ( 
    SELECT
        ...
        CASE WHEN representation_pr.id IS NULL 
           THEN COALESCE(cl.quantity,il.quantity)         
           ELSE null END                                                                 AS "Total Quantity",
        CASE WHEN representation_pr.id IS NULL
           THEN COALESCE(-1*cl.total_amount,il.total_amount,-1*art.amount)                
           ELSE null END                                                                 AS "Total Sale Amount",
        CASE WHEN representation_pr.id IS NULL
           THEN COALESCE(-1*cl.net_amount,il.net_amount,-1*art.amount)
           ELSE null   END                                                               AS "Total Net Amount",
        CASE WHEN representation_pr.id IS NULL
           THEN COALESCE( -1*(cl.total_amount - cl.net_amount), il.total_amount - il.net_amount,0)  
           ELSE null  END                                                                AS "Total Tax Amount",
        CASE WHEN representation_pr.id IS NULL
           THEN ROUND(COALESCE(vl_cl.rate, vl.rate ),2)   
           ELSE null END                                                                 AS "Tax Rate",
        pr.agr_subid
    FROM
        payment_requests pr
    CROSS JOIN
    ...
    LEFT JOIN 
       payment_requests representation_pr 
    ON
       representation_pr.inv_coll_center = pr.inv_coll_center
       AND representation_pr.inv_coll_id = pr.inv_coll_id
       AND representation_pr.inv_coll_subid = pr.inv_coll_subid
       AND representation_pr.request_type = 6
       AND representation_pr.req_date >= params.first_day_of_previous_month 
       AND representation_pr.req_date < params.first_day_of_selected_month
       AND representation_pr.state != 8  --- cancelled
       AND representation_pr.clearinghouse_id = 2  -- BACS UK
       AND representation_pr.subid != pr.subid
    LEFT JOIN
    ...
    WHERE
      pr.req_date >= params.first_day_of_previous_month 
      AND pr.req_date < params.first_day_of_selected_month
      AND pr.state != 8  --- cancelled
      AND pr.clearinghouse_id = 2  -- BACS UK
      AND pr.req_amount != 0
