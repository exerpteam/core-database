...
SELECT
     ...
     ins.FULLNAME "PT Name",
			 ...
			 per.sex as "Gender",
     CASE  per.PERSONTYPE WHEN 2 THEN 'STAFF' ELSE NULL END AS "Staff Member",			 
 ...
 WHERE
     ...
     actgr.NAME IN ('Personal training','Pool PT', 'In 2 Wellness', 'Mind and Body 121', 'Reformer Pilates 121')
 ...
 GROUP BY
     ...
     c.NAME,
	  ins.Center ||'p'|| ins.id,
	  per.external_id,
	  --floor(months_between(TRUNC(CURRENT_TIMESTAMP),per.BIRTHDATE) / 12),
			 per.sex,
     ins.FULLNAME,
     per.Center,
     per.id,
