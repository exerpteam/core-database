 ...
 FROM
     (
         SELECT
             ...
             CASE
    WHEN first_value(i2.PREVIOUS_TRANSACTION) OVER (
             PARTITION BY i2.pid
             ORDER BY i2.PREVIOUS_TRANSACTION DESC NULLS LAST
         ) IS NULL
         OR (
             date_part(
                 'year',
                 age(
                     i2.LAST_TRANS::date,
                     first_value(i2.PREVIOUS_TRANSACTION::date) OVER (
                         PARTITION BY i2.pid
                         ORDER BY i2.PREVIOUS_TRANSACTION DESC NULLS LAST
                     )
                 )
         ...
         OR TO_CHAR(
                first_value(i2.PREVIOUS_TRANSACTION) OVER (
                    PARTITION BY i2.pid
                    ORDER BY i2.PREVIOUS_TRANSACTION DESC NULLS LAST
                ),
                'YYYY-MM-DD'
