...
/* Exerp GO profile check */
    go_check as (
        select
                SUM(CASE WHEN sp.globalid = 'ExerpGoProfileSettings' THEN true ELSE false END) AS "go_check"                       
        from systemproperties sp 
        where sp.globalid = 'ExerpGoProfileSettings'
    ),
...
/* Main query. licenses, area */    
    base AS  (   
        SELECT DISTINCT                     
                ...
                c.state AS "State",
                round(c.latitude, 6) as "Latitude", 
                round(c.longitude, 6) as "Longitude",
