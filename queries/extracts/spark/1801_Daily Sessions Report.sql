...
SELECT
    ...
    COUNT(
        CASE
            WHEN t.activity_group = 'PT'
            THEN 1
        END) AS "PT COUNT FOR THE DAY",
    COUNT(
        CASE
            WHEN t.activity_group = 'PT'
            AND t.state = 'CANCELLED'
            THEN 1
        END) AS "PT CANCELLED FOR THE DAY",
    SUM(
        CASE
            WHEN t.activity_group = 'PT'
            THEN t.price
        END) AS "PT SESSIONS KWD FOR THE DAY",
    COUNT(
        CASE
            WHEN t.activity_group = 'LAB'
            THEN 1
        END) AS "LAB COUNT FOR THE DAY",
    COUNT(
        CASE
            WHEN t.activity_group = 'LAB'
            AND t.state = 'CANCELLED'
            THEN 1
        END) AS "LAB CANCELLED FOR THE DAY",
    SUM(
        CASE
            WHEN t.activity_group = 'LAB'
            THEN t.price
        END) AS "LAB SESSIONS KWD FOR THE DAY",
    COUNT(
        CASE
            WHEN t.activity_group = 'Consultation'
            THEN 1
        END) AS "CONSULTATION COUNT FOR THE DAY",
    COUNT(
        CASE
            WHEN t.activity_group = 'Consultation'
            AND t.state = 'CANCELLED'
            THEN 1
        END) AS "CONSULTATION CANCELLED FOR THE DAY",
    SUM(
        CASE
            WHEN t.activity_group = 'Consultation'
            THEN t.price
        END) AS "CONSULTATION SESSIONS KWD FOR THE DAY"
FROM
    (
        SELECT
            --   su.person_center ||'p'|| su.person_id                   AS "Staff ID",
            -- sta.fullname                                            AS "Staff name",
            TO_CHAR(longtodate(bo.starttime), 'YYYY-MM-DD') AS start_date,
