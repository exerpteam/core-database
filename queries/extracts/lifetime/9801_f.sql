...
t.CENTERNAME as centername,
  ...
  --  COALESCE(camp_gender,'') camp_gender,
    ...
    otc,
    asthma,
     STRING_AGG(CAST(t.parent AS TEXT),'  ' ) as parent
FROM
    (
        WITH
            params AS
        ...
        STRING_AGG(CAST(rel_per.fullname AS TEXT),'  ' ) as parent,
       qa.subid as qasubid,
		...
		left join relatives r on p.center = r.relativecenter and p.id = r.relativeid and r.rtype = 14
		left join lifetime.persons rel_per on rel_per.center =r.center and rel_per.id = r.id and rel_per.fullname <> ('API User') and rel_per.fullname <> ('Exerp Admin')
		and CAST(EXTRACT( YEAR FROM (AGE(now(),rel_per.birthdate)))AS INTEGER) > 18 		
        LEFT JOIN
...
--        AND immunization_current.question_id = 15 
    ...
    WHERE
     ...
     and a.external_id in (:external_id) --701591130151
        group by rel_per.id,pa.id,qa.id,rel_per.fullname,b.center,b.id,b.name,ap1.txtvalue,cap1.txtvalue,ap2.txtvalue,cap2.txtvalue,ap3.txtvalue,ec1r.txtvalue,
