...
-- https://clublead.atlassian.net/browse/ST-16299
WITH
    params AS materialized
    (
        SELECT
            c.id                                                                          AS center,
            CAST(datetolongc(TO_CHAR(CURRENT_DATE,'YYYY-MM-DD HH24:MI:SS'), c.id) AS BIGINT) AS
            FROM_DATE,
            CAST(datetolongc(TO_CHAR(CURRENT_DATE+1,'YYYY-MM-DD HH24:MI:SS'), c.id) AS BIGINT)-1 AS
            TO_DATE
        FROM
            centers c
    )
