...
(
        SELECT  
                ...
                (CASE
                        WHEN (xmlexists('//clipcardType/period/text()' PASSING BY REF pmp_xml.pxml)) THEN
                                UNNEST(xpath('//clipcardType/period/text()', pmp_xml.pxml))         
                        ELSE
                                NULL
                END) AS expiry,
                (CASE
                        WHEN (xmlexists('//clipcardType/period/@unit' PASSING BY REF pmp_xml.pxml)) THEN
                                UNNEST(xpath('//clipcardType/period/@unit', pmp_xml.pxml))         
                        ELSE
                                NULL
                END) AS expiryUnit,
                UNNEST(xpath('//clipcardType/product/privilegeNeeded/text()', pmp_xml.pxml)) AS requiresPrivileges,
                (CASE
                        WHEN (xmlexists('//clipcardType/buyoutFeePercentage/text()' PASSING BY REF pmp_xml.pxml)) THEN
                                UNNEST(xpath('//clipcardType/buyoutFeePercentage/text()', pmp_xml.pxml))         
                        ELSE
                                NULL
                END) AS buyoutFeePercentage,
                UNNEST(xpath('//clipcardType/product/showOnWeb/text()', pmp_xml.pxml)) AS showonWeb,
                (CASE
                        WHEN (xmlexists('//clipcardType/product/maxBuy/@qty' PASSING BY REF pmp_xml.pxml)) THEN
                                UNNEST(xpath('//clipcardType/product/maxBuy/@qty', pmp_xml.pxml))       
                        ELSE
                                NULL
                END) AS purchaseFrequencyMaxBuy,
                (CASE
                        WHEN (xmlexists('//clipcardType/product/maxBuy/period/@unit' PASSING BY REF pmp_xml.pxml)) THEN
                                UNNEST(xpath('//clipcardType/product/maxBuy/period/@unit', pmp_xml.pxml))       
                        ELSE
                                NULL
                END) AS purchaseFrequencyPeriodUnit,
                (CASE
                        WHEN (xmlexists('//clipcardType/product/maxBuy/period/text()' PASSING BY REF pmp_xml.pxml)) THEN
                                UNNEST(xpath('//clipcardType/product/maxBuy/period/text()', pmp_xml.pxml))       
                        ELSE
                                NULL
                ...
                END) as productType,
                (CASE
                        WHEN (xmlexists('//clipcardType/product/requiredRole/text()' PASSING BY REF pmp_xml.pxml)) THEN
                                UNNEST(xpath('//clipcardType/product/requiredRole/text()', pmp_xml.pxml))       
                        ELSE
                                NULL
                END) AS requiredRole_t1,
                (CASE
                        WHEN (xmlexists('//clipcardType/product/assignedStaffGroup/text()' PASSING BY REF pmp_xml.pxml)) THEN
                                UNNEST(xpath('//clipcardType/product/assignedStaffGroup/text()', pmp_xml.pxml))
                        ELSE
                                NULL
