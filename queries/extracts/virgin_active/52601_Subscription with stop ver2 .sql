...
--  
         ...
         SELECT DISTINCT
             ...
             CASE
                 WHEN qa.number_answer IS NOT NULL
                   ...
                   THEN CAST(COALESCE((xpath('//question[id/text()='||qa.QUESTION_ID ||']/options/option[id/text()='||qa.NUMBER_ANSWER ||']/optionText/text()', xmlparse(document convert_from(q.QUESTIONS,'UTF-8'))))[1]) AS VARCHAR) 
                 ELSE NULL
             END                                                                                                              AS Reason,
 ...
 (SELECT
    ...
    "ID"||'qu'||"QQID"||'an'||"AID" AS                "ANSWER_ID",
         ...
         FROM
			 ...
			 THEN TO_DATE(TRIM(substr(convert_from(je.big_text,'UTF-8'),LENGTH('Subscription credit date: '),8)),'MM/DD/YY') 
                 ...
                 ON
                     ...
                     AND je.jetype = 3
                     AND je.name = 'Apply: Stop subscriptions' ) t
         WHERE
