...
-- https://clublead.atlassian.net/browse/ES-2908
WITH
    params AS
    (
        SELECT
...
SELECT
    ...
    DECODE(sales.PAYER_CENTER || 'p' || sales.PAYER_ID, 'p', sales.PERSON_CENTER || 'p' || sales.PERSON_ID, sales.PAYER_CENTER || 'p' || sales.PAYER_ID) payer_id,
...
WHERE
    ...
    AND (
        debit.EXTERNAL_ID IN ('101120',
                              '102300',
                              '101416'
                              )
        OR credit.EXTERNAL_ID IN ('101120',
                              	  '102300',
                              	  '101416'                              									) )
GROUP BY
pu.LAST_ACTIVE_START_DATE,
