...
-- Version of the Vitality Invoice report which can be run for a selected time period. If you change the query, change it in report 12001 as well.
https://clublead.atlassian.net/browse/ST-3180
-- Parameters: FromDate(LONG_DATE),ToDate(LONG_DATE),Scope(SCOPE)
WITH
    params AS
    (
        SELECT
            CAST(extract(epoch FROM timezone('Europe/London',CAST(date_trunc('month',
            CURRENT_TIMESTAMP) - interval '1 month ' AS TIMESTAMP))) AS BIGINT)*1000 AS FROMDATE,
            CAST(extract(epoch FROM timezone('Europe/London',CAST(date_trunc('month',
            CURRENT_TIMESTAMP) AS TIMESTAMP))) AS BIGINT)*1000 AS TODATE
    )
SELECT
    c.SHORTNAME club,
    ...
    SUM(COALESCE(remit_gross_amount, 0)) + SUM(COALESCE(membership_fee,0)) + SUM(COALESCE(admin_fee
    , 0)) + SUM(COALESCE(sales_commission, 0 )) + SUM(COALESCE(pru_Assessment, 0)) "Total Invoice",
    NULL " ",
    SUM(COALESCE(membership_fee,0)) + SUM(COALESCE(admin_fee, 0)) + SUM(COALESCE(sales_commission,
    0)) + SUM(COALESCE(pru_Assessment, 0)) "Gross charge to Vitality",
    NULL " ",
    ...
    (
        SELECT
            ...
            CASE
                ...
                THEN
                    CASE
                        WHEN SCL1.SUB_STATE = 1
                        THEN 'NONE'
                        WHEN SCL1.SUB_STATE = 2
                        THEN 'AWAITING_ACTIVATION'
                        WHEN SCL1.SUB_STATE = 3
                        THEN 'UPGRADED'
                        WHEN SCL1.SUB_STATE = 4
                        THEN 'DOWNGRADED'
                        WHEN SCL1.SUB_STATE = 5
                        THEN 'EXTENDED'
                        WHEN SCL1.SUB_STATE = 6
                        THEN 'TRANSFERRED'
                        WHEN SCL1.SUB_STATE = 7
                        THEN 'REGRETTED'
                        WHEN SCL1.SUB_STATE = 8
                        THEN 'CANCELLED'
                        WHEN SCL1.SUB_STATE = 9
                        THEN 'BLOCKED'
                        ELSE 'UNKNOWN'
                    END
                ELSE NULL
            ...
            END  admin_fee,
            NULL pru_Assessment,
            sales.sales_type || '-' || sales.CENTER || '-' || sales.ID || '-' || sales.SUB_ID
            sales_id
        FROM
            persons p
        CROSS JOIN
        ...
        AND (
                ...
                (
                    SELECT
                        1
                    FROM
                        subscriptiontypes pro_st
                    JOIN
                        PRODUCT_AND_PRODUCT_GROUP_LINK pro_pgl
                    ON
                        pro_pgl.PRODUCT_CENTER = pro_st.center
                    AND pro_pgl.PRODUCT_ID = pro_st.id
                    AND pro_pgl.PRODUCT_GROUP_ID = 1601
                    WHERE
                        pro_st.PRORATAPRODUCT_CENTER = prod.center
                    AND pro_st.PRORATAPRODUCT_ID = prod.id)
            OR  EXISTS
                (
                    SELECT
                        *
        ...
        SELECT
            ...
            c.SHORTNAME || ' - ' || act.NAME || ': ' || TO_CHAR(longtodateTZ(bk.STARTTIME,
            'Europe/London'), 'YYYY-MM-DD HH24:MI') description,
