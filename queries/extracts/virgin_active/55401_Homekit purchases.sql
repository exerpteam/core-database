...
--  
 ...
 LEFT JOIN
         ...
         ON
             ext_email.PERSONCENTER = P.Center
             AND ext_email.PERSONID = P.ID
             AND ext_email.NAME ='_eClub_Email'
             AND ext_email.TXTVALUE IS NOT NULL
 WHERE
     longtodate(inv.TRANS_TIME) between (current_timestamp-1) and (current_timestamp)
     AND
