SELECT DISTINCT
    ...
    nvl(FIRST_VALUE(invl.TOTAL_AMOUNT) OVER (PARTITION BY s.CENTER, s.ID ORDER BY invl.SUBID ASC),0) ProRata1,
    nvl(FIRST_VALUE(invl.TOTAL_AMOUNT) OVER (PARTITION BY s.CENTER, s.ID ORDER BY invl.SUBID DESC),0) ProRata2,
