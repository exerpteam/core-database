SELECT
    ...
    invl.QUANTITY,
	TO_CHAR(invl.TOTAL_AMOUNT / (1 + invl.rate),'FM99999999999999999990.00') NET_AMOUNT,
    TO_CHAR(invl.TOTAL_AMOUNT - (invl.TOTAL_AMOUNT / (1 + invl.rate)),'FM99999999999999999990.00') VAT_AMOUNT,
    prod.NAME PROD_NAME,
    prod.CENTER || 'prod' || prod.ID PROD_ID,
	invl.REBOOKING_TO_CENTER as REBOOKINGTOCENTER,
	vat.EXTERNAL_ID
FROM
    INVOICES inv
...
LEFT OUTER JOIN ACCOUNT_TRANS trans
ON invl.VAT_ACC_TRANS_CENTER = trans.CENTER
and invl.VAT_ACC_TRANS_ID = trans.ID
and invl.VAT_ACC_TRANS_SUBID = trans.SUBID
LEFT OUTER JOIN VAT_TYPES vat
ON  vat.CENTER = trans.VAT_TYPE_CENTER
AND vat.ID = trans.VAT_TYPE_ID


where inv.center IN(select c.ID from CENTERS c where  c.COUNTRY = 'IT' ) 