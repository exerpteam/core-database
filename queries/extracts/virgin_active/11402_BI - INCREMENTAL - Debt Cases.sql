...
SELECT
            c.id,
            cast(datetolongtz(TO_CHAR(TO_DATE(getCenterTime(c.id),'YYYY-MM-DD') - interval '5 days', 'YYYY-MM-DD HH24:MI'), c.time_zone) as bigint) AS FROMDATE,
            cast(datetolongtz(TO_CHAR(TO_DATE(getCenterTime(c.id),'YYYY-MM-DD') + interval '1 days',  'YYYY-MM-DD HH24:MI'), c.time_zone) as bigint) AS TODATE
        FROM
            centers c
        WHERE
            id IN ($$scope$$)
