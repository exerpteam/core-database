 SELECT
         TUTTO.Club AS "CLUB",
         TUTTO.VenditeTotali AS "Vendite Totali",
         TUTTO.VenditeWeb AS "Vendite Web",
         TUTTO.VenditeNonWeb AS "Vendite Non Web"
 FROM
 (
         SELECT
                 ' ' || VENDITE_PER_CLUB.SHORTNAME as Club,
                 SUM(VENDITE_PER_CLUB.venditeTotali) as VenditeTotali,
                 SUM(VENDITE_PER_CLUB.Web) as VenditeWeb,
                 SUM(VENDITE_PER_CLUB.NonWeb) as VenditeNonWeb
         FROM
         (
                 SELECT
                         cen.SHORTNAME,
         ...
         GROUP BY VENDITE_PER_CLUB.SHORTNAME
         UNION
         SELECT
                'VIRGIN ACTIVE TOTALE' as Club,
                t2.*
         FROM
         (
                 SELECT
