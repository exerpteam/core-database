...
--  
 ...
 --  CASE WHEN pp.center IS NOT NULL THEN PExtEmail.TXTVALUE ELSE ExtEmail.TXTVALUE END "Payer email address",
   ...
   CASE  m.DELIVERYMETHOD  WHEN 0 THEN  'STAFF'  WHEN 1 THEN  'EMAIL'  WHEN 2 THEN  'SMS'  WHEN 3 THEN  'PERSINTF' WHEN 4 THEN  'BLOCKPERSINTF' WHEN 5 THEN  'LETTER' ELSE 'UNDEFINED' END "Communication method",
   mpp.SUBJECT "Message sent to payer",
   CASE  mpp.DELIVERYMETHOD  WHEN 0 THEN  'STAFF'  WHEN 1 THEN  'EMAIL'  WHEN 2 THEN  'SMS'  WHEN 3 THEN  'PERSINTF' WHEN 4 THEN  'BLOCKPERSINTF' WHEN 5 THEN  'LETTER' ELSE 'UNDEFINED' END "Communication method",
 ...
 ON
     ...
     AND pp.ID = rel.ID
 LEFT JOIN
         MESSAGES mpp on mpp.CENTER = pp.CENTER and mpp.ID = pp.ID and mpp.SENTTIME between (je.CREATION_TIME - 10000) and (je.CREATION_TIME + 10000)
 LEFT JOIN
 ...
 WHERE
     ...
     and floor(months_between(to_timestamp(getcentertime(c.id),'YYYY-MM-DD HH24:MI:SS'), p.BIRTHDATE) /12) between $$age_from$$ and $$age_to$$
