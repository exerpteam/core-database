 ...
 SELECT
     ...
     cast(extract ('year' from age(p.BIRTHDATE)) as integer) "Member age",
     ...
     TO_CHAR(longToDateC(par.CREATION_TIME,book.CENTER),'YYYY-MM-DD') "Date booking made",
     CASE
         WHEN par.STATE = 'CANCELLED'
         THEN par.CANCELATION_REASON
         ELSE par.STATE
     END "Booking status",
         CASE par.USER_INTERFACE_TYPE  WHEN 0 THEN 'OTHER'  WHEN 1 THEN 'CLIENT' WHEN 2 THEN 'WEB' WHEN 3 THEN 'KIOSK' WHEN 4 THEN 'SCRIPT' ELSE 'UNKNOWN' END AS "User interface"
 FROM
     PARTICIPATIONS par
 JOIN
     PERSONS p
 ...
 and
 par.CANCELATION_REASON = 'NO_SHOW'
 ORDER BY
     c.SHORTNAME,
     book.STARTTIME,
     act.NAME
