...
-- Subscription with Stop per schedulazione 
 WITH
     v_sub AS
     (
         SELECT
         ...
         WHERE
             ...
             AND TRUNC(sub.end_date) BETWEEN to_date('2020-08-01','yyyy-mm-dd') AND to_date('2020-08-31','yyyy-mm-dd')
             AND EXISTS
             (
                 SELECT
                     1
     ...
     (
         SELECT DISTINCT
             ...
             CASE
                 ...
                 THEN 
				     CAST(COALESCE((xpath('//question[id/text()='||qa.QUESTION_ID ||']/options/option[id/text()='||qa.number_answer ||']/optionText/text()', xmlparse(document convert_from(q.QUESTIONS,'UTF-8'))))[1]) AS VARCHAR) 
                 ELSE NULL
             END                                                                                                              AS Reason,
