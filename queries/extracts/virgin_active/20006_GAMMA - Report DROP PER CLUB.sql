 ...
 P.LASTNAME AS COGNOME,
  FLOOR((TRUNC(CURRENT_DATE) - P.BIRTHDATE) /365.242199) AS ETA,
   PR.NAME AS PRODOTTO,
...
END AS SUB_STATE,
 ...
 -- Updated
 AND (( Extract(month from S.END_DATE) = Extract(month FROM CURRENT_DATE)
 AND Extract(year from S.END_DATE)::integer = Extract(year FROM CURRENT_DATE)::integer) or ((Extract(month from S.END_DATE)::integer != Extract(month FROM CURRENT_DATE)::integer OR   Extract(year from S.END_DATE)::integer! = Extract(year FROM CURRENT_DATE)::integer)) AND p.STATUS NOT IN(1,3,4))) p
 GROUP BY
 NUMEROABBONAMENTO,
 NOME,
 COGNOME,
