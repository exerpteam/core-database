...
--  
 SELECT DISTINCT
     ...
     COALESCE(FIRST_VALUE(invl.TOTAL_AMOUNT) OVER (PARTITION BY s.CENTER, s.ID ORDER BY invl.SUBID ASC),0) "PRORATA1",
     COALESCE(FIRST_VALUE(invl.TOTAL_AMOUNT) OVER (PARTITION BY s.CENTER, s.ID ORDER BY invl.SUBID DESC),0) "PRORATA2",
     s.BINDING_PRICE "Amount",
     perCreation.TXTVALUE "JoinDate",
     s.START_DATE "ContractStart",
     s.BINDING_END_DATE "EarliestEnd",
     COALESCE(ss.PRICE_NEW,0) + COALESCE(ss.PRICE_ADMIN_FEE,0) "ActivateFee",
     (REGEXP_MATCHES(prod.name,'([[:digit:]]+)%','i'))[1] "Discount",
     /*pp.PRICE_MODIFICATION_AMOUNT "Discount",*/
     ca.NAME "PlanType",
     trim(PRU_ENTITY_NUMBER.TXTVALUE) "AuthCode",
     trim(PBLookupPartnerErrorCode.TXTVALUE) AS "ERRORCODE",
 ...
 WHERE
     ...
     AND s.CREATION_TIME BETWEEN dateToLong(TO_CHAR(TRUNC(current_timestamp)-1,'YYYY-MM-DD HH24:MI')) AND dateToLong(TO_CHAR(TRUNC(current_timestamp),'YYYY-MM-DD HH24:MI'))-1
