SELECT
     ...
     sales.email.PERSONID SALES_PERSON,
     t1.AF AS AF,
     ...
     CASE
         ...
         ELSE 'N'
     END COMMISSIONABLE,
     mycomp.email.PERSONID COMPANY_RELATION,
 ...
 SELECT
     ...
     TO_CHAR(longtodateC(SU.CREATION_TIME, SU.CENTER),'DD/MM/YYYY HH:MM:SS AM') AS created,
     CASE
     ...
     WHEN su.SUB_STATE = 9
     THEN 'BLOCKED'
     WHEN su.SUB_STATE = 10
     THEN 'CHANGED'
     ELSE 'UNKNOWN'
     END AS SUB_SUB_STATE,
 ...
 LEFT OUTER JOIN
     (
         ...
         WHERE
             pg.ID IN (4001, 4601,5208,30203,30801)
         AND c.COUNTRY ='IT'
         GROUP BY
             s.CENTER,
             s.ID ) spg
 ...
 WHERE
   ...
   */
     ...
     AND PRG.ID NOT IN (5405,5611,5613,5406,5407,5615,30203,30801)
