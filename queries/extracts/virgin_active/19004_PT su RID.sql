         ...
         ON
          ...
          (
                 SELECT
                     mpr.ID,
                     mpr2.SCOPE_ID ,
                     CAST((xpath('//product/prices/price/normalPrice/text()', xmlparse(document convert_from(COALESCE
                     (mpr2.PRODUCT,mpr.PRODUCT),'UTF-8'))))[1] AS VARCHAR) AS price,
                     CAST((xpath('//product/prices/price/@start', xmlparse(document convert_from(COALESCE
                     (mpr2.PRODUCT,mpr.PRODUCT),'UTF-8'))))[1] AS VARCHAR) AS changeDate
                 FROM
                     MASTERPRODUCTREGISTER mpr
                 LEFT JOIN
                     MASTERPRODUCTREGISTER mpr2
                 ON
                     mpr2.GLOBALID = mpr.GLOBALID
                 AND mpr2.SCOPE_TYPE = 'C'
                 ) q ON q.ID = sa.ADDON_PRODUCT_ID and q.SCOPE_ID = sa.CENTER_ID
         left join (
         SELECT
                mpr.ID,
                CAST((xpath('//product/prices/price/normalPrice/text()', xmlparse(document convert_from
                ( mpr.PRODUCT,'UTF-8'))))[1] AS VARCHAR) AS price,
                CAST((xpath('//product/prices/price/@start', xmlparse(document convert_from(COALESCE
                ( mpr.PRODUCT,'UTF-8'),'UTF-8'))))[1] AS VARCHAR) AS changeDate
        FROM
        MASTERPRODUCTREGISTER mpr) q2 ON q2.ID = sa.ADDON_PRODUCT_ID
         JOIN
             PRODUCTS prod
         ON
         ...
         WHERE
             ...
             (
                 ...
                 WHERE
                     ...
                     AND link.PRODUCT_GROUP_ID = 271 ) )q1
 LEFT JOIN
