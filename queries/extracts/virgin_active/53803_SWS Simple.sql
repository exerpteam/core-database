...
-- https://clublead.atlassian.net/browse/ST-12767
 WITH
     v_sub AS
     (
         SELECT
         ...
         FROM
             (
                 SELECT
                     ...
                     rank() over (partition BY sub.center, sub.id ORDER BY je.creation_time DESC)    AS rnk,
                     rank() over (partition BY tsub.center, tsub.id ORDER BY tje.creation_time DESC) AS trnk
                 FROM
                     v_sub sub
                 LEFT JOIN
                     subscriptions tsub
