...
WITH
 ...
 ON
     (
         (sales.REBOOKING_TO_CENTER IS NOT NULL AND PARAMS.centerid = sales.REBOOKING_TO_CENTER)
         OR
         (sales.REBOOKING_TO_CENTER IS NULL AND PARAMS.centerid = sales.ACCOUNT_TRANS_CENTER)
     )
 JOIN
     PRODUCTS prod
 ON
 ...
 WHERE
     sales.TRANS_TIME >= CAST(PARAMS.FROMDATE AS BIGINT)
     AND sales.TRANS_TIME < CAST(PARAMS.TODATE AS BIGINT)
     AND ( (
