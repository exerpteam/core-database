...
FROM
    (
        SELECT
            ...
            DECODE(i2.CASH_TO_INCLUDE,1,'Subscription Ended/','') || DECODE(i2.ENDING_SUB,1,'Subscription Ended/','') || DECODE(i2.NEW_DEBT_CASE,1,'Debt Case/','') || DECODE(i2.ENTERING_FREEZE,1,'Freeze/','') || DECODE(i2.NEW_AGREEMENT_CASE,1,'DDI Case/','') TERMINATION_TYPE,
            MAX(
                CASE
                    ...
                    THEN EXTRACTVALUE(xmltype(
                            ...
                            END ), '//question[id/text()='||TO_CHAR(qa.QUESTION_ID)||']/options/option[id/text()='||TO_CHAR(qa.NUMBER_ANSWER)||']/optionText')
        ...
        FROM
            (
                ...
                SELECT
                    CASE
                        /* The cash subs that was in WINDOW at cut date and when the report was ran have sub state NONE should be included*/
                        WHEN CASH_TO_INCLUDE = 1
                        THEN 1                    
                        /* All cash subs that has not been covered by above and where we don't hve a a freeze, debt, or agreement case should not be in the report */
                        WHEN (LIVE_CLOSING = 1
        ...
        GROUP BY
            ...
            CASH_TO_INCLUDE,
            pg.NAME ,
            DECODE(i2.CASH_TO_INCLUDE,1,'Subscription Ended/','') || DECODE(i2.ENDING_SUB,1,'Subscription Ended/','') || DECODE(i2.NEW_DEBT_CASE,1,'Debt Case/','') || DECODE(i2.ENTERING_FREEZE,1,'Freeze/','') || DECODE(i2.NEW_AGREEMENT_CASE,1,'DDI Case/',''),
