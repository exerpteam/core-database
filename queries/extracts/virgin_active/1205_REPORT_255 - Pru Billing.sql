SELECT
    ...
    pruEntityNo.TXTVALUE "Pru Entity",
    CASE
...
dataset.remit_rack_gross_amount "Gross Rack Rate to Member",
    ...
    SUM(remit_gross_amount) "Pru Rate to Member",
...
SUM(remit_gross_amount) "Member Payment",
    ...
    SUM(sales_commission) "Sales Commission",
    SUM(pru_Assessment) "Fitness Assesment",
    SUM(NVL(remit_gross_amount, 0)) + SUM(NVL(membership_fee,0)) + SUM(NVL(admin_fee, 0)) + SUM(NVL(sales_commission, 0
    )) + SUM(NVL(pru_Assessment, 0)) "Total Invoice",
null " ",
    SUM(NVL(membership_fee,0)) + SUM(NVL(admin_fee, 0)) + SUM(NVL(sales_commission, 0)) + SUM(NVL(pru_Assessment, 0))
    "Gross charge to Pru",
null " ",
    dataset.sales_id "Sales ID"
    --    ROUND((NVL(SUM(sales_commission),0) + NVL(SUM(membership_fee),0) + NVL(SUM(admin_fee),0) + NVL(SUM
    -- (pru_Assessment),
    --    0)) /120*100, 2) CODA,
FROM
    (
        SELECT
            ...
            sales.PRODUCT_NAME,
            sales.PRODUCT_TYPE,
            exerpro.longtodateTZ(subs.CREATION_TIME, 'Europe/London') join_date,
            subs.START_DATE,
            subs.END_DATE,
            CASE
                WHEN subs.START_DATE IS NOT NULL
                THEN DECODE (SCL1.STATEID, 2,'ACTIVE', 3,'ENDED', 4,'FROZEN', 7,'WINDOW', 8,'CREATED',
                    '             
UNKNOWN')
                ELSE NULL
            END STATE,
            CASE
                WHEN subs.START_DATE IS NOT NULL
                THEN DECODE (SCL1.SUB_STATE, 1,'NONE', 2,'AWAITING_ACTIVATION', 3,'UPGRADED', 4,
                    '             
DOWNGRADED', 5, 'EXTENDED', 6, 'TRANSFERRED',7,'REGRETTED',8,'CANCELLED',9,'BLOCKED'
                    ,'UNKNOWN')
                ELSE NULL
            END SUB_STATE,
            ...
            CASE
                WHEN sales.PRODUCT_TYPE = 'JOINING_FEE'
                    ...
                    AND p.LAST_ACTIVE_START_DATE > exerpro.longtodateTZ(sales.ENTRY_TIME, 'Europe/London') - 30
                    AND cag.EXTERNAL_ID NOT LIKE 'VITPLNY%'
                THEN 10
                WHEN sales.PRODUCT_TYPE = 'JOINING_FEE'
        ...
        ON
            (
                ...
                AND SCL1.BOOK_START_TIME < $$ToDate$$ + (1000*60*60*24) + 2000
                AND (
                    SCL1.BOOK_END_TIME IS NULL
                    OR SCL1.BOOK_END_TIME >= $$ToDate$$ + (1000*60*60*24) + 2000)
        ...
        WHERE
            ...
            AND (
                sales.PRODUCT_NAME LIKE 'ProRata%Pru %'
                OR EXISTS
                (
                    SELECT
                        *
        ...
        SELECT
            ...
            c.SHORTNAME || ' - ' || act.NAME || ': ' || TO_CHAR(exerpro.longtodateTZ(bk.STARTTIME, 'Europe/London'),
...
LEFT JOIN
    RELATIVES comp_rel
ON
    comp_rel.center=dataset.person_center
    AND comp_rel.id=dataset.person_id
    AND comp_rel.RTYPE = 3
    AND comp_rel.STATUS < 3
LEFT JOIN
    COMPANYAGREEMENTS cag
...
ON
    dataset.person_center = pruEntityNo.PERSONCENTER
    AND dataset.person_id = pruEntityNo.PERSONID
    AND pruEntityNo.name = '_eClub_PBLookupPartnerPersonId'
LEFT JOIN
    PERSON_EXT_ATTRS pruAuthoriseCode
ON
    dataset.person_center = pruAuthoriseCode.PERSONCENTER
    AND dataset.person_id = pruAuthoriseCode.PERSONID
    AND pruAuthoriseCode.name = '_eClub_PBActivationAuthorizationCode'
LEFT JOIN
    PERSON_EXT_ATTRS pruAuthoriseCodeOverride
ON
    dataset.person_center = pruAuthoriseCodeOverride.PERSONCENTER
    AND dataset.person_id = pruAuthoriseCodeOverride.PERSONID
    AND pruAuthoriseCodeOverride.name = '_eClub_PBActivationAuthorizationCodeOverride'
LEFT JOIN
    PERSON_EXT_ATTRS pruActivateErrorCode
ON
    dataset.person_center = pruActivateErrorCode.PERSONCENTER
    AND dataset.person_id = pruActivateErrorCode.PERSONID
