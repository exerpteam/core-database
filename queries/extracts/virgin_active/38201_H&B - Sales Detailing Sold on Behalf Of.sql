 SELECT DISTINCT
     ...
     COALESCE(s.NET_AMOUNT, s.total_amount)                              AS "NON VAT Amount",
     s.total_amount                                                 AS "Total Sales Amount",
     s.PAYER_CENTER ||'p' || s.PAYER_ID                             AS "Payer ID",
     invp.fullname                                                  AS "Sales Employee Name",
     CASE WHEN invemp.center IS NOT NULL THEN  invemp.center || 'emp' || invemp.id ELSE  NULL END AS "Sales Employee Id",
     p.fullname                                                     AS "Sold on Behalf Of Name",
     CASE WHEN emp.center IS NOT NULL THEN  emp.center|| 'emp' || emp.id ELSE  NULL END           AS "Sold on Behalf Of Employee Id"
 FROM
     SALES_VW s
 LEFT JOIN
