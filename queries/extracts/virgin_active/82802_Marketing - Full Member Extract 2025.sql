...
-- Created by RG 060625 as requested by Emma Beevers for the details that marketing need for any member over 15 and only with email
 ...
 SELECT DISTINCT
     ...
     t1.MemberID    AS "Member ID",
         ...
         CASE 
                WHEN t1.CanEmail = 'true' THEN 'Yes'
                WHEN t1.CanEmail = 'false' THEN 'No'
                ELSE 'No'
         END AS "CanEmail",
 ...
 FROM
  ...
  and pg.GRANTER_SERVICE = 'CompanyAgreement'
         ...
         WHERE
             ...
             AND email.TXTVALUE IS NOT NULL
			 AND S.Sub_state <> '9'
			 AND floor(months_between(current_timestamp, P.BIRTHDATE) / 12) > 15
			 AND pr.GlobalID NOT IN
				(
'RCO_PRO-FEES',
'PT_BY_DD_-_EXPERT',
'PT_BY_DD_-_ICON',
...
'PT_BY_DD_-_PERSONAL',
'PT_BY_DD_-_REFORMER',
'PT_BY_DD_-_REFORMER_EXPERT',
'NUTRITION_BY_DD_-_EXPERT',
'NUTRITION_BY_DD_-_ICON',
'NUTRITION_BY_DD_-_MASTER',
'NUTRITION_BY_DD_-_PERSONAL',
'UK306',
'UK426',
'UK328',
'UK329',
