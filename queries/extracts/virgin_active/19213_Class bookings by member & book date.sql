 ...
 SELECT
     ...
     TRUNC(months_between(CURRENT_TIMESTAMP, p.BIRTHDATE)/12) "Member age",
     ...
     CASE
         ...
         ELSE par.STATE
     END "Booking status",
         CASE par.USER_INTERFACE_TYPE  WHEN 1 THEN 'STAFF' WHEN 2 THEN 'WEB' WHEN 3 THEN 'KIOSK' WHEN 4 THEN 'SCRIPT' WHEN 5 THEN 'API' WHEN 6 THEN 'MOBILE API' WHEN 0 THEN 'OTHER' END AS "User interface"
 FROM
     PARTICIPATIONS par
 JOIN
     PERSONS p
