...
CASE WHEN pr.CLEARINGHOUSE_ID = 803 THEN '04' ELSE '02' END as PAYMENT_METHOD,
c.EXTERNAL_ID,
agr.TEXT,
ART3.ID,
CASE WHEN art3.ID IS NULL THEN prs.OPEN_AMOUNT ELSE art3.AMOUNT END as OPEN_AMOUNT,
pr.REQ_DATE AS DUE_DATE,
Exerpro.longtodate(art1.ENTRY_TIME) as BOOK_DATE,
...
ON
  pr.CENTER = ar.CENTER
    AND pr.ID = ar.id  
LEFT JOIN CH_AND_PCC_LINK chpc
on chpcp.ID = pr.CLEARINGHOUSE_ID
...
and ((extract(MONTH FROM pr.req_date)  = extract(MONTH FROM ADD_MONTHS(LAST_DAY(SYSDATE),-1))  AND extract(YEAR FROM pr.req_date)  = extract(YEAR FROM ADD_MONTHS(LAST_DAY(SYSDATE),-1)))
 or (extract(MONTH FROM pr.req_date)  = extract(MONTH FROM ADD_MONTHS(LAST_DAY(SYSDATE),-2)) and extract(YEAR FROM pr.req_date)  = extract(YEAR FROM ADD_MONTHS(LAST_DAY(SYSDATE),-2))))
AND pr.STATE IS NOT NULL
AND ART.REF_TYPE = 'INVOICE'
AND PR.STATE IN (3,5,12,17)
AND (ART3.SUBID IS NOT NULL OR OPEN_AMOUNT  > 0)
AND art1.ID IS NOT NULL
GROUP BY p1.CENTER,
P1.ID, 
pr.CLEARINGHOUSE_ID, 
CASE WHEN pr.CLEARINGHOUSE_ID = 803 THEN '04' ELSE '02' END, 
agr.TEXT,
pr.REQ_DATE, 
Exerpro.longtodate(art1.ENTRY_TIME),
 vat.EXTERNAL_ID, 
 vat.RATE,
 c.EXTERNAL_ID,
CASE WHEN art3.ID IS NULL THEN prs.OPEN_AMOUNT ELSE art3.AMOUNT END,
ART3.ID
