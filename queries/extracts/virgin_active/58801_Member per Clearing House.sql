...
--  
 ...
 p.fullname as Name,
       CASE pag.STATE WHEN 1 THEN 'Created' WHEN 2 THEN 'Sent' WHEN 3 THEN 'Failed' WHEN 4 THEN 'OK' WHEN 5 THEN 'Ended, bank' WHEN 6 THEN 'Ended, clearing house' WHEN 7 THEN 'Ended, debtor'
          WHEN 8 THEN 'Cancelled, not sent' WHEN 9 THEN 'Cancelled, sent' WHEN 10 THEN 'Ended, creditor' WHEN 11 THEN 'No agreement (deprecated)'
          WHEN 12 THEN 'Cash payment (deprecated)' WHEN 13 THEN 'Agreement not needed (invoice payment)' WHEN 14 THEN 'Agreement information incomplete'
          WHEN 15 THEN 'Transfer' WHEN 16 THEN 'Agreement Recreated' WHEN 17 THEN  'Signature missing'  ELSE 'UNDEFINED' END AS "Status",
 pag.EXAMPLE_REFERENCE as KID,
 CASE pcc.RENEWAL_POLICY WHEN 5 THEN 'prepaid' WHEN 9 THEN 'postpaid' ELSE 'Undefined' END as "Agreement policy",
 a.name as scope,
 ...
 and pcc.id = pag.PAYMENT_CYCLE_CONFIG_ID
 JOIN
     AREAS a
 ON
     ch.SCOPE_ID = a.ID
