SELECT DISTINCT
    ...
    SUM(ACTIVE_CLOSING) + SUM(BLOCKED) + SUM(MISSING_AGREEMENT) + SUM(FROZEN) + SUM(DEFERRED_ONE_MONTH) + SUM(DEFERRED_TWO_MONTHS) + SUM
...
FROM
    (
        SELECT
            ...
            END ACTIVE_CLOSING,
            CASE
                WHEN SCL1.STATEID IN (2,4,8) AND SCL1.SUB_STATE IN (9)                    
                THEN -1
                ELSE 0
            END BLOCKED,
            CASE
            ...
            CASE
                WHEN SCL1.STATEID IN (8)
                    ...
                    AND memberAgreementCase.PERSONCENTER IS NULL
                    AND opAgreementCase.RELATIVECENTER IS NULL
                    AND SU.START_DATE <= add_months(longtodate(1456700400000),1)
                THEN -1
                ELSE 0
            ...
            CASE
                WHEN SCL1.STATEID IN (8)
                    ...
                    AND opAgreementCase.RELATIVECENTER IS NULL
                    AND SU.START_DATE > add_months(longtodate(1456700400000),1)
                    AND SU.START_DATE <= add_months(longtodate(1456700400000),2)
                THEN -1
                ELSE 0
        ...
        ON
            (
                ...
                AND SCL1.STATEID IN (2,
                                     4,7,8)
                AND SCL1.BOOK_START_TIME < 1456700400000 + (1000*60*60*24) + 2000
                AND SCL1.ENTRY_START_TIME < 1456700400000 + (1000*60*60*33) )
            AND (
                SCL1.ENTRY_END_TIME IS NULL
                OR SCL1.ENTRY_END_TIME > 1456700400000 + (1000*60*60*33) )
        INNER JOIN
            PRODUCTS PR
        ON
            (
        ...
        LEFT JOIN
            (
                ...
                WHERE
                    cc.MISSINGPAYMENT = 0
                    AND cc.STARTDATE < longtodateTZ(1456700400000, 'Europe/London') + 1
                    AND (
                        cc.CLOSED = 0
                        OR (
                            cc.CLOSED = 1
                            AND cc.CLOSED_DATETIME > 1456700400000 + (1000*60*60*24)))) memberAgreementCase
        ON
            ...
            (
                ...
                WHERE
                    ...
                    AND (
                        ...
                        OR (
                            cc.CLOSED = 1
                            AND cc.CLOSED_DATETIME > 1456700400000 + (1000*60*60*24)))) opAgreementCase
        ON
