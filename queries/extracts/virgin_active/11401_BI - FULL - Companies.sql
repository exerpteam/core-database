...
FROM
    (
        SELECT
            ...
            p.city                             AS "CITY",
            account_manager.external_id        AS "ACCOUNT_MANAGER_ID",
            CASE
                WHEN (p.status = 7)
                THEN 'DELETED'::text
                ELSE 'ACTIVE'::text
        ...
        ON
            (((
                ...
                AND (
                        rel.relativeid = account_manager.id))))
        LEFT JOIN
            zipcodes z
        ON
            ((((
                            z.country)::text = (p.country)::text)
                AND ((
                            z.zipcode)::text = (p.zipcode)::text)
                AND ((
                            z.city)::text = (p.city)::text))))
        LEFT JOIN
        ...
        ON
            (((
                ...
                AND (
                        mother_comapny_rel.relativeid = p.id)
                AND (
                        mother_comapny_rel.rtype = 6)
                AND (
                        mother_comapny_rel.status = 1))))
        LEFT JOIN
        ...
        ON
            (((
                ...
                AND (
                        mother_comapny.id = mother_comapny_rel.id))))
        WHERE
            ((
                    p.sex)::text = 'C'::text) ) biview
WHERE
    CAST(biview."HOME_CENTER_ID" AS INT) IN ($$Scope$$)