...
SELECT 
    c.NAME                                   Club,
    to_char(exerpro.longToDate(scl.ENTRY_START_TIME),'YYYY-MM-DD') MEMBER_CREATED,
    p.CENTER || 'p' || p.ID "Member ID",
    p.FULLNAME "Member Name",
    nvl2(par.CENTER,1,0) "Induction Booked",
    exerpro.longToDate(par.CREATION_TIME) "Date Booked",
    CASE
        WHEN par.STATE = 'PARTICIPATION'
        THEN 1
        ELSE 0
    END AS "Induction Attended",
    exerpro.longToDate(par.SHOWUP_TIME) "Date Attended",
...
(
	...
	and prod.PRIMARY_PRODUCT_GROUP_ID not in (219,239,242)
)