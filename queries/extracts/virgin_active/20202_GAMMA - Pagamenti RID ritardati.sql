...
and pr.req_date  <= ADD_MONTHS(LAST_DAY(SYSDATE),-2)
and extract(day from pr.req_date) <= 2
and Extract(MONTH FROM(longtodate(prs.LAST_MODIFIED))) = Extract(MONTH FROM(ADD_MONTHS(SYSDATE,-1)))
and Extract(YEAR FROM(longtodate(prs.LAST_MODIFIED))) = Extract(YEAR FROM(ADD_MONTHS(SYSDATE,-1)))
