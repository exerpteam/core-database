...
WITH
    ...
    (
        SELECT
            ...
            BIGINT)*1000 AS CreationFrom,
            CAST(extract(epoch FROM timezone('Europe/London', CAST($$ToDate$$ AS timestamptz) +
            interval '1 day')) AS BIGINT)*1000 AS CreationTo,
            id
        FROM
            centers c
        WHERE
...
on par.id = sub.CENTER
 ...
 ON
     (
         ...
         AND SCL1.STATEID IN (2,
                              4,8)
         AND SCL1.ENTRY_START_TIME >= par.CreationFrom
         AND (
             SCL1.ENTRY_END_TIME IS NULL
             OR SCL1.ENTRY_END_TIME < par.CreationTo))
 LEFT JOIN
 ...
 ON
     owner.center = Pend.PERSONCENTER
 ...
 WHERE
  --   ss.SUBSCRIPTION_CENTER IN ($$Scope$$)
     sub.CREATION_TIME BETWEEN  par.CreationFrom AND par.CreationTo
     AND NOT EXISTS
     (
         SELECT
             *
     ...
     (
         ...
         WHERE
             ...
             AND SCL1.ENTRY_START_TIME BETWEEN  par.CreationFrom AND par.CreationTo)
     AND EXISTS
     (
         SELECT
             *
...
CROSS JOIN params2 par2
 WHERE
         M.Pend_Cleared =  par2.y
 AND
         M.Commissionable = par2.n