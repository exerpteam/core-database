...
SELECT
    ...
    NULL " ",
    dataset.remit_rack_gross_amount "List Price To Member",
    ...
    SUM(NVL(remit_gross_amount, 0)) + SUM(NVL(membership_fee,0)) + SUM(NVL(admin_fee, 0)) + SUM(NVL(sales_commission, 0 )) + SUM(NVL(pru_Assessment, 0)) "Total Invoice",
    NULL " ",
    SUM(NVL(membership_fee,0)) + SUM(NVL(admin_fee, 0)) + SUM(NVL(sales_commission, 0)) + SUM(NVL(pru_Assessment, 0)) "Gross charge to Vitality",
    NULL " ",
...
DOWNGRADED', 5, 'EXTENDED', 6, 'TRANSFERRED',7,'REGRETTED',8,'CANCELLED',9,'BLOCKED' ,'UNKNOWN')
                ELSE NULL
            END                                           SUB_STATE,
        ...
        SELECT
            ...
            c.SHORTNAME || ' - ' || act.NAME || ': ' || TO_CHAR(exerpro.longtodateTZ(bk.STARTTIME, 'Europe/London'), 'YYYY-MM-DD HH24:MI') description,
