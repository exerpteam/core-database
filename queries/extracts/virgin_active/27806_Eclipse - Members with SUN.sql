 WITH
     ...
     (
         SELECT DISTINCT
             ...
             CASE ei.IDMETHOD  WHEN 1 THEN  'BARCODE'  WHEN 2 THEN  'MAGNETIC_CARD'  WHEN 3 THEN  'SSN'  WHEN 4 THEN  'RFID_CARD'  WHEN 5 THEN  'PIN'  WHEN 6 THEN  'ANTI DROWN'  WHEN 7 THEN  'QRCODE'  ELSE 'UNKNOWN' END as MembercardType,
             ei."identity"                                                   AS membercardid,
             pa.REF                                                          AS dd_referenceid,
             pa.CLEARINGHOUSE_REF                                            AS dd_contractid,
                         CASE  pa.clearinghouse  WHEN 1 THEN  '836301'  WHEN 601 THEN  '837394'  WHEN 201 THEN  '768308'  WHEN 401 THEN  '911116' END     AS dd_SUN,
             ...
             fh.FreezeTo,
             fh.FreezeReason,
             CASE WHEN fh.FreezeFrom IS NOT NULL THEN  freezeproduct.price ELSE  null END as freezeprice,
             ...
             CASE
                 WHEN sbp.type = 'DEBT_COLLECTION' THEN
                     'DEBT_COLLECTION'
                 ELSE
                     sbp.text
             END                                                                                                     AS blockreason,
         ...
         LEFT JOIN
            ( SELECT sbp.subscription_center,
                     sbp.subscription_id,
                     max(sbp.entry_time) max_entry
              FROM
                 subscription_blocked_period sbp
              WHERE sbp.state = 'ACTIVE' AND sbp.type IN ('DEBT_COLLECTION', 'EMPLOYEE', 'SANCTION')
              GROUP BY sbp.subscription_center,
                     sbp.subscription_id
            ) latest_sbp
         ON
                 ...
                 OR (has_clipcard.OWNER_CENTER IS NOT NULL
                      AND NOT EXISTS
                         ...
                         WHERE
                             ...
                             AND s2.OWNER_ID = p.ID
                             AND COALESCE(s2.end_date,current_timestamp) > COALESCE(s.END_DATE,current_timestamp)))
                 -- inactive member from last 24 months
                 OR (
                     p.status = 2
                     AND EXISTS
                     (
                         ...
                         WHERE
                             ...
                             AND scl.BOOK_END_TIME IS NULL
                             AND scl.STATEID=2
                             AND scl.ENTRY_START_TIME > datetolong(TO_CHAR(add_months(CURRENT_TIMESTAMP,-24),'YYYY-MM-DD HH24:MI')))
                     AND NOT EXISTS
                     (
                         SELECT
                             1
 ...
 --    COUNTRY ,
  ...
  --   CASE WHEN ADDONNAME IS NOT NULL THEN  DECODE(MEMBERSHIPTYPE ELSE  'DD', 1, 0 END, null)           AS ADDONCOUNT1 ,
  ...
  --   ADDONNAME2 ,
  --   CASE WHEN ADDONNAME2 IS NOT NULL THEN  DECODE(MEMBERSHIPTYPE ELSE  'DD', 1, 0 END, null)           AS ADDONCOUNT2 ,
  --   ADDONPRICE2 ,
  --   ADDONSTARTDATE2 ,
  --   ADDONENDDATE2 ,
   --  ADDONBINDINGENDDATE2 ,
  --   ADDONNAME3 ,
  --   CASE WHEN ADDONNAME3 IS NOT NULL THEN  DECODE(MEMBERSHIPTYPE ELSE  'DD', 1, 0 END, null)           AS ADDONCOUNT3 ,
   --  ADDONPRICE3 ,
   --  ADDONSTARTDATE3 ,
   --  ADDONENDDATE3 ,
   --  ADDONBINDINGENDDATE3 ,
   --  ADDONNAME4 ,
   --  CASE WHEN ADDONNAME4 IS NOT NULL THEN  DECODE(MEMBERSHIPTYPE ELSE  'DD', 1, 0 END, null)           AS ADDONCOUNT4 ,
  --   ADDONPRICE4 ,
   --  ADDONSTARTDATE4 ,
   --  ADDONENDDATE4 ,
   --  ADDONBINDINGENDDATE4 ,
   --  ADDONNAME5 ,
   --  CASE WHEN ADDONNAME5 IS NOT NULL THEN  DECODE(MEMBERSHIPTYPE ELSE  'DD', 1, 0 END, null)           AS ADDONCOUNT5 ,
   --  ADDONPRICE5 ,
   --  ADDONSTARTDATE5 ,
   --  ADDONENDDATE5 ,
