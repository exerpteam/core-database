...
FROM
    (
        SELECT
			...
			NewsLetters.TXTVALUE                                                                                            										AS "Can Email",
		...
		LEFT JOIN
            PERSON_EXT_ATTRS NewsLetters
        ON
            p.center=NewsLetters.PERSONCENTER
            AND p.id=NewsLetters.PERSONID
            AND NewsLetters.name='eClubIsAcceptingEmailNewsLetters'
            AND NewsLetters.TXTVALUE IS NOT NULL
        JOIN
            CENTERS c
        ON
            c.id = p.CENTER
