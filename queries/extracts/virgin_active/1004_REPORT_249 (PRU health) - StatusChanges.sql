 ...
 SELECT DISTINCT
     ...
     END AS "Club",
     /* bigint */
     trim(pruRef.TXTVALUE) "EntityNumber",
     /* varchar(20) */
     trim(p.EXTERNAL_ID) "ExerpMemberID",
     /* varchar(20) */
     trim(con.TXTVALUE) "LegacyMemberID",
     i1.EffectiveDate AS "EFFECTIVEDATE",
     i1.TRANSFERRED_CENTER AS "TRANSFERREDCLUB",
     i1.RecordType AS "RECORDTYPE"
 FROM
     (
         /* Transferred members */
         SELECT
         ...
         ON
             ...
             AND scl.BOOK_START_TIME BETWEEN datetolongc(to_char(date_trunc('day',to_date(getcentertime(s.CENTER), 'YYYY-MM-DD') -  interval '1 day'),'YYYY-MM-DD HH24:MI'),s.CENTER)
                                         AND datetolongc(to_char(date_trunc('day',to_date(getcentertime(s.CENTER), 'YYYY-MM-DD')),'YYYY-MM-DD HH24:MI'),s.CENTER)-1
         UNION ALL
         /* Rejoiners */
         SELECT
             s.OWNER_CENTER,
         ...
         ON
             ...
             AND pg.NAME = 'Pru'
         WHERE
             s.CREATION_TIME BETWEEN datetolongc(to_char(date_trunc('day',to_date(getcentertime(s.CENTER), 'YYYY-MM-DD') -  interval '1 day'),'YYYY-MM-DD HH24:MI'),s.CENTER)
                                         AND datetolongc(to_char(date_trunc('day',to_date(getcentertime(s.CENTER), 'YYYY-MM-DD')),'YYYY-MM-DD HH24:MI'),s.CENTER)-1
             ...
             (
                 SELECT
                     ...
                     s2.id,
                     s2.SUB_STATE,
                     DATE_TRUNC('day',s.START_DATE) - DATE_TRUNC('day',s2.END_DATE) days_between
                 FROM
                     SUBSCRIPTIONS s2
         ...
         SELECT
             ...
             'Cancel' RecordType ,
             s.START_DATE,
             date_trunc('day',to_date(getcentertime(s.CENTER), 'YYYY-MM-DD')) EffectiveDate
         FROM
             SUBSCRIPTIONS s
         JOIN RELATIVES rel
         ON
         ...
         WHERE
             ...
             s.SUB_STATE NOT IN (5,6, 10)
             /* They should have been created the day before yesterday else just a adjustment during the day */
             AND s.CREATION_TIME < datetolongc(to_char(date_trunc('day',to_date(getcentertime(s.CENTER), 'YYYY-MM-DD') -  interval '1 day'),'YYYY-MM-DD HH24:MI'),s.CENTER)
             AND date_trunc('day',longToDate(scl.BOOK_START_TIME)) = date_trunc('day',to_date(getcentertime(s.CENTER), 'YYYY-MM-DD'))
     )
     i1
     /****************************/
