...
-- https://clublead.atlassian.net/browse/ST-11051
 /*
 VA free period due to corona virus problem
 */
 WITH
     params AS
     (
 ...
 SELECT
         ...
         CASE s.SUB_STATE WHEN 1 THEN 'NONE' WHEN 2 THEN 'AWAITING_ACTIVATION' WHEN 3 THEN 'UPGRADED' WHEN 4 THEN 'DOWNGRADED' WHEN 5 THEN 'EXTENDED' WHEN 6 THEN 'TRANSFERRED' WHEN 7 THEN 'REGRETTED' WHEN 8 THEN 'CANCELLED' WHEN 9 THEN 'BLOCKED' WHEN 10 THEN 'CHANGED' ELSE 'Undefined' END AS "Subscription sub state",
         (CASE WHEN s.BINDING_END_DATE > current_date THEN s.BINDING_PRICE ELSE s.SUBSCRIPTION_PRICE END) AS "Subscription price",
 ...
 FROM
     (
         SELECT DISTINCT
             ...
             'COVID-19 measures (ST-11051)'    AS Text,
             a.SubscriptionName,
             a.TransferDate,
             COALESCE(
                        (
         ...
         FROM
             (
                 ...
                 JOIN
                     centers c ON s.center = c.id and c.COUNTRY = 'GB'
                 CROSS JOIN
                     params
                 JOIN
                 ...
                 WHERE
                     ...
                     AND pr.GLOBALID NOT IN ('UK316', 'UK317')
 ...
 ON
     ...
     AND s.id = b.id
 JOIN
     persons p ON s.OWNER_CENTER = p.CENTER AND s.OWNER_ID = p.ID
 WHERE
