...
SELECT
    ...
    i1.CLUB,
CONCAT(CONCAT(cast(i1.OWNER_CENTER as char(3)),'p'), cast(i1.OWNER_ID as varchar(8))) as personId,
CONCAT(CONCAT(cast(i1.CENTER as char(3)),'ss'), cast(i1.ID as varchar(8))) as subscriptionId,
 i1.DATE_JOINED,
	i1.START_DATE,
i1.BINDING_END_DATE,
	i1.END_DATE,
...
i1.MEMBERSHIP,
 ...
 SUM(
   ...
   NVL(ccd.CODE,MAX(camp.CODE)) AS code,
    MAX(
        CASE
            WHEN attivaz.center IS NOT NULL
            THEN invl.TOTAL_AMOUNT
            ELSE 0
        END ) ATTIVAZIONI,
        CASE WHEN sst.CODE = NVL(ccd.CODE,MAX(camp.CODE)) THEN NULL ELSE sst.CODE END as CODICESTARTUP
FROM
    (
        SELECT
            /*+ NO_BIND_AWARE */
            ...
            ss.OWNER_CENTER,
            ss.OWNER_ID,
            sub.STARTUP_FREE_PERIOD_ID,
            centre.SHORTNAME                      club,
            TO_CHAR(sub.start_date, 'DD-MM-YYYY') start_date,
			TO_CHAR(sub.end_date, 'DD-MM-YYYY') end_date,
 			TO_CHAR(sub.binding_end_date, 'DD-MM-YYYY') binding_end_date,
            sub.CAMPAIGN_CODE_ID,
            CASE
            ...
            TO_CHAR(exerpro.longtodateTZ(sub.CREATION_TIME, 'Europe/Copenhagen'), 'DD-MM-YYYY') DATE_JOINED,
        ...
        WHERE
            ...
            (
                ...
                WHERE
                    ...
                    AND (
                        ...
                        OR oldSub.ID <> sub.ID)
                    AND oldSub.END_DATE + 30 > exerpro.longtodateTZ(sub.CREATION_TIME, 'Europe/Copenhagen')
                    AND (
                        oldSub.STATE != 5
                        AND NOT(
                            oldSub.STATE = 3
            ...
            (
                ...
                WHERE
                    ...
                    AND pgl.PRODUCT_GROUP_ID IN (5401,5402,246,248)) 
			AND NOT EXISTS
            (
                SELECT
                    *
                ...
                WHERE
                    pgl.PRODUCT_CENTER = prod.CENTER
                    AND pgl.PRODUCT_ID = prod.ID
					AND pgl.PRODUCT_GROUP_ID IN (5406, 5407)) ) i1
LEFT JOIN
    INVOICES inv
ON
...
ON
    ...
    AND invl.ID = inv.ID
    AND exerpro.longToDate(inv.TRANS_TIME) BETWEEN i1.sales_date AND i1.sales_date + 29
left JOIN
    PRODUCTS prod
ON
    ...
    PRODUCTS af
ON
    af.CENTER = invl.PRODUCTCENTER
    AND af.ID = invl.PRODUCTID
    AND af.PRIMARY_PRODUCT_GROUP_ID IN (5417)
	
	left JOIN
    PRODUCTS attivaz
ON
    attivaz.CENTER = invl.PRODUCTCENTER
    AND attivaz.ID = invl.PRODUCTID
    AND attivaz.GLOBALID = 'ATTIVAZIONE'
	
	LEFT JOIN 
	(SELECT pu.TARGET_CENTER, pu.TARGET_ID, pu.TARGET_SUBID, cc.CODE
	FROM PRIVILEGE_USAGES pu 
           INNER JOIN  CAMPAIGN_CODES cc
        
        ON
            pu.CAMPAIGN_CODE_ID = cc.ID
            AND pu.TARGET_SERVICE in ('InvoiceLine','SubscriptionPrice')
            AND pu.PRIVILEGE_TYPE = 'PRODUCT') camp
 ON
            invl.CENTER = camp.TARGET_CENTER
            AND invl.ID = camp.TARGET_ID
            AND invl.SUBID = camp.TARGET_SUBID
	LEFT JOIN CAMPAIGN_CODES ccd
	ON ccd.ID = i1.CAMPAIGN_CODE_ID

    left outer join CAMPAIGN_CODES sst
	ON sst.CAMPAIGN_ID = i1.STARTUP_FREE_PERIOD_ID

WHERE invl.ID IS NOT NULL
