...
(pr.CLEARINGHOUSE_ID IN (803,
	...
	'dd/mm/YYYY')))
	and pr.req_date <= ADD_MONTHS(LAST_DAY(SYSDATE),
	-2)
	and extract(DAY
FROM
	pr.req_date) <= 3
	...
	and Extract(MONTH
FROM
	(Exerpro.longtodate(prs.PAID_STATE_LAST_ENTRY_TIME))) = Extract(MONTH
FROM
	(ADD_MONTHS(SYSDATE,
	-1)))
	and Extract(YEAR
FROM
	(Exerpro.longtodate(prs.PAID_STATE_LAST_ENTRY_TIME))) = Extract(YEAR
FROM
	(ADD_MONTHS(SYSDATE,
	-1)))
	and Extract(MONTH
FROM
	(Exerpro.longtodate(art3.ENTRY_TIME))) = Extract(MONTH
FROM
	(ADD_MONTHS(SYSDATE,
	-1)))
	and Extract(YEAR
FROM
	(Exerpro.longtodate(art3.ENTRY_TIME))) = Extract(YEAR
FROM
	(ADD_MONTHS(SYSDATE,
	-1)))
