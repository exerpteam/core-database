...
FROM
    (
        SELECT
            ...
            TO_CHAR(exerpro.longtodateTZ(SU.CREATION_TIME, 'Europe/Copenhagen'), 'YYYY-MM-DD') CREATION,
            TO_CHAR(exerpro.longtodateTZ(SCL1.ENTRY_START_TIME, 'Europe/Copenhagen'), 'YYYY-MM-DD HH24:MI') ENTRY_START,
            TO_CHAR(exerpro.longtodateTZ(SCL1.ENTRY_END_TIME, 'Europe/Copenhagen'), 'YYYY-MM-DD HH24:MI') ENTRY_END,
            CASE
            ...
            CASE
                WHEN (SCL1.SUB_STATE IN (9))
                    AND SCL1.ENTRY_START_TIME < exerpro.datetolong(TO_CHAR(TRUNC(exerpro.longtodateTZ($$CutDate$$,
                    'Europe/Copenhagen'), 'mm'), 'YYYY-MM-DD HH24:MI'))
                THEN -1
                ELSE 0
            END BLOCKED,
            CASE
                WHEN SCL1.SUB_STATE IN (9)
                THEN TO_CHAR(exerpro.longtodateTZ(SCL1.ENTRY_START_TIME, 'Europe/Copenhagen'), 'YYYY-MM-DD HH24:MI')
                ELSE NULL
        ...
        ON
            (
                ...
                AND SCL1.ENTRY_START_TIME < $$CutDate$$ + (1000*60*60*33) )
            AND (
                SCL1.ENTRY_END_TIME IS NULL
                OR SCL1.ENTRY_END_TIME > $$CutDate$$ + (1000*60*60*33) )
        INNER JOIN
            PRODUCTS PR
        ON
            (
        ...
        LEFT JOIN
            (
                ...
                WHERE
                    cc.MISSINGPAYMENT = 0
                    AND cc.STARTDATE < exerpro.longtodateTZ($$CutDate$$, 'Europe/Copenhagen') + 1
                    AND (
                        cc.CLOSED = 0
                        OR (
                            cc.CLOSED = 1
                            AND cc.CLOSED_DATETIME > $$CutDate$$ + (1000*60*60*24)))) memberAgreementCase
        ON
            ...
            (
                ...
                WHERE
                    ...
                    AND (
                        ...
                        OR (
                            cc.CLOSED = 1
                            AND cc.CLOSED_DATETIME > $$CutDate$$ + (1000*60*60*24)))) opAgreementCase
        ON
