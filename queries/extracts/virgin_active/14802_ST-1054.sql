...
FROM
    (
        SELECT
            ...
            eClub_PhoneSMS.txtvalue phone,
            eClub_Email.txtvalue email,
            count(atts.start_time) attends,
            longToDate(max(atts.start_time)) last_attend,
            CASH_TO_INCLUDE
        FROM
            (
                /********************* DRAFT 2 PART BEGIN **************************/
                ...
                FROM
                    (
                        ...
                        FROM
                            (
                            /********************* START WD1 ***************************/
                            /*select s.CENTER SUB_CENTER,s.ID SUB_ID,1 LIVE_CLOSING,1 WINDOW from SUBSCRIPTIONS s where s.CENTER = 102 -- and s.id = 634*/
                             WITH
     params AS
     (
         SELECT
     ...
     SUB_ID
                        ...
                        LEFT JOIN
                            (
                                ...
                                WHERE
                                    cc.MISSINGPAYMENT = 1
                                    AND cc.STARTDATE <= add_months(exerpro.longtodateTZ($$CutDate$$, 'Europe/London'),-1)
        ...
        ON
            ...
            AND p.id = s.OWNER_ID
        LEFT JOIN person_ext_attrs eClub_PhoneSMS ON eClub_PhoneSMS.personcenter = p.center AND eClub_PhoneSMS.personid = p.id AND eClub_PhoneSMS.name = '_eClub_PhoneSMS'
        LEFT JOIN person_ext_attrs eClub_Email ON eClub_Email.personcenter = p.center AND eClub_Email.personid = p.id AND eClub_Email.name = '_eClub_Email'
        LEFT join ATTENDS atts on atts.PERSON_CENTER = p.CENTER and atts.person_id = p.ID and atts.start_time between dateToLong(to_char(sysdate - 90,'YYYY-MM-dd HH24:MI')) and     
        dateToLong(to_char(sysdate ,'YYYY-MM-dd HH24:MI'))   
        JOIN
            CENTERS c
        ON
            c.id = p.CENTER
        ...
        WHERE
            i2.SHOWS_IN_ETR > 0
        GROUP BY
            eClub_PhoneSMS.txtvalue ,
            eClub_Email.txtvalue ,
            c.SHORTNAME ,
            p.CENTER,
            p.ID ,
