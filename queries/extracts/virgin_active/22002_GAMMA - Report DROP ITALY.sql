...
--  
 ...
 INNER JOIN CENTERS ON S.CENTER = CENTERS.ID
 LEFT JOIN
     INVOICELINES invl
 ON
 ...
 WHERE
  QUN.CENTER IN (101,102,103,104,105,106,107,108,201,202,203,204,205,206,207,208,209,210,211,212,213,214,215,216,217,218,219,220,221,222,223,224,225,226,227,228,229,230,231,232,233)
 ...
 INNER JOIN
 (select concat(concat(cast(qun.center as char(3)),'p'),cast(qun.id as varchar(10))) as PERSONID, QC.STARTDATE, QC.STOPDATE, longToDate(qun.LOG_TIME) as answerDate, case when q.QUESTIONS IS NOT NULL AND qa.NUMBER_ANSWER IS NOT NULL AND qa.QUESTION_ID IS NOT NULL 
 then 
 CAST(COALESCE(
      (xpath('//question[id/text()='||qa.QUESTION_ID ||']/options/option[id/text()='||qa.NUMBER_ANSWER ||']/optionText/text()', xmlparse(document convert_from(q.QUESTIONS,'UTF-8'))))[1]) AS VARCHAR) end
...
-- as LEAVING_REASON, QUN.ID, QUN.CENTER, QUN.SUBID, longToDate(qun.LOG_TIME) AS ANSWER_DATE
 ...
 ON
     q.ID = QC.QUESTIONNAIRE
 where   QUN.CENTER IN (101,102,103,104,105,106,107,108,201,202,203,204,205,206,207,208,209,210,211,212,213,214,215,216,217,218,219,220,221,222,223,224,225,226,227,228,229,230,231,232,233)
 and q.questions is not null
 AND S.END_DATE >= :End_Date_From AND S.END_DATE <= :End_Date_To
 ...
 where
 P.CENTER IN (101,102,103,104,105,106,107,108,201,202,203,204,205,206,207,208,209,210,211,212,213,214,215,216,217,218,219,220,221,222,223,224,225,226,227,228,229,230,231,232,233)
 AND S.END_DATE >= :End_Date_From AND S.END_DATE <= :End_Date_To
 AND S.SUB_STATE IN(1,9)
 ...
 WHERE P.CENTER IN (101,102,103,104,105,106,107,108,201,202,203,204,205,206,207,208,209,210,211,212,213,214,215,216,217,218,219,220,221,222,223,224,225,226,227,228,229,230,231,232,233)
 AND S.END_DATE >= :End_Date_From AND S.END_DATE <= :End_Date_To
 ...
 AND (( Extract(month from S.END_DATE)::integer = Extract(month FROM CURRENT_DATE)::integer
 AND Extract(year from S.END_DATE)::integer = Extract(year FROM CURRENT_DATE)::integer) or ((Extract(month from S.END_DATE)::integer != Extract(month FROM CURRENT_DATE)::integer OR   Extract(year from S.END_DATE)::integer! = Extract(year FROM CURRENT_DATE)::integer)) AND p.STATUS NOT IN(1,3,4))
 ) t1
 group by
 CLUB,
 NUMEROABBONAMENTO,
