WITH
    ...
    (
        SELECT DISTINCT
            ...
            -- person creation date is first membership start date. if it is null then creation
            -- date
            TO_CHAR(COALESCE(first_sub.start_date, to_date(personCreation.txtvalue, 'YYYY-MM-DD')),
            'YYYY-MM-DD')                      AS personCreationDate,
            ...
            --DECODE(ei.IDMETHOD, 1, 'BARCODE', 2, 'MAGNETIC_CARD', 3, 'SSN', 4, 'RFID_CARD', 5, '
            -- PIN', 6, 'ANTI DROWN', 7, 'QRCODE', 'UNKNOWN')
            -- AS MembercardType,
            CASE
                WHEN ei_barcode.ID IS NOT NULL
            ...
            CASE
                ...
                THEN
                    CASE sbp.type
                        WHEN 'DEBT_COLLECTION'
                        THEN 'DEBT_COLLECTION'
                        ELSE sbp.text
                    END
                ELSE NULL
            END AS blockreason,
            TO_CHAR(longToDateC(last_attend.last_attendtime, last_attend.person_center),
            'yyyy-MM-dd HH24:MI:SS') AS LastVisitTime,
            CASE
            ...
            OR  ( ccc.id IS NOT NULL
                AND EXISTS
                    (
                        ...
                        AND scl.STATEID=2
                        AND scl.ENTRY_START_TIME > datetolong(TO_CHAR(add_months(CURRENT_TIMESTAMP,
                ...
                AND NOT EXISTS
                    (
                        ...
                        AND COALESCE(s2.end_date,CURRENT_TIMESTAMP) > COALESCE(s.END_DATE,
                            CURRENT_TIMESTAMP)))
                -- members having clip card
            OR  ( has_clipcard.OWNER_CENTER IS NOT NULL
                AND NOT EXISTS
                    (
                        SELECT
                            1
                ...
                AND ( pay_for.PAYER_CENTER IS NOT NULL
                    OR  EXISTS
                        (
                            ...
                            AND scl.BOOK_END_TIME IS NULL
                            AND scl.STATEID=2
                            AND scl.ENTRY_START_TIME > datetolong(TO_CHAR(add_months
                                (CURRENT_TIMESTAMP,-1),'YYYY-MM-DD HH24:MI'))))
                AND NOT EXISTS
                    (
                        SELECT
                            1
    ...
    (
        SELECT
            v_main.* ,
            LEAD(ADDONNAME,1) OVER (PARTITION BY PERSONID, MEMBERSHIPID, MEMBERCARDID ORDER BY
            ADDONSTARTDATE, ADDONNAME) AS ADDONNAME2 ,
            LEAD(ADDONPRICE,1) OVER (PARTITION BY PERSONID, MEMBERSHIPID, MEMBERCARDID ORDER BY
            ADDONSTARTDATE, ADDONNAME) AS ADDONPRICE2 ,
            LEAD(ADDONSTARTDATE,1) OVER (PARTITION BY PERSONID, MEMBERSHIPID, MEMBERCARDID ORDER BY
            ADDONSTARTDATE, ADDONNAME) AS ADDONSTARTDATE2 ,
            LEAD(ADDONENDDATE,1) OVER (PARTITION BY PERSONID, MEMBERSHIPID, MEMBERCARDID ORDER BY
            ADDONSTARTDATE, ADDONNAME) AS ADDONENDDATE2 ,
            LEAD(ADDONBINDINGENDDATE,1) OVER (PARTITION BY PERSONID, MEMBERSHIPID, MEMBERCARDID
            ORDER BY ADDONSTARTDATE, ADDONNAME) AS ADDONBINDINGENDDATE2
            --
            ,
            LEAD(ADDONNAME,2) OVER (PARTITION BY PERSONID, MEMBERSHIPID, MEMBERCARDID ORDER BY
            ADDONSTARTDATE, ADDONNAME) AS ADDONNAME3 ,
            LEAD(ADDONPRICE,2) OVER (PARTITION BY PERSONID, MEMBERSHIPID, MEMBERCARDID ORDER BY
            ADDONSTARTDATE, ADDONNAME) AS ADDONPRICE3 ,
            LEAD(ADDONSTARTDATE,2) OVER (PARTITION BY PERSONID, MEMBERSHIPID, MEMBERCARDID ORDER BY
            ADDONSTARTDATE, ADDONNAME) AS ADDONSTARTDATE3 ,
            LEAD(ADDONENDDATE,2) OVER (PARTITION BY PERSONID, MEMBERSHIPID, MEMBERCARDID ORDER BY
            ADDONSTARTDATE, ADDONNAME) AS ADDONENDDATE3 ,
            LEAD(ADDONBINDINGENDDATE,2) OVER (PARTITION BY PERSONID, MEMBERSHIPID, MEMBERCARDID
            ORDER BY ADDONSTARTDATE, ADDONNAME) AS ADDONBINDINGENDDATE3
            --
            ,
            LEAD(ADDONNAME,3) OVER (PARTITION BY PERSONID, MEMBERSHIPID, MEMBERCARDID ORDER BY
            ADDONSTARTDATE, ADDONNAME) AS ADDONNAME4 ,
            LEAD(ADDONPRICE,3) OVER (PARTITION BY PERSONID, MEMBERSHIPID, MEMBERCARDID ORDER BY
            ADDONSTARTDATE, ADDONNAME) AS ADDONPRICE4 ,
            LEAD(ADDONSTARTDATE,3) OVER (PARTITION BY PERSONID, MEMBERSHIPID, MEMBERCARDID ORDER BY
            ADDONSTARTDATE, ADDONNAME) AS ADDONSTARTDATE4 ,
            LEAD(ADDONENDDATE,3) OVER (PARTITION BY PERSONID, MEMBERSHIPID, MEMBERCARDID ORDER BY
            ADDONSTARTDATE, ADDONNAME) AS ADDONENDDATE4 ,
            LEAD(ADDONBINDINGENDDATE,3) OVER (PARTITION BY PERSONID, MEMBERSHIPID, MEMBERCARDID
            ORDER BY ADDONSTARTDATE, ADDONNAME) AS ADDONBINDINGENDDATE4
            --
            ,
            LEAD(ADDONNAME,4) OVER (PARTITION BY PERSONID, MEMBERSHIPID, MEMBERCARDID ORDER BY
            ADDONSTARTDATE, ADDONNAME) AS ADDONNAME5 ,
            LEAD(ADDONPRICE,4) OVER (PARTITION BY PERSONID, MEMBERSHIPID, MEMBERCARDID ORDER BY
            ADDONSTARTDATE, ADDONNAME) AS ADDONPRICE5 ,
            LEAD(ADDONSTARTDATE,4) OVER (PARTITION BY PERSONID, MEMBERSHIPID, MEMBERCARDID ORDER BY
            ADDONSTARTDATE, ADDONNAME) AS ADDONSTARTDATE5 ,
            LEAD(ADDONENDDATE,4) OVER (PARTITION BY PERSONID, MEMBERSHIPID, MEMBERCARDID ORDER BY
            ADDONSTARTDATE, ADDONNAME) AS ADDONENDDATE5 ,
            LEAD(ADDONBINDINGENDDATE,4) OVER (PARTITION BY PERSONID, MEMBERSHIPID, MEMBERCARDID
            ORDER BY ADDONSTARTDATE, ADDONNAME) AS ADDONBINDINGENDDATE5
            --
            ,
            ROW_NUMBER() OVER (PARTITION BY PERSONID, MEMBERSHIPID, MEMBERCARDID ORDER BY
            ADDONSTARTDATE, ADDONNAME) AS ADDONSEQ
        FROM
            v_main
    )
SELECT
    ...
    BIRTHDATE ,
    GENDER ,
    REPLACE(REPLACE(REPLACE(REPLACE(TRIM(BOTH ' ' FROM ADDRESSLINE1), CHR(13), '[CR]'), CHR(10),
    '[LF]'),';',''),'"','[qt]') AS ADDRESSLINE1 ,
    REPLACE(REPLACE(REPLACE(REPLACE(TRIM(BOTH ' ' FROM ADDRESSLINE2), CHR(13), '[CR]'), CHR(10),
    '[LF]'),';',''),'"','[qt]') AS ADDRESSLINE2 ,
    REPLACE(REPLACE(REPLACE(REPLACE(TRIM(BOTH ' ' FROM ADDRESSLINE3), CHR(13), '[CR]'), CHR(10),
    '[LF]'),';',''),'"','[qt]')                         AS ADDRESSLINE3 ,
    ...
    MEMBERCARDTYPE ,
    REPLACE(REPLACE(TRIM(BOTH ' ' FROM MEMBERCARDID), ';', '') ,chr(63),'') AS MEMBERCARDID ,
    DD_REFERENCEID ,
    DD_CONTRACTID ,
    DD_BANKREG ,
