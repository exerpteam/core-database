...
WITH
    ...
    (
        SELECT
	              ...
	              CAST(extract(epoch from timezone('Europe/London',CAST(date_trunc('month', current_timestamp) - interval '1 month ' AS TIMESTAMP))) AS BIGINT)*1000 AS FROMDATE,
				  CAST(extract(epoch from timezone('Europe/London',CAST(date_trunc('month', current_timestamp)  AS TIMESTAMP))) AS BIGINT)*1000 AS TODATE
    )
 SELECT
     c.SHORTNAME AS "CLUB",
    ...
    -- SUM(admin_fee) "Admin Fee",
     ...
     SUM(COALESCE(remit_gross_amount, 0)) + SUM(COALESCE(membership_fee,0)) + SUM(COALESCE(sales_commission, 0
     )) + SUM(COALESCE(pru_Assessment, 0)) "Total Invoice",
     NULL " ",
     SUM(COALESCE(membership_fee,0)) + SUM(COALESCE(sales_commission, 0)) + SUM(COALESCE(pru_Assessment, 0))
 ...
 DOWNGRADED'  WHEN 5 THEN  'EXTENDED'  WHEN 6 THEN  'TRANSFERRED' WHEN 7 THEN 'REGRETTED' WHEN 8 THEN 'CANCELLED' WHEN 9 THEN 'BLOCKED'
             ...
             CASE
                 WHEN sales.PRODUCT_TYPE IN ('SUBS_PERIOD',
                     ...
                     AND instr(sales.PRODUCT_NAME, '50%') > 0
                 THEN ROUND(SUM(sales.TOTAL_AMOUNT) * 0.08 * 2, 2)
                 WHEN sales.PRODUCT_TYPE IN ('SUBS_PERIOD',
                                             'SUBS_PRORATA')
                     AND (cag.EXTERNAL_ID IS NULL
                         OR cag.EXTERNAL_ID NOT LIKE 'VITPLNY%')
                     AND instr(sales.PRODUCT_NAME, '25%') > 0
                 THEN ROUND(SUM(sales.TOTAL_AMOUNT) * 0.08 * 4, 2)
                 ELSE NULL
             END membership_fee,
             CASE
                 WHEN sales.PRODUCT_TYPE IN ('SUBS_PERIOD',
