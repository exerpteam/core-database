 ...
 CASE pr.STATE  WHEN 1 THEN  'PS_NEW'  WHEN 2 THEN  'PS_SENT'  WHEN 3 THEN  'PS_DONE'  WHEN 5 THEN  'PS_REJECTED_BY_CLEARINGHOUSE'  WHEN 12 THEN  'PS_FAIL_NO_CREDITOR'  WHEN 17 THEN 'PS_FAIL_REJ_DEB_REVOKED' END "Stato", PR.STATE,
 ...
 and iz.id = p1.id
 WHERE
 pr.center IN(select c.ID from CENTERS c where  c.COUNTRY = 'IT') and extract(month from pr.req_date) = EXTRACT(month FROM  ADD_MONTHS(CURRENT_TIMESTAMP,-13))
 and extract(year from  pr.req_date) = extract(YEAR FROM ADD_MONTHS(CURRENT_TIMESTAMP,-13))
