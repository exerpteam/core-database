WITH
    ...
    (
        SELECT
            CAST(datetolongC(TO_CHAR(CAST(:FromDate AS DATE), 'YYYY-MM-dd HH24:MI'),c.id) AS BIGINT
            )    AS FROMDATE,
            c.id AS CENTER_ID,
            CAST((datetolongC(TO_CHAR(CAST(:ToDate AS DATE) + INTERVAL '1 DAY' - INTERVAL
            '1 SECOND', 'YYYY-MM-dd HH24:MI:SS'),c.id)) AS BIGINT ) AS TODATE
        FROM
            centers c
        WHERE
            country = 'IT'
