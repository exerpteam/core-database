SELECT
    TO_CHAR(longtodateTZ($$BookDate$$, 'Europe/London'), 'YYYY-MM-DD') "Date",
    extId MeasureID,
...
FROM
    (
        ...
        JOIN
            CENTERS club
        ON
            acc.CENTER = club.ID and club.COUNTRY = 'GB'
        LEFT JOIN
            (
                SELECT
                ...
                WHERE
                    art.AMOUNT <> 0
                    AND art.TRANS_TIME >= $$BookDate$$ - (1000*60*60)
                    AND art.TRANS_TIME < $$BookDate$$ + (1000*60*60*23)
                    AND NOT(
                        art.TRANSFERRED = 1
