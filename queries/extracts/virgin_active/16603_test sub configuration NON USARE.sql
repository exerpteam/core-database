 ...
 SELECT
     ...
     FIRST_VALUE(t.description) OVER (PARTITION BY mpr.DEFINITION_KEY ORDER BY mpr.SCOPE_TYPE desc) contract,
 --      -- Added by VA
 --      CASE
 ...
 --      -- End Change by VA
     ...
     CASE
         ...
         THEN CAST((xpath('FREEZELIMIT/FREEZEAPPLYPERIOD/FREEZEDURATION/@LENGTH', xmlparse(document convert_from(st.FREEZELIMIT,'UTF8'))))[1] AS VARCHAR) || ' ' || CAST((xpath('FREEZELIMIT/FREEZEAPPLYPERIOD/FREEZEDURATION/@UNIT', xmlparse(document convert_from(st.FREEZELIMIT,'UTF8'))))[1] AS VARCHAR)
		 ELSE NULL
     END AS "Within a period of",
     CASE
         WHEN st.FREEZELIMIT IS NOT NULL
         THEN CAST((xpath('FREEZELIMIT/@MINDURATION', xmlparse(document convert_from(st.FREEZELIMIT,'UTF8'))))[1] AS VARCHAR)
		 ELSE NULL
     END AS "Min duration",
     CASE
         WHEN st.FREEZELIMIT IS NOT NULL
		 THEN CAST((xpath('FREEZELIMIT/@MINDURATION_UNIT', xmlparse(document convert_from(st.FREEZELIMIT,'UTF8'))))[1] AS VARCHAR)
         ELSE NULL
     END AS "Min duration Unit",
     CASE
         WHEN st.FREEZELIMIT IS NOT NULL
         THEN CAST((xpath('FREEZELIMIT/@MAXDURATION', xmlparse(document convert_from(st.FREEZELIMIT,'UTF8'))))[1] AS VARCHAR)
		 ELSE NULL
     END AS "Max duration",
     CASE
         WHEN st.FREEZELIMIT IS NOT NULL
         THEN CAST((xpath('FREEZELIMIT/@MAXDURATION_UNIT', xmlparse(document convert_from(st.FREEZELIMIT,'UTF8'))))[1] AS VARCHAR)
		 ELSE NULL
