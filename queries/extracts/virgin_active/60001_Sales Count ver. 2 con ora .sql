...
SELECT
 ...
 WHERE
     ...
     AND LongToDatec(SU.CREATION_TIME, su.center) >= CAST($$SalesFromDate$$ AS DATE)
     AND LongToDatec(SU.CREATION_TIME, su.center) < CAST($$SalesToDate$$ AS DATE) + interval '1 day')
     AND ss.TYPE IN(1,2)
     AND (
         spg.CENTER IS NULL
