 ...
 SELECT
     ...
     COALESCE(camps."CODE", cc_startup.CODE)             AS CAMPAIGN_CODE,
     ...
     CASE  p.STATUS  WHEN 0 THEN 'LEAD'  WHEN 1 THEN 'ACTIVE'  WHEN 2 THEN 'INACTIVE'  WHEN 3 THEN 'TEMPORARYINACTIVE'  WHEN 4 THEN 'TRANSFERED'  WHEN 5 THEN 
          'DUPLICATE'  WHEN 6 THEN 'PROSPECT'  WHEN 7 THEN 'DELETED' WHEN 8 THEN  'ANONYMIZED'  WHEN 9 THEN  'CONTACT'  ELSE 'UNKNOWN' END AS  MEMBER_STATUS,
     CASE  su.STATE  WHEN 2 THEN 'ACTIVE'  WHEN 3 THEN 'ENDED'  WHEN 4 THEN 'FROZEN'  WHEN 7 THEN 'WINDOW'  WHEN 8 THEN 'CREATED' ELSE 'UNKNOWN' END AS  SUB_STATUS,
     CASE  su.SUB_STATE  WHEN 1 THEN 'NONE'  WHEN 2 THEN 'AWAITING_ACTIVATION'  WHEN 3 THEN 'UPGRADED'  WHEN 4 THEN 'DOWNGRADED'  WHEN 5 THEN 
         'EXTENDED'  WHEN 6 THEN  'TRANSFERRED' WHEN 7 THEN 'REGRETTED' WHEN 8 THEN 'CANCELLED' WHEN 9 THEN 'BLOCKED' WHEN 10 THEN 'CHANGED' ELSE 'UNKNOWN' END AS  SUB_SUB_STATE,
 ...
 (
     SELECT
        pu.TARGET_CENTER, pu.TARGET_ID, string_agg(cc.CODE, ',' order by pu.CAMPAIGN_CODE_ID) AS "CODE"
     FROM
        PRIVILEGE_USAGES pu
     JOIN
        CAMPAIGN_CODES cc
 ...
 WHERE
     ...
     SS.SUBSCRIPTION_TYPE_CENTER IN ($$Scope$$)
        AND LongToDate(SU.CREATION_TIME) >= CAST(:SalesFromDate AS DATE)
     AND LongToDate(SU.CREATION_TIME) < CAST(:SalesToDate AS DATE) + interval '1 day')
     AND ss.TYPE IN(1,2)
     AND (
         spg.CENTER IS NULL
   ...
   */
     ...
     AND NOT EXISTS (
        ...
        WHERE
           ...
           AND sw.ID <> su.ID
                   --1000*60*60*24*30
                   AND scl.ENTRY_START_TIME >= su.CREATION_TIME - 2592000000
 ...
 (
     SELECT
        ...
        sa.SUBSCRIPTION_ID,
        string_agg (mpr.CACHED_PRODUCTNAME, ', ' ORDER BY mpr.CACHED_PRODUCTNAME) AS "ADD_ON"
     FROM
