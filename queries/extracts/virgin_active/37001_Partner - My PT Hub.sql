WITH
    ...
    (
        SELECT
                ...
                datetolongTZ(TO_CHAR(trunc(add_months(sysdate,1), 'MONTH'), 'YYYY-MM-DD HH24:MI'),'Europe/London') AS endOfMonth,
...
SELECT
        ...
        DECODE(bp.VALID_FOR,'LSS',cc.CENTER,VALID_FOR) AS "Where clips can be used"
FROM
        PARAMS
...
WHERE 
        cc.VALID_UNTIL >= params.today
        AND p.EXTERNAL_ID = :MemberId
        AND EXISTS     
        (
	       SELECT 1 
               FROM PRODUCTS pro
               JOIN PRODUCT_AND_PRODUCT_GROUP_LINK ppgl ON ppgl.PRODUCT_CENTER = pro.CENTER AND ppgl.PRODUCT_ID = pro.ID
               JOIN PRODUCT_GROUP pg ON pg.id = ppgl.PRODUCT_GROUP_ID AND pg.name in ('PT Clipcards','Personal Training','PT Top Ups', 'PT Packs',
                    'PT Rental', 'PT by DD subscriptions', 'PT DD Master', 'PT DD Standard', 'PT New Members', 'PT Promotions', 'PT DD ICON') 
               WHERE
...
SELECT
        ...
        nvl(to_number(regexp_replace(bpg.name,'[[:alpha:]]|_')),-1) AS  "Duration of the sessions",
        TO_CHAR(nvl(sa.END_DATE,longtodateTZ(params.endOfMonth-60000,'Europe/London')),'YYYY-MM-DD') AS "Expiry of active sessions",
        p.EXTERNAL_ID AS "Member external ID",
        ...
        Trainer_Level."Trainer Level",
        DECODE(bp.VALID_FOR,'LSS',sa.CENTER_ID, VALID_FOR) AS "Where clips can be used"
FROM
        PARAMS
CROSS JOIN PERSONS p
...
LEFT JOIN
        (
                ...
                JOIN    PRIVILEGE_USAGES pu ON bg.id = pu.PRIVILEGE_ID
                WHERE 
			pu.TARGET_START_TIME >= params.prevMonth 
			AND pu.TARGET_START_TIME < params.endOfMonth
