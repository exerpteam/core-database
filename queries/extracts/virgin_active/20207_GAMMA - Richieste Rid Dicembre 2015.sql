...
WHERE 
pr.center IN(select c.ID from CENTERS c where  c.COUNTRY = 'IT') and extract(month from pr.req_date) = EXTRACT(month FROM  ADD_MONTHS(SYSDATE,-11))
and extract(year from  pr.req_date) = extract(YEAR FROM ADD_MONTHS(SYSDATE,-11))
