WITH params AS MATERIALIZED
    ...
    (
        SELECT
            ...
            DATE_TRUNC('month',TO_DATE(getCenterTime(c.id),'YYYY-MM-DD')- interval '1' month)+4 AS fromdate, 
	    DATE_TRUNC('month',TO_DATE(getCenterTime(c.id),'YYYY-MM-DD')- interval '1' month)+5 AS todate
	    from
	    centers c
   

 where c.id not in (233,229) and c.country = 'IT'
...
(
        ...
        WHERE 
                longtodate(act.ENTRY_TIME) >= params.fromDate
                AND LONGTODATE(act.ENTRY_TIME) < params.toDate
