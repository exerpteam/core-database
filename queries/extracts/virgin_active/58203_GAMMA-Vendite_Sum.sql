 SELECT
         SUM(venditeTotali) as VenditeTotali,
         SUM(VenditeNonWeb) as VenditeNonWeb,
         SUM(VenditeWeb) as VenditeWeb
  FROM(
 SELECT
     cen.SHORTNAME,
     1 as VenditeTotali,
     CASE WHEN  ss.EMPLOYEE_CENTER= 100 AND  ss.EMPLOYEE_ID = 26801 THEN 0 else 1
         END as VenditeNonWeb,
         CASE WHEN  ss.EMPLOYEE_CENTER= 100 AND  ss.EMPLOYEE_ID = 26801 THEN 1 else 0
         END as VenditeWeb
 FROM
 ...
 INNER JOIN
     CENTERS cen
 ON
     cen.ID = p.CENTER AND cen.COUNTRY='IT'
 LEFT JOIN
     PRODUCT_GROUP prg
 ON
