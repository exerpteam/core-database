...
-- https://clublead.atlassian.net/browse/ST-5868
     ...
     replace(replace(replace(replace(replace(replace(replace(concat(concat(concat('<?xml version="1.0" encoding="UTF-8"?><FatturaElettronica xmlns'||chr(58)||'xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns'||chr(58)||'xsd="http://www.w3.org/2001/XMLSchema" versione="FPR12" xmlns="http://ivaservizi.agenziaentrate.gov.it/docs/xsd/fatture/v1.2">',
       ...
       xmlelement(name "FatturaElettronicaHeader",
             ...
             xmlelement(name "CessionarioCommittente",
                 xmlelement(name "DatiAnagrafici",
                                    xmlelement(name "IdFiscaleIVA",
                        xmlelement(name "IdPaese", CASE  WHEN data_for_Xml."IdCodice_Comment" is null THEN null ELSE data_for_Xml."IdPaese" END),
 ...
 SELECT
     ...
     CASE  WHEN codice.TXTVALUE is null THEN '0000000' ELSE codice.TXTVALUE END "CodiceDestinatario",
     CASE p.center  WHEN 209 THEN  '06371530962'   WHEN 223 THEN  '03032340980' WHEN 224 THEN  '03032340980' WHEN 225 THEN  '03032340980'  WHEN 229 THEN  '09634340963'  ELSE '03641880962'  END  "IdCodice",
     CASE p.center  WHEN 209 THEN  'Club Milano Corso Como S.r.l.'   WHEN 223 THEN 'Club Milano City S.r.l.' WHEN 224 THEN 'Club Milano City S.r.l.' WHEN 225 THEN 'Club Milano City S.r.l.' WHEN 229 THEN  'Revolution s.r.l. Unipersonale'  ELSE 'Virgin Active Italia S.pa.'  END  "Denominazione_VAI",
     CASE
     ...
     COALESCE(invname.TXTVALUE, p.FULLNAME) "Denominazione_Cust",
     COALESCE(invadr1.TXTVALUE, p.ADDRESS1)  "Indirizzo2",
     COALESCE(invzip.TXTVALUE, p.ZIPCODE) "CAP2",
     COALESCE(invcity.TXTVALUE, p.CITY) "Comune2",
     COALESCE(invnation.TXTVALUE, p.COUNTRY) "Nazione2",
 ...
 WITH
     ...
     (
         SELECT
             /*+ materialize */
             datetolongTZ(TO_CHAR(TRUNC(CURRENT_TIMESTAMP)-:offset, 'YYYY-MM-dd HH24:MI'), 'Europe/Rome')   AS StartDateLong,
             datetolongTZ(TO_CHAR(TRUNC(CURRENT_TIMESTAMP), 'YYYY-MM-dd HH24:MI'), 'Europe/Rome')   AS EndDateLong
 ...
 WITH
     ...
     (
         SELECT
             /*+ materialize */
             TRUNC(CURRENT_TIMESTAMP)-:offset  AS StartDate,
             TRUNC(CURRENT_TIMESTAMP)          AS EndDate
 ...
 WHERE
    ...
    AND prs.ORIGINAL_DUE_DATE >= params.StartDate
    AND prs.ORIGINAL_DUE_DATE < params.EndDate
 UNION ALL
 -- Credit Notes
 SELECT
     ar.CUSTOMERCENTER PersonCenter,
