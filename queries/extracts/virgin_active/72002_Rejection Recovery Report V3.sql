...
--             XMLTABLE('//attributes/attribute' passing XMLType(q.MIMEVALUE,871) columns attributeid VARCHAR2(100) PATH '@id', attributename VARCHAR2(100) PATH '@name' )x
 ...
 SELECT DISTINCT
     ...
     CASE
		 WHEN art.text like 'Automatic placement: Saferpay VAI%' AND paid_pr.rejected_reason_code IS NULL THEN 'Normal Collection'
                 WHEN art.text like 'Automatic placement: Saferpay VAI%' AND paid_pr.rejected_reason_code IS NOT NULL THEN 'Late Payment on Representation'
                 WHEN art.text like 'Cash collection payment received' THEN 'Cash collection payment received'
