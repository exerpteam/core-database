...
--  
 SELECT
  ...
  i1.DATE_JOINED,
         i1.START_DATE,
         coalesce(i1.BINDING_END_DATE,i1.END_DATE) as END_DATE,
  i1.MEMBER_NAME,
 i1.MEMBERSHIP,
     i1.SALES_PERSON,
  SUM(
         ...
         END ) AF,
    i1.COMMISSIONABLE,
    COALESCE(ccd.CODE,MAX(camp.CODE)) AS code,
     MAX(
         CASE
             ...
             ELSE 0
         END ) ATTIVAZIONI,
         CASE WHEN sst.CODE = COALESCE(ccd.CODE,MAX(camp.CODE)) THEN NULL ELSE sst.CODE END as CODICESTARTUP
 FROM
     (
         SELECT
             /*+ NO_BIND_AWARE */
