...
--  
 SELECT
     ...
     CASE
        WHEN i1.COMMISSIONABLE = 'Y' THEN
            validateperson.FULLNAME
        ELSE
            null
     END AS "Validated Person Name"
 FROM
     (
         SELECT
             /*+ NO_BIND_AWARE */
             ...
             pcl.employee_center as vald_emp_center,
             pcl.employee_id as vald_emp_id,
             rank() over (partition BY pcl.PERSON_CENTER,pcl.PERSON_ID ORDER BY pcl.ENTRY_TIME DESC) AS rnk
         FROM
         ...
         ON
             pcl.person_center = owner.CENTER
             and pcl.person_id = owner.ID
             AND pcl.change_attribute IN ('Sono_presenti_tutti_i _dati_anagrafici', 'Sono_presenti_ID_e_codice_fiscale', 'CONDIZIONIGENERALIFIRMATO', 'Sono_presenti_i_dati_bancari_o_il_pagamento_cash',
             'LADATAINIZIOEVALIDA', 'PRESENTEILBADGEAZIENDALE', 'MODULODIPAGAMENTOFIRMATO', 'Sono_presenti_le_firme_del_titolare_dei_pagamenti')
         WHERE
 ...
 LEFT JOIN
     EMPLOYEES validateemp
 ON
     validateemp.CENTER = i1.vald_emp_center
     AND validateemp.ID = i1.vald_emp_id
 LEFT JOIN
     PERSONS validateperson
 ON
     validateperson.CENTER = validateemp.personcenter
     AND validateperson.ID  = validateemp.personid
 WHERE
     rnk = 1
 GROUP BY
     i1.SALES_DATE,
