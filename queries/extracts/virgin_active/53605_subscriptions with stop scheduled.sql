 WITH
  lastmonth AS
  (SELECT  /* materialize */
         trunc(add_months(CURRENT_TIMESTAMP,-1),'MONTH') as fromdate,
         trunc(CURRENT_TIMESTAMP,'MONTH')  AS TODATE
     ...
     (
         ...
         WHERE
            sub.owner_center IN ($$Scope$$)
             AND TRUNC(sub.end_date) >= lastmonth.FROMDATE
             AND TRUNC(sub.end_date) < lastmonth.TODATE
             AND EXISTS
             (
                 SELECT
                     1
         ...
         SELECT DISTINCT
             ...
             CASE
                 WHEN qa.number_answer IS NOT NULL
                   ...
                   THEN EXTRACTVALUE(xmltype(UTL_I18N.RAW_TO_CHAR(DBMS_LOB.SUBSTR(q.QUESTIONS, 2000,1), 'UTF8') || UTL_I18N.RAW_TO_CHAR(DBMS_LOB.SUBSTR(q.QUESTIONS, 2000,2001), 'UTF8')), '//question[id/text()='|| qa.QUESTION_ID ||']/options/option[id/text()='|| qa.number_answer ||']/optionText')
                 ELSE NULL
             END                                                                                                              AS Reason,
 ...
 (SELECT distinct
     Qx.ID                                         AS "ID",
     Qx.name                                       AS "NAME",
     Qx.ID||'qu'||x.questionid                     AS "QUESTION_ID",
     CAST ( UPPER(x.questiontype)AS VARCHAR(255)) AS "QUESTION_TYPE",
     CAST ( x.questiontext AS VARCHAR(255))       AS "QUESTION_TEXT",
     CASE
         WHEN y.answerid IS NOT NULL
         THEN Qx.ID||'qu'||x.questionid||'an'||y.answerid
         ELSE NULL
     END                                  AS "ANSWER_ID",
     CAST ( y.answertext AS VARCHAR(255)) AS "ANSWER_TEXT",
     qx.CREATION_TIME                      AS "CREATION_DATE",
     y.code
 FROM
     QUESTIONNAIRES Qx,
     XMLTABLE('//question' passing XMLType(qx.QUESTIONS,871) columns questionid VARCHAR2(10) PATH 'id', questiontext VARCHAR2(200) PATH 'questionText', questiontype VARCHAR2(10) PATH 'questionType',answer XMLTYPE PATH './options/option')x
 LEFT OUTER JOIN
     XMLTABLE('option' passing x.answer columns answerid VARCHAR2(10) PATH 'id', answertext VARCHAR2(50) PATH 'optionText' , Code VARCHAR2(10) PATH 'code' )y
 ON
     1=1 ) code
 on qaa.RESULT_CODE = code.code
 and code.id = q.id
         WHERE
         ...
         FROM
             (
                 SELECT
                     ...
                     CASE
                         WHEN TO_NUMBER(REPLACE(TRIM(SUBSTR(UTL_I18N.RAW_TO_CHAR(DBMS_LOB.SUBSTR(je.big_text, 2000,1), 'UTF8'), LENGTH('Subscription credit date: '),3)), '/', '')) <= 12
                         THEN TO_DATE(TRIM(SUBSTR(UTL_I18N.RAW_TO_CHAR(DBMS_LOB.SUBSTR(je.big_text, 2000,1), 'UTF8'), LENGTH('Subscription credit date: '), 8)), 'MM/DD/YY')
                     END                                                                                      AS stop_date,
                     rank() over (partition BY je.person_center, je.person_id ORDER BY je.creation_time DESC) AS rnk
                 FROM
                     v_sub s
                 ...
                 ON
                     ...
                     AND je.jetype = 3
                     AND je.name = 'Apply: Stop subscriptions' )
         WHERE
 ...
 SELECT DISTINCT
     ...
     CASE
         WHEN sub.st_type = 0
         THEN 'CASH'
         WHEN quest.ANSWER_TEXT IS NOT NULL
         THEN quest.ANSWER_TEXT
