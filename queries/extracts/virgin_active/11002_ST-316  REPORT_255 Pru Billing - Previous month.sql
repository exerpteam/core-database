...
WITH
    ...
    (
        SELECT
            /*+ materialize */
            exerpro.datetolong(to_char(trunc(trunc(sysdate, 'mm') - 1, 'MM'), 'YYYY-MM-DD HH24:MI')) AS FROMDATE,
            exerpro.datetolong(to_char(trunc(sysdate, 'mm') - 1, 'YYYY-MM-DD HH24:MI')) + (1000*60*60*24) AS TODATE
        FROM
            dual
    )
SELECT
