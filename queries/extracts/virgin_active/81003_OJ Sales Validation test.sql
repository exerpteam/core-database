SELECT
    ...
    -- Add the CASE statement to categorize the employee
    CASE
        WHEN ss.EMPLOYEE_CENTER || 'emp' || ss.EMPLOYEE_ID = '4emp13601' THEN 'Online Joiner'
        ELSE 'In client'
    END as Sale_Type,  -- This is the new column
...
WHERE
    ...
    (
        ...
        ON
            ...
            AND oldsubprod.name NOT IN ('Free Adult Guest', 'Paying Adult Guest')
        WHERE
...
GROUP BY
    sub.start_date,
    ss.EMPLOYEE_CENTER || 'emp' || ss.EMPLOYEE_ID,
    --ss.EMPLOYEE_ID,
    centre.SHORTNAME,
