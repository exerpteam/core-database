...
--  
 WITH
     ...
     (
         SELECT distinct
             ...
             qaa.log_time,
             qc.questionnaire,
             CAST(code."ANSWER_TEXT" AS VARCHAR),
             --    CASE
         --        WHEN qa.number_answer IS NOT NULL
     --                AND q.id IS NOT NULL
                 --              THEN code.ANSWER_TEXT
       --          ELSE NULL
   --          END                                                                      --AS Reason,
 ...
 (SELECT
    ...
    "NAME",
    "CREATION_DATE",
    (xpath('//question/id/text()',xml_element))[1]                       AS "QUESTION_ID",
    ...
    (xpath('//id/text()',unnest(xpath('//question/options/option',xml_element))))[1] AS "ANSWER_ID"
    ,
    (xpath('//optionText/text()',unnest(xpath('//question/options/option',xml_element))))[1] AS
                                                                                      "ANSWER_TEXT",
    (xpath('//code/text()',unnest(xpath('//question/options/option',xml_element))))[1] AS code
FROM
    (
        SELECT
            Q.id                                                                 AS "ID",
            ...
            unnest(xpath('//question',xmlparse(document convert_from(q.QUESTIONS, 'UTF-8')))) AS
            xml_element
        FROM
            QUESTIONNAIRES q) qx ) code
 on qaa.RESULT_CODE = CAST(code.code AS VARCHAR)
 and code."ID" = q.id
         WHERE
 ...
 SELECT DISTINCT
     ...
     quest."ANSWER_TEXT"                                                                                                                                                                 AS "Cancellation Reasons",
