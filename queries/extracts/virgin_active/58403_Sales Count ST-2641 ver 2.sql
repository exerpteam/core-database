    ...
    (
        ...
        LEFT JOIN
            (
                SELECT
                    pu.TARGET_CENTER,
                    pu.TARGET_ID,
                    string_agg(cc.CODE, ',' ORDER BY pu.CAMPAIGN_CODE_ID) AS "CODE"
                FROM
                    PRIVILEGE_USAGES pu
                JOIN
                    CAMPAIGN_CODES cc
     ...
     AND LongToDate(SU.CREATION_TIME) < CAST($$SalesToDate$$ AS DATE) + interval '1 day')
        ...
        AND PRG.ID NOT IN (5405,5611,5613,5406,5407,5615,30203,30801)
            -- excludes subscription sales after a regrest/cancellation unless the regretted/
            -- cancelled
            ...
            (
                ...
                AND NOT EXISTS
                    (
                        ...
                        WHERE
                            pl.PRODUCT_CENTER = sw.SUBSCRIPTIONTYPE_CENTER
