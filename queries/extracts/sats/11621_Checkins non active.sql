SELECT  /*+ index(Checkin_Log IDX_CHKIN_TIME) */
    ...
    to_char(eclub2.longtodate(Checkin_Log.Checkin_Time), 'YYYY-MM-dd')AS CheckinDate,
    per.address1,
    per.address2,
    per.zipcode,
    z.city,
...
and Checkin_Log.CHECKIN_TIME  >= eclub2.datetolong(to_char(exerpsysdate() - :Days_back_in_time,'yyyy-mm-dd HH24:MI'))
and Checkin_Log.CHECKIN_TIME <= (eclub2.datetolong(to_char(exerpsysdate() - 1, 'yyyy-mm-dd HH24:MI'))+ 24*3600*1000)
and per.status not in(1,3)
