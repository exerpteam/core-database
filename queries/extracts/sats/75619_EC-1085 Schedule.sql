 WITH
     ...
     (
         SELECT
             /*+ materialize */
             cast(datetolongC(TO_CHAR(TRUNC(TO_DATE(getCenterTime(c.ID),'YYYY-MM-DD HH24:MI') - 30),'YYYY-MM-DD HH24:MI'),c.ID) AS BIGINT)      AS StartDate,
             cast((datetolongC(TO_CHAR(TRUNC(TO_DATE(getCenterTime(c.ID),'YYYY-MM-DD HH24:MI')+1),'YYYY-MM-DD HH24:MI'),c.ID)-1) AS BIGINT)  AS EndDate,
             c.ID                                                                                                                AS CENTER
         FROM
             CENTERS c
     )
 SELECT
     ...
     PARAMS.StartDate AS "Startd",
     PARAMS.EndDate   AS "Enddate",
