SELECT
        t3.*,
        (CASE t3.PRODUCT_GLOBALID
                        WHEN 'ALL_INCLUSIVE_BND' THEN ROUND(t3.CREDIT_AMOUNT*0.224,2)  
                        WHEN 'ALL_INCLUSIVE_NOBND' THEN ROUND(t3.CREDIT_AMOUNT*0.201,2)
                        WHEN 'NORDIC_DAYTIME_BND_GX' THEN ROUND(t3.CREDIT_AMOUNT*0.155,2)
                        WHEN 'NORDIC_DAYTIME_BND_CON' THEN ROUND(t3.CREDIT_AMOUNT*0.27,2)
                        WHEN 'NORDIC_DAYTIME_NOBND_CON' THEN ROUND(t3.CREDIT_AMOUNT*0.237,2)
                        WHEN 'NORDIC_DAYTIME_NOBND_GX' THEN ROUND(t3.CREDIT_AMOUNT*0.134,2)
                        WHEN 'NORDIC_ALLDAY_BND_GX' THEN ROUND(t3.CREDIT_AMOUNT*0.142,2)
                        WHEN 'NORDIC_ALLDAY_BND_CON' THEN ROUND(t3.CREDIT_AMOUNT*0.249,2)
                        WHEN 'NORDIC_ALLDAY_NOBND_CON' THEN ROUND(t3.CREDIT_AMOUNT*0.222,2)
                        WHEN 'NORDIC_ALLDAY_NOBND_GX' THEN ROUND(t3.CREDIT_AMOUNT*0.124,2)
                        WHEN '1CLUB_DAYTIME_BND_GX' THEN ROUND(t3.CREDIT_AMOUNT*0.177,2)
                        WHEN '1CLUB_DAYTIME_BND_CON' THEN ROUND(t3.CREDIT_AMOUNT*0.302,2)
                        WHEN '1CLUB_DAYTIME_NOBND_CON' THEN ROUND(t3.CREDIT_AMOUNT*0.263,2)
                        WHEN '1CLUB_DAYTIME_NOBND_GX' THEN ROUND(t3.CREDIT_AMOUNT*0.15,2)
                        WHEN '1CLUB_ALLDAY_BND_GX' THEN ROUND(t3.CREDIT_AMOUNT*0.16,2)
                        WHEN '1CLUB_ALLDAY_BND_CON' THEN ROUND(t3.CREDIT_AMOUNT*0.277,2)
                        WHEN '1CLUB_ALLDAY_NOBND_CON' THEN ROUND(t3.CREDIT_AMOUNT*0.243,2)
                        WHEN '1CLUB_ALLDAY_NOBND_GX' THEN ROUND(t3.CREDIT_AMOUNT*0.138,2)
                        WHEN 'REGION_DAYTIME_BND_GX' THEN ROUND(t3.CREDIT_AMOUNT*0.158,2)
                        WHEN 'REGION_DAYTIME_BND_CON' THEN ROUND(t3.CREDIT_AMOUNT*0.273,2)
                        WHEN 'REGION_DAYTIME_NOBND_CON' THEN ROUND(t3.CREDIT_AMOUNT*0.24,2)
                        WHEN 'REGION_DAYTIME_NOBND_GX' THEN ROUND(t3.CREDIT_AMOUNT*0.136,2)
                        WHEN 'REGION_ALLDAY_BND_GX' THEN ROUND(t3.CREDIT_AMOUNT*0.144,2)
                        WHEN 'REGION_ALLDAY_BND_CON' THEN ROUND(t3.CREDIT_AMOUNT*0.253,2)
                        WHEN 'REGION_ALLDAY_NOBND_CON' THEN ROUND(t3.CREDIT_AMOUNT*0.224,2)
                        WHEN 'REGION_ALLDAY_NOBND_GX' THEN ROUND(t3.CREDIT_AMOUNT*0.126,2)
                        ELSE 0
...
(
        SELECT
                t2.PERSONID,
                t2.PAYER_CENTER || 'p' || t2.PAYER_ID AS PAYERID,
                t2.SEX,
                t2.PERSONSTATUS,
...
:toDate AS TO_DATE,
                        ...
                        SELECT
                                ...
                                s.OWNER_CENTER AS SUB_OWNER_CENTER,
                        ...
                        JOIN PERSONS p ON i.PAYER_CENTER = p.CENTER AND i.PAYER_ID = p.ID
...
:toDate AS TO_DATE,
                        ...
                        SELECT
                                ...
                                spp.TO_DATE AS SPP_TO_DATE,
                                spp.TO_DATE - spp.FROM_DATE +1 AS INVOICE_DAYS,
                                ilspon.TOTAL_AMOUNT AS INV_TOTAL_AMOUNT,
                                GREATEST(spp.FROM_DATE, params.FROM_DATE) AS CREDIT_PERIOD_START,
                                LEAST(spp.TO_DATE, params.TO_DATE) AS CREDIT_PERIOD_END,
                                pr.PTYPE,
                                params.TODAYS_DATE AS BOOK_DATE,
                        ...
                        JOIN INVOICE_LINES_MT ilspon
                                ON ilspon.CENTER = i.SPONSOR_INVOICE_CENTER AND ilspon.ID = i.SPONSOR_INVOICE_ID AND ilspon.SUBID = il.SPONSOR_INVOICE_SUBID
                        JOIN INVOICES ispon
                                ON ispon.CENTER = ilspon.CENTER AND ispon.ID = ilspon.ID
                        JOIN PERSONS p ON ispon.PAYER_CENTER = p.CENTER AND ispon.PAYER_ID = p.ID
                        WHERE pr.GLOBALID IN ('ALL_INCLUSIVE_BND','ALL_INCLUSIVE_NOBND','NORDIC_DAYTIME_BND_GX','NORDIC_DAYTIME_BND_CON','NORDIC_DAYTIME_NOBND_CON',
                                'NORDIC_DAYTIME_NOBND_GX','NORDIC_ALLDAY_BND_GX','NORDIC_ALLDAY_BND_CON','NORDIC_ALLDAY_NOBND_CON','NORDIC_ALLDAY_NOBND_GX',
                                '1CLUB_DAYTIME_BND_GX','1CLUB_DAYTIME_BND_CON','1CLUB_DAYTIME_NOBND_CON','1CLUB_DAYTIME_NOBND_GX','1CLUB_ALLDAY_BND_GX',
                                '1CLUB_ALLDAY_BND_CON','1CLUB_ALLDAY_NOBND_CON','1CLUB_ALLDAY_NOBND_GX','REGION_DAYTIME_BND_GX','REGION_DAYTIME_BND_CON',
