SELECT DISTINCT
        ...
        barcode_card.IDENTITY                               AS barcode_card_id,
        rfid_card.identity AS rfid_card_id,
        p.external_id,
        ...
        pa.state,
        CASE 
                WHEN (op.center IS NULL AND pa.state IS NOT NULL AND (has_sub.owner_center IS NOT NULL OR pay_for.payer_center IS NOT NULL))
                THEN 
                        (CASE pa.state
                                WHEN 1 THEN 'Created'
                                WHEN 2 THEN 'Sent'
...
JOIN sats.CENTERS center
        ON p.center = center.id
LEFT JOIN sats.PERSON_EXT_ATTRS home
        ON p.center=home.PERSONCENTER
        AND p.id=home.PERSONID
        AND home.name='_eClub_PhoneHome'
LEFT JOIN sats.PERSON_EXT_ATTRS mobile
        ON p.center=mobile.PERSONCENTER
        AND p.id=mobile.PERSONID
        AND mobile.name='_eClub_PhoneSMS'
LEFT JOIN sats.PERSON_EXT_ATTRS workphone
        ON p.center=workphone.PERSONCENTER
        AND p.id=workphone.PERSONID
        AND workphone.name='_eClub_PhoneWork'
LEFT JOIN sats.PERSON_EXT_ATTRS email
        ON p.center=email.PERSONCENTER
        AND p.id=email.PERSONID
        AND email.name='_eClub_Email'
LEFT JOIN sats.PERSON_EXT_ATTRS personcomment
        ON p.center=personcomment.PERSONCENTER
        AND p.id=personcomment.PERSONID
        AND personcomment.name='_eClub_Comment'
LEFT JOIN sats.ACCOUNT_RECEIVABLES payment_ar
        ON payment_ar.CUSTOMERCENTER = p.center
        AND payment_ar.CUSTOMERID = p.id
        AND payment_ar.AR_TYPE = 4
LEFT JOIN sats.ACCOUNT_RECEIVABLES cash_ar
        ON cash_ar.CUSTOMERCENTER=p.center
        AND cash_ar.CUSTOMERID=p.id
        AND cash_ar.AR_TYPE = 1
LEFT JOIN sats.RELATIVES comp_rel
        ON comp_rel.center=p.center
        AND comp_rel.id=p.id
        AND comp_rel.RTYPE = 3
        AND comp_rel.STATUS < 3
LEFT JOIN sats.COMPANYAGREEMENTS cag
        ON cag.center= comp_rel.RELATIVECENTER
        AND cag.id=comp_rel.RELATIVEID
        AND cag.subid = comp_rel.RELATIVESUBID
LEFT JOIN sats.persons comp
        ON comp.center = cag.center
        AND comp.id=cag.id
LEFT JOIN sats.entityidentifiers barcode_card
        ON barcode_card.ref_center = p.center
        AND barcode_card.ref_id = p.id
        AND barcode_card.entitystatus = 1
        AND barcode_card.idmethod = 1
LEFT JOIN sats.entityidentifiers rfid_card
        ON rfid_card.ref_center = p.center
        AND rfid_card.ref_id = p.id
        AND rfid_card.entitystatus = 1
        AND rfid_card.idmethod = 4
LEFT JOIN sats.PAYMENT_ACCOUNTS paymentaccount
        ON paymentaccount.center = payment_ar.center
        AND paymentaccount.id = payment_ar.id
LEFT JOIN sats.PAYMENT_AGREEMENTS pa
        ON paymentaccount.active_agr_center = pa.center
        AND paymentaccount.active_agr_id = pa.id
        AND paymentaccount.active_agr_subid = pa.subid
LEFT JOIN sats.clearinghouses ch
        ON pa.clearinghouse = ch.id
LEFT JOIN sats.RELATIVES op_rel
        ON op_rel.relativecenter=p.center
        AND op_rel.relativeid=p.id
        AND op_rel.RTYPE = 12
        AND op_rel.STATUS < 3
LEFT JOIN sats.PERSONS op
        ON op.center = op_rel.center
        AND op.id = op_rel.id
LEFT JOIN sats.ACCOUNT_RECEIVABLES otherPayerAR
        ON otherPayerAR.CUSTOMERCENTER = op.center
...
ON
    ...
    AND ( (
        ...
        OR (
            p.PERSONTYPE = 6
            AND pt_rel.RTYPE = 4 ) )
LEFT JOIN sats.PERSONS pt_rel_p
        ON pt_rel_p.center = pt_rel.relativecenter
...
WHERE
        ...
        ( 
                (
                        p.status IN (1,3) AND p.persontype NOT IN (2) 
                )
