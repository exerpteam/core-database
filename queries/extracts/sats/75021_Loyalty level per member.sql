 SELECT
     ...
     then results.COUNT_ACTIVE_DAYS+cast(results.daysplus as integer)
     else results.COUNT_ACTIVE_DAYS
 end as COUNT_ACTIVE_DAYS,
     case
     when results.checkinsplus is not null
     then results.TOTAL_CHECKINS+cast(results.checkinsplus as integer)
         ...
         FROM
             (
               ...
               LEFT JOIN
                 ...
                 WHERE
                     p.external_id = :externalid ) innersql
         LEFT JOIN
             checkins ch
         ON
