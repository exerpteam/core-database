...
-- Ohter Payer -- 3
  ...
  other_payer				         	  AS Other_Payer,
  other_payer_id						  AS Other_Payer_ID,
...
-- Other Payer -- 3     
      ...
      op.fullNAME						          AS other_payer,
      op.center||'p'||op.id 			          AS other_payer_id,
...
-- Other Payer --
    ...
    LEFT JOIN 
      sats.relatives rel5 
    ON
      comp.CENTER = rel5.CENTER
    AND
      comp.ID = rel5.ID 
    AND
      rel5.RTYPE = 12 -- other payer
    AND 
      rel5.status = 1
    ...
    LEFT JOIN 
      sats.persons op 
    ON
      op.center = rel5.RELATIVECENTER AND
      op.id = rel5.RELATIVEID
...
-- Invoice --
   ...
   LEFT JOIN
      sats.person_Ext_Attrs invoice_adress
    ON
   ...
   LEFT JOIN
      sats.person_Ext_Attrs invoice_adress2
    ON
   ...
   LEFT JOIN
      sats.person_Ext_Attrs invoice_adress3
    ON
   ...
   LEFT JOIN
      sats.person_Ext_Attrs invoice_adress4
    ON
  ...
  LEFT JOIN
      sats.person_Ext_Attrs invoice_adress5
    ON
  ...
  LEFT JOIN
      sats.person_Ext_Attrs invoice_adress6
    ON
...
-- Ext attributes Company --
    ...
    LEFT JOIN
      sats.person_Ext_Attrs Company_comment
    ON
    ...
    LEFT JOIN
      sats.person_Ext_Attrs Company_billing_id
    ON
