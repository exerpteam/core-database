...
(
        ...
        WHERE
                ...
                (
                        SELECT 1
                        FROM sats.state_change_log scl
                        WHERE
                                scl.entry_type = 1
                ...
                (
                        SELECT 1
                        FROM sats.account_receivables ar
                        WHERE
                ...
                (
                        SELECT 1
                        FROM sats.relatives r
                        INNER JOIN sats.persons p2 ON r.relativecenter = p2.center AND r.relativeid = p2.id AND r.rtype <> 3
                        WHERE
                                r.center = p.center
                                AND r.id = p.id
                                AND r.rtype = 14
...
(
        ...
        JOIN sats.subscriptions s ON em.center = s.owner_center AND em.id = s.owner_id 
        WHERE
                s.state != 3
                OR
                s.end_date IS NULL
...
(
        SELECT 
                ...
                em.*
        FROM eligible_members em
        JOIN sats.subscriptions s ON em.center = s.owner_center AND em.id = s.owner_id AND s.end_date IS NULL
        JOIN sats.subscriptiontypes st ON s.subscriptiontype_center = st.center AND s.subscriptiontype_id = st.id AND st.st_type NOT IN (0)
        WHERE
...
(
        SELECT
                ...
                em.*
        FROM eligible_members em
        JOIN sats.clipcards cc ON em.center = cc.owner_center AND em.id = cc.owner_id
        WHERE
                cc.cancelled = false
...
(
        SELECT
                ...
                em.*
        FROM eligible_members em
        JOIN sats.relatives r ON em.center = r.center AND em.id = r.id AND r.rtype = 12 AND r.status NOT IN (3)
),
condition_6 AS
(
        SELECT
                ...
                ar.balance
        FROM eligible_members em 
        JOIN sats.account_receivables ar ON em.center = ar.customercenter AND em.id = ar.customerid AND ar.state NOT IN (4)
        WHERE
                ar.balance != 0
                AND ar.ar_type = 1
),
...
(
        SELECT
                ...
                em.*
        FROM eligible_members em 
        JOIN sats.employees emp ON em.center = emp.personcenter AND em.id = emp.personid
        WHERE
                emp.blocked = false
),
condition_11 AS
(
        SELECT
                ...
                em.*
        FROM eligible_members em 
        JOIN sats.relatives r ON em.center = r.relativecenter AND em.id = r.relativeid
        WHERE
                em.persontype = 2
                AND r.rtype = 1
...
(
        SELECT
                ...
                em.*
        FROM eligible_members em 
        JOIN sats.participations part ON em.center = part.participant_center AND em.id = part.participant_id
...
(
        SELECT
                ...
                em.*
        FROM eligible_members em 
        JOIN sats.cashcollectioncases ccc ON em.center = ccc.personcenter AND em.id = ccc.personid
        WHERE
                ccc.closed = false
),
condition_14 AS
(
        ...
        FROM eligible_members em 
        JOIN sats.cashcollectioncases ccc ON em.center = ccc.personcenter AND em.id = ccc.personid
        JOIN sats.cashcollection_requests ccr ON ccr.center = ccc.center AND ccr.id = ccc.id
        WHERE
                ccr.state = 0
),
condition_15 AS
(
        SELECT
                ...
                em.*
        FROM eligible_members em 
        JOIN sats.account_receivables ar ON em.center = ar.customercenter AND em.id = ar.customerid
        JOIN sats.payment_accounts pac ON ar.center = pac.center AND ar.id = pac.id
        JOIN sats.payment_agreements pag ON pac.center = pag.center AND pac.id = pag.id
        WHERE
                ar.ar_type = 4
...
(
        ...
        JOIN sats.payment_accounts pac ON ar.center = pac.center AND ar.id = pac.id
        JOIN sats.payment_agreements pag ON pac.center = pag.center AND pac.id = pag.id
        JOIN sats.payment_requests pr ON pag.center = pr.center AND pag.id = pr.id AND pag.subid = pr.agr_subid
        WHERE
                ar.ar_type = 4
