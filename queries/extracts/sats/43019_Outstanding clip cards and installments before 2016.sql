WITH
    ...
    (
        SELECT
            /*+ materialize */
            exerpro.datetolong(to_char(to_date('2015-12-31', 'YYYY-MM-DD'), 'YYYY-MM-DD HH24:MI')) + (1000*60*60*24) AS TODATE
        FROM
            dual
    )
SELECT DISTINCT
    ...
    CASE 
         WHEN DECODE(insp.id,NULL,'No','Yes') = 'Yes' and invl.TOTAL_AMOUNT> 0 THEN      
    ...
    CASE 
         WHEN DECODE(insp.id,NULL,'No','Yes') = 'Yes' and invl.TOTAL_AMOUNT> 0 THEN        
             NVL((ABS(ar_per.ar_trans_amount) + NVL(ABS(pay_per.pr_amount), 0))/invl.QUANTITY, 0)
         ELSE
           0
...
WHERE
    ...
    AND ( (c.clips_initial -   NVL(clip_usage.clip_used,0) > 0) or (invl.TOTAL_AMOUNT> 0 AND  NVL(ar_per.ar_trans_count,0) >0) )
