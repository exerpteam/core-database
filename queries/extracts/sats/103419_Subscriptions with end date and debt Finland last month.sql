...
WITH
    ...
    (   SELECT
            ...
            fromDate,
            DATE_TRUNC('month',TO_DATE(getCenterTime(c.id),'YYYY-MM-DD'))- interval '1 day' AS
                    toDate,
            c.id AS centerID
        FROM
            centers c
...
JOIN --subquery ranking the member's subscriptions with latest as 1
    (   SELECT
        ...
        centers ce
        ON
        ce.id = p.current_person_center
        WHERE
        ce.country = 'FI') latestsub
ON
...
AND sub.end_date >= par.fromDate
AND sub.end_date <= par.toDate
