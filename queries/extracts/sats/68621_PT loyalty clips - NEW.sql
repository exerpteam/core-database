...
--  
 ...
 (
     SELECT /*+ materialize */
             ID AS Center,
             datetolongTZ(TO_CHAR(TRUNC(current_date-7 , 'DDD'), 'YYYY-MM-DD HH24:MI'), TIME_ZONE) AS SevenDays,
             datetolongTZ(TO_CHAR(TRUNC(current_date-6 , 'DDD'), 'YYYY-MM-DD HH24:MI'), TIME_ZONE) AS SixDays
         FROM
              CENTERS
         WHERE ID in ($$scope$$)
 )
 ...
 WHERE
     inv.TRANS_TIME >= PARAMS.SEVENDAYS
 AND p.CENTER IN ($$center$$)
 --AND inv.PAYER_CENTER IN ($$center$$)
