...
--  
 -- Once a to-do is marked as complete, all comments pertaining to that to-do have a status of
 -- complete. Therefore, the final COMMENT_DATE is taken as the TO-DO completed time.
 WITH
     params AS
     (
         SELECT
             /*+ materialize */
             datetolong(TO_CHAR(to_date($$fromdate$$,'YYYY-MM-DD'), 'YYYY-MM-dd HH24:MI')) AS StartDate,
             datetolong(TO_CHAR(to_date($$todate$$,'YYYY-MM-DD'), 'YYYY-MM-dd HH24:MI')) AS EndDate
 ...
 SELECT
     ...
     CASE t.TODO_GROUP_ID
        ...
        WHEN 1804 THEN 'VW - Payment Query'
        ELSE 'UNKNOWN'
      END AS TODO_GROUP,
 ...
 CROSS JOIN
     params
 WHERE
     t.CREATION_TIME BETWEEN StartDate AND EndDate
 AND t.SUBJECT = 'CV19 F'
