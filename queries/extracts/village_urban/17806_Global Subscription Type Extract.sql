...
SELECT DISTINCT P.CENTER AS CENTER, P.ID AS ID, P.FULLNAME AS FULLNAME, 
        ...
        P.LAST_ACTIVE_END_DATE IS NULL THEN TRUNC(TO_DATE('2023-07-13', 
        'YYYY-MM-DD') - P.LAST_ACTIVE_START_DATE) + 1 WHEN 
        P.LAST_ACTIVE_START_DATE IS NULL THEN 0 ELSE P.MEMBERDAYS END 
        AS UNBROKEN_MEMBER_DAYS, CASE WHEN P.LAST_ACTIVE_END_DATE IS 
        NULL THEN TRUNC(TO_DATE('2023-07-13', 'YYYY-MM-DD') - 
    ...
    from PERSONS AS P LEFT 
    join SUBSCRIPTIONS AS S ON (P.CENTER = S.OWNER_CENTER AND P.ID = 
        S.OWNER_ID) LEFT 
    join CENTERS AS CE ON P.CENTER = CE.ID LEFT 
    join ENTITYIDENTIFIERS AS EI1 ON (P.CENTER = EI1.REF_CENTER AND P.ID = 
        EI1.REF_ID AND EI1.IDMETHOD = 1 AND EI1.ENTITYSTATUS = 1) LEFT 
    join ENTITYIDENTIFIERS AS EI2 ON (P.CENTER = EI2.REF_CENTER AND P.ID = 
        EI2.REF_ID AND EI2.IDMETHOD = 2 AND EI2.ENTITYSTATUS = 1) LEFT 
    join ENTITYIDENTIFIERS AS EI3 ON (P.CENTER = EI3.REF_CENTER AND P.ID = 
        EI3.REF_ID AND EI3.IDMETHOD = 4 AND EI3.ENTITYSTATUS = 1) LEFT 
    join ENTITYIDENTIFIERS AS EI4 ON (P.CENTER = EI4.REF_CENTER AND P.ID = 
        EI4.REF_ID AND EI4.IDMETHOD = 5 AND EI4.ENTITYSTATUS = 1) LEFT 
    join RELATIVES AS RL ON (P.CENTER = RL.RELATIVECENTER AND P.ID = 
        RL.RELATIVEID AND RL.RTYPE = 2 AND RL.STATUS <> 3) LEFT 
    join PERSONS AS PE1 ON (PE1.CENTER = RL.CENTER AND PE1.ID = RL.ID) LEFT 
    join PERSONS AS PE2 ON (PE2.CENTER = P.CURRENT_PERSON_CENTER AND PE2.ID 
        = P.CURRENT_PERSON_ID) LEFT 
    join PERSON_EXT_ATTRS AS PEA1 ON (PEA1.PERSONCENTER = P.CENTER AND 
        PEA1.PERSONID = P.ID AND PEA1.NAME = '_eClub_PhoneHome') LEFT 
    join PERSON_EXT_ATTRS AS PEA2 ON (PEA2.PERSONCENTER = 
        P.CURRENT_PERSON_CENTER AND PEA2.PERSONID = P.CURRENT_PERSON_ID 
        AND PEA2.NAME = '_eClub_PhoneSMS') LEFT 
    join PERSON_EXT_ATTRS AS PEA3 ON (PEA3.PERSONCENTER = 
        P.CURRENT_PERSON_CENTER AND PEA3.PERSONID = P.CURRENT_PERSON_ID 
        AND PEA3.NAME = '_eClub_Email') 
    where P.CENTER IN (:Center)  AND (S.SUBSCRIPTIONTYPE_CENTER, S.SUBSCRIPTIONTYPE_ID) in(SELECT center,id FROM PRODUCTS WHERE PTYPE = 10 AND GLOBALID IN ( :globalsubtype ) AND center IN (S.CENTER))