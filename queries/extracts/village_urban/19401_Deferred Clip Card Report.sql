...
WITH
    ...
    (
        ...
        WHERE
            (SCL.ENTRY_TYPE = 6
            AND SCL.STATEID = 1
            AND SCL.BOOK_START_TIME < cast(GETENDOFDAY(:fromdate, SCL.CENTER)AS BIGINT)
            AND (SCL.BOOK_END_TIME IS NULL
                OR  SCL.BOOK_END_TIME >= cast(GETENDOFDAY(:fromdate, SCL.CENTER)AS BIGINT))
            AND (SCL.ENTRY_END_TIME IS NULL
                OR  SCL.ENTRY_END_TIME >= cast(GETENDOFDAY(:fromdate, SCL.CENTER)AS BIGINT))
            AND SCL.CENTER IN             
                                (:scope)
)
    )
    ...
    (
        ...
        WHERE
            (
             (CC.CANCELLED = false
                OR  CC.CANCELLATION_TIME > cast(GETENDOFDAY(:fromdate, CC.CENTER)AS BIGINT))
    ...
    (
        ...
        ON
            ...
            AND (CCU.STATE IN ('ACTIVE',
                               'PLANNED')
                AND CCU.TIME <= cast(GETENDOFDAY(:fromdate, CCU.CARD_CENTER)AS BIGINT)))
        GROUP BY
            CC_TRANS.CURRENT_CC_CENTER,
            CC_TRANS.CURRENT_CC_ID,
            CC_TRANS.CURRENT_CC_SUBID
...
FROM
    (
        ...
        WHERE
            ...
            AND INV.TRANS_TIME <= cast(GETENDOFDAY(:fromdate, CC.CENTER)AS BIGINT)
            AND CC.CLIPS_INITIAL > 0)
        ORDER BY
            CC.OWNER_CENTER,
            CC.OWNER_ID,
