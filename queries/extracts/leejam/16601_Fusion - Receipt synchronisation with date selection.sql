...
-- EC-10261- Fusion - Receipt synchronisation - recurring and cash (with date selection)
        WITH
...
FROM             
       (       
        ...
        FROM
            (
                ...
                WHERE
                        ...
                        AND 
                        crt.CRTTYPE NOT IN (4,5,10,11,14,15,16,17,18,19,20,21,100,101)
                        AND
                        (crt.coment IS NULL 
                        OR crt.coment NOT LIKE 'Manual registered payment of request:%')) crt_trans
        GROUP BY
        ...
        SELECT  
                ...
                ,NULL AS transaction_id
                ,CASE 
                        WHEN art.info IS NULL THEN art.text
                        ELSE art.info
                END AS "ReceiptNumber"
        ...
        WHERE 
                ...
                ar.ar_type IN (1,4)
                AND
                art.employeecenter ||'emp'||art.employeeid in ('100emp2002','100emp4202','100emp12801','100emp23002','100emp23402','100emp19001','100emp27601')
                AND 
...
FROM
        ...
        SELECT DISTINCT 
                ...
                END AS "CustomerAccountNumber"
                ,t1."CurrencyCode"
                ,CAST(t1."ClubNumber" AS VARCHAR) AS "Club Number"
                ,t1."ExerpInvoiceID" AS "Exerp Invoice ID"
                ,'individual-recurring' AS "Payment Source"
                ,t1.a
                ,t1.subid                                   
        FROM       
                ( 
                SELECT
                    ...
                    ,CRTTYPE        AS crt_type 
                    ,CASE
                        WHEN CRT.CENTER IN (100,101) THEN arti.ref_center
                        ELSE CRT.CENTER
                    END AS crt_center 
                    ,arti.ref_CENTER     AS inv_center 
                    ,crt.coment     AS coment 
                    ,NULL           AS art_info 
                    ,NULL           AS transaction_id 
                    ,CASE
                        ...
                        WHEN cct.receipt_number IS NOT NULL THEN cct.receipt_number
                        WHEN cct.receipt_number IS NULL AND cct.transaction_id IS NOT NULL THEN cct.transaction_id
                        WHEN cct.receipt_number IS NULL AND cct.transaction_id IS NULL AND arti.ref_center IS NOT NULL THEN arti.ref_center||'inv'||arti.ref_id
                    END AS "ReceiptNumber" 
                    ,crt.transtime AS "ReceiptDate" 
                    ,CASE
                        WHEN arti.center IS NOT NULL THEN arti.trans_time
                        WHEN artc.center IS NOT NULL THEN artc.trans_time
                        ELSE NULL
                    ...
                    ,crt.center AS "ClubNumber" 
                    ,CASE
                        WHEN arti.ref_center IS NOT NULL THEN arti.ref_center||'inv'||arti.ref_id
                        ELSE NULL
                    END AS "ExerpInvoiceID" 
                    ,crt.center 
                    ,p.sex 
                ...
                JOIN
                        account_trans act
                                ...
                                AND act.subid = crt.gltranssubid +1
                                AND crt.coment like 'Manual registered payment of request%' 
                JOIN
                        leejam.ar_trans artc
                                ON artc.ref_center = act.center
                                AND artc.ref_id = act.id
                                AND artc.ref_subid = act.subid
                JOIN
                        art_match artm
                        ON artm.art_paying_center = artc.center
                        AND artm.art_paying_id = artc.id
                        AND artm.art_paying_subid = artc.subid
                JOIN
                        ar_trans arti
                        ON artm.art_paid_center = arti.center
                        AND artm.art_paid_id = arti.id
                        AND artm.art_paid_subid = arti.subid                                                                                                                  
                LEFT JOIN
                        creditcardtransactions cct
                                ON cct.gl_trans_center = crt.gltranscenter
                                AND cct.gl_trans_id = crt.gltransid
...
SELECT
        t."BusinessUnitName"
        ,t."ReceiptMethodName"
        ,MAX(t."ReceiptNumber") AS "ReceiptNumber"
        ,t.crt_type
        ,t.ReceiptDate
        ,t.GlDate                                                           
        ,t."Amount"
...
FROM
        (                     
         SELECT DISTINCT
                c.org_code AS "BusinessUnitName"
                ,'' AS "ReceiptMethodName"
                ,pr.full_reference AS "ReceiptNumber"
                ,CASE
                        WHEN art.ref_type = 'INVOICE' THEN 1
                        ELSE 0
                END AS crt_type
                ,TO_CHAR(longtodatec(artp.entry_time,artp.center),'yyyy-MM-dd') AS ReceiptDate
                ,TO_CHAR(longtodatec(art.entry_time,art.center),'yyyy-MM-dd')  AS GlDate                                                           
                ,CAST(ROUND(invl.total_amount,2) AS VARCHAR)  AS "Amount"
                ,p.fullname AS "CustomerName"                        
                ,p.external_id AS "CustomerAccountNumber"
                ,'SAR' AS "CurrencyCode"
                ,CAST(c.id AS VARCHAR) AS "Club Number"
                ,inv.center||'inv'||inv.id AS "Exerp Invoice ID"
                ,'Individual-recurring' AS "Payment Source"
        FROM
                leejam.persons p
        ...
        JOIN
                ...
                AND art.ref_type = 'INVOICE'  
        JOIN
                leejam.ar_trans artp               
                ON prs.center = artp.payreq_spec_center
                AND prs.id = artp.payreq_spec_id
                AND prs.subid = artp.payreq_spec_subid
                AND prs.cancelled IS FALSE
        ...
        JOIN
                params
                ON params.center_id = artp.center                    
        WHERE
                p.center IN (:Scope)
                AND
                artp.entry_time BETWEEN params.FromDate AND params.ToDate
                AND
                invl.total_amount > 0
        )t
GROUP BY
