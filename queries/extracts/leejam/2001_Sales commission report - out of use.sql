...
SELECT DISTINCT
        ...
        ,t1."Billed For"
        ,t1."Base Cost"
        ,SUM(t1."Actual Cost") AS "Actual Cost"
        ,SUM(t1."Bill Amount") AS "Bill Amount"
...
FROM
        ...
        PREVIOUS_SUB AS
                ...
                FROM
                        ...
                        SELECT 
                                ...
                                ,maxsub.LastSub
                                ,max(oldsub.end_date) as enddate
                        FROM
                                ELIGIBLE_SUBSCRIPTIONS es
                        JOIN           
                                (
                               SELECT 
                                ...
                                JOIN
                                        leejam.persons p
                                        ON es.center = p.current_person_center
                                        AND es.id = p.current_person_id  
                                JOIN
                                        subscriptions s
                                ...
                                JOIN
                                        ...
                                        ON c.id = s.center                                    
                                WHERE 
                                        es.subcenter||'ss'||es.subid != s.center||'ss'||s.id
                                        AND 
                                        s.sub_state != 8
                                GROUP BY 
                        ...
                        JOIN
                                leejam.persons p   
                                        ON p.current_person_center = maxsub.owner_center
                                        AND p.current_person_id = maxsub.owner_id                                 
                        JOIN
                                subscriptions oldsub
                                        ON oldsub.owner_center = p.center
                                        AND oldsub.owner_id = p.id
                ...
                JOIN
                        leejam.persons p   
                        ON p.current_person_center = t1.owner_center
                        AND p.current_person_id = t1.owner_id
                JOIN
                        subscriptions oldsub
                        ON oldsub.owner_center = p.center
                        AND oldsub.owner_id = p.id
                        AND oldsub.end_date = t1.enddate
                        AND oldsub.id = t1.LastSub
                        AND oldsub.sub_state != 8
                JOIN
        ...
        INVOICE AS
                ...
                JOIN 
                        ...
                        AND es.subid = spp.id
                        AND spp.spp_type IN (8)--SPP_type NORMAL included due to Leejam wanting the new calculated invoice to appear when stop date has changed
                        --AND spp.spp_state = 1
                JOIN 
        ...
        CORPORATE AS
                ...
                SUB_TRANSFERRED AS
                        ...
                        WHERE
                                ...
                                AND 
                                s.invoiceline_center IS NULL
                        )                                                                                                                                                                                  
                -------------Final Script---------
                SELECT DISTINCT -----Subscriptions
                        CASE
                                WHEN ps.center IS NULL THEN 'New Sale'
                                WHEN es.start_date - ps.end_date > 90 AND ps.state NOT IN (2,4) THEN 'New Sale - Above 90 days'
                                WHEN es.start_date - ps.end_date <= 90 AND ps.state NOT IN (2,4) THEN 'Renewal - With In 90 Days'
                                WHEN ps.state IN (2,4) THEN 'Renewal - Not expired'
                        END AS "Status" 
                        ,CASE
                ...
                LEFT JOIN
                        ...
                        AND tp.id = es.id
                LEFT JOIN
                        CORPORATE_TRANSFERRED ctp
                        ON ctp.center = es.center
                        AND ctp.id = es.id
                LEFT JOIN
                        SUB_TRANSFERRED subt
