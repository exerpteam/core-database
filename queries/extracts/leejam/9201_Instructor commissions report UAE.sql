...
WITH
    ...
    (
        ...
        AND (
            ...
            AND ac.id IN (203,1401,1601,1,2))
        JOIN
        ...
        ON
            instructor.center = pea2.personcenter
        AND instructor.id = pea2.personid
        AND pea2.name = 'SCALEPT'
        LEFT JOIN
            leejam.staff_groups sg
        ON
        ...
        GROUP BY
            ...
            longtodatec(b.stoptime,b.center) ,
            c.name ,
            p.center||'p'||p.id ,
            p.fullname ,
            pro.name ,
            cc.clips_initial ,
            invl.net_amount ,
            longtodatec(cc.valid_from,cc.center) ,
            longtodatec(cc.valid_until,cc.center) ,
            cc.clips_left ,
            part.state ,
            part.showup_using_card,
            acs.staff_group_id,
            su.booking_id,
            pea2.txtvalue
    )
...
SELECT DISTINCT
   ...
   CASE
        WHEN m2.partstate = 'PARTICIPATION'
        AND m2.staff_group_id IN (1,2,201)
        AND m2.txtvalue2 = 'Old'
        THEN ROUND((m2."Clip card price" / m2."Total number of clips") * (m1.salary/100),2)
        WHEN m2.partstate = 'PARTICIPATION'
        AND m2.staff_group_id IN (1,2,201)
        AND m2.txtvalue2 = 'New'
        AND m3.TotalBooking BETWEEN 1 AND 50
        THEN ROUND((m2."Clip card price" / m2."Total number of clips") * (0.30),2)
        WHEN m2.partstate = 'PARTICIPATION'
        AND m2.staff_group_id IN (1,2,201)
        AND m2.txtvalue2 = 'New'
        AND m3.TotalBooking BETWEEN 51 AND 69
        THEN ROUND((m2."Clip card price" / m2."Total number of clips") * (0.35),2)
        WHEN m2.partstate = 'PARTICIPATION'
        AND m2.staff_group_id IN (1,2,201)
        AND m2.txtvalue2 = 'New'
        AND m3.TotalBooking BETWEEN 70 AND 89
        THEN ROUND((m2."Clip card price" / m2."Total number of clips") * (0.40),2)
        WHEN m2.partstate = 'PARTICIPATION'
        AND m2.staff_group_id IN (1,2,201)
        AND m2.txtvalue2 = 'New'
        AND m3.TotalBooking BETWEEN 90 AND 119
        THEN ROUND((m2."Clip card price" / m2."Total number of clips") * (0.45),2)
        WHEN m2.partstate = 'PARTICIPATION'
        AND m2.staff_group_id IN (1,2,201)
        AND m2.txtvalue2 = 'New'
        AND m3.TotalBooking BETWEEN 120 AND 139
        THEN ROUND((m2."Clip card price" / m2."Total number of clips") * (0.50),2)
        WHEN m2.partstate = 'PARTICIPATION'
        AND m2.staff_group_id IN (1,2,201)
        AND m2.txtvalue2 = 'New'
        AND m3.TotalBooking BETWEEN 140 AND 169
        THEN ROUND((m2."Clip card price" / m2."Total number of clips") * (0.55),2)
        WHEN m2.partstate = 'PARTICIPATION'
        AND m2.staff_group_id IN (1,2,201)
        AND m2.txtvalue2 = 'New'
        AND m3.TotalBooking BETWEEN 170 AND 210
        THEN ROUND((m2."Clip card price" / m2."Total number of clips") * (0.60),2)
        WHEN m2.partstate = 'PARTICIPATION'
        AND m2.staff_group_id NOT IN (1,2,201,202,203,204)
        THEN ROUND((m2."Clip card price" / m2."Total number of clips") * (m1.salary/100),2)
        WHEN m2.partstate = 'PARTICIPATION'
        AND m2.staff_group_id IN (202,203,204)
       -- AND m2.txtvalue2 = 'New'
        AND m4.TotalBooking BETWEEN 1 AND 6
        THEN ROUND((m2."Clip card price" / m2."Total number of clips") * (0.30),2)
        WHEN m2.partstate = 'PARTICIPATION'
        AND m2.staff_group_id IN (202,203,204)
       -- AND m2.txtvalue2 = 'New'
        AND m4.TotalBooking BETWEEN 7 AND 12
        THEN ROUND((m2."Clip card price" / m2."Total number of clips") * (0.40),2)
        WHEN m2.partstate = 'PARTICIPATION'
        AND m2.staff_group_id IN (202,203,204)
        -- AND m2.txtvalue2 = 'New'
        AND m4.TotalBooking BETWEEN 13 AND 7000
        THEN ROUND((m2."Clip card price" / m2."Total number of clips") * (0.50),2)
        ELSE NULL
...
AND m2.staff_group_id = m3.StaffGroupID
LEFT JOIN
    materialized_4 m4
ON
    m2.instructor_external_id = m4.Instructor
AND m2.booking_id = m4.booking_id