...
SELECT DISTINCT
        ...
        CASE
                WHEN part.state = 'PARTICIPATION' THEN 'PARTICIPATED'
                WHEN part.state = 'CANCELLED' THEN 'NO_SHOW'
        END                                             AS "Session status" ,
        part.showup_using_card                          AS "Membercard swiped" ,
        CASE
                WHEN acs.staff_group_id = 201 THEN person_salary.salary
                ELSE ROUND((invl.net_amount / cc.clips_initial) * (person_salary.salary/100),2) 
...
WHERE
        (part.state = 'PARTICIPATION' OR (part.state = 'CANCELLED' AND part.cancelation_reason = 'NO_SHOW'))
        AND 
