    ...
    (
     ...
     TO_CHAR(longtodatec(act.ENTRY_TIME,act.CENTER), 'YYYY-MM-dd HH24:MI') AS Entrytime,
        ...
        JOIN
            CENTERS c
        ON
            c.ID = act.CENTER         AND act.MAIN_TRANSCENTER IS NULL
        LEFT JOIN
...
LEFT JOIN
    ...
    LEFT JOIN
        ...
        and (creditAccount.EXTERNAL_ID in (:external_id) or debitAccount.EXTERNAL_ID in (:external_id))
        ORDER BY
            act.TRANS_TIME ) t1
LEFT JOIN
    PERSONS p
