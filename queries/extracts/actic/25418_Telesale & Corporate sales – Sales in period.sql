...
SELECT
	...
	ss.PRICE_NEW*0.9434 AS JoiningOriginal_excl_VAT,
	ss.PRICE_NEW_DISCOUNT AS JoiningRebate,
	ss.PRICE_NEW_SPONSORED AS JoiningSponsored,
	(ss.PRICE_NEW - ss.PRICE_NEW_DISCOUNT - ss.PRICE_NEW_SPONSORED) as JoiningCustomer,
	(ss.PRICE_INITIAL + ss.PRICE_INITIAL_DISCOUNT) as NormalPrice,
	ss.PRICE_INITIAL_SPONSORED AS InitialSponsored,
	ss.PRICE_INITIAL_DISCOUNT AS InitialRebate,
	ss.PRICE_INITIAL AS Price_After_Rebate,
	cc2.CODE as campaign_code_used,
...
-- calculate cash price	per month
	...
	CASE
		WHEN (trunc(months_between(sub.BINDING_END_DATE + 1,ss.START_DATE))) IS NULL THEN 1
		ELSE (trunc(months_between(sub.BINDING_END_DATE + 1,ss.START_DATE))) END * 
		CASE
		WHEN ss.SUBSCRIPTION_TYPE_TYPE = 0 THEN trunc((ss.PRICE_PERIOD*0.9434) / MONTHS_BETWEEN((ss.END_DATE + 1), ss.START_DATE), 2) -- cash pr month
		WHEN ss.SUBSCRIPTION_TYPE_TYPE = 1 THEN (ss.PRICE_PERIOD*0.9434)
	END   AS "MonthlyPrice*BindingMonths",
...
--	sp.COMENT AS sp_COMENT,
	...
	CASE
		WHEN (trunc(months_between(sub.BINDING_END_DATE + 1,ss.START_DATE))) IS NULL THEN 1
		ELSE (trunc(months_between(sub.BINDING_END_DATE + 1,ss.START_DATE)))
...
AND ss.SALES_DATE BETWEEN (:FromDate) AND (:ToDate)
	AND ss.OWNER_TYPE != 2
	AND ss.EMPLOYEE_CENTER IN (100)
	AND ss.EMPLOYEE_ID IN (6204)
AND ss.TYPE = 1
AND ss.CANCELLATION_DATE IS NULL

	--AND prod.PRIMARY_PRODUCT_GROUP_ID NOT IN (826)
