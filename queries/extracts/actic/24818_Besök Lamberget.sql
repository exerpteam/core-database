SELECT
	...
	DECODE (per.PERSONTYPE, 0,'PRIVATE', 1,'STUDENT', 2,'STAFF', 3,'FRIEND', 4,'CORPORATE', 5,'ONEMANCORPORATE', 6,'FAMILY', 7,'SENIOR', 8,'GUEST', 9,'CONTACT', 'UNKNOWN') AS PERSONTYPE, -- changed 2015-02-18
    DECODE (per.STATUS, 0,'LEAD', 1,'ACTIVE', 2,'INACTIVE', 3,'TEMPORARY INACTIVE', 4,'TRANSFERED', 5,'DUPLICATE', 6, 'PROSPECT',7,'DELETED',9,'CONTACT', 'UNKNOWN')  AS PERSONSTATUS, -- changed 2015-02-18
...
LEFT JOIN 
	(
		...
		WHERE
			...
			AND sub.START_DATE <= TRUNC(SYSDATE -1) -- Date
			AND
				(sub.END_DATE IS NULL
				OR sub.END_DATE >= TRUNC(SYSDATE -1)) -- Date
	) newsub
ON
...
ON
    ...
    AND longToDate(compRel.ENTRY_START_TIME) < TRUNC(SYSDATE) -- Date
    AND
        (compRel.ENTRY_END_TIME IS NULL
        OR longToDate(compRel.ENTRY_END_TIME) > TRUNC(SYSDATE -1))
