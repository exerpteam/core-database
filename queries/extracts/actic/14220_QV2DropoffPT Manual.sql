...
SELECT 
    ...
    DECODE (scl_sub.STATEID, 2,'ACTIVE', 3,'ENDED', 4,'FROZEN', 7,'WINDOW', 8,'CREATED','UNKNOWN') AS subscription_STATE,
	DECODE (scl_sub.SUB_STATE, 1,'NONE', 2,'AWAITING_ACTIVATION', 3,'UPGRADED', 4,'DOWNGRADED', 5,'EXTENDED', 6, 'TRANSFERRED',7,'REGRETTED',8,'CANCELLED',9,'BLOCKED','UNKNOWN')  AS SUBSCRIPTION_SUB_STATE,
...
/*
	AND sub.END_DATE BETWEEN 
		CASE
			WHEN st.ST_TYPE = 1 THEN fromDate - 1	-- eft
			WHEN st.ST_TYPE = 0 THEN fromDate - 30 - 1 -- cash
		END 
		
		AND
		CASE
			WHEN st.ST_TYPE = 1 THEN toDate - 1
			WHEN st.ST_TYPE = 0 THEN toDate - 30 - 1
		END
	AND sub.SUB_STATE !=
		CASE
