...
SELECT
   ...
   ,i2.center_name AS "CENTER_NAME"
  , i2.TODAY AS "TODAY"
  ,i2.COMPANY_NAME AS "COMPANY_NAME"
  ,i2.ADDRESS1 AS "ADDRESS1"
  ,i2.ADDRESS2 AS "ADDRESS2"
  ,i2.ZIPCODE AS "ZIPCODE"
  ,i2.CITY AS "CITY"
  ,i2.SPLIT_RATE_COMMUNITY AS "SPLIT_RATE_COMMUNITY"
  , i1.CLUB AS "CLUB"
  , i1.CR_ID AS "CR_ID"
  , i1.SALES_CENTER AS "SALES_CENTER"
  , i1.SALES_ID AS "SALES_ID"
  , i1.BETAL_DATUM AS "BETAL_DATUM"
  , i1.MEDLEMS_NUMMER AS "MEDLEMS_NUMMER"
  , i1.KUND_TYP AS "KUND_TYP"
  , i1.TYP AS "TYP"
  , i1.ANTAL || ' '                        AS "ANTAL"
  ...
  , CAST(i2.SPLIT_RATE_COMMUNITY * SUM(i1.NETTO) over (partition BY 1) AS NUMERIC) AS "COMMUNITY_PART"
  , i1.MOMS AS "MOMS"
  ...
  , i1.CASH AS "CASH"
  , i1.CARD AS "CARD"
  , i1.AR_CASH AS "AR_CASH"
  , i1.AR_PAY AS "AR_PAY"
  , i1.CHANGE AS "CHANGE"
  , i1.GIFTCARD AS "GIFTCARD"
  , i1.CUSTOM AS "CUSTOM"
  , i1.OTHER AS "OTHER"
   ,i1.WEB AS "WEB"
  ,i1.ptype
  ...
  ,i1.shared_municipality AS "SHARED_MUNICIPALITY"
  ,i1.type as "TYPE"
  ,i1.trans_id AS "TRANS_ID"
FROM
	PARAMS params 
CROSS JOIN
    (
        ...
        WHERE
            (
				/*Include all purchases made in center and by memberservice*/
                INV0.CASHREGISTER_CENTER = $$center$$ 
                AND IL.TOTAL_AMOUNT <> 0
                AND INV0.ENTRY_TIME >= $$periodFromLong$$
                AND INV0.ENTRY_TIME < $$periodToLong$$ + 3600*1000*24
or(INV0.CASHREGISTER_CENTER = 100 and per.center = $$center$$ 
        ...
        WHERE
            (
                ...
                AND CN.ENTRY_TIME < $$periodToLong$$ + 3600*1000*24
or (cn.CASHREGISTER_CENTER = 100 and per.center = $$center$$ AND CN.ENTRY_TIME >= $$periodFromLong$$
                AND CN.ENTRY_TIME < $$periodToLong$$ + 3600*1000*24)
		AND NOT EXISTS
            (
                SELECT
        ...
        UNION ALL
		/*Union all purchases made on web*/
		SELECT
            INV0.CASHREGISTER_CENTER                                   AS CLUB
          , INV0.CASHREGISTER_ID                                       AS CR_ID
          , INV0.CENTER                                                AS SALES_CENTER
          ...
          , 0 CUSTOM
          , 0 OTHER
		  , SUM(ROUND(IL.TOTAL_AMOUNT - (IL.TOTAL_AMOUNT * (1 - (1 / (1 + IL.RATE)))),2)) WEB
          ,prod.PTYPE
          ,CASE WHEN spl.PRODUCT_CENTER IS NOT NULL THEN 1 ELSE 0 END                    SHARED_MUNICIPALITY
          ,'INVOICE'                                       as TYPE
          ,IL.CENTER || 'inv' || IL.ID || 'ln' || IL.SUBID trans_id
        ...
        WHERE
            (
                INV0.CASHREGISTER_CENTER IS NULL 
				AND IL.PERSON_CENTER = $$center$$ 
				AND INV0.EMPLOYEE_CENTER = 100
				AND INV0.EMPLOYEE_ID = 6204
                AND IL.TOTAL_AMOUNT <> 0
                AND INV0.ENTRY_TIME >= $$periodFromLong$$
                AND INV0.ENTRY_TIME < $$periodToLong$$ + 3600*1000*24
		AND NOT EXISTS
...
JOIN
    (
        SELECT
          ...
          ,p.ZIPCODE
          ,p.CITY
          ,atts.TXTVALUE::float / 100 SPLIT_RATE_COMMUNITY
          ,c.NAME              CENTER_NAME
        FROM
            PERSONS p
		CROSS JOIN PARAMS params
