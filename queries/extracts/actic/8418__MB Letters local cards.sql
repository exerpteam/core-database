...
SELECT
	...
	*/  
	...
	/* Check if senior and trunc price */
	CASE
		WHEN per.PERSONTYPE = 7
		THEN trunc(defaultprod_eft.PRICE * 0.70, -1) + 9
		
		ELSE defaultprod_eft.PRICE
	END EFT_12_M,
	CASE
		WHEN per.PERSONTYPE = 7
		THEN trunc(defaultprod_cash.PRICE * 0.70, -2) + 95
...
ON
    cen.ID = per.CENTER
JOIN PRODUCTS defaultprod_eft
ON
    per.CENTER = defaultprod_eft.CENTER
    AND defaultprod_eft.GLOBALID = 'EFT_12_M_RECURRING'
JOIN PRODUCTS defaultprod_cash
ON
    per.CENTER = defaultprod_cash.CENTER
    AND defaultprod_cash.GLOBALID = 'CASH_12_M'
...
WHERE
	...
	AND sub.center in (34, 58, 53, 27, 152, 99, 158, 16, 133, 84, 83, 4, 143, 13, 20, 68, 77, 134, 108, 5)
