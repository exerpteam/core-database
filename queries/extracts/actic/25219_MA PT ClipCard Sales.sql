...
-- HH added centername 2019-12-16
...
WHERE
	i.CENTER IN (:Scope)
	--AND i.TRANS_TIME >= datetolong(TO_CHAR(TRUNC(current_timestamp, 'DDD'), 'YYYY-MM-DD HH24:MI')) -- yesterday at midnight
	--AND i.TRANS_TIME < datetolong(TO_CHAR(TRUNC(current_timestamp, 'DDD'), 'YYYY-MM-DD HH24:MI')) + 86399*1000 -- yesterday at midnight +24 hours in ms
-- ROUND round to nearest..
--	AND i.TRANS_TIME >= datetolong(TO_CHAR(ROUND(current_timestamp), 'YYYY-MM-DD HH24:MI')) -- yesterday at midnight
--	AND i.TRANS_TIME < datetolong(TO_CHAR(ROUND(current_timestamp), 'YYYY-MM-DD HH24:MI')) + 86399*1000 -- yesterday at midnight +24 hours in ms
AND i.TRANS_TIME BETWEEN (:fromDate) AND (:toDate + 3600*1000*24-1) --long date
		AND prod.ptype = 4 
	 AND pg.NAME in ('Personal Training','PT Installment')
...
WHERE
	c.CENTER IN (:Scope)
	--AND c.TRANS_TIME >= datetolong(TO_CHAR(TRUNC(current_timestamp, 'DDD'), 'YYYY-MM-DD HH24:MI')) -- yesterday at midnight
	--AND c.TRANS_TIME < datetolong(TO_CHAR(TRUNC(current_timestamp, 'DDD'), 'YYYY-MM-DD HH24:MI')) + 86399*1000 -- yesterday at midnight +24 hours in ms
-- error after midnight as ROUND rounds to nearest..
--	AND c.TRANS_TIME >= datetolong(TO_CHAR(ROUND(current_timestamp -2), 'YYYY-MM-DD HH24:MI')) -- yesterday at midnight
--	AND c.TRANS_TIME < datetolong(TO_CHAR(ROUND(current_timestamp -2), 'YYYY-MM-DD HH24:MI')) + 86399*1000 -- yesterday at midnight +24 hours in ms
AND c.TRANS_TIME BETWEEN (:fromDate) AND (:toDate + 3600*1000*24-1) --long date
	AND prod.ptype = 4 
	 AND pg.NAME in ('Personal Training','PT Installment','ClipCard-Base')
