...
SELECT
    center,
    SUM(totaldays) days_total,
    SUM(realized_days) days_realized,
    SUM(deferred_days) days_deferred,
    SUM(realized_amount) sum_realized_amount,
    SUM(deferred_amount) sum_deferred_amount
FROM
    (
        SELECT
    sub.center,
