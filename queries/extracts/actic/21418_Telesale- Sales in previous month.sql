...
WHERE
	...
	AND ss.SALES_DATE  >= TRUNC(ADD_MONTHS(exerpsysdate(), -1), 'MM')
 	AND ss.SALES_DATE < TRUNC(LAST_DAY(ADD_MONTHS(exerpsysdate(), -1)))
	AND ss.OWNER_TYPE != 2
	AND ss.EMPLOYEE_CENTER IN (100, 45)
	AND ss.EMPLOYEE_ID IN (12609, 22603, 48212, 56218, 17010, 17410)
