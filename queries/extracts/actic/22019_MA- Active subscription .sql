...
--	TO_CHAR(sub.END_DATE, 'YYYY-MM-DD')				AS end_DATE,
	...
	sp.FROM_DATE AS Current_Price_From,
	sp.TO_DATE AS Current_Price_To,
	sp.COMENT AS Current_price_COMENT,
...
ON
	...
	AND sp.FROM_DATE <= TRUNC(exerpsysdate())
    AND
        (sp.TO_DATE IS NULL
        OR sp.TO_DATE >= TRUNC(exerpsysdate()))
	AND SP.CANCELLED = 0
-----------------------------------------------------------------	
...
WHERE
	...
	AND scl_ptype.PERSONTYPE NOT IN 2 -- exclude staff
	AND prod.PRIMARY_PRODUCT_GROUP_ID IN (7, 8, 9, 218, 222, 10, 11, 219, 221)
	-- Product groups
