...
WITH
    ...
    (
        SELECT
				...
				datetolongTZ(TO_CHAR(TRUNC(to_date(getcentertime(c.id), 'YYYY-MM-DD HH24:MI')-1), 'YYYY-MM-DD HH24:MI'),co.DEFAULTTIMEZONE) AS previousDayLongDate,
                c.ID AS CenterID
        FROM CENTERS c
...
SELECT
    ...
    DECODE (per.PERSONTYPE, 0,'PRIVATE', 1,'STUDENT', 2,'STAFF', 3,'FRIEND', 4,'CORPORATE', 5,
    'ONEMANCORPORATE', 6,'FAMILY', 7,'SENIOR', 8,'GUEST', 9,'CONTACT', 'UNKNOWN') AS PERSONTYPE, --
    -- changed 2015-02-18
    DECODE (per.STATUS, 0,'LEAD', 1,'ACTIVE', 2,'INACTIVE', 3,'TEMPORARY INACTIVE', 4,'TRANSFERED',
    5,'DUPLICATE', 6, 'PROSPECT',7,'DELETED',9,'CONTACT', 'UNKNOWN') AS PERSONSTATUS, -- changed
    -- 2015-02-18
    TO_CHAR(TRUNC(months_between(TRUNC(SYSDATE), per.birthdate)/12)) AS Age,
    per.SEX                                                          AS Gender, -- add 2015-02-18
    CASE
        WHEN newsub.CENTER IS NOT NULL
        THEN newsub.CENTER || 'ss' || newsub.ID
        ELSE NULL
    END                                         AS SubscriptionId,
    DECODE(newsub.ST_TYPE, 0, 'CASH', 1, 'EFT') AS PaymentType,
    CASE
        WHEN compRel.RELATIVECENTER IS NOT NULL
        THEN compRel.RELATIVECENTER || 'p' || compRel.RELATIVEID
        ELSE NULL
    END              AS Company,
    ...
    TO_CHAR(TRUNC(longToDate(cil.CHECKIN_TIME), 'HH'), 'HH24') || ' - ' || TO_CHAR(TRUNC(longToDate
    (cil.CHECKIN_TIME), 'HH') + 1/24, 'HH24') AS Hour,
    CASE
...
AND (
        att.START_TIME - cil.CHECKIN_TIME) BETWEEN -60000 AND 60000 --one minute +- between checkin
    -- and attend is allowed
LEFT JOIN
...
AND (
        par.SHOWUP_TIME - cil.CHECKIN_TIME) BETWEEN -60000 AND 60000 --one minute +- between
    -- checkin and class participation is allowed
LEFT JOIN
    BOOKINGS bk
ON
...
LEFT JOIN
    (
        ...
        FROM
            SUBSCRIPTIONS sub
		JOIN PARAMS params2 ON params2.CenterID = sub.CENTER
        LEFT JOIN
        ...
        WHERE
            prod.PRIMARY_PRODUCT_GROUP_ID IN (7,
                                              8,
                                              9,
                                              10,
                                              11,
                                              ...
                                              222)
            -- Product groups
            -- 7-9 = EFT Subscriptions, 10-12 = CASH subscriptions, 218+222 = EFT campaign
            -- subscriptions, 219+221 = CASH campaign subscriptions
            -- 18 = add-on subscriptions, 624 = Lifestyle, 826 = Excluded
        AND sub.START_DATE <= params2.previousDayDate -- Date
        AND (
                sub.END_DATE IS NULL
            OR  sub.END_DATE >= params2.previousDayDate) -- Date
    ) newsub
ON
...
ON
    compRel.CENTER = per.CENTER
AND compRel.ID= per.ID
AND compRel.ENTRY_START_TIME < params.todayLongDate -- Date
AND (
        compRel.ENTRY_END_TIME IS NULL
    OR  compRel.ENTRY_END_TIME > params.previousDayLongDate)
LEFT JOIN
...
AND cil.CHECKIN_TIME BETWEEN params.previousDayLongDate AND
    (
        params.todayLongDate)
ORDER BY
    cil.CHECKIN_CENTER,
    cil.CHECKIN_TIME