...
SELECT
    ...
    'ONEMANCORPORATE', 6,'FAMILY', 7,'SENIOR', 8,'GUEST', 9,'CONTACT', NULL) AS "PERSONTYPE", --
    -- null for anonymous
    DECODE (per.STATUS, 0,'LEAD', 1,'ACTIVE', 2,'INACTIVE', 3,'TEMPORARY INACTIVE', 4,'TRANSFERED',
    5,'DUPLICATE', 6, 'PROSPECT',7,'DELETED',9,'CONTACT', NULL) AS PERSONSTATUS, -- null for
    -- anonymous
...
JOIN	
	PARAMS params ON params.CenterID = i.CENTER
JOIN
...
WHERE
    i.CENTER IN (select distinct c.id
from AREAS a
join AREA_CENTERS ac 
            on ac.AREA = a.ID
            and a.NAME like '%Germany%'
            and a.ROOT_AREA=19
join CENTERS c on
            c.ID = ac.CENTER)
...
AND i.TRANS_TIME < params.toDate
    ...
    -- AND i.TRANS_TIME >= datetolong(TO_CHAR(ROUND(exerpsysdate() -2), 'YYYY-MM-DD HH24:MI')) --
    -- yesterday at midnight
    -- AND i.TRANS_TIME < datetolong(TO_CHAR(ROUND(exerpsysdate() -2), 'YYYY-MM-DD HH24:MI')) + 86399*1000
    -- yesterday at midnight +24 hours in ms
...
SELECT
    ...
    END AS PersonID,
    DECODE (per.PERSONTYPE, 0,'PRIVATE', 1,'STUDENT', 2,'STAFF', 3,'FRIEND', 4,'CORPORATE', 5,
    'ONEMANCORPORATE', 6,'FAMILY', 7,'SENIOR', 8,'GUEST', 9,'CONTACT', NULL) AS PERSONTYPE, -- null
    -- for anonymous
    DECODE (per.STATUS, 0,'LEAD', 1,'ACTIVE', 2,'INACTIVE', 3,'TEMPORARY INACTIVE', 4,'TRANSFERED',
    5,'DUPLICATE', 6, 'PROSPECT',7,'DELETED',9,'CONTACT', NULL) AS PERSONSTATUS, -- null for
    -- anonymous
...
JOIN PARAMS params ON params.CenterID = c.CENTER
JOIN
    CREDIT_NOTE_LINES cl -- all lines/product in a receipt
ON
    cl.center = c.center
AND cl.id = c.id
JOIN
...
AND emp.PERSONID = emp_person.ID
WHERE
    c.CENTER IN (select distinct c.id
from AREAS a
join AREA_CENTERS ac 
            on ac.AREA = a.ID
            and a.NAME like '%Germany%'
...
join CENTERS c on
            c.ID = ac.CENTER)
AND c.TRANS_TIME >= params.fromDate
    -- yesterday at midnight
AND c.TRANS_TIME < params.toDate
    -- yesterday at midnight +24 hours in ms
    -- error after midnight as ROUND rounds to nearest..
    -- AND c.TRANS_TIME >= datetolong(TO_CHAR(ROUND(exerpsysdate() -2), 'YYYY-MM-DD HH24:MI')) --
    -- yesterday at midnight
    -- AND c.TRANS_TIME < datetolong(TO_CHAR(ROUND(exerpsysdate() -2), 'YYYY-MM-DD HH24:MI')) + 86399*1000
    -- yesterday at midnight +24 hours in ms
