SELECT
    ...
    i1.type ,
    i1.trans_id,
    i1.SALES_CENTER || TO_CHAR(exerpsysdate(),'YYYYMM')       INV_ID,
    ...
    i2.BILLING_NBR,
    i2.CONTACT_PERSON,
    i2.CONTACT_WORK_PHONE,
    i2.CONTACT_EMAIL,
    COUNT(i1.ANTAL) over (partition BY 1) AS "COUNT"
FROM
    (
        SELECT
        ...
        WHERE
            (
                ...
                AND INV0.ENTRY_TIME >= dateToLongC(TO_CHAR(TRUNC(ADD_MONTHS((LAST_DAY(exerpsysdate())+1),-2)), 'YYYYMMdd HH24:MI'),$$center$$)
                AND INV0.ENTRY_TIME < dateToLongC(TO_CHAR(TRUNC(ADD_MONTHS((LAST_DAY(exerpsysdate())+1),-1)), 'YYYYMMdd HH24:MI'),$$center$$) )
        GROUP BY
        ...
        WHERE
            (
                ...
                AND CN.ENTRY_TIME >= dateToLongC(TO_CHAR(TRUNC(ADD_MONTHS((LAST_DAY(exerpsysdate())+1),-2)), 'YYYYMMdd HH24:MI'),$$center$$)
                AND CN.ENTRY_TIME < dateToLongC(TO_CHAR(TRUNC(ADD_MONTHS((LAST_DAY(exerpsysdate())+1),-1)), 'YYYYMMdd HH24:MI'),$$center$$) )
        GROUP BY
...
JOIN
    (
        SELECT
            TO_CHAR(exerpsysdate() ,'YYYY-MM-DD') today ,
            ...
            NVL(p.SSN,' ')                          ORG_NBR ,
            c.NAME                                  CENTER_NAME,
            NVL(EXT_ACCOUNT_NUMBER.TXTVALUE,' ') AS ACCOUNT_NUMBER,
            NVL(EXT_VAT.TXTVALUE ,' ')           AS VAT,
            NVL(bn.TXTVALUE ,' ')                AS BILLING_NBR,
            NVL(cp.TXTVALUE ,' ')                AS CONTACT_WORK_PHONE,
            NVL(ce.txtvalue ,' ')                AS CONTACT_EMAIL,
            NVL(contact_p.fullname ,' ')         AS CONTACT_PERSON
        FROM
            PERSONS p
        JOIN
            CENTERS c
        ...
        ON
            p.center = EXT_ACCOUNT_NUMBER.PERSONCENTER
            AND p.id = EXT_ACCOUNT_NUMBER.PERSONID
            AND EXT_ACCOUNT_NUMBER.NAME = 'AccountNumber'
        LEFT JOIN
            PERSON_EXT_ATTRS bn
        ON
            bn.PERSONCENTER = p.CENTER
            AND bn.PERSONID = p.ID
            AND bn.NAME = '_eClub_BillingNumber'
        LEFT JOIN
            PERSON_EXT_ATTRS EXT_VAT
        ON
            p.center = EXT_VAT.PERSONCENTER
            AND p.id = EXT_VAT.PERSONID
        ...
        LEFT JOIN
            PERSONS contact_p
        ON
            contact_p.CENTER = rel.RELATIVECENTER
            AND contact_p.id = rel.RELATIVEID
        LEFT JOIN
            PERSON_EXT_ATTRS ce
        ON
            ce.PERSONCENTER = contact_p.CENTER
            AND ce.PERSONID = contact_p.id
            AND ce.NAME = '_eClub_Email'
        LEFT JOIN
            PERSON_EXT_ATTRS cp
        ON
            cp.PERSONCENTER = contact_p.CENTER
            AND cp.PERSONID = contact_p.id
