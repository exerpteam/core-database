...
/**
* Creator: Exerp
* Purpose: Get a summary of all recurring payments within a period. 
* Supposed to be published as a Jasper report.
* Note: Community reports OLD - this is deprecated
*/
WITH params as (

        select /*+ materialize */ $$FromDate$$ as FromDate,
        $$ToDate$$ as ToDate,
        $$Club$$ as Club from dual
) 
...
SELECT /*+ NO_BIND_AWARE */
    ...
    CASE
        ...
        THEN SUM(sales.excluding_vat) - nvl(to_number(adminfee1.TXTVALUE, '999999.99999'), 0) - nvl(to_number(adminfee2.TXTVALUE,
            '999999.99999'), 0)
        WHEN SUM(sales.total_amount) < 0
    ...
    CASE
        WHEN SUM(sales.total_amount) > 0
        THEN to_number(splitrate.TXTVALUE, '999999.99999') / 100 * (SUM(sales.excluding_vat) - nvl(to_number(
            adminfee1.TXTVALUE, '999999.99999'), 0) - nvl(to_number(adminfee2.TXTVALUE, '999999.99999'), 0))
        WHEN SUM(sales.total_amount) < 0
...
FROM
    ...
    (
		SELECT /*+ INDEX(ACCOUNT_TRANS IDX_ACT_INFO_TIME) */
		...
		WHERE
		    ...
		    AND artSales.REF_CENTER = params.Club
		    AND act.TRANS_TIME >= params.FromDate
		    AND act.TRANS_TIME < params.ToDate + 1000*60*60*24
		      AND act.center in (select id from centers where country = 'NO')
		UNION ALL
		SELECT
		    p.CENTER || 'p' || p.id payerid,
		    prs.ref,
...
WHERE payments.REF_CENTER = params.Club
GROUP BY
