...
SELECT
    ...
    ,TO_CHAR(longtodate(:ToDate), 'dd.MM.yyyy') period_to
  ,i2.center_name
  , i2.TODAY
  ,i2.COMPANY_NAME
  ,i2.ADDRESS1
...
FROM
    (
        ...
        ON
            ...
            AND spl.PRODUCT_GROUP_ID IN (8825)
        INNER JOIN
            INVOICES INV0
        ON
            (
...
or(INV0.CASHREGISTER_CENTER = 100 and per.center = $$center$$ 		    AND INV0.ENTRY_TIME  >= :FromDate
    AND INV0.ENTRY_TIME < :ToDate + (1000*60*60*24))
		AND NOT EXISTS
            (
                SELECT
    ...
    AND CRR.STARTTIME  >= :FromDate
    AND CRR.REPORTTIME < :ToDate + (1000*60*60*24)
 )
        LEFT JOIN
            PERSONS PER
        ON
...
or(cn.CASHREGISTER_CENTER = 100 and per.center = $$center$$ AND CN.ENTRY_TIME  >= :FromDate
    AND CN.ENTRY_TIME < :ToDate + (1000*60*60*24))       
 		AND NOT EXISTS
            (
                SELECT
