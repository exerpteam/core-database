...
/**
* Creator: Exerp
* Purpose: List prices for companyagreements by center.
*/
SELECT
	DISTINCT club.ID, 
	club.SHORTNAME,
	...
	ca.AVAILABILITY CA_AVAILABLE,
	prod.NAME product,
	pp.VALID_FOR privilege_valid,
...
WHERE
	club.id = :scope
	AND (ca.AVAILABILITY LIKE '%C' || club.id || '%' OR ca.AVAILABILITY = :incl_country)
	AND
	(
		ca.STOP_NEW_DATE IS NULL
		OR ca.STOP_NEW_DATE > current_timestamp
	)
	AND ca.state::varchar IN (:ca_state)
