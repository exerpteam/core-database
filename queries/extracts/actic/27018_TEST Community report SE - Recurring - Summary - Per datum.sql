   ...
   ( SELECT :Club id from dual) center JOIN
    ...
    (
		SELECT /*+ INDEX(ACCOUNT_TRANS IDX_ACT_INFO_TIME) */
		    ...
		    prs.ref,
		    xx.amount,
		    TO_CHAR(longtodate(xx.paymentDate), 'YYYY-MM-DD') paymentDate,
		    artSales.REF_CENTER,
		    artSales.REF_ID,
		    artSales.REF_TYPE,
		    CASE
		...
		(
		select sum(art.amount) Amount, art.Info Info , act.Info_Type, p.CENTER || 'p' || p.id payerid, p.SEX, max(art.TRANS_TIME) PAYMENTDATE
		from
		    ACCOUNT_TRANS act
		JOIN AR_TRANS art
		ON
		...
		WHERE
		    act.CENTER <= 500
        group by art.Info, act.Info_Type, p.CENTER, p.id, p.SEX
		    ) xx
		...
		WHERE
		    xx.INFO_TYPE IN (3, 16)
		    AND artSales.REF_CENTER = :Club
		    AND xx.PAYMENTDATE >= :FromDate
		    AND xx.PAYMENTDATE < :ToDate + 1000*60*60*24
		UNION ALL
		SELECT
