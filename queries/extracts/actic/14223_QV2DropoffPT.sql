 ...
 WITH
    ...
    (
        SELECT
				...
				TRUNC(to_date(getcentertime(c.id), 'YYYY-MM-DD HH24:MI')-1-30) AS cutDateCash,
                c.ID AS CenterID
        FROM CENTERS c
...
--  DECODE (p.PERSONTYPE, 0,'PRIVATE', 1,'STUDENT', 2,'STAFF', 3,'FRIEND', 4,'CORPORATE', 5,'ONEMANCORPORATE', 6,'FAMILY', 7,'SENIOR', 8,'GUEST', 9,'CONTACT', 'UNKNOWN') AS Current_PTYPE,
...
--	AND sub.END_DATE = TRUNC(current_timestamp -1)
--	AND scl_ptype.STATEID != 2 -- changed to historical persontype
	...
	AND sub.END_DATE BETWEEN 
		CASE
			WHEN st.ST_TYPE = 1 THEN params.previousDay	 -- eft
			WHEN st.ST_TYPE = 0 THEN params.cutDateCash -- cash
		END 
		
		AND
		CASE
			WHEN st.ST_TYPE = 1 THEN params.previousDay
			WHEN st.ST_TYPE = 0 THEN params.cutDateCash
		END
	AND sub.SUB_STATE !=
		CASE
