...
SELECT
	...
	TO_CHAR(trunc(months_between(TRUNC(exerpsysdate()), p1.birthdate)/12)) AS Age,
...
WHERE
	...
	AND (a.person_center||'p'||a.person_ID) IN (:memberid)
...
SELECT
	...
	TO_CHAR(trunc(months_between(TRUNC(exerpsysdate()), p2.birthdate)/12)) AS Age,
...
AND (b.OWNER_CENTER||'p'||b.OWNER_ID) IN (:memberid)
