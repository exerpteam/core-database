        ...
        SELECT
            ...
            (
                SELECT DISTINCT
                    ...
                    --,
                    --sum(spp1.TO_DATE + 1 - greatest(spp1.FROM_DATE, dat.LastSale)) days
                FROM
                    PERSONS p1
                JOIN
                    SUBSCRIPTIONS sub1
        ...
        FROM
            (
                ...
                GROUP BY
                    ...
                    p.id ) dat ) dat2
WHERE
    earliest_newsub_deduction_date <= SYSDATE
    AND (
        LastSale IS NULL
        OR add_months(dat2.LastSale, 12 + NVL(freeze_months, 0)) <= SYSDATE )