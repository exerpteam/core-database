...
----------------------------------------------------------------
	...
	sub.OWNER_CENTER || 'p' || sub.OWNER_ID 		AS PersonId,
		TO_CHAR(trunc(months_between(TRUNC(sysdate),scl_ptype.birthdate)/12)) AS Age,
...
prod.PRIMARY_PRODUCT_GROUP_ID AS PrimaryGroupId,
scl_ptype.SSN AS SSN
	-------------------------------------------------------------
...
WHERE
	...
	AND sub.START_DATE <= TRUNC(:TheDate) -- Date
	AND
		(sub.END_DATE IS NULL
		OR sub.END_DATE >= TRUNC(:TheDate)) -- Date
	-------------------
	...
	-------------------
	AND longToDate(sub.CREATION_TIME) < TRUNC(:TheDate) -- make sure we dont include sales with start date in past
	...
	AND prod.PRIMARY_PRODUCT_GROUP_ID IN (7, 8, 9, 10, 11, 12, 218, 219, 221, 222, 826)
	-- Product groups
