...
-- added by MB
/* remove to see if this makes duplicates
...
ON
    ...
    AND longToDate(scl_ptype.ENTRY_START_TIME) < TRUNC(exerpsysdate()) -- Date
    AND
        (scl_ptype.ENTRY_END_TIME IS NULL
        OR longToDate(scl_ptype.ENTRY_END_TIME) > TRUNC(exerpsysdate() -1))
*/
-----------------------------------------------------------------	
...
WHERE
	...
	AND p.PERSONTYPE IN (:Persontype) -- only include choosen persontypes
