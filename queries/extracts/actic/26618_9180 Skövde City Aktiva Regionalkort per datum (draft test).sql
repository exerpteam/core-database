...
* Used as a report within the CommunityReports.
*
* Note: Is hardcoded and named for Skovde City (180). Should probably be edited with this as parameter instead.
*
*/
SELECT
----------------------------------------------------------------
	...
	(TRUNC(MONTHS_BETWEEN(cast(:MemberBaseDate as date), p.BIRTHDATE)/12))::varchar AS Age,
	p.SEX AS Gender,
...
--	sp.PRICE AS Monthly_Price, -- is null if there is no price changes 
	...
	cast(:MemberBaseDate as date) AS MEMBERBASEDATE
...
LEFT JOIN
	(
		...
		ON
			...
			AND companyAgrRel.RTYPE = 3 -- Relationstatus company
		WHERE
...
prod.name Like '%regionalt%')
	...
	AND sub.OWNER_CENTER IN (180) -- Hardcoded to 180 Skövde City. No good solution.
	...
	AND scl_substate.STATEID IN (2) -- only include subscriptions that are active
