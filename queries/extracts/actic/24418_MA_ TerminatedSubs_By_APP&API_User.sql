...
--  
/*
 * Terminated Cards APP API User
 * mimics the Subscription sales report Exerp
 * filter on: Termination Date
 */
...
WHERE
    ...
    AND ss.TERMINATION_DATE >= :FromDate 
   	AND ss.TERMINATION_DATE < :ToDate + 1
	AND ss.OWNER_TYPE != 2
	AND ss.CANCELLATION_EMPLOYEE_CENTER = 100
	AND ss.CANCELLATION_EMPLOYEE_ID IN (6204, 15203)
