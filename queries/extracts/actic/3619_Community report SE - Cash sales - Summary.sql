/**
* Creator: Exerp
* Purpose: Summary of Cash Sales during a given period.
* Note: Community reports OLD - this is deprecated
*
*
*/
SELECT
    sales_center,
    sales_club,
    cashRegisterCenter,
    pname product,
    COUNT(*) antal,
    tot_amount price,
    SUM(excl_vat) excl_vat,
    SUM(incl_vat) incl_vat,
    SUM(excl_vat) + SUM(incl_vat) total,
    vat_rate
FROM
    (
        SELECT
            sales_center,
            sales_club,
            cashRegisterCenter,
            dato,
            pname,
            vat_rate,
            person_Id,
            person_Type,
            emp_id,
            SUM(excluding_Vat) excl_vat,
            SUM(included_Vat) incl_vat,
            SUM(total_Amount) tot_amount
        FROM
            (
                SELECT
                    i.center sales_center,
                    club.NAME sales_club,
                    (
                        SELECT DISTINCT
                            crt.center
                        FROM
                            CASHREGISTERTRANSACTIONS crt
                        WHERE
                            crt.CENTER = i.CASHREGISTER_CENTER
                            AND crt.ID = i.CASHREGISTER_ID
                            AND crt.PAYSESSIONID = i.PAYSESSIONID
                    )
                    cashRegisterCenter,
                    TO_CHAR(longtodate(i.TRANS_TIME), 'YYYY-MM-DD HH24:MI') dato,
                    prod.NAME pname,
                    ROUND(il.TOTAL_AMOUNT - (il.TOTAL_AMOUNT * (1-(1/(1+il.RATE)))),2) excluding_Vat,
                    ROUND(il.TOTAL_AMOUNT * (1-(1/(1+il.RATE))),2) included_Vat,
                    ROUND(il.TOTAL_AMOUNT, 2) total_Amount,
                    il.RATE vat_rate,
                    CASE
                        WHEN il.PERSON_CENTER IS NOT NULL
                        THEN il.PERSON_CENTER || 'p' || il.PERSON_ID
                        ELSE NULL
                    END person_id,
                    CASE
                        WHEN il.PERSON_CENTER IS NOT NULL
                        THEN DECODE ( per.PERSONTYPE, 0,'PRIVATE', 1,'STUDENT', 2,'STAFF', 3,'FRIEND', 4,'CORPORATE', 5
                            , 'ONEMANCORPORATE', 6, 'FAMILY', 7,'SENIOR', 8,'GUEST','UNKNOWN')
                        ELSE NULL
                    END person_type,
                    i.EMPLOYEE_CENTER || 'emp' || i.EMPLOYEE_ID emp_id
                FROM
                    INVOICES i
                JOIN INVOICELINES il
                ON
                    il.center = i.center
                    AND il.id = i.id
                JOIN PRODUCTS prod
                ON
                    prod.center = il.PRODUCTCENTER
                    AND prod.id = il.PRODUCTID
                JOIN CENTERS club
                ON
                    i.center = club.id
                LEFT JOIN PERSONS per
                ON
                    il.PERSON_CENTER = per.center
                    AND il.PERSON_ID = per.id
                WHERE
                    i.CENTER = :club
                    AND il.TOTAL_AMOUNT <> 0
                    AND i.TRANS_TIME >= :fromDate
                    AND i.TRANS_TIME < :toDate + 60*60*1000*24
					AND prod.PTYPE not in (8,9)
                    AND
                    (
                        i.EMPLOYEE_CENTER, i.EMPLOYEE_ID
                    )
                    NOT IN ((100,1))
                    AND NOT EXISTS
                    (
                        SELECT
                            *
                        FROM
                            AR_TRANS art
                        JOIN ACCOUNT_RECEIVABLES ar
                        ON
                            ar.center = art.center
                            AND ar.id = art.id
                        WHERE
                            art.REF_CENTER = i.CENTER
                            AND art.REF_ID = i.ID
                            AND art.REF_TYPE = 'INVOICE'
                            AND ar.AR_TYPE = 4
                    )
                UNION ALL
                SELECT
                    c.center sales_center,
                    club.NAME sales_club,
                    (
                        SELECT DISTINCT
                            crt.center
                        FROM
                            CASHREGISTERTRANSACTIONS crt
                        WHERE
                            crt.CENTER = c.CASHREGISTER_CENTER
                            AND crt.ID = c.CASHREGISTER_ID
                            AND crt.PAYSESSIONID = c.PAYSESSIONID
                    )
                    cashRegisterCenter,
                    TO_CHAR(longtodate(c.TRANS_TIME), 'YYYY-MM-DD HH24:MI') dato,
                    prod.NAME pname,
                    -ROUND(cl.TOTAL_AMOUNT - ROUND(cl.TOTAL_AMOUNT * (1-(1/(1+cl.RATE))), 2), 2) excluding_Vat,
                    -ROUND(cl.TOTAL_AMOUNT * (1-(1/(1+cl.RATE))), 2) included_Vat,
                    -ROUND(cl.TOTAL_AMOUNT, 2) total_Amount,
                    cl.RATE vat_rate,
                    CASE
                        WHEN cl.PERSON_CENTER IS NOT NULL
                        THEN cl.PERSON_CENTER || 'p' || cl.PERSON_ID
                        ELSE NULL
                    END person_id,
                    CASE
                        WHEN cl.PERSON_CENTER IS NOT NULL
                        THEN DECODE ( per.PERSONTYPE, 0,'PRIVATE', 1,'STUDENT', 2,'STAFF', 3,'FRIEND', 4,'CORPORATE', 5
                            , 'ONEMANCORPORATE', 6, 'FAMILY', 7,'SENIOR', 8,'GUEST','UNKNOWN')
                        ELSE NULL
                    END person_type,
                    c.EMPLOYEE_CENTER || 'emp' || c.EMPLOYEE_ID emp_id
                FROM
                    CREDIT_NOTES c
                JOIN CREDIT_NOTE_LINES cl
                ON
                    cl.center = c.center
                    AND cl.id = c.id
                JOIN PRODUCTS prod
                ON
                    prod.center = cl.PRODUCTCENTER
                    AND prod.id = cl.PRODUCTID
                JOIN CENTERS club
                ON
                    c.center = club.id
                LEFT JOIN PERSONS per
                ON
                    cl.PERSON_CENTER = per.center
                    AND cl.PERSON_ID = per.id
                WHERE
                    c.CENTER = :club
                    AND cl.TOTAL_AMOUNT <> 0
                    AND c.TRANS_TIME >= :fromDate
                    AND c.TRANS_TIME < :toDate + 60*60*1000*24
					AND prod.PTYPE not in (8,9)
                    AND
                    (
                        c.EMPLOYEE_CENTER, c.EMPLOYEE_ID
                    )
                    NOT IN ((100,1))
                    AND NOT EXISTS
                    (
                        SELECT
                            *
                        FROM
                            AR_TRANS art
                        JOIN ACCOUNT_RECEIVABLES ar
                        ON
                            ar.center = art.center
                            AND ar.id = art.id
                        WHERE
                            art.REF_CENTER = c.CENTER
                            AND art.REF_ID = c.ID
                            AND art.REF_TYPE = 'CREDIT_NOTE'
                            AND ar.AR_TYPE = 4
                    )
            )
        GROUP BY
            sales_center,
            sales_club,
            cashRegisterCenter,
            dato,
            pname,
            vat_rate,
            person_Id,
            person_Type,
            emp_id
        ORDER BY
            1,
            2 DESC,
            3
    )
GROUP BY
    sales_center,
    sales_club,
    cashRegisterCenter,
    pname,
    tot_amount,
    vat_rate
ORDER BY
    1,2,3,4,5
