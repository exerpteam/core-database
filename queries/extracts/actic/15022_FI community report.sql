SELECT  /*+ NO_BIND_AWARE */
    ...
    CASE
        ...
        ELSE 0
    END FEE_APPLIED,
    TO_CHAR(longtodateTZ((:FromDate ), 'Europe/Helsinki'), 'dd.MM.yyyy') PeriodStart,
    TO_CHAR(longtodateTZ(:ToDate, 'Europe/Helsinki'), 'dd.MM.yyyy') PeriodEnd,
   ...
   ( SELECT :Club id from dual) center JOIN
    ...
    (
		SELECT /*+ INDEX(ACCOUNT_TRANS IDX_ACT_INFO_TIME) */
		    ...
		    '' as ref,
		    art.AMOUNT,
		    TO_CHAR(longtodateTZ(art.TRANS_TIME, 'Europe/Helsinki'), 'YYYY-MM-DD') paymentDate,
		    artSales.REF_CENTER,
		    artSales.REF_ID,
		    artSales.REF_TYPE,
		    CASE
...
*/
		...
		JOIN ART_MATCH arm on arm.ART_PAYING_CENTER = art.center and arm.ART_PAYING_ID = art.id and arm.ART_PAYING_SUBID = art.subid and arm.CANCELLED_TIME is null
		
		JOIN AR_TRANS artSales on artSales.center = arm.ART_PAID_CENTER and artSales.id = arm.ART_PAID_ID and artSales.subid = arm.ART_PAID_SUBID AND artSales.REF_TYPE IN ('INVOICE', 'CREDIT_NOTE')
		...
		WHERE
		    ...
		    AND act.CENTER in (select id from centers where id >= 600)
