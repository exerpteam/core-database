...
* Used by financedepartment to create rentreports for municipality clubs.
* Note: Should be renamed due to not connected to specific center.
*/
WITH
    params AS
    (
        SELECT
            dateToLongC(TO_CHAR(trunc((:fromDate)::date),'YYYY-MM-DD'),:Club)   FromDate 
            ,dateToLongC(TO_CHAR(trunc((:toDate)::date),'YYYY-MM-DD'),:Club)   ToDate             
        FROM
        	CENTERS c
        WHERE
	    c.id = :Club
...
SELECT
 ...
 , SUM(0)  "INCL_VAT"
  ...
  , CASE
        WHEN SUM(sales.total_amount) > 0
        THEN SUM(sales.total_amount) - COALESCE(to_number(adminfee2.TXTVALUE, '999999.99999'), 0) - COALESCE(to_number(adminfee2.TXTVALUE, '999999.99999'), 0)
        WHEN SUM(sales.total_amount) < 0
        THEN SUM(sales.total_amount)
        ELSE 0
    END                                           "SPLITAMOUNT"
  , to_number(splitrate.TXTVALUE, '999999.99999') "SPLITRATE"
  , SUM (sales.total_amount) "COMMUNITYPART"
  , CASE
        WHEN SUM(sales.total_amount) > 0
        THEN 1
        ELSE 0
        ...
        WHERE
            ...
            (
                ...
                FROM
                    centers
                WHERE
                    id <= 200) -- Not a very good solution...
        UNION ALL
        SELECT
            p.CENTER || 'p' || p.id payerid
          , prs.ref
...
JOIN
    ...
    (
        ...
        WHERE
            ...
            AND prod.PTYPE IN (5,10,12) --ProductType
			AND prod.GLOBALID = 'EFT_0_M_GROUP_TRAINING'
...
ON
    ...
    AND splitrate.NAME = 'SPLITRATECOMMUNITY2'
WHERE
