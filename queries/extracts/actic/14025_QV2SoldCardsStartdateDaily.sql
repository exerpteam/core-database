...
/* SoldCardsDailyManual */
/* mimics the Subscription sales report in Exerp */
-- TODO
-- is the persontype correct historically
-- correct monthly price on salesdate
...
SELECT
    ...
    CASE  ss.OWNER_TYPE  WHEN 0 THEN 'PRIVATE'  WHEN 1 THEN 'STUDENT'  WHEN 2 THEN 'STAFF'  WHEN 3 THEN 'FRIEND'  WHEN 4 THEN 'CORPORATE'  WHEN 5 THEN 'ONEMANCORPORATE'  WHEN 6 THEN 'FAMILY'  WHEN 7 THEN 'SENIOR'  WHEN 8 THEN 'GUEST' ELSE 'UNKNOWN' END AS PersonType,
...
JOIN PARAMS params ON params.CenterID = ss.SUBSCRIPTION_CENTER
...
WHERE
	...
	--AND ss.START_DATE BETWEEN TRUNC(current_timestamp - 1) AND TRUNC(current_timestamp - 1)
	AND ss.START_DATE = params.previousDay
	AND ss.OWNER_TYPE != 2
