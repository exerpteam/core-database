...
--	AND i.TRANS_TIME >= datetolong(TO_CHAR(ROUND(exerpsysdate() -2), 'YYYY-MM-DD HH24:MI')) -- yesterday at midnight
--	AND i.TRANS_TIME < datetolong(TO_CHAR(ROUND(exerpsysdate() -2), 'YYYY-MM-DD HH24:MI')) + 86399*1000 -- yesterday at midnight +24 hours in ms
...
-- persons
	...
	DECODE (per.PERSONTYPE, 0,'PRIVATE', 1,'STUDENT', 2,'STAFF', 3,'FRIEND', 4,'CORPORATE', 5,'ONEMANCORPORATE', 6,'FAMILY', 7,'SENIOR', 8,'GUEST', 9,'CONTACT', NULL) AS PERSONTYPE, -- null for anonymous
    DECODE (per.STATUS, 0,'LEAD', 1,'ACTIVE', 2,'INACTIVE', 3,'TEMPORARY INACTIVE', 4,'TRANSFERED', 5,'DUPLICATE', 6, 'PROSPECT',7,'DELETED',9,'CONTACT', NULL)  AS PERSONSTATUS, -- null for anonymous
	TO_CHAR(trunc(months_between(TRUNC(exerpsysdate()date), per.birthdate)/12)) AS Age,
	per.SEX AS GENDER
...
--	AND c.TRANS_TIME >= datetolong(TO_CHAR(ROUND(exerpsysdate() -2), 'YYYY-MM-DD HH24:MI')) -- yesterday at midnight
--	AND c.TRANS_TIME < datetolong(TO_CHAR(ROUND(exerpsysdate() -2), 'YYYY-MM-DD HH24:MI')) + 86399*1000 -- yesterday at midnight +24 hours in ms
