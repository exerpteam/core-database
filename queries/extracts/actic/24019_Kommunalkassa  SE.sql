...
SELECT
    --TO_CHAR(TRUNC(ADD_MONTHS((LAST_DAY(current_timestamp)+1),-2)),'YYYY-MM-DD') PERIOD_FROM
  --TO_CHAR(TRUNC(ADD_MONTHS((LAST_DAY(current_timestamp)),-1)),'YYYY-MM-DD')    period_to
  --i1.CLUB
  --i1.CR_ID
   i1.SALES_CENTER
  --i1.SALES_ID
    ...
    (
        ...
        WHERE
            (
				...
				AND NOT (INV0.EMPLOYEE_CENTER = 100 AND INV0.EMPLOYEE_ID = 6204) -- Remove API User
			)
        GROUP BY
