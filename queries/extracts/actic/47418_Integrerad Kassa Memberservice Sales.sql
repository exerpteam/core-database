...
-- IK-report for sales made by memberservice
WITH
...
JOIN
    (
        ...
        WHERE
            (p.LASTNAME LIKE '9' || REPLACE(lpad((:center)::varchar,3),' ','0') || ' %') or (p.LASTNAME LIKE '9' || REPLACE(lpad((:center)::varchar,4),' ','0') || ' %')) company
ON
    1 = 1
WHERE
	per.CENTER = :center
	AND	(
		...
		OR (INV0.EMPLOYEE_CENTER = 100 AND INV0.EMPLOYEE_ID = 6204)
		OR
		(INV0.EMPLOYEE_CENTER = 100 AND INV0.EMPLOYEE_ID = 49204)
	)
