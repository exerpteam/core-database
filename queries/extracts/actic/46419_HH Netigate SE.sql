WITH
    ...
    (
        SELECT
            /*+ materialize */
            datetolongTZ(TO_CHAR((TRUNC(SYSDATE) - INTERVAL '35' DAY) , 'YYYY-MM-dd HH24:MI'),
            'Europe/Stockholm') AS DAYS_AGO_35,
            datetolongTZ(TO_CHAR((TRUNC(SYSDATE) - INTERVAL '34' DAY) , 'YYYY-MM-dd HH24:MI'),
            'Europe/Stockholm') AS DAYS_AGO_34,
            datetolongTZ(TO_CHAR((TRUNC(SYSDATE) - INTERVAL '65' DAY) , 'YYYY-MM-dd HH24:MI'),
            'Europe/Stockholm')                AS DAYS_AGO_65,
            TRUNC(SYSDATE) - INTERVAL '35' DAY AS DAYS_AGO_35_date,
            TRUNC(SYSDATE) - INTERVAL '34' DAY AS DAYS_AGO_34_date
        FROM
            dual
    )
SELECT
...
FROM
    (
        ...
        ORDER BY
            st.entry_start_time)
WHERE
    (STATE_FROM,state_to) IN ((0,1),(2,1), (6,1), (9,1))
    AND rnk = 2
) t1
ON 
    t1.center = p.center
        ...
        FROM
            (
                SELECT
                    ...
                    par.PARTICIPANT_ID,
                    CASE
                        WHEN (ag.id = 2203 OR  ag.top_node_id = 2203)
                        THEN 1
                        ELSE 0
                    END AS pt,
                    1      total
...
WHERE
	...
	AND PR.NAME NOT LIKE ('%Actic Anywhere%')