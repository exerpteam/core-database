...
LEFT JOIN
    ar_trans art ON inv.center = art.ref_center AND inv.id = art.ref_id AND art.ref_type = 'INVOICE'
LEFT JOIN
    account_receivables ar ON ar.center = art.center AND ar.id = art.id                   
LEFT JOIN
    accounts credit ON credit.center = act.credit_accountcenter AND credit.id = act.credit_accountid                     
JOIN
    centers c ON c.id = inv.center                          
JOIN
...
LEFT JOIN
    ar_trans art ON cn.center = art.ref_center AND cn.id = art.ref_id AND art.ref_type = 'CREDIT_NOTE' 
LEFT JOIN
    account_receivables ar ON ar.center = art.center AND ar.id = art.id                     
LEFT JOIN
    accounts debit ON debit.center = act.debit_accountcenter AND debit.id = act.debit_accountid 
JOIN
    centers c ON c.id = cn.center                                  
JOIN
...
FROM
    account_trans act
JOIN
    ar_trans art ON act.center = art.ref_center AND act.id = art.ref_id AND act.subid = art.ref_subid AND art.ref_type = 'ACCOUNT_TRANS'
JOIN
    accounts credit ON credit.center = act.credit_accountcenter AND credit.id = act.credit_accountid
JOIN
    accounts debit ON debit.center = act.debit_accountcenter AND debit.id = act.debit_accountid 
JOIN
    centers c ON c.id = act.center   
JOIN
    account_receivables ar ON ar.center = art.center AND ar.id= art.id AND ar.ar_type != 6               
LEFT JOIN
    (
        SELECT
            pr.center,
        ...
        FROM
            payment_requests pr
        JOIN
            payment_agreements pag ON pr.center = pag.center AND pr.id = pag.id AND pr.agr_subid = pag.subid 
        JOIN
