...
WITH
  ...
  (
      SELECT
          /*+ materialize */
          datetolongC(TO_CHAR(CAST('2025-05-16' AS DATE), 'YYYY-MM-dd HH24:MI'),c.id) AS FromDate,
          c.id AS CENTER_ID,
...
SELECT   
        ...
        ,CASE
                WHEN eac.balance IS NULL THEN 0
                ELSE eac.balance
        END AS "Member Debt Collector Balance"
        ,CASE
                WHEN arip.balance IS NULL THEN 0
                ELSE arip.balance
