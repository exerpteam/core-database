...
(
    SELECT
        datetolongC(TO_CHAR(TO_DATE('01/01/2026', 'DD/MM/YYYY'), 'YYYY-MM-dd HH24:MI'), c.id) AS FromDate,
        c.id AS CENTER_ID,
        CAST((datetolongC(TO_CHAR((TO_DATE('31/01/2026', 'DD/MM/YYYY') + INTERVAL '1 day'), 'YYYY-MM-dd HH24:MI'), c.id) - 1) AS BIGINT) AS ToDate
    FROM
        centers c
)
SELECT DISTINCT 
