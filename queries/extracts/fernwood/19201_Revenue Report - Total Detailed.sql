...
-- Full detail of every transaction in payment account, installment account and POS between a date range.
Note there is a column that tells you which ones are paid on installment.
This report also includes all payments made on the payment account and the installment account.
WITH params AS
(
    SELECT
