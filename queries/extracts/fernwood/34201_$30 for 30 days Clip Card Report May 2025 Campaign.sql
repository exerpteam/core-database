...
FROM
        ...
        WITH
          ...
          (
              SELECT
                  /*+ materialize */
datetolongC('2025-05-01 00:00', c.id) AS FromDate,
    c.id AS CENTER_ID,
    CAST((datetolongC('2025-05-31 00:00', c.id) - 1) AS BIGINT) AS ToDate
        FROM
                  centers c
          )
        SELECT distinct
        ...
        WHERE 
                ...
                AND 
                cc.valid_from BETWEEN params.FromDate AND params.ToDate
                AND 
                p.CENTER IN (:Scope)
                AND
                pgl.product_group_id IS NULL
				AND pro.name = '$30 for 30 Days'        
        )t1                