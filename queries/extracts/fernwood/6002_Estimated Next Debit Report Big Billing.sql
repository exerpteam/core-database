...
WITH PARAMS AS (
  SELECT
    ...
    (
      ...
      + (
          ((4 - (EXTRACT(ISODOW FROM (now() AT TIME ZONE 'Australia/Sydney')::date))::int + 7) % 7)
          * INTERVAL '1 day'
        )
    )::date AS FROM_DATE,
    (
    ...
    11 AS PAYMENT_CYCLE   -- <-- set to 4 (small) or 11 (big)
),
...
(
  SELECT
    ...
    s.start_date, s.binding_end_date, s.billed_until_date, s.end_date,
    s.individual_price, s.subscription_price, s.binding_price,
    par.FROM_DATE, par.TO_DATE,
  ...
  JOIN payment_agreements pag
   ...
   AND pag.individual_deduction_day = par.PAYMENT_CYCLE
   AND pag.state = 4  /* ONLY OK */
  WHERE s.state IN (2,4,8)
    AND s.start_date <= par.TO_DATE
    AND (s.end_date IS NULL OR s.end_date >= par.FROM_DATE)
    AND (s.billed_until_date IS NULL OR s.billed_until_date < par.TO_DATE)
...
(
  SELECT t2.subscription_center, t2.subscription_id,
         SUM(num_freeze_days) AS total_freeze_days,
         t2.FreezePrice
  FROM (
    SELECT t1.*,
    ...
    FROM (
      SELECT
        sfp.subscription_center, sfp.subscription_id,
        sfp.start_date, sfp.end_date,
  ...
  GROUP BY t2.subscription_center, t2.subscription_id, t2.FreezePrice
),
...
(
  ...
  FROM (
    SELECT t1.*, t1.free_end - t1.free_start + 1 AS num_free_days
    FROM (
      SELECT
        srp.subscription_center, srp.subscription_id,
        srp.start_date, srp.end_date, srp.freeze_period, srp.type,
  ...
  GROUP BY t2.subscription_center, t2.subscription_id
),
...
(
  ...
  FROM (
    SELECT
      sa.subscription_center, sa.subscription_id, sa.id,
    ...
    WHERE sa.cancelled = FALSE
      ...
      AND (sa.end_date IS NULL OR sa.end_date >= es.FROM_DATE)
...
(
  SELECT t2.subscription_center, t2.subscription_id, t2.id,
         SUM(num_freeze_days_addon) AS total_freeze_days_addon
  FROM (
    SELECT t1.*,
           t1.freeze_end_addon - t1.freeze_start_addon + 1 AS num_freeze_days_addon
    FROM (
      SELECT
        sa.id, sfp.subscription_center, sfp.subscription_id,
        sfp.start_date, sfp.end_date,
  ...
  GROUP BY t2.subscription_center, t2.subscription_id, t2.id
),
...
(
  ...
  FROM (
    ...
    FROM (
      SELECT
        sa.id, srp.subscription_center, srp.subscription_id,
        srp.start_date, srp.end_date, srp.freeze_period, srp.type,
  ...
  FROM (
    ...
    FROM (
      SELECT
        ...
        sa.theorical_days_to_invoice_addon
          - COALESCE(fzpa.total_freeze_days_addon, 0)
          - COALESCE(fpa.total_free_days_addon,    0) AS days_to_invoice_addon
      FROM SUBSCRIPTION_ADDONS sa
      LEFT JOIN FREEZE_PERIOD_ADDON fzpa ON sa.id = fzpa.id
      LEFT JOIN FREE_PERIOD_ADDON fpa  ON sa.id = fpa.id
    ) t1
    WHERE t1.days_to_invoice_addon > 0
  ) t2
  GROUP BY t2.subscription_center, t2.subscription_id
),
...
(
  SELECT
    t2.subscription_center, t2.subscription_id,
    t2.daily_price_update, SUM(num_price_days) AS total_price_days
  FROM (
    SELECT
      t1.*,
    ...
    FROM (
      SELECT
        ...
        sp.from_date, sp.to_date, sp.price,
      ...
      WHERE sp.cancelled = FALSE
        ...
        AND (sp.to_date IS NULL OR sp.to_date >= par.FROM_DATE)
        AND es.subscription_price <> sp.price
    ) t1
  ) t2
  GROUP BY t2.subscription_center, t2.subscription_id, t2.daily_price_update
),
...
(
  ...
  FROM (
    SELECT
      es.*,
      freeze_p.total_freeze_days, free_p.total_free_days,
      freeze_p.FreezePrice, addtot.total_amount_addons,
      SP.total_price_days, SP.daily_price_update
    FROM ELIGIBLE_SUB es
    LEFT JOIN FREEZE_PERIOD_SUB freeze_p
      ON es.center = freeze_p.subscription_center
     AND es.id     = freeze_p.subscription_id
    LEFT JOIN FREE_PERIOD_SUB free_p
      ON es.center = free_p.subscription_center
     AND es.id     = free_p.subscription_id
    LEFT JOIN ADDONS_TOTAL addtot
      ON addtot.subscription_center = es.center
     AND addtot.subscription_id     = es.id
...
(
  SELECT
    ...
    t."Deduction Date",
    CASE
      WHEN (t."Renewal amount" + t."Installment plan amount" + t."Manual Invoices/Credits amount" + t."Overdue amount") < 0
        THEN (t."Renewal amount" + t."Installment plan amount" + t."Manual Invoices/Credits amount" + t."Overdue amount")
      ELSE 0
    ...
    CASE WHEN t.clearinghouse = 1 THEN 'Bank Account'
      ...
      FROM (
        ...
        FROM (
          SELECT DISTINCT
            ...
            t1.*,
            CASE
              WHEN t1.total_price_days IS NULL
                THEN ROUND(COALESCE(t1.FreezePrice, t1.daily_price) * t1.days_to_invoice, 2) + COALESCE(t1.total_amount_addons, 0)
              ELSE ROUND(((t1.days_to_invoice - t1.total_price_days) * COALESCE(t1.FreezePrice, t1.daily_price))
                         + (t1.total_price_days * COALESCE(t1.FreezePrice, t1.daily_price_update)), 2)
                   + COALESCE(t1.total_amount_addons, 0)
            END AS amount_to_invoice_sub
          FROM (
            SELECT
              ddl.center, ddl.id, ddl.owner_center, ddl.owner_id,
              ddl.start_date, ddl.binding_end_date, ddl.billed_until_date, ddl.end_date,
              ddl.subscription_price, ddl.binding_price,
              ddl.period_from, ddl.period_to,
              ddl.total_freeze_days, ddl.total_free_days,
              ...
              ddl.total_price_days, ddl.daily_price_update,
              ddl.FreezePrice, ddl.total_amount_addons
            FROM DRILL_DOWN_LIST ddl
          ) t1
          WHERE t1.days_to_invoice > 0
        ) t2
        CROSS JOIN PARAMS par
        WHERE t2.owner_center IN (:Scope)
        GROUP BY t2.PersonId, par.FROM_DATE, t2.owner_center

        UNION ALL

        /* Installment plans due inside window */
        SELECT
        ...
        FROM (
          ...
          JOIN payment_agreements pag
           ...
           AND pag.individual_deduction_day = par.PAYMENT_CYCLE
           AND pag.state = 4  /* ONLY OK */
          WHERE art.due_date BETWEEN par.FROM_DATE AND par.TO_DATE
            AND ar.ar_type = 6
            AND ar.customercenter IN (:Scope)
        ) t1
        GROUP BY t1.customercenter, t1.customerid, t1.due_date

        UNION ALL

        /* Overdue before the window */
        SELECT
          t1.customercenter || 'p' || t1.customerid AS "Person ID",
          0, 0,
          t1."Overdue/Outstanding Amount",
          0,
          par.FROM_DATE AS "Deduction Date"
        FROM (
          SELECT DISTINCT
          ...
          JOIN payment_agreements pag
           ...
           AND pag.individual_deduction_day = par.PAYMENT_CYCLE
           AND pag.state = 4  /* ONLY OK */
          WHERE art.due_date < par.FROM_DATE
            AND ar.customercenter IN (:Scope)
          GROUP BY ar.customercenter, ar.customerid, ar.balance, pag.individual_deduction_day
        ) t1
        CROSS JOIN PARAMS par

        UNION ALL

        /* Manual invoices/credits dated null or equal to FROM_DATE */
        SELECT
          t1.customercenter || 'p' || t1.customerid AS "Person ID",
          0, 0, 0,
          t1."Manual Invoices/Credits",
          par.FROM_DATE AS "Deduction Date"
        FROM (
          SELECT DISTINCT
            ar.customercenter, ar.customerid,
          ...
          JOIN payment_agreements pag
           ...
           AND pag.individual_deduction_day = par.PAYMENT_CYCLE
           AND pag.state = 4  /* ONLY OK */
          WHERE (art.due_date IS NULL OR art.due_date = par.FROM_DATE)
            AND ar.customercenter IN (:Scope)
            AND art.text NOT LIKE '%(Auto Renewal)'
          GROUP BY ar.customercenter, ar.customerid, ar.balance, pag.individual_deduction_day
        ) t1
        CROSS JOIN PARAMS par
      ) t1
...
---------- Main output (4 columns) -----------
SELECT
  fl."Person ID",
  fl."Member Name",
  fl."Person Status",
  CASE
    WHEN fl."Payment Agreement State" IN ('Ended = bank','Ended = creditor','Failed') THEN 0
  ...
  fl."Clearing House"
FROM FIRST_LIST fl
WHERE fl."Payment Agreement State" = 'OK'   /* ONLY OK */
...
SELECT
  ...
  p.fullname AS "Member Name",
  bi_decode_field('PERSONS','STATUS', p.status) AS "Person Status",
  0 AS "Total Debit BEFORE Renewal",
  CASE
    WHEN pag.state IN (5,10,3) THEN 0
...
JOIN payment_agreements pag
 ...
 AND pag.active = 'true'
 AND pag.state = 4  /* ONLY OK */
WHERE p.center||'p'||p.id NOT IN (SELECT "Person ID" FROM FIRST_LIST)
  AND p.center IN (:Scope)
  AND ar.balance < 0
  AND pag.individual_deduction_day = par.PAYMENT_CYCLE

ORDER BY 2, 1;  -- sort by Member Name then Person ID
