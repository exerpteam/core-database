...
FROM clipcards c_org
JOIN invoice_lines_mt il ON c_org.invoiceline_center = il.center AND c_org.invoiceline_id = il.id AND c_org.invoiceline_subid = il.subid
JOIN invoices i ON i.center = il.center AND i.id = il.id
LEFT JOIN clipcards c_new ON c_new.transfer_from_clipcard_center = c_org.center AND c_new.transfer_from_clipcard_id = c_org.id AND c_new.transfer_from_clipcard_subid = c_org.subid
WHERE
        (il.person_center, il.person_id) != (i.payer_center, i.payer_id)
AND
		c_org.finished = 'FALSE'
AND
		i.payer_center = :scope
