...
-- https://clublead.atlassian.net/browse/EC-4922
WITH
        max_sub AS
                (
                SELECT 
...
FROM
        ...
        SELECT DISTINCT
                ...
                ,c.name
                ,CASE
                        WHEN -(date_part('year',age(p.birthdate, sub.start_date))) < 18 THEN ' - (MINOR)'
                        ELSE ''
                END AS "Age"
                ,CASE
        ...
        WHERE 
                ...
                AND
                pr.due_date BETWEEN :BillingStart AND :BillingEnd
                AND 
                p.center in (:scope)
                AND
                p.center NOT IN (204,303,305,309,311,314,320,321,601,602,702,327)  
                AND
