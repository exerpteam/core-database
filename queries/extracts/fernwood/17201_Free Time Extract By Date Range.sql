...
JOIN
        persons emp_name
        ON emp_name.center = e.personcenter 
        AND emp_name.id = e.personid
JOIN
        centers c
...
WHERE
	...
	AND
(
	(srp.start_date Between :FromDate AND :ToDate)
	OR
	(srp.end_date Between :FromDate AND :ToDate)
	OR
	(srp.start_date < :FromDate AND srp.end_date > :ToDate)
	)