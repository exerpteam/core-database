...
WHERE
  ...
  AND (
        COALESCE(cnl.text,'')          ILIKE ANY (ARRAY['%goodwill%','%waiv%','%debt cancelled%','%not pursuing%'])
     OR COALESCE(cn.text,'')           ILIKE ANY (ARRAY['%goodwill%','%waiv%','%debt cancelled%','%not pursuing%'])
     OR COALESCE(cn.coment,'')         ILIKE ANY (ARRAY['%goodwill%','%waiv%','%debt cancelled%','%not pursuing%'])
     OR COALESCE(art.text,'')          ILIKE ANY (ARRAY['%goodwill%','%waiv%','%debt cancelled%','%not pursuing%'])
     OR COALESCE(cnl.cancel_reason,'') ILIKE ANY (ARRAY['%goodwill%','%waiv%','%debt cancelled%','%not pursuing%'])
  )
