...
(
    SELECT
        datetolongC(TO_CHAR(CAST('2026-02-01' AS DATE), 'YYYY-MM-dd HH24:MI'),c.id) AS FromDate,
        c.id AS CENTER_ID,
        CAST((datetolongC(TO_CHAR((CAST('2026-02-28' AS DATE) + INTERVAL '1 day'), 'YYYY-MM-dd HH24:MI'),c.id) - 1) AS BIGINT) AS ToDate
    FROM
        centers c
),
filtered_data AS
