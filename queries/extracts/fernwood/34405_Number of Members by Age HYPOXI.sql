SELECT
  ...
  COUNT(CASE WHEN derived.age_group = '19 to 24' THEN 1 END) AS "19 to 24",
  COUNT(CASE WHEN derived.age_group = '25 - 34' THEN 1 END) AS "25 - 34",
  COUNT(CASE WHEN derived.age_group = '35 - 44' THEN 1 END) AS "35 - 44",
  COUNT(CASE WHEN derived.age_group = '45 - 54' THEN 1 END) AS "45 - 54",
  COUNT(CASE WHEN derived.age_group = '55 - 64' THEN 1 END) AS "55 - 64",
  COUNT(CASE WHEN derived.age_group = '65 - 74' THEN 1 END) AS "65 - 74",
  COUNT(CASE WHEN derived.age_group = '75 to 100' THEN 1 END) AS "75 to 100",
  COUNT(*) AS "Total"
FROM (
  ...
  WHERE
    ...
    AND s.state IN (2, 4)
    AND (
      LOWER(prod.name) LIKE '%hypoxi%' OR
      LOWER(prod.name) LIKE '%hdc%'
    )
