...
WITH 
    ...
    SELECT
        ...
        CASE 
            ...
            ELSE 'Unknown'
        END AS subscription_status,  -- New column for subscription status
        ...
        sp.from_date AS last_price_adjustment_date,
        ...
        ROW_NUMBER() OVER (PARTITION BY s.center, s.id ORDER BY sp.from_date DESC) AS rn
    FROM 
        persons p
    JOIN
        subscriptions s
    ...
    WHERE 
        p.status IN (1, 3)  -- Include Active and Frozen members
        AND p.persontype != 2  -- Exclude staff
        AND (s.end_date IS NULL OR s.end_date > CURRENT_DATE + INTERVAL '60 days')  -- Exclude subscriptions ending in the next 60 days
        AND s.start_date < CURRENT_DATE + INTERVAL '1 day'  -- Exclude subscriptions starting tomorrow or in the future
...
SELECT 
    ...
    am.subscription_status AS "Subscription Status",  -- New column added
    ...
    TO_CHAR(subscription_end_date, 'DD-MM-YYYY') AS "Subscription End Date",
    TO_CHAR(binding_end_date, 'DD-MM-YYYY') AS "Binding End Date",
    GREATEST(0, binding_end_date - CURRENT_DATE) AS "Days until outside binding end date",
    TO_CHAR(last_price_adjustment_date, 'DD-MM-YYYY') AS "Last Price Adjustment Date",
...
CASE 
        WHEN visits_last_90_days > 0 THEN 'Yes'
        ELSE 'No'
