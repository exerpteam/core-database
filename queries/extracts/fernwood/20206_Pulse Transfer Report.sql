SELECT
    ...
    olds.start_date AS "Old Subscription Start Date",
    olds.owner_center || 'ss' || olds.id AS "Old Subscription ID",  -- Old Subscription ID
    pnew.name AS "New Subscription",
    news.start_date AS "New Subscription Start Date",
    news.owner_center || 'ss' || news.id AS "New Subscription ID",  -- New Subscription ID
...
WHERE
    ...
    AND trd.txtvalue BETWEEN :FROM AND :TO;  -- Filter by the date of transfer
