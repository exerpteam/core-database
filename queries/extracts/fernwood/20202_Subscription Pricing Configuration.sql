...
WITH params AS MATERIALIZED (
    ...
    FROM centers c
    WHERE c.id IN (:Scope)  -- Use parameter filtering
),
pmp_xml AS 
(
    SELECT 
    ...
    JOIN params p ON m.scope_id = p.CENTER_ID  -- Minimal change: Join with filtered centers
    WHERE 
...
(
        ...
        (
                ...
                WHERE
                        m.scope_id IN (:Scope)
						AND m.state NOT IN ('DELETED','INACTIVE')
                        AND m.cached_producttype = 10
        ) 
        SELECT 
                mpr.id,
        ...
        JOIN product_group ppg ON mpr.primary_product_group_id = ppg.id
