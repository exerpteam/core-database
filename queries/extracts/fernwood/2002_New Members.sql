...
-- https://clublead.atlassian.net/browse/ST-13018
https://clublead.atlassian.net/browse/EC-1506
WITH
  params AS
  (
      SELECT
...
SELECT 
        ...
        ,CASE
                ...
                WHEN rea.center IS NULL AND scl.SUB_STATE = 4 THEN ts.subscription_price
                ELSE sp.price
        END AS "DD Amount"
FROM
        subscriptions s        
...
LEFT JOIN
        subscriptions ts
        ON ts.owner_center = p.center
        AND ts.owner_id = p.id
        AND ts.state IN (2,4)
        AND scl.SUB_STATE = 4                        
WHERE 
