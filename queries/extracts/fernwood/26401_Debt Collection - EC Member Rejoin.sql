...
WHERE
  /* Date range: past 31 days up to today (inclusive) */
  art.trans_time BETWEEN
      datetolongC(TO_CHAR((CURRENT_DATE - INTERVAL '31 days')::timestamp, 'YYYY-MM-DD HH24:MI'), art.center)
  AND datetolongC(TO_CHAR((CURRENT_DATE)::timestamp, 'YYYY-MM-DD HH24:MI'), art.center) + 86400*1000

  /* Scope (multi-centre friendly) */
  AND cn.center IN (:Scope)

  /* Match 'EC Member Rejoining' allowing extra spaces/commas/variations.
     We normalize each text by lowercasing and removing spaces/commas, then look for 'ecmemberrejoin'. */
  AND (
       regexp_replace(lower(coalesce(cnl.text,'')),          '[ ,]+', '', 'g') LIKE '%ecmemberrejoin%'
    OR regexp_replace(lower(coalesce(cn.text,'')),           '[ ,]+', '', 'g') LIKE '%ecmemberrejoin%'
    OR regexp_replace(lower(coalesce(cn.coment,'')),         '[ ,]+', '', 'g') LIKE '%ecmemberrejoin%'
    OR regexp_replace(lower(coalesce(art.text,'')),          '[ ,]+', '', 'g') LIKE '%ecmemberrejoin%'
    OR regexp_replace(lower(coalesce(cnl.cancel_reason,'')), '[ ,]+', '', 'g') LIKE '%ecmemberrejoin%'
  )

/* No goodwill/waiver filters, no canceltype exclusions */
