WITH
  ...
  (
      SELECT
          /*+ materialize */
          datetolongC(TO_CHAR(CURRENT_DATE, 'YYYY-MM-dd HH24:MI'),c.id) AS FromDate,
          c.id AS CENTER_ID,
          CAST((datetolongC(TO_CHAR((CURRENT_DATE + INTERVAL '1 day'), 'YYYY-MM-dd HH24:MI'),c.id) - 1) AS BIGINT) AS ToDate
      FROM
          centers c
      WHERE
          c.id IN (:Scope)
...
SELECT  
        ...
        ,TO_CHAR(longtodateC(t.creation_time, t.center), 'DD/MM/YYYY') AS "Task creation date"
        ,t.status AS "Task Status"
        ,ts.name AS "Task Step"
        ,assignee.fullname AS "Assigned to"
        ,TO_CHAR(longtodateC(t.last_update_time, t.asignee_center), 'DD/MM/YYYY') AS "Last Updated"
...
WHERE
        ...
        AND TRUNC(t.follow_up) = TRUNC(CURRENT_DATE)
        AND t.status = 'OPEN'