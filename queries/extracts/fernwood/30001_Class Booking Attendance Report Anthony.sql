...
WITH
    ...
    (
        SELECT
            /*+ materialize */
            datetolongC(TO_CHAR(CAST(CURRENT_DATE AS DATE), 'YYYY-MM-dd HH24:MI'), c.id) AS FromDate,
            c.id AS CENTER_ID,
            CAST((datetolongC(TO_CHAR(CAST(CURRENT_DATE + 1 AS DATE), 'YYYY-MM-dd HH24:MI'), c.id) - 1) AS BIGINT) AS ToDate
        FROM
            centers c
    ),
    comp AS
...
SELECT  
	...
	TO_CHAR(CAST(longtodateC(b.starttime, b.center) AS DATE), 'DD/MM/YYYY') AS "Class Date",
...
WHERE 
    ...
    AND 
    b.name NOT IN ('Massage Chair Booking', 'Fernwood Tour', 'Chatbot Appointment');