...
WITH j AS (
  ...
  WHERE
      ...
      -- Flexible match for: "eCollect - Member Rejoin: Debt Remains with eCollect"
      AND lower(coalesce(je.name,'')) LIKE '%ecollect%'
      AND lower(coalesce(je.name,'')) LIKE '%rejoin%'
      AND lower(coalesce(je.name,'')) LIKE '%debt%'
      AND lower(coalesce(je.name,'')) LIKE '%remain%'
)
