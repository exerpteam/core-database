...
-- Use this report for Payroll. This report excludes sessions where the member may have attended a PT session but she did not have a clip available to use - effectively not paying for the session. Trainers must ensure that members have clips available to use and if not, arranging for the member to buy a session. 
WITH
  params AS
  (
      SELECT
...
JOIN 
        ...
        ON b.activity = ac.id
        AND ac.id in 
(803,2,601,3401,3,9003,24608,24207,3,23445,24405,23449,23861,33208,33413,45209,9404,15406,27401,38615,38819,45208,52804,55003,56403,58604,59407,59409,59410,59605,59606,59803,8802,9405,9406,21615,21616,21812,22212,23413,26211,26417,26418,26618,59401,59402,59403,59404,59405,59602,59801,59802)
JOIN 
        STAFF_USAGE su
