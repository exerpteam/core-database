 ...
 WHERE
     (
         ...
         AND CIL.CHECKIN_TIME >= :start_time
         AND CIL.CHECKIN_TIME <= :end_time
         AND NOT EXISTS
         (
             SELECT
                 1
         ...
         (
             ...
             WHERE
                 (
                     ...
                     AND SCL99.BOOK_START_TIME <= :end_time
                     AND (
                         SCL99.BOOK_END_TIME IS NULL
                         OR SCL99.BOOK_END_TIME >= :end_time)
                     AND SCL99.STATEID IN (0,1,2,3,4,5,6,7,8))))
 GROUP BY
     TRUNC(longToDateTZ(CIL.CHECKIN_TIME,'Europe/London'),'HH'),
     centers.name,
         TO_CHAR(longToDateTZ(:start_time,'Europe/London'),'HH24:MI')
 ORDER BY
     centers.name,
     TRUNC(longToDateTZ(CIL.CHECKIN_TIME,'Europe/London'),'HH')
