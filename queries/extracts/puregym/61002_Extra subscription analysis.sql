...
-- https://clublead.atlassian.net/browse/ST-6925
https://clublead.atlassian.net/browse/ST-8559
WITH
    params AS
    (
        SELECT
    ...
    (
        ...
        WHERE
            s.owner_center IN ($$Scope$$)
            AND pg.name = 'Plus Subscriptions MS'
            /*Do not count members who still have an active subscription within the product group 'Active subscriptions MS' during filter period unless they are transferred during filter period. Do not count same subscription from above */
            AND NOT EXISTS
            (
                SELECT
                    1
    ...
    ,
    v_feat_count_pivot AS
    (
        SELECT
            *
        FROM
            v_feat_count PIVOT ( MIN(Total) FOR feature IN ('Extra Open Member'           AS "Extra Open Member",
                                                            'Extra Joiner Member'         AS "Extra Joiner Member",
                                                            'Extra Switch Member'         AS "Extra Switch Member",
                                                            'Extra Leaver Member'         AS "Extra Leaver Member",
                                                            'Extra Close Member'          AS "Extra Close Member",
                                                            'Extra Bolt On Open Member'   AS "Extra Bolt On Open Member",
                                                            'Extra Bolt On Joiner Member' AS "Extra Bolt On Joiner Member",
                                                            'Extra Bolt On Leaver Member' AS "Extra Bolt On Leaver Member",
                                                            'Extra Bolt On Close Member'  AS "Extra Bolt On Close Member"))
        ORDER BY
            1
    )
SELECT
    p.center,
    NVL(p."Extra Open Member", 0)           AS "Extra Open Member",
    NVL(p."Extra Joiner Member", 0)         AS "Extra Joiner Member",
    NVL(p."Extra Switch Member", 0)         AS "Extra Switch Member",
    NVL(p."Extra Leaver Member", 0)         AS "Extra Leaver Member",
    NVL(p."Extra Close Member", 0)          AS "Extra Close Member",
    NVL(p."Extra Bolt On Open Member", 0)   AS "Extra Bolt On Open Member",
    NVL(p."Extra Bolt On Joiner Member", 0) AS "Extra Bolt On Joiner Member",
    NVL(p."Extra Bolt On Leaver Member", 0) AS "Extra Bolt On Leaver Member",
    NVL(p."Extra Bolt On Close Member", 0)  AS "Extra Bolt On Close Member"
FROM
    v_feat_count_pivot p