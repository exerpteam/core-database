 WITH
     ...
     (
         SELECT
             /*+ materialize */
	     CAST(extract(epoch from CURRENT_DATE-1) * 1000 AS BIGINT) AS from_time,
	     CAST(extract(epoch from CURRENT_DATE-1) * 1000 AS BIGINT) AS to_time,
             TRUNC(CURRENT_TIMESTAMP)-1 AS from_date,
             TRUNC(CURRENT_TIMESTAMP)-1 AS to_date
                 ...
                 WHERE
                     ...
                     (
                         ...
                         WHERE
                             ...
                             AND TRUNC(longtodateC(scl2.ENTRY_START_TIME,scl.center))=TRUNC(longtodateC(s.CREATION_TIME,s.center)))) t
         WHERE
             rnk=1 ) ss
 ON
