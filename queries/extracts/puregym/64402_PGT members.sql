...
-- https://clublead.atlassian.net/browse/ST-11579
 ...
 SELECT
     ...
     CASE
         WHEN cc.CENTER IS NOT NULL OR cc_op.CENTER IS NOT NULL THEN 'Yes'
         ELSE 'No'
     END                                                         AS "Debt case",
 ...
 WHERE
     ...
     AND
     (
      ($$product_group_selected$$ in (6407,5602) AND EXISTS (select 1 from PRODUCT_AND_PRODUCT_GROUP_LINK ppl WHERE ppl.PRODUCT_CENTER = prod.CENTER AND ppl.PRODUCT_ID = prod.ID AND  ppl.PRODUCT_GROUP_ID in ($$product_group_selected$$)))
      OR
      (($$product_group_selected$$ = 1 AND NOT EXISTS (select 1 from PRODUCT_AND_PRODUCT_GROUP_LINK ppl WHERE ppl.PRODUCT_CENTER = prod.CENTER AND ppl.PRODUCT_ID = prod.ID AND  ppl.PRODUCT_GROUP_ID in (6407,5602, 401))))
     )
 AND
         (
                sf.ID IS NOT NULL
