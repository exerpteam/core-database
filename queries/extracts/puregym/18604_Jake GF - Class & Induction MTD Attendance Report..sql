...
SELECT
    ...
    CASE
        ...
        ELSE TO_CHAR(SUM(ShowUps.ManualClasses) / DECODE(SUM(MEMBERS.total),0,1,SUM(MEMBERS.total))*100,'FM9990.00' )||'%'
    END                                                                  AS "% Manual showup Classes",
    SUM(NVL(ShowUps.AutoClasses,0))                                      AS "Auto showup Classes count",    
    CASE
        WHEN CENTERS.ID IS NOT NULL
        THEN TO_CHAR(SUM(NVL((ShowUps.AutoClasses / DECODE(MEMBERS.total,0,1,MEMBERS.total))*100,0)),'FM990.00')||'%'
