...
SELECT
    ...
    TO_CHAR(longtodateC(ch.checkin_time, ch.checkin_center), 'dd/MM/YYYY HH12:MI:SS AM')  AS Checkin_start,
    TO_CHAR(longtodateC(ch.checkout_time, ch.checkin_center), 'dd/MM/YYYY HH12:MI:SS AM') AS Checkout_time
FROM
    persons p
JOIN
    centers c
...
AND (p.firstname LIKE 'CBRE%'
	OR p.firstname LIKE '%CBRE'
	OR p.lastname LIKE '%CBRE'
	OR p.lastname LIKE 'CBRE%')
ORDER BY
p.fullname,
ch.checkin_center,
