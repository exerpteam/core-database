...
-- https://clublead.atlassian.net/browse/ST-6544
 WITH
     PARAMS AS Materialized
     (
         SELECT 
                 :fromDate AS fromDate,
                 :toDate + 86400000 AS toDate
 ...
 (
         SELECT
                 TO_CHAR(longtodateTZ(t1.ENTRY_START_TIME, 'Europe/London'), 'YYYY-MM-DD HH24:MI:SS') AS ReferralDate,
                 longtodateTZ(t1.ENTRY_START_TIME, 'Europe/London') AS ReferralDateComp,
                 ...
                 CASE  referrer.STATUS  WHEN 0 THEN 'LEAD'  WHEN 1 THEN 'ACTIVE'  WHEN 2 THEN 'INACTIVE'  WHEN 3 THEN 'TEMPORARYINACTIVE'
                        WHEN 4 THEN 'TRANSFERED'  WHEN 5 THEN 'DUPLICATE'  WHEN 6 THEN 'PROSPECT'  WHEN 7 THEN 'DELETED' WHEN 9 THEN  'CONTACT' ELSE 'UNKNOWN' END AS ReferrerStatus,         
                 (CASE referrer.SEX
                                 WHEN 'M' THEN 'Male'
                                 WHEN 'F' THEN 'Female'
                                 ELSE 'Undefined'
                 ...
                 CASE  referral.STATUS  WHEN 0 THEN 'LEAD'  WHEN 1 THEN 'ACTIVE'  WHEN 2 THEN 'INACTIVE'  WHEN 3 THEN 'TEMPORARYINACTIVE'
                          WHEN 4 THEN 'TRANSFERED'  WHEN 5 THEN 'DUPLICATE'  WHEN 6 THEN 'PROSPECT'  WHEN 7 THEN 'DELETED' WHEN 9 THEN  'CONTACT' ELSE 'UNKNOWN' END AS ReferralStatus,                         
                 (CASE referral.SEX
                         WHEN 'M' THEN 'Male'
                         WHEN 'F' THEN 'Female'
                         ELSE 'Undefined'
         ...
         LEFT JOIN PERSONS referrer
                 ON
                         referrer.CENTER = t1.referrerCENTER
                         AND referrer.ID = t1.referrerID
         LEFT JOIN PERSON_EXT_ATTRS referrerMobile
                 ON
         ...
         LEFT JOIN PERSON_EXT_ATTRS referrerPhoneHome
                 ON
         ...
         LEFT JOIN SUBSCRIPTIONS s  -- THIS JOIN WILL RETURN DUPLICATES
                 ON
         ...
         LEFT JOIN CENTERS cen2
                 ON
                         cen2.ID = t1.referralCENTER
         LEFT JOIN PERSONS referral
                 ON
                         referral.CENTER = t1.referralCENTER
                         AND referral.ID = t1.referralID
         LEFT JOIN PERSON_EXT_ATTRS referralMobile
                 ON
         ...
         LEFT JOIN PERSON_EXT_ATTRS referralPhoneHome
                 ON
         ...
         LEFT JOIN PERSON_EXT_ATTRS referralEmail
                 ON
         ...
         LEFT JOIN PERSON_EXT_ATTRS referralOldCode
                 ON
         ...
         WHERE
                 ...
                 AND t1.ENTRY_START_TIME < PARAMS.toDate
 ) t1
 LEFT JOIN
 (
         SELECT
         ...
         FROM PERSONS referrer
         CROSS JOIN PARAMS
         JOIN RELATIVES rel ON rel.RELATIVECENTER = referrer.center AND rel.RELATIVEID = referrer.id AND rel.RTYPE = 13
         JOIN PERSONS referral ON referral.center = rel.center  AND referral.id = rel.id -- new member
         JOIN STATE_CHANGE_LOG scl ON scl.center = rel.center AND scl.id = rel.id AND scl.subid = rel.SUBID AND scl.ENTRY_TYPE = 4 AND scl.STATEID = 1
         JOIN SUBSCRIPTIONS sref ON sref.OWNER_CENTER = referrer.CENTER AND sref.OWNER_ID = referrer.ID
         JOIN STATE_CHANGE_LOG sclsub
                 ON
                     ...
                     AND (sclsub.ENTRY_END_TIME IS NULL OR sclsub.ENTRY_END_TIME > scl.ENTRY_START_TIME)
                     AND sclsub.STATEID IN (2,4)
         JOIN PRODUCTS pr ON sref.SUBSCRIPTIONTYPE_CENTER = pr.CENTER AND sref.SUBSCRIPTIONTYPE_ID = pr.ID
         WHERE
             referrer.center IN ($$scope$$)
             AND (
             ...
             AND scl.ENTRY_START_TIME < PARAMS.toDate
 ) t2
 ON
 ...
 (
         SELECT
                 ...
                 sclsub.*,
                 longtodateTZ(sref.CREATION_TIME, 'Europe/London') AS SubCreationTime
         FROM PERSONS referrer
         CROSS JOIN PARAMS
         JOIN RELATIVES rel ON rel.RELATIVECENTER = referrer.center AND rel.RELATIVEID = referrer.id AND rel.RTYPE = 13
         JOIN PERSONS referral ON referral.center = rel.center  AND referral.id = rel.id -- new member
         JOIN STATE_CHANGE_LOG scl ON scl.center = rel.center AND scl.id = rel.id AND scl.subid = rel.SUBID AND scl.ENTRY_TYPE = 4 AND scl.STATEID = 1
         JOIN SUBSCRIPTIONS sref ON sref.OWNER_CENTER = referral.CENTER AND sref.OWNER_ID = referral.ID
         JOIN STATE_CHANGE_LOG sclsub
                 ON
                     sclsub.CENTER = sref.CENTER
                     AND sclsub.ID = sref.ID
 ...
 JOIN PERSONS currentReferral ON currentReferral.CENTER = t1.CURRENT_PERSON_CENTER AND currentReferral.ID = t1.CURRENT_PERSON_ID
 WHERE
         t3.SubCreationTime IS NOT NULL
         AND
         TRUNC(t3.SubCreationTime) = TRUNC(t1.ReferralDateComp)
