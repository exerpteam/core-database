 WITH
     ...
     (
        SELECT 
            id                                                                                  AS center,
            CAST(datetolongTZ(TO_CHAR(TRUNC(CAST($$from_date$$ AS DATE) , 'DDD'), 'YYYY-MM-DD HH24:MI'), 'Europe/London') AS BIGINT) AS STARTTIME ,
            CAST(datetolongTZ(TO_CHAR(TRUNC(CAST($$to_date$$ AS DATE), 'DDD'), 'YYYY-MM-DD HH24:MI'), 'Europe/London') AS BIGINT)    AS ENDTIME,
            CAST(datetolongTZ(TO_CHAR(TRUNC(CAST($$to_date$$ AS DATE) +1, 'DDD'), 'YYYY-MM-DD HH24:MI'), 'Europe/London')  AS BIGINT) AS HARDCLOSETIME
        FROM
            centers
        where id in ($$scope$$) 
     ),
 ...
 SELECT
     ...
     ST AS "Joiners SPC",
     CASE
         WHEN SUBSTR(RE,LENGTH(RE))=' '
         THEN SUBSTR(RE,1,LENGTH(RE)-1)
         ELSE RE
 ...
 FROM
     (
         SELECT
             ...
             SUBSTR(p.ZIPCODE,1,LENGTH(p.ZIPCODE)-3)    RE
         FROM
             (
