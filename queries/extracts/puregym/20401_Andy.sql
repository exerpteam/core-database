WITH
    ...
    (
        SELECT
            ...
            datetolongTZ(TO_CHAR(SYSDATE -$$offset$$ ,'YYYY-MM-DD') || ' 00:00', 'Europe/London')
            AS p_start_next_day
        FROM
            dual
    )
SELECT
    ...
    SUM(
        CASE
            WHEN SU_GLOBALID IN ('GYMFLEX_12M_EFT',
                                 'GYMFLEX_9M_EFT')
                AND SCL1_STATEID != 4
            THEN 1
            ELSE 0
    ...
    SUM(
        CASE
            WHEN SU_GLOBALID IN ('9_MONTH_DAY_PASS')
                AND SCL1_STATEID != 4
            THEN 1
            ELSE 0
            ...
            (
                ...
                (
                    SELECT
                        ...
                        id
                    FROM
                        V_EXCLUDED_SUBSCRIPTIONS) ) --and c.id in (1, 12, 33)
    )
GROUP BY
