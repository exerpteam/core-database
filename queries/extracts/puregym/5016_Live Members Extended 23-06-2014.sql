...
SELECT /*+ NO_BIND_AWARE */ DISTINCT 
    ...
    TO_CHAR(NVL(SPP.SUBSCRIPTION_PRICE, NVL(SP.PRICE, SU.SUBSCRIPTION_PRICE))) AS CURRENT_PRICE,
    TO_CHAR(NVL(SP.PRICE, SU.SUBSCRIPTION_PRICE))                     AS NORMAL_PRICE,
    CASE
    ...
    CASE
        ...
        THEN TO_CHAR(longtodateTZ(ci.MaxExerp, 'Europe/London'), 'YYYY-MM-DD HH24:MI')
        WHEN ci.MaxExerp IS NULL
        THEN NOMICAL.EVENTDATETIME
        ELSE TO_CHAR(longtodateTZ(ci.MaxExerp, 'Europe/London'), 'YYYY-MM-DD HH24:MI')
    END LastGymVisit,
    CASE
        WHEN NOMICAL.ID IS NULL
    ...
    p.BIRTHDATE,
    DECODE(pl_mem.is_now,0,'No',NULL,'No','Yes')   AS "Current PLoser Member",
    DECODE(pl_mem.has_ever,0,'No',NULL,'No','Yes') AS "Past PLoser Member",
    ...
    (
        SELECT
           ...
           -- MAX(ci.id)                                                                         AS id ,
            ...
            MAX(ci.CHECKIN_TIME) AS MaxExerp
        FROM
            CHECKINS ci
        ...
        ON
          ...
          --WHERE (ci.id < 30000000 or ci.id > 50000000)
        GROUP BY
            ci.PERSON_CENTER,
            ci.PERSON_ID ) ci
ON
...
LEFT JOIN
    CHECKINS latest_ci
ON
    ci.MaxExerp = latest_ci.CHECKIN_TIME
    and latest_ci.PERSON_CENTER = ci.PERSON_CENTER
    and latest_ci.PERSON_ID = ci.PERSON_ID
    /* Participations */
LEFT JOIN
    (
        SELECT
...
--	AND ST.ST_TYPE = 1
    AND (ST.CENTER, ST.ID) not in (select /*+ materialize */ center,id from V_EXCLUDED_SUBSCRIPTIONS)
LEFT JOIN
