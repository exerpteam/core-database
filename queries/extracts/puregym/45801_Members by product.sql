...
-- https://clublead.atlassian.net/browse/ST-3657
https://clublead.atlassian.net/browse/ES-7866
 WITH
     dates AS Materialized
     (
	    SELECT CAST(datetolongTZ(to_char(d1,'YYYY-MM-DD HH24:MI'),'Europe/London') AS BIGINT) AS dt, 
				d1 AS start_date
		FROM
            (SELECT * FROM
 ...
 SELECT
     ...
     CENTER                           AS "Club ID",
     C_NAME                           AS "Club Name",
     CASE  PERSON_TYPE  WHEN 0 THEN 'Private'  WHEN 1 THEN 'Student'  WHEN 2 THEN 'Staff'  WHEN 3 THEN 'Friend'  WHEN 4 THEN 'Corporate'  WHEN 5 THEN 
     'Onemancorporate'  WHEN 6 THEN 'Family'  WHEN 7 THEN 'Senior'  WHEN 8 THEN 'Guest' ELSE 'Unknown' END AS "Person Type",
 ...
 FROM
     (
                 ...
                 OR  (
                     ...
                     AND NOT EXISTS
                         (
                             ...
                             ON
                                 s2.center = scl3.center
                             AND s2.id = scl3.id
                             WHERE
                                 s2.owner_id = su.owner_id
                             AND s2.owner_center = su.owner_center
                             AND scl3.stateid IN(2,4)
                             AND SCL3.BOOK_START_TIME < dates.dt + 1000*60*60*24
                             AND (
                                     SCL3.BOOK_END_TIME IS NULL
                                 OR  SCL3.BOOK_END_TIME >= dates.dt + 1000*60*60*24)
                             AND SCL3.ENTRY_START_TIME < dates.dt + 1000*60*60*24 )))
         ...
         AND c.id IN ($$scope$$) ) t2
 GROUP BY
     CENTER,
     C_NAME,
     CASE  PERSON_TYPE  WHEN 0 THEN 'Private'  WHEN 1 THEN 'Student'  WHEN 2 THEN 'Staff'  WHEN 3 THEN 'Friend'  WHEN 4 THEN 'Corporate'  WHEN 5 THEN 
     'Onemancorporate'  WHEN 6 THEN 'Family'  WHEN 7 THEN 'Senior'  WHEN 8 THEN 'Guest' ELSE 'Unknown' END ,
     PRICE,
     SU_GLOBALID,
     CAMPAIGN_OR_COMPANY,
     dt
