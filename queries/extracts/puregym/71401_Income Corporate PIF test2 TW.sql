...
--  
 SELECT
     ...
     t1.amount           AS "Debit Amount",
     t1.CreditExternalId AS "Credit",
     t1.CreditAccount    AS "Credit Account",
     t1.amount           AS "Credit Amount",
     t1.VAT              AS "VAT",
     t1.VATtype          AS "VAT type",
     t1.fromdate1        as "Start date",
     t1.todate1          as "End date",
     t1.actcenter        as "Cost center",
     t1.context          as "consolidated text",
 ...
 FROM
     (
	  WITH
		...
		(
         SELECT
             CAST(dateToLongC(TO_CHAR(CAST(:fromDate AS DATE), 'YYYY-MM-dd HH24:MI'), 100) AS BIGINT)  AS FromDate,
             CAST((dateToLongC(TO_CHAR(CAST(:toDate AS DATE), 'YYYY-MM-dd HH24:MI'), 100)+ 86400 * 1000)-1 AS BIGINT) AS ToDate
            
		)
	  ,
	  selected_ACT AS Materialized
	  (
	    SELECT a.*
		FROM
			ACCOUNT_TRANS a
		...
		WHERE
	a.CENTER IN (:Scope)
	AND a.TRANS_TIME >= params.fromDate
        AND a.TRANS_TIME < params.toDate
        AND a.MAIN_TRANSCENTER IS NULL
         ...
         SELECT
          ...
          ( case
               WHEN act.TRANS_TYPE=4
                     THEN spp.FROM_DATE
                     WHEN act.TRANS_TYPE=5
                     THEN sppcn.FROM_DATE else null END) as fromdate1,
                 spp.FROM_DATE as fromdate,
                 ( case
               WHEN act.TRANS_TYPE=4
                     THEN spp.TO_DATE
                     WHEN act.TRANS_TYPE=5
                     THEN sppcn.TO_DATE else null END) as todate1,
                     spp.TO_DATE as todate,
             ar.*,
             act.center as actcenter,
             act.id as actid,
             act.subid as actsubid,
             act.TRANS_TYPE as transtype,
             art.text as arttext,
             i.text as itext,
             cr.coment as crtext,
             (CASE
        ...
        LEFT JOIN
         ...
         AND ar.ID = art.ID
         WHERE
          (creditAccount.EXTERNAL_ID in (:externalid) or debitAccount.EXTERNAL_ID in (:externalid))
    ) t1
 LEFT JOIN
     PERSONS p
 ON
