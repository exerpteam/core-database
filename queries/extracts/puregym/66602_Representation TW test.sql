...
left JOIN PUREGYM.CASHCOLLECTIONCASES cc on cc.PERSONCENTER = p.center and cc.personid = p.id and cc.MISSINGPAYMENT = 1 and cc.CLOSED = 0
...
--		nvl(ci.RECEIVED_DATE, prs.ORIGINAL_DUE_DATE) <= (SYSDATE - --7 - (decode(1 + TRUNC (sysdate) - TRUNC (sysdate, 'IW'),4,1,0))  --))
  --  AND (
