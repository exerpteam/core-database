...
-- https://clublead.atlassian.net/browse/ST-8185
WITH 
    RECURSIVE area_tree
    (
        id,
    ...
    (   SELECT
        ...
        AND NOT EXISTS
            (   SELECT
                ...
                AND pl.PRODUCT_GROUP_ID IN (9,6410) )
            -- Step 2:
        ...
        AND EXISTS
            (   SELECT
                ...
                WHERE
                    pl.PRODUCT_CENTER = pr.CENTER
                AND pl.PRODUCT_ID = pr.ID
                AND pl.PRODUCT_GROUP_ID IN (6407,
                                            6408,
                                            10001) )
            -- Step 4.2:
        ...
        AND NOT EXISTS
            (   SELECT
                ...
                WHERE
                    pl.PRODUCT_CENTER = pr.CENTER
                AND pl.PRODUCT_ID = pr.ID
                AND pl.PRODUCT_GROUP_ID = 6407
                AND 
                    (
                        pr.PRICE >= :NEW_PRICE + 5
                    OR  pr.PRICE < :CURRENT_PRICE + 5 ) )
            -- Step 6:
...
AND EXISTS
    (   SELECT
        ...
        WHERE
            ...
            --to_number(REPLACE(t1.name,'£')) < :NEW_PRICE 
    )