 ...
 WITH
     ...
     (
         ...
         FROM
             (
                 ...
                 WHERE
                     CAST(s.center AS VARCHAR) IN ($$Scope$$)
                     ...
                     /* Exclude subscriptions from those product groups. Staff Subscription, Staff / PT Subscriptions MS, PGT */
                     AND NOT EXISTS
                     (
                         SELECT
                             1
                         ...
                         WHERE
                             ...
                             AND ppl.PRODUCT_GROUP_ID IN (401,6409,11801))
                     AND NOT EXISTS
                     (
                         SELECT
                             1
                         ...
                         WHERE
                             ...
                             AND srd1.end_date = COALESCE(s.end_date, to_date('01-01-2100', 'dd-MM-yyyy'))) ) a
         CROSS JOIN
             params
         LEFT JOIN
             subscription_reduced_period existsrd
         ON
             existsrd.subscription_center = a.center
             AND existsrd.subscription_id = a.id
    ...
    ,
     ...
     (
         SELECT
             sub.*,
             LEAD(sub.ExistingSavedFreezStart,1) OVER (PARTITION BY sub.SubscriptionId ORDER BY sub.ExistingSavedFreezStart) AS ExistingSavedFreezStart2 ,
             LEAD(sub.ExistingSavedFreezeEnd,1) OVER (PARTITION BY sub.SubscriptionId ORDER BY sub.ExistingSavedFreezeEnd)   AS ExistingSavedFreezEnd2 ,
             LEAD(sub.ExistingSavedFreezStart,2) OVER (PARTITION BY sub.SubscriptionId ORDER BY sub.ExistingSavedFreezStart) AS ExistingSavedFreezStart3 ,
             LEAD(sub.ExistingSavedFreezeEnd,2) OVER (PARTITION BY sub.SubscriptionId ORDER BY sub.ExistingSavedFreezeEnd)   AS ExistingSavedFreezEnd3 ,
             LEAD(sub.ExistingSavedFreezStart,3) OVER (PARTITION BY sub.SubscriptionId ORDER BY sub.ExistingSavedFreezStart) AS ExistingSavedFreezStart4 ,
             LEAD(sub.ExistingSavedFreezeEnd,3) OVER (PARTITION BY sub.SubscriptionId ORDER BY sub.ExistingSavedFreezeEnd)   AS ExistingSavedFreezEnd4 ,
             LEAD(sub.ExistingSavedFreezStart,4) OVER (PARTITION BY sub.SubscriptionId ORDER BY sub.ExistingSavedFreezStart) AS ExistingSavedFreezStart5 ,
             LEAD(sub.ExistingSavedFreezeEnd,4) OVER (PARTITION BY sub.SubscriptionId ORDER BY sub.ExistingSavedFreezeEnd)   AS ExistingSavedFreezEnd5 ,
             ROW_NUMBER() OVER (PARTITION BY sub.SubscriptionId ORDER BY sub.ExistingSavedFreezStart)                        AS ADDONSEQ
         FROM
             v_sub sub
     )
 SELECT
