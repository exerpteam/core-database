SELECT distinct
    ...
    COALESCE(p_new.external_id, p_old.external_id) AS external_id,
...
JOIN PERSONS p_old
   ON p_old.center = ar.customercenter
  AND p_old.id     = ar.customerid
LEFT JOIN PERSONS p_new
   ON p_new.center = p_old.transfers_current_prs_center
  AND p_new.id     = p_old.transfers_current_prs_id
JOIN
