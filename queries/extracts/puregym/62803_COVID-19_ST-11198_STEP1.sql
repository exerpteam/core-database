...
WITH
    ...
    (
        SELECT
            ...
            to_date('20-05-2020', 'dd-MM-yyyy') AS EndDate,
            0          AS numberOfDays
...
SELECT
    ...
    s.billed_until_date,
    CASE  s.STATE  WHEN 2 THEN 'ACTIVE'  WHEN 3 THEN 'ENDED'  WHEN 4 THEN 'FROZEN'  WHEN 7 THEN 'WINDOW'  WHEN 8 THEN 'CREATED' ELSE 'Undefined' END AS STATE,
...
FROM
    (
        SELECT DISTINCT
            ...
            'COVID-19 measures (ST-11198) STEP1'    AS Text,
            a.TransferDate,
            COALESCE(
                       (
                       SELECT
