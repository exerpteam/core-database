...
WITH
    ...
    (
        SELECT
            CAST(dateToLongC(TO_CHAR(
                CASE
                    WHEN EXTRACT(ISODOW FROM TO_DATE(getCenterTime(c.id), 'YYYY-MM-DD')) = 1
                    THEN TO_DATE(getCenterTime(c.id), 'YYYY-MM-DD') - INTERVAL '7 days'
                    ELSE TO_DATE(getCenterTime(c.id), 'YYYY-MM-DD') - INTERVAL '1 day' * (EXTRACT
                        (ISODOW FROM TO_DATE(getCenterTime(c.id), 'YYYY-MM-DD')) - 1)
                END, 'YYYY-MM-DD'), c.id) AS bigint) AS fromDate,
            CAST(dateToLongC(TO_CHAR(TO_DATE(getCenterTime(c.id), 'YYYY-MM-DD') - interval '1 Day',
            'YYYY-MM-DD'),c.id ) AS bigint)+1000*60*60*24-1 AS toDate,
            c.id                                            AS centerID,
