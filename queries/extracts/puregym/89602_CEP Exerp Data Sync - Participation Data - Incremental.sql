...
(
    SELECT
        datetolongC(TO_CHAR(TO_DATE(center_time,'YYYY-MM-DD') - INTERVAL '5 days', 'YYYY-MM-DD'), c.id) AS fromdate,
        datetolongC(TO_CHAR(TO_DATE(center_time,'YYYY-MM-DD'), 'YYYY-MM-DD'), c.id) - 1 AS todate,
        c.id
    FROM (
        SELECT id, getCenterTime(id) AS center_time
        FROM centers
        WHERE id IN (:Scope)
    ) c
)
...
WHERE
    par.last_modified between params.fromdate AND params.todate
