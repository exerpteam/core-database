SELECT distinct
    ...
    NVL (CI_Centers_Count.CentersCount,0)         AS CentersCount,
    NVL(Home_Secondary_Count.HomeClub,0)          AS "HomeClub",
    NVL(Home_Secondary_Count.SecondaryClub,0)     AS "SecondaryClub",
...
FROM
    (
        ...
        LEFT JOIN
            PUREGYM.PERSON_EXT_ATTRS pea_secgymId
        ON
        ...
        LEFT JOIN
            PUREGYM.CENTERS center1
        ON
            center1.ID = EXERP_CI.CHECKIN_CENTER
        LEFT JOIN
            PUREGYM.CENTERS center2
        ON
...
LEFT JOIN
    (
        ...
        GROUP BY
            p.center,
            p.id) CI_Centers_Count
ON
    m.P_CENTER = CI_Centers_Count.center
    AND m.P_ID=CI_Centers_Count.id
LEFT JOIN
    (
        SELECT
            ci.PERSON_CENTER,
            ...
            SUM(
                CASE
                    WHEN ci.CHECKIN_CENTER=ci.PERSON_CENTER
                    THEN 1
                    ELSE 0
                END) AS HomeClub,
            SUM(
                CASE
                    WHEN ci.CHECKIN_CENTER = ext.TXTVALUE
                    THEN 1
                    ELSE 0
                END) AS SecondaryClub
        FROM
            PUREGYM.CHECKINS ci
        LEFT JOIN
        ...
        GROUP BY
            ci.PERSON_CENTER,
            ci.PERSON_ID) Home_Secondary_Count
ON
    m.P_CENTER=Home_Secondary_Count.PERSON_CENTER
    AND m.P_ID = Home_Secondary_Count.PERSON_ID
LEFT JOIN
