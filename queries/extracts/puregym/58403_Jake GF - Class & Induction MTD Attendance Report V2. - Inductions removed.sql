...
SELECT
    ...
    CASE
        WHEN CENTERS.ID IS NOT NULL
        THEN TO_CHAR(SUM(NVL(((ShowUps.Manualinductions+ShowUps.AutoInductions) / POSITIVEGAIN.totalJoiners)*100,0)),'FM990.00')||'%'
...
LEFT JOIN
    (
        ...
        ON
            ac.ID = bo.ACTIVITY
            AND ac.ACTIVITY_GROUP_ID IN (1,202,203,3601)
        WHERE
...
JOIN
    (
        ...
        ON
            ac.ID = bo.ACTIVITY
            AND ac.ACTIVITY_GROUP_ID IN (1,202,3601)
        WHERE
		    bo.state = 'ACTIVE'
            AND bo.STARTTIME >= PARAMS.fromdate
            AND bo.STARTTIME < (PARAMS.todate + 86400000)
