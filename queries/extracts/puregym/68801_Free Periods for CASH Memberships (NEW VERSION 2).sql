 ...
 SELECT
     ...
     (
         CASE
             ...
             THEN TO_CHAR((b.sub_end_date + interval '1' DAY * (b.free_theoric_length - b.existing_freeze_days_in_period - b.given_covid19_freeday_inperiod)) + (params.EndDate - b.sub_end_date), 'YYYY-MM-DD')
     ...
     (
         SELECT --a.center - mod(a.center,20) as threadgroup,
             ...
             COALESCE(
                       (
                       SELECT
                           SUM( cast(least(srd2.end_date,a.fullfreezeend) as date) - cast(greatest(srd2.start_date,a.fullfreezestart) as date) + 1)
                       FROM
             ...
             (cast(a.fullfreezeend as date) - cast(a.fullfreezestart as date))+1 AS free_theoric_length
         FROM
             (
                 SELECT
                     s.center,
                 ...
                 LEFT JOIN
                     (
                         SELECT
                             s3.center,
                             s3.id,
                             (s3.end_date - interval '1' DAY * SUM(srd3.end_date - srd3.start_date + 1)) AS orig_end_date,
                 ...
                 WHERE
                     ...
                     AND cast(s.CENTER as varchar) IN ($$Scope$$)
                     ...
                     (
                         ...
                         WHERE
                             ...
                             AND ppl.PRODUCT_GROUP_ID IN (401,6409,11801) ) ) a
         CROSS JOIN
             params ) b
 CROSS JOIN
     params
 WHERE
     (
         b.free_theoric_length - b.existing_freeze_days_in_period - b.given_covid19_freeday_inperiod) > 0
     --and b.id < 10
 ORDER BY
     1,2,3
