...
JOIN puregym.persons p ON p.center = t2.center AND p.id = t2.id
JOIN puregym.account_receivables ar ON p.center = ar.customercenter AND p.id = ar.customerid AND ar.ar_type = 4
JOIN puregym.payment_accounts pac ON ar.center = pac.center AND ar.id = pac.id
JOIN puregym.payment_agreements pag ON pac.active_agr_center = pag.center AND pac.active_agr_id = pag.id AND pac.active_agr_subid = pag.subid
LEFT JOIN puregym.person_ext_attrs pea_email ON pea_email.PERSONCENTER = p.center AND pea_email.PERSONID = p.id  AND pea_email.NAME = '_eClub_Email'
LEFT JOIN puregym.person_ext_attrs pea_mobile ON pea_mobile.PERSONCENTER = p.center AND pea_mobile.PERSONID = p.id AND pea_mobile.NAME = '_eClub_PhoneSMS'
LEFT JOIN puregym.cashcollectioncases cc ON cc.personcenter = p.center AND cc.personid = p.id AND cc.missingpayment = true AND cc.CLOSED = false