...
--  
 ...
 v0 AS
 (
 SELECT  substr(messages.subject, position(' IN ' IN messages.subject) + 4) club_name
       , messages.senttime offline_time
       , CASE message_type_id
           WHEN 123 THEN
             CASE LEAD(message_type_id, 1) OVER (PARTITION BY substr(messages.subject, position(' IN ' IN messages.subject) + 4) ORDER BY messages.senttime)
             WHEN 124 THEN
               LEAD(senttime, 1) OVER (PARTITION BY substr(messages.subject, position(' IN ' IN messages.subject) + 4) ORDER BY messages.senttime)
             ELSE
               NULL
             END
           ELSE
   ...
   FROM messages
     JOIN event_type_config
      ...
      AND event_type_config.event_type_id in (123, 124)
      AND event_type_config.state = 'ACTIVE'
      --AND messages.center in (100)
     -- AND messages.id in (201)
  WHERE 1=1
