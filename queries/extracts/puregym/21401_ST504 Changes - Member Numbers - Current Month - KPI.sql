SELECT
    ...
    decode(REGION,'Justin Way',1,'Graeme Penny',2,'Jake Rostron',3,'Matt Buckley',4,'Johanna Brogan',5,'Matt Tomlinson',6,'Naj Chekara',7,'Malcom Armstrong',8,'Barry Ashby',9,'Andrew Ingham',10,'John Maddox',11,'Jacqui Pope',12) as "Region Number",
    ...
    CASE
        WHEN (add_months(TRUNC(SYSDATE -1 -$$offset$$ ,'MM'),1)-TRUNC(SYSDATE-1 -$$offset$$ ,'DDD')-1) = 0
        THEN SUM(MONTHNETGAINDIFF)
        ELSE CEIL(SUM(MONTHNETGAINDIFF)/(add_months(TRUNC(SYSDATE -1 -$$offset$$ ,'MM'),1)-TRUNC(SYSDATE-1 -$$offset$$ ,'DDD')-1) *-1)
    END                         AS "Month Net Gain Req Per Day",
    SUM(MEMBERSSTARTMONTH)      AS "Start of Month",
    SUM(JOINERS)                AS "Joiners",
    SUM(YEST_JOINERS)           AS "Yesterday's Joiners",
    SUM(REJOINERS+REINSTATED)   AS "Rejoiners",
    SUM(CANCELS)                AS "Cancels",
    SUM(YEST_CANCELS)           AS "Yesterday's Cancels",
    SUM(MEMBERS)                AS "Members closing",   
    SUM(MEMBERSYESTLASTYEAR)    AS "Members closing Yest Last Year", 
...
FROM
    (
        SELECT
            ...
            MCYL.VALUE  AS MEMBERSYESTLASTYEAR,
            (
                SELECT
                    SUM(D.VALUE)
                FROM
            ...
            (
                ...
                WHERE
                    ...
                    AND D.FIELD = F.ID
                    AND C.ID = D.CENTER
                    AND D.FOR_DATE = PARAMS.REPORTDATE-1 ) AS YEST_JOINERS,                    
            (
                SELECT
                    SUM(D.VALUE)
                FROM
            ...
            (
                ...
                WHERE
                    ...
                    AND D.FIELD = F.ID
                    AND C.ID = D.CENTER
                    AND D.FOR_DATE = PARAMS.REPORTDATE-1) AS YEST_CANCELS,                    
            MC.VALUE-MS.VALUE                                                                    AS NETGAIN,
            NVL(MCY.VALUE,0)-NVL(MSY.VALUE,0)                                                    AS NETGAIN_YESTERDAY,
        ...
        WHERE
            ...
            AND MC.FIELD = MF.ID
            AND MCY.FIELD = MF.ID
            AND MCYL.FIELD = MF.ID            
            AND MC.FOR_DATE = PARAMS.REPORTDATE
            AND MCY.FOR_DATE = PARAMS.REPORTDATE - 1
            AND MCYL.FOR_DATE =  to_date(to_char(PARAMS.REPORTDATE - 1, 'DDMM') || to_char(PARAMS.REPORTDATE - 1, 'YYYY')-1, 'DDMMYYYY')
