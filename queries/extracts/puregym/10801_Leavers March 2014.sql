...
SELECT
    ...
    NVL(ch.this_month,0)                                                                                                                                          this_month,
    NVL(ch.last_3_months,0)                                                                                                                                       last_3_months,
    ...
    NVL(par.num ,0)                                  AS "Class usage last 3 months",
    DECODE(par.induction,1,'yes','no')               AS "Induction Participation",
    NVL(future.same,0) "future days in same gym",
    NVL(future.other,0) "future days in other gyms"
FROM
    PUREGYM.PERSONS p
JOIN
    PUREGYM.PERSONS p1
...
ON
    ...
    AND s.OWNER_ID = p.id
    AND s.END_DATE BETWEEN to_date('2014-03-01','yyyy-MM-dd') AND to_date('2014-03-31','yyyy-MM-dd')
...
LEFT JOIN
    (
        ...
        WHERE
            ...
            -- AND ch.CHECKIN_TIME BETWEEN dateToLongtz(TO_CHAR(TRUNC(add_months(s.END_DATE, -3)), 'YYYY-MM-dd HH24:MI'),'Europe/London') AND dateToLongtz(TO_CHAR(s.END_DATE, 'YYYY-MM-dd HH24:MI'), 'Europe/London')
