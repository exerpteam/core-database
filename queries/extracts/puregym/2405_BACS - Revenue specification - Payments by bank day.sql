...
-- payments by bank day, including bounces after period
 SELECT
    BANK_DATE,
    SUM(
        CASE
            WHEN CreditDebit = 'CREDIT'
            THEN TOTAL
            ELSE 0
        END ) AS "CREDIT",
    SUM(
        CASE
            WHEN CreditDebit = 'DEBIT'
            THEN TOTAL
            ELSE 0
        END ) AS "DEBIT"
 FROM
     (
         ...
         FROM
             (
                 SELECT
                     ...
                     p.FULLNAME,
                     CASE pr.REQUEST_TYPE  WHEN 1 THEN  'PAYMENT'  WHEN 5 THEN  'REFUND'  WHEN 6 THEN  'REPRESENTATION'  WHEN 8 THEN 
                     'ZERO'  ELSE 'UNKNOWN' END as type,
                     CASE pr.STATE  WHEN '1' THEN  'New'  WHEN '2' THEN  'Sent'  WHEN '3' THEN  'Done'  WHEN '4' THEN  'Done maual'  WHEN '5' THEN 
                     'Rejected, clearinghouse'  WHEN '6' THEN  'Rejected, bank'  WHEN '7' THEN  'Rejected, debtor'  WHEN '8' THEN 
                     'Cancelled'  WHEN '12' THEN 'Failed, no creditor'  WHEN '17' THEN  'Rejected, debtor'  WHEN '19' THEN 
                     'Failed, not supported' END as                       state,
                 ...
                 AND pr.DUE_DATE <= :toDate) collections
         UNION ALL
         SELECT
             bounces.BANK_DATE,
         ...
         FROM
             (
                 ...
                 WHERE
                     pr.DUE_DATE >= :fromDate
                 AND pr.DUE_DATE <= :toDate) bounces ) t
 group by BANK_DATE
 ORDER BY
     BANK_DATE
