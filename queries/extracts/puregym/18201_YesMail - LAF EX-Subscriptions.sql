WITH
    ...
    (
        SELECT
            --  TRUNC(SYSDATE-2 ,'DDD')
            -- p_date,
            datetolongTZ(TO_CHAR(SYSDATE+1 ,'YYYY-MM-DD') || ' 00:00', 'Europe/London') AS
            p_start_next_day
        FROM
            dual
    )
SELECT DISTINCT
    ...
    DECODE (su.STATE, 2,'ACTIVE', 3,'ENDED', 4,'FROZEN', 7,'WINDOW', 8,'CREATED','UNKNOWN') AS
    STATE,
    DECODE (su.SUB_STATE, 1,'NONE', 2,'AWAITING_ACTIVATION', 3,'UPGRADED', 4,'DOWNGRADED', 5,
    'EXTENDED', 6, 'TRANSFERRED',7,'REGRETTED',8,'CANCELLED',9,'BLOCKED','UNKNOWN') AS SUB_STATE,
    DECODE(sc.NEW_SUBSCRIPTION_CENTER||'ss'||sc.NEW_SUBSCRIPTION_ID,'ss',NULL,
    sc.NEW_SUBSCRIPTION_CENTER||'ss'||sc.NEW_SUBSCRIPTION_ID) AS NEW_SUBSCRIPTION_ID
FROM
    PUREGYM.PERSONS p
    /* Check-ins */
JOIN
