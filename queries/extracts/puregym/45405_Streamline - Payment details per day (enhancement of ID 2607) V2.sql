...
WITH
    ...
    (   SELECT
            ...
            CASE WHEN (je.creation_time >= CAST(datetolongTZ(to_char(CAST($$fromdate$$ AS DATE), 'YYYY-MM-dd HH24:MI'),  'Europe/London') AS BIGINT) - 300000
                                AND je.creation_time <  CAST(datetolongTZ(to_char(CAST($$todate$$ AS DATE)+1, 'YYYY-MM-dd HH24:MI'),  'Europe/London') AS BIGINT) + 300000
        ...
        WHERE
            ...
            AND act.trans_time >= CAST(datetolongTZ(to_char(CAST($$fromdate$$ AS DATE), 'YYYY-MM-dd HH24:MI'),  'Europe/London') AS BIGINT)
            AND act.trans_time <  CAST(datetolongTZ(to_char(CAST($$todate$$ AS DATE)+1, 'YYYY-MM-dd HH24:MI'),  'Europe/London') AS BIGINT)
    )
    ,
    base2 AS
    (   SELECT
...
SELECT
    TO_CHAR(longtodateTZ(act_trans_time, 'Europe/London'), 'DD/MM/YYYY HH24:MI:SS') AS "DATETIME",
    ...
    CASE WHEN(art_amount > 0)
            ...
            ELSE 0
    END                                                                             AS PAYMENT,
    TO_CHAR(longtodateTZ(act_entry_time, 'Europe/London'), 'DD/MM/YYYY HH24:MI:SS') AS "ENTRYTIME",
