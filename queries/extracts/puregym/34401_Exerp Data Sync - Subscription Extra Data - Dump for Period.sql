...
WITH
    ...
    (
        SELECT
            /*+ materialize  */
            datetolongC(TO_CHAR(TRUNC($$fromDate$$), 'YYYY-MM-DD HH24:MI'), any_club_in_scope.id) AS FROMDATE,
            datetolongC(TO_CHAR(TRUNC($$toDate$$)+1, 'YYYY-MM-DD HH24:MI'), any_club_in_scope.id) AS TODATE
        FROM
            dual
