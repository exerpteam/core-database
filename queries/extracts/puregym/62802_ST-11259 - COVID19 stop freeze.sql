WITH
    ...
    (
        SELECT
            ...
            to_date('2020-05-02','YYYY-MM-DD') AS to_date,
...
FROM
    (
        SELECT
            ...
            spp.TO_DATE - spp.FROM_DATE                                                 AS inv_days,
            il.TOTAL_AMOUNT                                                        INV_TOTAL_AMOUNT,
            GREATEST(spp.FROM_DATE ,srp.start_date, params.from_Date) AS
            credit_period_start,
            LEAST(spp.TO_DATE,srp.end_date, to_Date('2020-05-02','yyyy-MM-dd')) AS
            credit_period_end,
            LEAST(spp.TO_DATE,srp.end_date, to_Date('2020-05-02','yyyy-MM-dd')) - GREATEST
            (spp.FROM_DATE , srp.start_date, params.from_Date) +1 AS
            covid_credit_days
        FROM
...
/*        JOIN
            products fpr
        ON
            fpr.center = st.FREEZEPERIODPRODUCT_CENTER
        AND fpr.id = FREEZEPERIODPRODUCT_ID*/
        JOIN
            products pr
        ON
