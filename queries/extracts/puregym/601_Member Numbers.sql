        ...
        SELECT
            ...
            CASE
                WHEN DEBTORS.CUSTOMERCENTER IS NULL
                    ...
                    AND floor(months_between(longtodate(:ToDate + (1000*60*60*24)), BIRTHDATE) / 12) >= 18
                    ...
                    (
                        ...
                        WHERE
                            (
                                ...
                                AND ST.ST_TYPE IN (1)
                                AND ((
                                        SCL.BOOK_START_TIME <= :FromDate
                                        AND (
                                            SCL.BOOK_END_TIME IS NULL
                                            OR SCL.BOOK_END_TIME > :FromDate ))
                                    OR (
                                        SCL.BOOK_START_TIME >= :FromDate
                                        AND SCL.BOOK_START_TIME <= :ToDate + (1000*60*60*24)) )
                    ...
                    (
                        ...
                        ON
                            (
                                ...
                                AND (
                                    ...
                                    OR SCL1.BOOK_END_TIME >= :ToDate + (1000*60*60*24) )
                        ...
                        ON
                            (
                                ...
                                AND SPP.ID = SU.ID
                                AND SPP.FROM_DATE <= longtodate(:ToDate + (1000*60*60*24))
                                AND SPP.TO_DATE >= longtodate(:ToDate + (1000*60*60*24))
                                AND SPP.SPP_STATE = 1
                                AND SPP.ENTRY_TIME <= :ToDate + (1000*60*60*24) + (6*60*60*1000) )
                        LEFT JOIN
                        ...
                        ON
                            (
                                ...
                                AND SP.FROM_DATE >= longtodate(:ToDate + (1000*60*60*24))
                                AND SP.PRICE > 0 )
                        WHERE
                            (
                                SU.CENTER IN (:Scope)
...
AND TO_CHAR(target.FOR_DATE, 'YYYY-MM-DD') = TO_CHAR(longtodate(:FromDate + (1000*60*60*24)), 'YYYY-MM-DD')
GROUP BY
