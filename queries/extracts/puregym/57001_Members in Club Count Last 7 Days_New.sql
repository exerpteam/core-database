WITH
    ...
    (
        SELECT
            /*+ materialize */
            dateToLongTZ(TO_CHAR(TRUNC(SYSDATE-7),'YYYY-MM-DD') || ' 00:00','UTC') from_date ,
    ...
    (
        SELECT
            /*+ materialize */
            dateToLongTZ(TO_CHAR(TRUNC(SYSDATE-7),'YYYY-MM-DD') || ' 00:00','UTC') + ((level ) * 1000 * 60 * 60)                                  lower_bound ,
            dateToLongTZ(TO_CHAR(TRUNC(SYSDATE-7),'YYYY-MM-DD') || ' 00:00','UTC') + ((level +1) * 1000 * 60 * 60) - 1                            higher_bound ,
            longToDateTZ(dateToLongTZ(TO_CHAR(TRUNC(SYSDATE-7),'YYYY-MM-DD') || ' 00:00','UTC') + ((level) * 1000 * 60 * 60),'UTC')     lower_date_time ,
            longToDateTZ(dateToLongTZ(TO_CHAR(TRUNC(SYSDATE-7),'YYYY-MM-DD') || ' 00:00','UTC') + ((level + 1) * 1000 * 60 * 60),'UTC') higher_date_time
        FROM
