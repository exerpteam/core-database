...
SELECT
    ...
    GREATEST(NVL(brc.maximum_participations, ac.max_participants), ac.max_participants)                                              AS "Resource capacity",
    ROUND((showup_waiting.total_showup/ GREATEST(NVL(brc.maximum_participations, ac.max_participants), ac.max_participants))*100, 1) AS "Resource show up ratio %",
    bo.WAITING_LIST_CAPACITY                                                                                                         AS "Waiting list capacity",
    showup_waiting.total_waiting                                                                                                     AS "Total waiting list",
    ROUND((showup_waiting.total_waiting/ DECODE(bo.WAITING_LIST_CAPACITY, 0, 1, bo.WAITING_LIST_CAPACITY))*100, 1)                   AS "Waiting list ratio %",
...
ON
    ac.ID = bo.ACTIVITY
JOIN
    PUREGYM.ACTIVITY_RESOURCE_CONFIGS arc
ON
    arc.ACTIVITY_ID =ac.id
JOIN
...
ON
    ...
    AND arc.BOOKING_RESOURCE_GROUP_ID = brc.GROUP_ID
JOIN
    CENTERS c
ON
    c.ID = bo.CENTER
...
WHERE
    ...
    AND bo.STATE='ACTIVE'
    AND bru.STARTTIME>= params.PeriodStart  
    AND bru.STOPTIME<= params.PeriodEnd      
...
GROUP BY
    ...
    GREATEST(NVL(brc.maximum_participations, ac.max_participants), ac.max_participants) ,
