...
SELECT
  ...
  , SUM(
        CASE
            ...
            THEN 1
            ELSE 0
        END) AS "Re-joiner Sales" /*Rejoiners_total  */
  , SUM(
        CASE
            WHEN joiners.owner_center IS NULL and il.TOTAL_AMOUNT != 0
            THEN 1
            ELSE 0
        END) AS Rejoiners_paid
  , SUM(
        CASE
            WHEN joiners.owner_center IS NULL 
            THEN il.TOTAL_AMOUNT
            ELSE 0
        END) AS "Rejoiners_JF_amount_£"
  , SUM(
        CASE
            WHEN joiners.owner_center IS NOT NULL 
            THEN 1
            ELSE 0
        END) AS "New Joiner sales" /*joiners_total*/
  , SUM(
        CASE
            WHEN joiners.owner_center IS NOT NULL  and il.TOTAL_AMOUNT != 0
            THEN 1
            ELSE 0
        END) AS "New Joiner Paid" /*joiners_paid*/
  , SUM(
        CASE
            WHEN joiners.owner_center IS NOT NULL  
            THEN il.TOTAL_AMOUNT
            ELSE 0
        END) AS "New joiners_JF_amount_£"
  , SUM(
        CASE
            WHEN  il.TOTAL_AMOUNT != 0
            THEN 1
            ELSE 0
        END) AS "Total number JF" /*TOTAL_JF*/
  , SUM(
        CASE
            WHEN  il.TOTAL_AMOUNT != 0
            THEN il.TOTAL_AMOUNT
            ELSE 0
        END) AS "TOTAL_JF_AMOUNT_£"
FROM
    PARAMS
...
LEFT JOIN
    (
        ...
        WHERE
            ...
            --            AND
            SCL.ENTRY_START_TIME < PARAMS.ENDTIME
