 ...
 FROM
     (
         SELECT
             ...
             CASE
                  WHEN FIRST_VALUE(i2.PREVIOUS_TRANSACTION) 
             OVER (PARTITION BY i2.pid ORDER BY i2.PREVIOUS_TRANSACTION DESC NULLS LAST) IS NULL
         OR DATE_PART('month', AGE(i2.LAST_TRANS, 
              FIRST_VALUE(i2.PREVIOUS_TRANSACTION::date) 
              OVER (PARTITION BY i2.pid ORDER BY i2.PREVIOUS_TRANSACTION DESC NULLS LAST))) 
            > $$inactive_period$$
         OR TO_CHAR(FIRST_VALUE(i2.PREVIOUS_TRANSACTION) 
              OVER (PARTITION BY i2.pid ORDER BY i2.PREVIOUS_TRANSACTION DESC NULLS LAST), 'YYYY-MM-DD') 
            = TO_CHAR(i2.LAST_TRANS, 'YYYY-MM-DD')
    THEN 'YES'
                 ELSE 'NO'
             END AS new_customer,
             CASE
             ...
             (
                 ...
                 FROM
                     (
						...
						ON 
							person.center = email.personcenter 
							AND person.id = email.personid 
                         ...
                         ON
                             ...
                             AND (
                                 spg.ID in (212,227)
                                 OR spg.parent_product_group_id in (212, 227))
                         WHERE
