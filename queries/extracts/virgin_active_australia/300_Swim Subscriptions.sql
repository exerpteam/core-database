...
SELECT distinct
 ...
 --            mpr2.ID      mpr2_id,
			...
			--PE.Fullname PT_NAME,
             ...
             cc.AMOUNT                                                                                                                       CC_AMOUNT,
             ROUND(EXTRACT(YEAR FROM AGE(CURRENT_TIMESTAMP, s.START_DATE)) * 12 +
      EXTRACT(MONTH FROM AGE(CURRENT_TIMESTAMP, s.START_DATE)) +
      EXTRACT(DAY FROM AGE(CURRENT_TIMESTAMP, s.START_DATE)) / 30.0, 2)                                                                                 months_from_start,
             atts.TXTVALUE                                                                                                                   title,
         ...
         WHERE
             ...
             (
                 ...
                 WHERE
                     ...
                     AND link.PRODUCT_GROUP_ID IN (291,300, 286) ) )q1
 LEFT JOIN
