...
--             END                                            AS NEXT_PRICE_CHANGE_DATE_DEF ,
             cc.AMOUNT                                                                                                                       CC_AMOUNT,
             ROUND(EXTRACT(YEAR FROM AGE(CURRENT_TIMESTAMP, sa.START_DATE)) * 12 +
      EXTRACT(MONTH FROM AGE(CURRENT_TIMESTAMP, sa.START_DATE)) +
      EXTRACT(DAY FROM AGE(CURRENT_TIMESTAMP, sa.START_DATE)) / 30.0, 2)
                                                                                  months_from_start,
             atts.TXTVALUE                                                                                                                   title,
             p.FIRSTNAME,
             p.LASTNAME,
