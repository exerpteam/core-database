...
-- Report detailing the refunds done by and the corresponding reasons
WITH
    PARAMS AS
    (
        SELECT
            CAST(extract(epoch FROM timezone('Australia/Sydney', CAST($$FromDate$$ AS timestamptz))) AS
            bigint)*1000 AS FromDate,
            CAST(extract(epoch FROM timezone('Australia/Sydney', CAST($$ToDate$$ AS timestamptz))) AS
    ...
    (
        ...
        AND COALESCE(sa.end_date, CURRENT_TIMESTAMP) > CURRENT_TIMESTAMP - INTERVAL '1 day'
        AND sa.cancelled = 0
        JOIN
