...
SELECT
    ...
    /*soldby.txtvalue  AS orig_sales_person,*/
...
JOIN
    STATE_CHANGE_LOG SCL1 ON SCL1.CENTER = sub.CENTER AND SCL1.ID = sub.ID AND SCL1.ENTRY_TYPE = 2
                         ...
                         AND SCL1.ENTRY_START_TIME >= EXTRACT(EPOCH FROM ($$CreationFrom$$::TIMESTAMP AT TIME ZONE 'Australia/Sydney')) * 1000
                         AND (SCL1.ENTRY_END_TIME IS NULL OR SCL1.ENTRY_END_TIME < (EXTRACT(EPOCH FROM $$CreationTo$$::TIMESTAMP AT TIME ZONE 'Australia/Sydney') * 1000 + 86400000))
LEFT JOIN 
...
LEFT JOIN 
    PERSON_EXT_ATTRS id_seen ON owner.center = id_seen.PERSONCENTER AND owner.id = id_seen.PERSONID AND id_seen.name = 'ID_SEEN_APPROVED'
LEFT JOIN 
    PERSON_EXT_ATTRS linked_member ON owner.center = linked_member.PERSONCENTER AND owner.id = linked_member.PERSONID AND linked_member.name = 'LINKED_MEMBER_VALID'
LEFT JOIN 
    PERSON_EXT_ATTRS Member_Details ON owner.center = Member_Details.PERSONCENTER AND owner.id = Member_Details.PERSONID AND Member_Details.name = 'MEMBER_PERSONAL_DETAILS_ACCURATE'
LEFT JOIN 
    PERSON_EXT_ATTRS parq ON owner.center = parq.PERSONCENTER AND owner.id = parq.PERSONID AND parq.name = 'PARQ_COMPLETED_ALL_NO'
LEFT JOIN 
    PERSON_EXT_ATTRS signatureTC ON owner.center = signatureTC.PERSONCENTER AND owner.id = signatureTC.PERSONID AND signatureTC.name = 'SIGNATURE_IN_PLACE_TOM'
LEFT JOIN 
    PERSON_EXT_ATTRS validStartdate ON owner.center = validStartdate.PERSONCENTER AND owner.id = validStartdate.PERSONID AND validStartdate.name = 'VALID_START_DATE'
LEFT JOIN 
    PERSON_EXT_ATTRS signatureDDI ON owner.center = signatureDDI.PERSONCENTER AND owner.id = signatureDDI.PERSONID AND signatureDDI.name = 'SIGNATURE_IN_PLACE_DDM'
LEFT JOIN 
...
LEFT JOIN 
    PERSONS salesPersonOverride ON salesPersonOverride.CENTER || 'p' || salesPersonOverride.ID = salesPersonOverrideExt.TXTVALUE -- Correctly joining salesPersonOverride
LEFT JOIN 
...
WHERE
    ...
    AND sub.CREATION_TIME >= EXTRACT(EPOCH FROM ($$CreationFrom$$::TIMESTAMP AT TIME ZONE 'Australia/Sydney')) * 1000
    AND sub.CREATION_TIME < (EXTRACT(EPOCH FROM $$CreationTo$$::TIMESTAMP AT TIME ZONE 'Australia/Sydney') * 1000 + 86400000)
   ...
   /* AND NOT EXISTS (
        ...
        JOIN PRODUCTS oldsubprod ON oldsubprod.CENTER = oldsub.subscriptiontype_center AND oldsubprod.ID = oldsub.subscriptiontype_id AND oldsubprod.name NOT IN ('Free Adult Guest', 'Paying Adult Guest','Trial Membership', 'Guest', 'Guest Pass Subscription', 'Online Membership','Vitality Online','Free Trial', 'Hyrox Training')
    ...
    AND NOT EXISTS (
        ...
        WHERE SCLCHECK.CENTER = sub.CENTER 
              ...
              AND SCL1.ENTRY_START_TIME >= EXTRACT(EPOCH FROM ($$CreationFrom$$::TIMESTAMP AT TIME ZONE 'Australia/Sydney')) * 1000
              AND SCL1.ENTRY_START_TIME < (EXTRACT(EPOCH FROM $$CreationTo$$::TIMESTAMP AT TIME ZONE 'Australia/Sydney') * 1000 + 86400000)
    )*/
    AND EXISTS (
        SELECT *
