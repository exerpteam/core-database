...
WITH
    ...
    (   SELECT
            c.id, 
            c.name,
            getstartofday((date_trunc('month', CURRENT_DATE) - interval '1 month')::DATE::VARCHAR, 
            c.id)::bigint AS first_day,
            getendofday((date_trunc('month', CURRENT_DATE) - interval '1 day')::DATE::VARCHAR, 
...
LEFT JOIN
 ...
 JOIN
     (
		...
		on pa.booking_center = pm1.id
         ...
         WHERE
             bo1.CENTER IN ($$scope$$)
             AND bo1.STARTTIME> pm1.first_day
             AND bo1.STARTTIME< pm1.last_day
 ...
 WHERE
     bo.STARTTIME> pm.first_day
     AND bo.STARTTIME< pm.last_day
     AND bo.CENTER IN ($$scope$$)
         AND bo.STATE='ACTIVE'
AND ag.ID not in (2,14)
 ORDER BY
     c.NAME,
     bo.starttime,
     bo.name
