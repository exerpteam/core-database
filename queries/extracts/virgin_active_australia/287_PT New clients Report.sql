...
--  
 ...
 ON
     (
         pg.ID in (212,227)
         OR pg.PARENT_PRODUCT_GROUP_ID in (212,227))
         and pg.STATE = 'ACTIVE'
 ...
 WHERE
     ...
     (
         ...
         WHERE
             ...
             and inv.TRANS_TIME BETWEEN 
      EXTRACT(EPOCH FROM $$fromDate$$::TIMESTAMP) * 1000
  AND EXTRACT(EPOCH FROM $$toDate$$::TIMESTAMP + INTERVAL '1 day' - INTERVAL '1 millisecond') * 1000
 GROUP BY
     invl.PERSON_CENTER,
     invl.PERSON_ID ,
     inv.PAYER_CENTER ,
