...
WITH
     ...
     (
         SELECT 
    -- Truncate the date and add 1 day (tomorrow)
    DATE_TRUNC('day', (TO_TIMESTAMP(getcentertime(4), 'YYYY-MM-DD HH24:MI') + INTERVAL '1 day')) AS tomorrowDate,
    
    -- Convert to Sydney time and get the Unix timestamp
    CAST(EXTRACT(EPOCH FROM (TO_TIMESTAMP(getcentertime(4), 'YYYY-MM-DD HH24:MI') + INTERVAL '1 day') AT TIME ZONE 'UTC' AT TIME ZONE 'Australia/Sydney') AS BIGINT) AS nextDayLongDate
                 
     )
 SELECT DISTINCT
     t1.Club                                                     AS "Club",
     t1.MemberID                                                 AS "Member ID",
	 t1.SubscriptionID											AS "Subscription ID",
     ...
     t1.LegacyId                                                 AS "Legacy ID",
     t1.LegacySubscription      										 AS                                           "Legacy Subscription",
	 ...
	 t1.ContractEndDate									AS "Contract End Date",
     t1.MembershipSubscription                   AS "Membership Subscription",
	 t1.ProductPrice							AS "Subscription Price",
 ...
 FROM
     (
         SELECT
             ...
             LegacyId.TXTVALUE                                                                                                   AS LegacyID,
			 LegacySubName.TXTVALUE                                                                                                   AS LegacySubscription,
			 ...
			 TO_CHAR(binding_end_date,'yyyy-MM-dd')																			AS ContractEndDate,
		...
		LEFT JOIN
             PERSON_EXT_ATTRS LegacyID
         ON
             p.center=LegacyID.PERSONCENTER
             AND p.id=LegacyID.PERSONID
			 AND LegacyId.Name = 'LegacyID'
             AND LegacyID.TXTVALUE IS NOT NULL
		LEFT JOIN
             PERSON_EXT_ATTRS LegacySubName
         ON
             p.center=LegacySubName.PERSONCENTER
             AND p.id=LegacySubName.PERSONID
			 AND LegacySubName.Name = 'LegacySubName'
             AND LegacySubName.TXTVALUE IS NOT NULL
         LEFT JOIN
