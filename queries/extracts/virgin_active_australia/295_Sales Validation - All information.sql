...
SELECT
    ...
    CASE 
        WHEN id_seen.TXTVALUE IN ('Y', 'NA')
             ...
             AND PENDCLEAREDRG.TXTVALUE IN ('Y', 'NA') 
        THEN 'YES' 
        ELSE 'NO' 
    ...
    PENDCLEAREDRG.TXTVALUE AS Pend_cleared
FROM 
...
ON 
    ...
    AND (SCL1.ENTRY_END_TIME IS NULL 
...
LEFT JOIN 
    PERSON_EXT_ATTRS PENDCLEAREDRG 
ON 
    owner.CENTER = PENDCLEAREDRG.PERSONCENTER 
    AND owner.ID = PENDCLEAREDRG.PERSONID 
    AND PENDCLEAREDRG.NAME = 'PENDCLEAREDRG' 
LEFT JOIN 
    EMPLOYEES emp 
ON 
...
WHERE 
    ...
    AND ss.SUBSCRIPTION_CENTER != '999' -- Exclude Online+
    AND NOT EXISTS
    (
        SELECT
            1
        ...
        ON 
            ...
            AND oldsubprod.NAME NOT IN ('Free Adult Guest', 'Paying Adult Guest', 'trial') 
        WHERE 
            ...
            AND (oldSub.STATE != 5 
                AND NOT (oldSub.STATE = 3 AND oldSub.SUB_STATE = 8))
    )
    AND NOT EXISTS
    (
        SELECT 
