...
SELECT DISTINCT ON (bk.center, bk.id)
     ...
     CASE
         WHEN mpr.CACHED_PRODUCTNAME IN ('PT - Kickstart Promo', 'PT - Staff', 'PT (No Level)', 'PT - Full Throttle Promo', 'Coaching Level 1', 'Coaching Level 0') THEN 'PT Level 1'
...
WHEN mpr.CACHED_PRODUCTNAME IN('Coaching Level 4','Coaching Level 4 Top Up') THEN 'PT Level 4'
     ...
     mpr.CACHED_PRODUCTNAME AS "Clip Card Used",
     CASE 
     ...
     --sales.product_normal_price AS "PT Revenue",
--sales.total_amount
FROM BOOKINGS bk
...
WHERE
    ...
    AND bk.STARTTIME >= (EXTRACT(EPOCH FROM (CURRENT_TIMESTAMP - INTERVAL '64 days')) * 1000)::bigint
    AND st.STATE = 'ACTIVE'
