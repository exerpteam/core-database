...
pr.price as PERSON_PRICE, 
s.individual_price AS BINDING_PRICE, 
     to_timestamp(s.CREATION_TIME / 1000)::DATE                                                                                                                          AS Join_Date,
     s.START_DATE                                                                                                                                                     AS START_DATE,
     s.binding_end_date                                                                                                                                            AS BINDING_DATE,
     s.end_date                                                                                                                                                       AS STOP_DATE,
     c_sub.SHORTNAME                                                                                                                                                                            AS center,
     a.NAME                                                                                                                                                                                 AS Region,
     ...
     CASE  referrer.PERSONTYPE  WHEN 0 THEN 'PRIVATE'  WHEN 1 THEN 'STUDENT'  WHEN 2 THEN 'STAFF'  WHEN 3 THEN 'FRIEND'  WHEN 4 THEN 'CORPORATE'  WHEN 5 THEN 'ONEMANCORPORATE'  WHEN 6 THEN 'FAMILY'  WHEN 7 THEN 'SENIOR'  WHEN 8 THEN 'GUEST' ELSE 'UNKNOWN' END                        AS referrer_PERSONTYPE,
	...
	rp.PRICE as refferrer_membership_price,
     rpea_email.TXTVALUE                                                                                                                                                                    AS referrer_email,
     rpea_sms.TXTVALUE                                                                                                                                                                      AS referrer_MOBILE_phone,
     to_timestamp(rs.CREATION_TIME / 1000)::DATE                                                                                                                           AS referrer_Join_Date,
     rs.START_DATE                                                                                                                                                    AS referrer_START_DATE,
     rs.binding_end_date                                                                                                                                           AS referrer_BINDING_DATE,
     rs.end_date                                                                                                                                                     AS referrer_STOP_DATE
 FROM
     PERSONS P
 JOIN
     SUBSCRIPTIONS S
 ...
 WHERE
     -- P.PERSONTYPE IN (4)
     --AND
     to_timestamp(s.CREATION_TIME / 1000)::DATE  BETWEEN $$from_date$$ AND $$to_date$$
     ...
     (
         ...
         WHERE
             (
                 s2.END_DATE > (s.START_DATE AT TIME ZONE 'Australia/Sydney' - INTERVAL '1 month')
