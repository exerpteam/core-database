...
JOIN
    STATE_CHANGE_LOG SCL1 ON (
        ...
        AND (SCL1.ENTRY_END_TIME IS NULL OR SCL1.ENTRY_END_TIME < (EXTRACT(EPOCH FROM $$CreationTo$$::TIMESTAMP) * 1000 + 86400000))
    )
LEFT JOIN
...
LEFT JOIN
    PERSON_EXT_ATTRS Pend ON owner.center = Pend.PERSONCENTER AND owner.id = Pend.PERSONID AND pend.name = 'PENDCLEAREDRG'
...
AND ss.SUBSCRIPTION_CENTER != '999'
    AND NOT EXISTS (
        ...
        JOIN PRODUCTS oldsubprod ON oldsubprod.CENTER = oldsub.subscriptiontype_center AND oldsubprod.ID = oldsub.subscriptiontype_id AND oldsubprod.name NOT IN ('Free Adult Guest', 'Paying Adult Guest','Trial Membership', 'Guest', 'Guest Pass Subscription', 'Online Membership','Hyrox Training')
