...
-- Version  of the sales validation report looking at the new extended attribute: Pend Cleared and commissionable = No
SELECT
    *
FROM
    (
            ...
            (
                ...
                ON
                    ...
                    AND SCL1.STATEID IN (2,
                                         4,8)
                    AND SCL1.ENTRY_START_TIME >= (EXTRACT(EPOCH FROM $$CreationFrom$$::TIMESTAMP) * 1000)::BIGINT 
                    AND (SCL1.ENTRY_END_TIME IS NULL OR SCL1.ENTRY_END_TIME < ((EXTRACT(EPOCH FROM $$CreationTo$$::TIMESTAMP) + 86400) * 1000)::BIGINT)
                LEFT JOIN
        ...
        SELECT
            ...
            TO_CHAR(to_timestamp(sub.CREATION_TIME / 1000.0) AT TIME ZONE 'UTC' AT TIME ZONE 'Australia/Sydney', 'DD-MM-YYYY') DATE_JOINED,
        ...
        AND sub_creationtime >= (EXTRACT(EPOCH FROM $$CreationFrom$$::TIMESTAMP) * 1000)::BIGINT
        AND sub_creationtime < ((EXTRACT(EPOCH FROM $$CreationTo$$::TIMESTAMP) + 86400) * 1000)::BIGINT
        AND NOT EXISTS
            (
                SELECT
                    *
                ...
                AND oldSub.END_DATE + INTERVAL '30 days' > to_timestamp(sub.CREATION_TIME / 1000.0) AT TIME ZONE 'UTC' AT TIME ZONE 'Australia/Sydney'
                AND (
                        oldSub.STATE != 5
                    AND NOT(
                            oldSub.STATE = 3
        ...
        AND NOT EXISTS
            (
                ...
                AND scl1_entrystart >= (EXTRACT(EPOCH FROM $$CreationFrom$$::TIMESTAMP) * 1000)::BIGINT
                AND scl1_entrystart < ((EXTRACT(EPOCH FROM $$CreationTo$$::TIMESTAMP) + 86400) * 1000)::BIGINT
        AND EXISTS
            (
                SELECT
                    *
