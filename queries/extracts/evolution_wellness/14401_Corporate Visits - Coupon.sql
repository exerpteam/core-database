...
JOIN
        ...
        ON ca.center = re.relativecenter
        AND ca.id = re.relativeid
        AND ca.subid = re.relativesubid
JOIN
        checkins ch
        ON ch.person_center = re.center
...
JOIN
        persons com
        ON re.relativecenter = com.center
...
JOIN
        ...
        JOIN
                ...
                AND pr.id = st.id
        JOIN
                evolutionwellness.product_and_product_group_link ppg
                ON ppg.product_center = pr.center
                AND ppg.product_id = pr.id
                AND ppg.product_group_id = 18223                
        WHERE
