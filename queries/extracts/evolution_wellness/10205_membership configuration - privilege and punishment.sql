...
(
        SELECT
                ...
                mpg.name
        FROM evolutionwellness.master_prod_and_prod_grp_link mpli
        JOIN evolutionwellness.product_group mpg ON mpli.product_group_id = mpg.id
        JOIN evolutionwellness.masterproductregister mpr ON mpr.id = mpli.master_product_id
        WHERE
...
(
        ...
        GROUP BY
                t2.granter_id, list_privset, punishment_name
),
full_collection AS
(
        SELECT
        ...
        (
                ...
                LEFT JOIN evolutionwellness.product_group ppg ON mpr.primary_product_group_id = ppg.id
        ...
        LEFT JOIN evolutionwellness.templates t ON t.id = t1.contract_template_id
     
)
SELECT fc.*
FROM full_collection fc
WHERE
        ('ID' = fc.countrySelected AND (fc.scope_type = 'T' OR (fc.scope_type = 'A' AND fc.scope_id IN (1,4,15,19,22,23,34)) OR (fc.scope_type = 'C' AND fc.country = 'ID')))
        OR
        ('PH' = fc.countrySelected AND (fc.scope_type = 'T' OR (fc.scope_type = 'A' AND fc.scope_id IN (1,18,24,25,40)) OR (fc.scope_type = 'C' AND fc.country = 'PH')))
        OR
        ('SG' = fc.countrySelected AND (fc.scope_type = 'T' OR (fc.scope_type = 'A' AND fc.scope_id IN (1,2,8,9,14)) OR (fc.scope_type = 'C' AND fc.country = 'SG')))
        OR
        ('MY' = fc.countrySelected AND (fc.scope_type = 'T' OR (fc.scope_type = 'A' AND fc.scope_id IN (1,7,13,17,32,33)) OR (fc.scope_type = 'C' AND fc.country = 'MY')))
        OR
        ('TH' = fc.countrySelected AND (fc.scope_type = 'T' OR (fc.scope_type = 'A' AND fc.scope_id IN (1,6,21,27,28,29,37,38)) OR (fc.scope_type = 'C' AND fc.country = 'TH')))
order by 6,1,2