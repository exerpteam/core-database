...
--Added Next Month Billing Date on 20250724
--Added distinct on 20250730
--Added Start Date on 20250804
...
,TO_CHAR(((DATE_TRUNC('month', s.START_DATE) + INTERVAL '1 month')::DATE), 'YYYY-MM-DD')        AS "First Billed Month"
        ,TO_CHAR(((DATE_TRUNC('month', CURRENT_DATE) + INTERVAL '1 month')::DATE), 'YYYY-MM-DD')        AS "Next Month Billing Date"
...
,TO_CHAR(DATE_TRUNC('month', s.BILLED_UNTIL_DATE), 'YYYY-MM-DD')                                AS "Last Billed Month"
...
LEFT JOIN
        evolutionwellness.centers pec
        ON pec.id = pi.center
JOIN
        evolutionwellness.subscriptions s
        ON s.owner_center = pi.center
        AND s.owner_id = pi.id
	AND s.state IN (2,4,7,8) --Active including future cancel date; 3= Cancelled
	--AND s.state IN (2,3,4,7,8)
LEFT JOIN
        evolutionwellness.subscription_freeze_period sf
        ON s.center = sf.subscription_center
        AND s.id = sf.subscription_id
		AND 
                (
                sf.start_date > current_date
                OR
                sf.end_date > current_date
                )
        AND sf.cancel_time IS NULL      
LEFT JOIN
