...
count_of_transactions AS
    (   
        ...
        FROM
                (
                SELECT DISTINCT
                        TO_CHAR(longtodatec(art.trans_time, art.center), 'YYYY-MM') AS bookdate
                        ,p.external_id as personid
                FROM
                        evolutionwellness.persons p
                JOIN
                        evolutionwellness.account_receivables ar
                ...
                WHERE
                        ...
                        art.amount < 0  
                        AND
                        art.text NOT IN ('Payment Reminder','FF Reject Fee','late payment','Late payment','Late Payment','late payment fee','Late payment fee','Late Payment Fee','late payment fee ')
                        AND 
                        art.text NOT LIKE '%Late Payment%'             
                )t
        GROUP BY
                t.personid 
    )                       
...
SELECT
    ...
    cot.count_of_invoices     AS "Number of Unpaid Invoices (Arrears)",
    CASE s.STATE
        WHEN 2
        THEN 'ACTIVE'
        WHEN 3
...
LEFT JOIN
    count_of_transactions cot
    ON cot.personid = p.external_id
WHERE
    ar.balance < 0
AND cc.closed IS FALSE
AND p.sex != 'C'
