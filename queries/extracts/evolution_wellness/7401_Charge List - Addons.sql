...
LEFT JOIN
    evolutionwellness.spp_invoicelines_link sppil
ON
    sppil.invoiceline_center = invl.center
AND sppil.invoiceline_id = invl.id
AND sppil.invoiceline_subid = invl.subid
LEFT JOIN
    evolutionwellness.subscriptions s
ON
    s.center = sppil.period_center
AND s.id = sppil.period_id
LEFT JOIN
    evolutionwellness.products spr
ON
