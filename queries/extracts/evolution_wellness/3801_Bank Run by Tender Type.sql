WITH
    ...
    , art_with_prs_overdue AS
    ( -- add 'overdue amount' from prs as an additional transaction line so it shows up in the 
    -- output
    SELECT
        art.center
        ,art.id
    ...
    SELECT
        ...
        ,prs.id
        ,0           AS subid
        , prs.center AS payreq_spec_center
        ,prs.id      AS payreq_spec_id
        ,prs.subid   AS payreq_spec_subid
        ,prs.included_overdue_amount*-1 as amount
        ,'OVERDUE_AMOUNT' AS ref_type
        ,NULL             AS ref_center
        ,NULL             AS ref_id
        ,NULL             AS ref_subid
        ,'Overdue Amount' AS text
    FROM
        evolutionwellness.payment_request_specifications prs
    WHERE
        prs.included_overdue_amount IS NOT NULL
    )
    , res AS
    ( SELECT
    ...
    LEFT JOIN
        evolutionwellness.invoicelines_vat_at_link vl
    ON
    ...
    LEFT JOIN
        evolutionwellness.product_account_configurations prac
    ON
    ...
    LEFT JOIN
        evolutionwellness.products prod_cl
    ON
        prod_cl.center = cl.productcenter
    AND prod_cl.id = cl.productid
    LEFT JOIN
        evolutionwellness.product_account_configurations prac_cl
    ON
    ...
    LEFT JOIN
        evolutionwellness.credit_note_line_vat_at_link vl_cl
    ON
