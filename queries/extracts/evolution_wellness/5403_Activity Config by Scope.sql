WITH 
        ...
        (
                ...
                *,
                ...
                WHERE
                  rnk = 1
                  --AND "CENTER_ID" IN (315)  
                )  
                                      
SELECT DISTINCT
       areas.name AS "Scope",
       --CASE WHEN areas.id = 4 THEN 'Celebrity Fitness'              --ID
       --WHEN areas.id in (15,22,23) THEN 'Fitness First'             --ID
	   --CASE WHEN areas.id IN (7,30) THEN 'Celebrity Fitness'        --MY
       --WHEN areas.id in (17,32,33) THEN 'Fitness First'             --MY
	   --CASE WHEN areas.id IN (6,26) THEN 'Celebrity Fitness'        --TH
	   --WHEN areas.id in (21,29,27,28,37) THEN 'Fitness First'       --TH
       --CASE WHEN areas.id in (18,24,25) THEN 'Fitness First'        --PH
       CASE WHEN areas.id in (2,8,9) THEN 'Fitness First'           --SG
        ELSE 'Unknown'  
        END AS "Scope Level",
        c.id AS "Center ID",
        ...
        ,CASE
                WHEN override.description IS NOT NULL THEN override.description
               ELSE a.description
        END AS "Public Description"    
                ,CASE
                WHEN override.max_waiting_list_participants IS NOT NULL THEN override.max_waiting_list_participants
               ELSE a.max_waiting_list_participants
        END AS "Maximum waiting list participants"
...
on 
    areas.id = ac2.area        
    AND areas.root_area = 1
    AND areas.id in (4,15,22,23)                 --ID
    --AND areas.id in (7,30,17,32,33)              --MY
    --AND	areas.id in (6,26,21,29,27,28,37)      --TH
    --AND	areas.id in (18,24,25)                 --PH
    --AND	areas.id in (2,8,9)                    --SG
LEFT JOIN
        override
...
LEFT JOIN
        ...
        ON pc.activity_id = a.id
        AND pc.name = 'Class'
LEFT JOIN
        evolutionwellness.booking_privilege_groups bp
        ON bp.id = pc.access_group_id                   
WHERE 
        a.state = 'ACTIVE'