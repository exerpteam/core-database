...
WITH
    ...
    ( SELECT
        ...
        , TO_CHAR(MAX(longtodatec(i.entry_time,999)) over
                                                           (
                                                       PARTITION BY
                                                           i.paysessionid
                                                           , p.center
                                                           , p.id),'yyyy-MM-dd hh24:mi') AS
        "Head Office Time"
        ...
        , (il.product_normal_price*il.quantity) - il.total_amount AS "Total Discount Amount"
        ...
        , i.payer_center
        , i.payer_id
        , COALESCE(st.name,prg.name) AS campaign_name
    FROM
    ...
    JOIN
        ...
        WHERE
            crt.crttype NOT IN (4,10,11,15,16,20 ) ) crt_center
    ON
    ...
    AND prg.id = pu.SOURCE_ID
    LEFT JOIN
        STARTUP_CAMPAIGN st
    ON
        pg.GRANTER_SERVICE IN ('StartupCampaign'
                               , 'RetentionCampaign')
    AND pg.GRANTER_ID = pu.SOURCE_ID
    WHERE
        (
    ...
    , shifts AS
    ( SELECT
        DISTINCT crl.cash_register_center
        ...
        , CASE
            ...
            THEN longtodatec(lag(log_time) over
                                                 (
                                             PARTITION BY
                                                 crl.cash_register_center
                                                 , crl.cash_register_id
                                             ORDER BY
                                                 log_time DESC),crl.cash_register_center)
        END AS register_close
        , CASE
            WHEN log_type = 'OPEN_CASH_REGISTER'
            THEN longtodatec(log_time,crl.cash_register_center)
            WHEN log_type = 'CLOSE_CASH_REGISTER'
            THEN longtodatec(lead(log_time) over
                                                  (
                                              PARTITION BY
                                                  crl.cash_register_center
                                                  , crl.cash_register_id
    ...
    ( SELECT
        ...
        "Head Office Time"
        , TO_CHAR(longtodatec(crt.transtime,crt.center),'yyyy-MM-dd hh24:mi')::TEXT AS "Club Time"
        , COALESCE(cpm.name, CASE crt.CRTTYPE
            WHEN 1
            THEN 'CASH'
            WHEN 2
        ...
        , crt.transtime            AS entry_time
        , COALESCE(crt.amount,0)   AS allocated_amount
        ,
        /*CASE
    ...
    AND
        (
            crt.coment IN('Manual registered payment of request: Payment open request'
                          ,
                          'Pembayaran permintaan terdaftar secara manual: Permintaan pembayaran terbuka'
                          , 'การชำระเงินคำขอที่ลงทะเบียนด้วยตนเอง: คำขอเปิดชำระเงิน') )
    )
    , manual_pr_payment_lines AS
    ( SELECT
        DISTINCT paysessionid
        , crt_center
        , crt_id
    ...
    ( SELECT
        ...
        , "Operator"
        , "Charge Date"
        , MAX("Shift Start Time") over (
                                    PARTITION BY
                                        line.paysessionid) AS "Shift Start Time"
        , MAX("Shift End Time") over (
                                  PARTITION BY
                                      line.paysessionid) AS "Shift End Time"
        , "Head Office Time"
        , "Club Time"
        , MAX("Workstation") over (
                               PARTITION BY
                                   line.paysessionid) AS "Workstation"
        , "Item"
        , "Item Description"
        , "Ledger Group"
        , "Ledger Group Code"
    ...
    FROM
        ( SELECT
            ...
            , SUM("Total Discount Amount") AS "Total Discount Amount"
            ,string_agg(campaign_name,',') AS "Campaign Name"
        ...
        SELECT
            ...
            , NULL::NUMERIC AS "Total Discount Amount"
        ...
        SELECT
            ...
            , "Club Time"
            , line_ref                               AS "Item"
            , 'Manual registered payment of request' AS "Item Description"
            , NULL::TEXT                             AS "Ledger Group"
            , NULL::TEXT                             AS "Ledger Group Code"
            , NULL::INTEGER                          AS "Quantity"
            , SUM(allocated_amount)                  AS "Total Sale Amount"
    ...
    FROM
        ( SELECT
            "Club Number"
            , "Tender Type"
            , "Shift Start Time"::DATE AS "Shift Start Time"
    ...
    FROM
        ( SELECT
            "Sale Line ID"
            , SUM(("Sale Line ID" IS NOT NULL)::INTEGER) over
                                                               (
                                                           PARTITION BY
                                                               "Sale Line ID") AS sl_dup
        FROM
            res
        WHERE
