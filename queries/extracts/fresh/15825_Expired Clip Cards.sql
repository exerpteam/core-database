 SELECT
     ...
     valid_until,
     name,
     --CASE (INSTALLMENT_PLAN_ID) WHEN MAX IS NULL THEN 'No' ELSE 'Yes' END AS "Installment Plan",
     CASE 
     WHEN MAX(INSTALLMENT_PLAN_ID) IS NULL 
     THEN 'No' 
     ELSE 'Yes'
 ...
 FROM
     (
         SELECT DISTINCT
             ...
             c.clips_left,
             c.clips_initial,
             (invl.TOTAL_AMOUNT / invl.QUANTITY)::varchar AS    price_per_clip_card,
             (invls.TOTAL_AMOUNT / invls.QUANTITY)::varchar  AS price_per_clip_card_spons,
