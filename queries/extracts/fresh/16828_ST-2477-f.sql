...
SELECT 
  ...
  TO_CHAR(longtodateC(ar.LAST_ENTRY_TIME,c.ID),'YYYY-MM-DD HH24:MI') AS LAST_ENTRY_TIME,
...
WHERE 
  ...
  AND 
  ((( (ar.BALANCE > -10 AND ar.BALANCE < 0) OR (ar.BALANCE < 10 AND ar.BALANCE > 0)) AND (c.COUNTRY <> 'FI'))
  OR
  (((ar.BALANCE > -1 AND ar.BALANCE < 0) OR (ar.BALANCE < 1 AND ar.BALANCE > 0)) AND (c.COUNTRY = 'FI'))
 ) 
) sq
WHERE 
 "Other Payer" is null