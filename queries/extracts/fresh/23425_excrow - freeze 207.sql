...
SELECT
    subscription_center||'ss'||subscription_id                                            AS MembershipId,
    TO_CHAR(START_DATE, 'YYYY-MM-DD')                                                 AS FreezeFrom,
    TO_CHAR(END_DATE, 'YYYY-MM-DD')                                                     AS FreezeTo,
    text                                                                            AS FreezeReason
FROM
    SUBSCRIPTION_FREEZE_PERIOD fpr
    join elig_subs es on fpr.subscription_center = es.center and fpr.subscription_id = es.id
WHERE
