 WITH
     ...
     (
         SELECT
             ...
             $$ToDate$$                                                              AS EndDate,
             datetolong(TO_CHAR(cast($$FromDate$$ as date), 'YYYY-MM-dd HH24:MI'))                   AS StartDateLong,
             (datetolong(TO_CHAR(cast($$ToDate$$ as date), 'YYYY-MM-dd HH24:MI'))+ 86400 * 1000)-1 AS EndDateLong
     )
     ,
     v_camp AS
     (
         SELECT
             pg.id,
             coalesce(prg.NAME,sc.NAME) AS name,
             CASE
 ...
 FROM
     (
         ...
         WHERE
             pu.USE_TIME BETWEEN params.StartDateLong AND params.EndDateLong ) t1
 GROUP BY
     name,
     type,
     code,
