...
https://clublead.atlassian.net/servicedesk/customer/portal/9/EC-5314
...
(
        SELECT
              ...
              GETSTARTOFDAY( CAST (to_date(:start_date, 'YYYY-MM-DD') AS TEXT), C.ID) AS start_date_selected,
                GETENDOFDAY(CAST(to_date(:end_date,'YYYY-MM-DD') AS TEXT), C.ID) AS end_date_selected,
                c.id AS center_id
        FROM centers c
        WHERE
                c.id IN ($$scope$$)
...
clip_usage_selected_mat as
(
  SELECT
                par.start_date_selected,
                par.end_date_selected,
                par.start_of_date,
                par.end_of_date,
                cc.center,
                cc.id,
                ...
                cc.clips_left,
                cc.valid_until,
                ABS(SUM(ccu.clips)) AS used_clips_timeframe
        FROM chelseapiers.clipcards cc
        JOIN params par
                ON par.center_id = cc.center
        INNER JOIN chelseapiers.invoices inv
        ...
        WHERE
                ...
                AND inv.trans_time <= par.end_of_date
                AND b.starttime >= start_date_selected 
                AND b.starttime <= par.end_date_selected 
        GROUP BY
                cc.center,
                cc.id,
                cc.subid,
...
SELECT
    ...
    ,TO_CHAR(longtodatec(last_clip_used.time, last_clip_used.center),'MM/DD/YYYY HH24:MI') AS "Last Used Time",
     CASE WHEN ccs.used_clips_timeframe IS NOT NULL THEN ccs.used_clips_timeframe ELSE 0 END AS "Realized During Selected Period",
     CAST(ROUND((COALESCE(ccs.used_clips_timeframe, 0) * (((il.total_amount + COALESCE(il2.total_amount, 0)) / (COALESCE
    (ilvatl.rate, 0) + 1)) / cc.clips_initial)),2) as numeric)     AS "Amount Used During Selected Period"

FROM clip_usage_mat cc
JOIN clip_usage_selected_mat ccs
        ON cc.center = ccs.center
        AND cc.id = ccs.id
        AND cc.subid = ccs.subid
