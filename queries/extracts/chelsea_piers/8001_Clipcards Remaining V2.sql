...
(
        ...
        WHERE
                ...
                ( 
                        ...
                        OR 
                        cc.valid_until >= par.start_of_date
                )
                ...
                (
                        SELECT
                                1
                        FROM chelseapiers.clipcards c2
                        WHERE
                                c2.transfer_from_clipcard_center = cc.center
                                AND c2.transfer_from_clipcard_id = cc.id
                                AND c2.transfer_from_clipcard_subid = cc.subid
                )
),
eligible_clipcards_rec AS
(
        SELECT
                cl_rec.*
        FROM
        (
                WITH RECURSIVE clipcards_rec AS
                (
                        SELECT
                                cc.center,
                                cc.id,
                                ...
                                cc.transfer_from_clipcard_id,
                                cc.transfer_from_clipcard_subid,
                                cc.center as orig_cc_center,
                                cc.id as orig_cc_id,
                                cc.subid as orig_cc_subid,
                                cc.invoiceline_center,
                                cc.invoiceline_id,
                                cc.invoiceline_subid,
                                cc.owner_center,
...
/*                        WHERE
                        ...
                        SELECT
                                ...
                                cc1.id,
                                cc1.subid,
                                cc1.transfer_from_clipcard_center,
                                cc1.transfer_from_clipcard_id,
                                cc1.transfer_from_clipcard_subid,
                                cc2.orig_cc_center,
                                cc2.orig_cc_id,
                                cc2.orig_cc_subid,
                                cc2.invoiceline_center,
                                cc2.invoiceline_id,
                                cc2.invoiceline_subid,
                                cc2.owner_center,
                                cc2.owner_id,
                                cc2.clips_initial,
                                cc2.clips_left,
                                cc2.valid_until
                        FROM chelseapiers.clipcards cc1
                        JOIN clipcards_rec cc2 
                                ON cc2.transfer_from_clipcard_center = cc1.center
                                AND cc2.transfer_from_clipcard_id = cc1.id 
                                AND cc2.transfer_from_clipcard_subid = cc1.subid
                )
                SELECT * FROM clipcards_rec
        ) cl_rec
),
clipcard_usage AS
(
        SELECT 
                ecl.orig_cc_center AS center,
                ecl.orig_cc_id AS id,
                ecl.orig_cc_subid AS subid,
                ecl.invoiceline_center,
                ecl.invoiceline_id,
                ecl.invoiceline_subid,
                ecl.owner_center,
                ecl.owner_id,
                ecl.clips_initial,
                ecl.clips_left,
        ...
        FROM eligible_clipcards_rec ecl
        JOIN params par
                ON par.center_id = ecl.center
        LEFT JOIN chelseapiers.card_clip_usages ccu
                ON ccu.card_center = ecl.center
                AND ccu.card_id = ecl.id
                AND ccu.card_subid = ecl.subid
                AND ccu.state IN ('ACTIVE','PLANNED')  -- PLANNED SHOULD NEVER BE HERE, BUT SINCE WE HAVE A BUG IN EXERP WE HAVE NO CHOICE
                AND ccu.time <= par.end_of_date
                AND ccu.type NOT IN ('TRANSFER_TO','TRANSFER_FROM')
...
SELECT
        ...
        CAST(ROUND(((il.total_amount + COALESCE(il2.total_amount, 0)) / (COALESCE(ilvatl.rate, 0) + 1)),2) as numeric) AS "Amount Excluding VAT",
        ...
        CAST(ROUND((COALESCE(used_clips, 0) * (((il.total_amount + COALESCE(il2.total_amount, 0)) / (COALESCE(ilvatl.rate, 0) + 1)) / cc.clips_initial)),2) as numeric)     AS "Used Amount",
        CAST(ROUND(LEAST(((cc.CLIPS_INITIAL - COALESCE(used_clips, 0)) * (((il.total_amount + COALESCE(il2.total_amount, 0)) / (COALESCE(ilvatl.rate, 0) + 1)) / cc.clips_initial)),
