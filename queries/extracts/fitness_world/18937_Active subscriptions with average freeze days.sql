...
select 
   center,
   shortname,
   TotalFaktureringsdage,
   TotalFryseDage,
   TotalFaktureringsdage - TotalFryseDage as NettoDage,
   NumberMemberships as AntalMedlemskaber,
   NumberMembers as AntalMedlemmer,
   round((TotalFaktureringsdage - TotalFryseDage) / NumberMemberships) as FaktureringsdagePerMedlemskab,
   round((TotalFaktureringsdage - TotalFryseDage) / NumberMembers) as FaktureringsdagePerMedlem,
   round(((TotalFaktureringsdage - TotalFryseDage) / NumberMemberships) / 30, 2) as AntalMaaneder
   from (
SELECT
        SU.CENTER,
        cen.SHORTNAME
        ,SUM(round(EXTRACT(DAY FROM (longtodate(:at_date + 86400000) - su.START_DATE))) - 1) as TotalFaktureringsdage
        ,SUM(round(freeze_length.Duration)) as TotalFryseDage
        ,count(distinct su.owner_id) as NumberMembers
        ,count(distinct su.id) as NumberMemberships
