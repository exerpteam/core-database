...
WITH
    ...
    eligible_products AS MATERIALIZED (
        SELECT
            ...
            cp.center || 'p' || cp.id AS payer_id,
            cp.external_id,
            mpr.cached_external_id AS master_cached_external_id,   
            cp.sex,
            FLOOR(months_between(TRUNC(CURRENT_TIMESTAMP), cp.birthdate) / 12) AS age,
            cp.birthdate,
            cp.firstname || ' ' || cp.lastname AS payer_name,
            CASE cp.persontype
                WHEN 0 THEN 'PRIVATE'
                WHEN 1 THEN 'STUDENT'
                WHEN 2 THEN 'STAFF'
            ...
            CASE 
            WHEN cr.NAME IS NULL OR cr.NAME = '' THEN 'N/A'
                ELSE cr.NAME
                END AS CashRegister,
            CASE
            ...
            CASE
                WHEN ps.name IS NULL OR ps.name = '' THEN 'N/A'
                ELSE ps.name
            END AS priv_set_name,
            CASE
        ...
        LEFT JOIN persons cp
           ON p.transfers_current_prs_center = cp.center
           AND p.transfers_current_prs_id = cp.id          
        JOIN centers c ON c.id = invl.center
        ...
        LEFT JOIN masterproductregister mpr
            ON mpr.globalid = prod.globalid   -- <-- CORRECTED JOIN
        ...
        LEFT JOIN privilege_sets ps ON ps.id = pg.privilege_set
        LEFT JOIN products cc
        ...
        LEFT JOIN masterproductregister ampr
        ...
        WHERE
            ...
            AND prod.ptype = 1
        GROUP BY
            c.id, cea.txt_value, cp.sex,
            months_between(TRUNC(CURRENT_TIMESTAMP), cp.birthdate) / 12,
            cp.birthdate, prod.name, prod.globalid, prod.price, prod.cost_price,
            inv.employee_center, inv.employee_id, staffp.fullname,
            cp.persontype, productGroup.name, cp.center, cp.id, cp.external_id,
            payer_id, cp.firstname, cp.lastname, prod.ptype,
...
SELECT
    ...
    CASE
       WHEN t2.priv_source_name IS NULL OR t2.priv_source_name = '' THEN 'N/A'
       ELSE t2.priv_source_name
    END AS "Privilege Source Name"
FROM (
    SELECT *
    FROM (
        SELECT
