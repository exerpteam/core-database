...
SELECT
    centerid,
    centername,
    stcam.ENDTIME AS campaign_endtime, -- Added column for stcam.ENDTIME
    ...
    REPLACE(REPLACE(REPLACE(TO_CHAR(MIN(joiningprice),'999G999G990D00'),'.','-'),',','.'),'-',',') AS CURRENT_LOWEST_JOINING_PRICE,
    REPLACE(REPLACE(REPLACE(TO_CHAR(MIN(joining.price),'999G999G990D00'),'.','-'),',','.'),'-',',') AS NORMAL_JOINING_PRICE
FROM
    (
        SELECT
...
GROUP BY
    centerid,
    centername,
    stcam.ENDTIME, -- Added line to include stcam.ENDTIME in the GROUP BY clause
    core.price
ORDER BY
    centerid;