...
WITH
    ...
    (
        SELECT
            datetolongTZ(TO_CHAR(date_trunc('day', CURRENT_DATE)- INTERVAL '2 days', 'YYYY-MM-DD HH24:MI'), 'Europe/Copenhagen')::bigint AS FROMDATE,
...
SELECT
    ...
    "PERSONSTATUS",
    "BLACKLISTED",
    TO_CHAR(longtodateTZ(j.creation_time, 'Europe/Copenhagen'), 'YYYY-MM-DD HH24:MI:SS') AS
    "BLACKLISTSTARTDATE",
    "MEMBERDAYS",
    ...
    "MEMBERSINCEDATE",
    TO_CHAR(longtodateTZ(pea.APPLOGIN_LAST_EDIT_TIME, 'Europe/Copenhagen'),
    ...
    CASE pea.AEM_TXTVALUE
        ...
        ELSE 0
    END                                                                 AS "EMAILOPTIN",
    TO_CHAR(longtodateTZ(pea.AEM_LAST_EDIT_TIME, 'Europe/Copenhagen'),'YYYY-MM-DD HH24:MI:SS') AS
    "EMAILOPTINDATE",
    ...
    TO_CHAR(longtodatetz(last_person_status.LastTime,'Europe/Copenhagen'),'YYYY-MM-DD') AS
                         "PERSONSTATUSLASTMODIFIEDDATE",
    TO_CHAR(longtodatetz(pea.ANL_LAST_EDIT_TIME,'Europe/Copenhagen'),'YYYY-MM-DD HH24:MI:SS') AS
    "NEWSLETTEROPTINDATE",
    ...
    TO_CHAR(longtodatetz(pea.PUSHNOTIFY_LAST_EDIT_TIME,'Europe/Copenhagen'),
    ...
    CASE pea.SMSMARKET_TXTVALUE
        ...
        ELSE 0
    END AS "SMSMARKETING",
    TO_CHAR(longtodatetz(pea.SMSMARKET_LAST_EDIT_TIME,'Europe/Copenhagen'),
    ...
    TO_CHAR(longtodatetz(pea.SOCIALMEDIA_LAST_EDIT_TIME,'Europe/Copenhagen'),
    ...
    CASE pea.ALLOWSURVEY_TXTVALUE
        ...
        ELSE 0
    END AS "ALLOWSURVEY",
    TO_CHAR (longtodatetz (pea.ALLOWSURVEY_LAST_EDIT_TIME, 'Europe/Copenhagen'),
...
FROM
    (
        SELECT
            ...
            TO_CHAR(longtodatetz(p.LAST_MODIFIED,'Europe/Copenhagen'),'YYYY-MM-DD HH24:MI:SS') AS
            "EXERPLASTMODIFIEDDATE"
        FROM
            persons p
        CROSS JOIN
...
(
    ...
    AND		per.CENTER = sub.OWNER_CENTER
            INNER JOIN SUBSCRIPTIONS b_sub
    ON		b_sub.OWNER_CENTER = rel.RELATIVECENTER
    AND		b_sub.OWNER_ID = rel.RELATIVEID
    AND		b_sub.STATE = 2
            INNER JOIN PRODUCTS b_prod
    ON		b_prod.CENTER = b_sub.SUBSCRIPTIONTYPE_CENTER
    AND		b_prod.ID = b_sub.SUBSCRIPTIONTYPE_ID
            INNER JOIN PERSONS b_per
    ON		b_per.ID = b_sub.OWNER_ID
    AND		b_per.CENTER = b_sub.OWNER_CENTER
    WHERE	rel.center IN (:scope)
    AND		rel.RTYPE = 4
    AND		rel.STATUS = 1
    AND		prod.GLOBALID = 'PLUS_FRIEND'
    AND		sub.STATE = 2
    AND		b_prod.GLOBALID != 'PLUS_FRIEND'
    AND		b_prod.GLOBALID like 'PLUS%'
	AND 	b_prod.BLOCKED = 0
) member_buddy
ON  selection.EXTERNAL_ID = member_buddy.LEAD_EXTERNAL_ID
LEFT JOIN
    JOURNALENTRIES j 
ON
