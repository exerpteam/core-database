...
WITH
        ...
        FROM
            (   SELECT
                ...
                AND pu.target_id = s.invoiceline_id
                    --AND pu.target_subid = s.invoiceline_subid
    ...
    (   SELECT
            ...
            CASE
                WHEN pp.ref_type = 'PRODUCT_GROUP'
                THEN t2.globalid
                ELSE pp.ref_globalid
            END AS prod_globalid,
            ps.name
        FROM
        ...
        LEFT JOIN
            (   SELECT
                ...
                FROM
                    fw.product_and_product_group_link pgl
                JOIN
                    products prod
                ON
                    prod.center = pgl.product_center
                AND prod.id = pgl.product_id ) t2
        ON
            t2.product_group_id = pp.ref_id
...
FROM
    (
        ...
        AND
            (
                prp.prod_globalid = su.globalid
            OR  prp.prod_globalid = 'PRORATA_'||su.globalid)
            /*JOIN
            privilege_sets ps
            ON
        ...
        ON
            prp.id = su.privilege_id
        AND prp.prod_globalid = 'CREATION_'||su.globalid
        JOIN
            privilege_sets ps
        ON
            ps.id = pg.privilege_set
        JOIN
            products jo
        ON
            jo.globalid = 'CREATION_'||su.globalid
        AND jo.center = su.center
        WHERE
            pg.granter_service = 'ReceiverGroup' ) sub
GROUP BY
    "EXTERNAL_ID",
    "MEMBERID",
    "SUBSCRIPTIONID",
