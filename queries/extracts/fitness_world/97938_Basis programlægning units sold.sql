...
SELECT
    ...
    FROM
        INVOICELINES invl
        JOIN INVOICES inv ON inv.CENTER = invl.CENTER AND inv.id = invl.id
        ...
        JOIN EMPLOYEES emp ON emp.CENTER = inv.EMPLOYEE_CENTER AND emp.ID = inv.EMPLOYEE_ID
        JOIN PERSONS pemp ON pemp.CENTER = emp.PERSONCENTER AND pemp.ID = emp.PERSONID
        LEFT JOIN PERSONS p ON p.CENTER = inv.PAYER_CENTER AND p.ID = inv.PAYER_ID
        LEFT JOIN AR_TRANS art ON art.REF_TYPE = 'INVOICE' AND art.REF_CENTER = inv.CENTER AND art.REF_ID = inv.ID
    WHERE
        inv.CENTER IN (:scope)
        AND inv.TRANS_TIME BETWEEN :startDate AND :endDate
        AND prod.GLOBALID = 'BASIS_PROGRAM_PT'
) subquery
GROUP BY subquery.CENTER