...
SELECT 
    case
        ...
        when levels.section_id is not null and ch_hf.id = 101 then 'A-' || levels.section_id || '-A'
        when levels.section_id is not null and ch_hf.id = 102 then 'A-' || levels.section_id || '-Z'
        else 'A-' || CR_PBSNR || lpad(CR_DEBGRPNO,5,'0') || '-B-' || LINE_IDX 
    end    as Rank,
    ...
    case
        ...
        when levels.clearinghouse is not null and ch_hf.id = 102 then 'BS992' || levels.CH_DATASUPPLIER_ID || 'BS1' || '0601' 
            || LPAD(levels.cnt_sections, 11, '0')  -- count sections
            ...
            || LPAD('0', 34, '0') 
        when levels.section_id is not null and ch_hf.id = 101 then 'BS012' || lpad(levels.CR_PBSNR, 8, '0') || '0117' || '     ' || lpad(levels.CR_DEBGRPNO, 5, '0') || RPAD(substr(levels.CH_NAME, 1, 15), 15, ' ') || '    ' || to_char(exerpsysdate(), 'DDMMYYYY') || LPAD(' ', 4, ' ') || LPAD(' ', 10, ' ')
        when levels.section_id is not null and ch_hf.id = 102 then 'BS092' || lpad(levels.CR_PBSNR, 8, '0') || '0117' || '00000' || lpad(levels.CR_DEBGRPNO, 5, '0') || '    ' 
            || LPAD(levels.cnt_lines / 9, 11, '0')  -- count 042
...
FROM
    (
        SELECT
            ...
            count(distinct flat.SECTION_ID) as cnt_sections,
            max(flat.CR_PBSNR) as CR_PBSNR,
        ...
        FROM
            (
                SELECT
                    ...
                    pa.CLEARINGHOUSE,
                    cr.FIELD_6 || lpad(cr.FIELD_5, 5, '0') as SECTION_ID,
...
--                AND pa.state = 4
            ...
            flat
        GROUP BY
            GROUPING SETS(flat.clearinghouse, SECTION_ID, Line)
    )
    levels
