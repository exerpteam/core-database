...
(
        SELECT
                ...
                datetolongC(TO_CHAR(TO_DATE(getCenterTime(c.id),'YYYY-MM-DD'), 'YYYY-MM-DD'), c.id) AS todayDateLong,
                TO_DATE(getCenterTime(c.id),'YYYY-MM-DD') AS todayDate,
                c.id
        FROM centers c
        WHERE c.id IN (:Scope)
),
...
LEFT JOIN fw.subscription_change sc
        ...
        AND (sc.cancel_time IS NULL OR sc.cancel_time > par.todayDateLong)
