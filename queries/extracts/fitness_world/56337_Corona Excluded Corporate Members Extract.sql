...
-- https://clublead.atlassian.net/browse/ST-11028
WITH
    sub AS
    (
        SELECT
        ...
        WHERE
            ...
            AND s.state IN (2,
                            4,
                            8)
            /* Exclude subscription ended before club closed date */
            AND (
                s.end_date IS NULL
                OR s.end_date >= to_date('12-03-2020', 'dd-MM-yyyy'))
    )
    ,
    v_pivot AS
    (
