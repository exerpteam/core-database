...
SELECT  '721' As SurveyID,
        ...
        s.END_DATE AS Membership_End_Date,
        p.Sex, 
...
(
  ...
  WHERE t.MYRANK = 1) c  -- last check-in 
JOIN
  PERSONS p  
ON
...
JOIN 
	PERSON_EXT_ATTRS survey
ON
	survey.PERSONCENTER = p.CENTER
	AND survey.PERSONID = p.ID
	AND survey.NAME = 'AllowSurvey'
	AND survey.TXTVALUE = 'true'

WHERE 
  p.center in (:scope)
  AND p.status IN (1) -- Person_status = Active
  AND p.FIRST_ACTIVE_START_DATE + INTERVAL '90' DAY <  longtodate(c.CHECKIN_TIME)  -- Customers must have trained for 3 months or more
  AND TO_CHAR(longtodate(c.CHECKIN_TIME), 'YYYY-MM-DD') =  TO_CHAR(current_timestamp - INTERVAL '1' DAY , 'YYYY-MM-DD')  -- customers who have trained yesterday. 
  AND ss.TERMINATION_DATE is null
