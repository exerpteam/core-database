...
,
    ...
    (
        SELECT
            ...
            CASE
                WHEN (inv.EMPLOYEE_CENTER,inv.EMPLOYEE_ID) = (100,5313)
                THEN substring(invl.text from '\(#(\d+)')
                ELSE NULL
            END AS slide_number,
            CASE prod.PTYPE
                WHEN 1
  ...
  prod.center as prod_center
        ...
        LEFT JOIN
            fw.products spr
        ON
            spr.center = s.subscriptiontype_center
        ...
        LEFT JOIN
            fw.masterproductregister ampr
        ON
            ampr.id = sa.addon_product_id
        LEFT JOIN
            fw.invoice_lines_mt il
        ON
            il.center = pu.SOURCE_CENTER
...
FROM
    (
        ...
        FROM
            (
                SELECT
                    ep.*,
                    rank() over (partition BY ep.payer_id, ep.product, ep."Invoice Date",
                    ep."Invoice Time" ORDER BY st."LEVEL" , CAST(ph.start_date ::text AS DATE) DESC) AS rnk,
                    ph.normal_price                                                     AS
                                     ph_normal_price,
   ...
   JOIN
                            scope_tree st
                        ON
                            ep.prod_center = st.center_id
                        AND ph.mpr_scope_type=st.scope_type
                        AND ph.mpr_scope_id=st.scope_id )t1
        WHERE
            t1.rnk = 1) t2
ORDER BY
    t2.product,
