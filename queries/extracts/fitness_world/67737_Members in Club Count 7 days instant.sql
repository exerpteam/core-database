...
WITH
    ...
    (
        SELECT
            /*+ materialize */
            dateToLongTZ(TO_CHAR(TRUNC(SYSDATE-7),'YYYY-MM-DD') || ' 00:00','CET') from_date ,
            dateToLongTZ(TO_CHAR(TRUNC(SYSDATE+1),'YYYY-MM-DD') || ' 00:00','CET')   to_date
        FROM
            dual
    )
    ,
    ...
    (
        SELECT
            /*+ materialize */
            dateToLongTZ(TO_CHAR(TRUNC(SYSDATE-7),'YYYY-MM-DD') || ' 00:00','CET') + ((level ) * 1000 * 60 * 60)                                  lower_bound ,
            dateToLongTZ(TO_CHAR(TRUNC(SYSDATE-7),'YYYY-MM-DD') || ' 00:00','CET') + ((level +1) * 1000 * 60 * 60) - 1                            higher_bound ,
            longToDateTZ(dateToLongTZ(TO_CHAR(TRUNC(SYSDATE-7),'YYYY-MM-DD') || ' 00:00','CET') + ((level) * 1000 * 60 * 60),'CET')     lower_date_time ,
            longToDateTZ(dateToLongTZ(TO_CHAR(TRUNC(SYSDATE-7),'YYYY-MM-DD') || ' 00:00','CET') + ((level + 1) * 1000 * 60 * 60),'CET') higher_date_time
        FROM
            dual CONNECT BY level <= (24 * 8)
    )
SELECT
