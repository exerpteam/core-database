...
prod.NAME,
to_char(longtodate(cl.CHECKIN_TIME),'DD-MM-YYYY HH24:MI') Checkin_time,
cl.checkin_center
FROM
...
WHERE
prod.GLOBALID IN ('NON_LOCAL_CHECKIN_FEE', 'CLUB_CHECKIN_FEE', 'NO_SHOW_FEE', 'VP_CLASS_BOOKING_FEE', 'VP_CLUB_CHECKIN_FEE')
AND inv.TRANS_TIME between (:StartDate) and (:EndDate)
AND inv.PAYER_CENTER in (:Person_homecenter)
AND to_char(longtodate(inv.TRANS_TIME),'DD-MM-YYYY HH24:MI') = to_char(longtodate(cl.CHECKIN_TIME),'DD-MM-YYYY HH24:MI')