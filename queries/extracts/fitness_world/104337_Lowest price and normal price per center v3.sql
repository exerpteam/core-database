...
WITH
	...
	AND mpr.globalid NOT LIKE 'CASH%'
	AND mpr.globalid NOT LIKE 'PREPAID%'
	AND mpr.globalid NOT LIKE 'PREAPAID%'
    ...
    (
        ...
        WHERE
            p.ptype = 5
        AND p.globalid NOT LIKE '%HOTEL_PAYPASS%'
        AND p.requiredrole IS NULL
        GROUP BY
            p.center
    )
SELECT
	...
	t2.centername,
	CASE
	WHEN t2.current_lowest_price IS NULL
	THEN t2.normal_price
	ELSE t2.current_lowest_price
	END AS current_lowest_price,
	t2.normal_price,
	t2.current_lowest_joining_price,
    joining.price AS normal_joining_price
FROM
    (
        SELECT
            core.center_id AS centerid,
            core.center_name AS centername,
            REPLACE(REPLACE(REPLACE(TO_CHAR(MIN(productprice),'999G999G990D00'),'.','-'),',','.'),
            '-',',') AS CURRENT_LOWEST_PRICE ,
            REPLACE(REPLACE(REPLACE(TO_CHAR(core.price,'999G999G990D00'),'.','-'),',','.'),'-',',')
            AS NORMAL_PRICE,
            REPLACE(REPLACE(REPLACE(TO_CHAR(MIN(joiningprice),'999G999G990D00'),'.','-'),',','.'),
            '-',',') AS CURRENT_LOWEST_JOINING_PRICE
        FROM
            (
                SELECT
                ...
                AND (
                        pp.valid_to > CAST(dateToLongC(getCenterTime(100), 100) AS bigint)
