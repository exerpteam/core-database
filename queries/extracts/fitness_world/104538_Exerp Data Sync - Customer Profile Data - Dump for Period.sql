...
pea.HOMEPHONE_TXTVALUE                                                                          AS "PHONE",
    ...
    "PERSONSTATUS",
    "BLACKLISTED",
    TO_CHAR(longtodateTZ(j.creation_time, 'Europe/Copenhagen'), 'YYYY-MM-DD HH24:MI:SS')        AS "BLACKLISTSTARTDATE",
    "MEMBERDAYS",
    "ACCUMULATEDMEMBERDAYS",
    "LASTACTIVESTARTDATE" ,
    "LASTACTIVEENDDATE",
    COALESCE(TO_CHAR(CAST("LASTACTIVESTARTDATE" AS DATE),'YYYY-MM-DD'), pea.CREATION_TXTVALUE)  AS "MEMBERSINCEDATE",
    ...
    CASE pea.AEM_TXTVALUE
        ...
        ELSE 0
    END                                                                                         AS "EMAILOPTIN",
    TO_CHAR(longtodateTZ(pea.AEM_LAST_EDIT_TIME, 'Europe/Copenhagen'),'YYYY-MM-DD HH24:MI:SS')  AS "EMAILOPTINDATE",
...
END                                                                                             AS "NEWSLETTEROPTIN",
    ...
    TO_CHAR(longtodatetz(last_person_status.LastTime,'Europe/Copenhagen'),'YYYY-MM-DD')         AS "PERSONSTATUSLASTMODIFIEDDATE",
    TO_CHAR(longtodatetz(pea.ANL_LAST_EDIT_TIME,'Europe/Copenhagen'),'YYYY-MM-DD HH24:MI:SS')   AS "NEWSLETTEROPTINDATE",
    CASE pea.PUSHNOTIFY_TXTVALUE
        WHEN 'true'
        THEN 1
        ELSE 0
END                                                                                             AS "PUSHNOTIFICATIONOPTIN",
    TO_CHAR(longtodatetz(pea.PUSHNOTIFY_LAST_EDIT_TIME,'Europe/Copenhagen'), 'YYYY-MM-DD HH24:MI:SS') AS "PUSHNOTIFICATIONOPTINDATE",
...
FROM
    (
        ...
        AND ( (
                p.last_modified >= $$fromdate$$
                AND p.last_modified < $$todate$$ + (86400 * 1000) )
            OR  (
                    e.start_time >= $$fromdate$$
                AND e.start_time < $$todate$$ + (86400 * 1000) )
            OR  (
                    deduction_day.last_modified >= $$fromdate$$
                AND deduction_day.last_modified < $$todate$$ + (86400 * 1000) ) ) ) selection
LEFT JOIN
    (
        SELECT
            scl.CENTER,
