...
SELECT 
    ...
    TO_CHAR(s.CREATION_TIME, 'DD-MM-YY HH24:MI:SS') AS CREATIONTIME  -- Use MI for minutes, not MM
FROM 
    PERSONS p
JOIN SUBSCRIPTIONS s ON s.OWNER_CENTER = p.CENTER AND s.OWNER_ID = p.ID
JOIN PERSON_EXT_ATTRS pea ON pea.PERSONCENTER = p.CENTER AND p.id = pea.personid AND pea.NAME = '_eClub_Email'
...
WHERE 
    s.CREATION_TIME BETWEEN :Oprettelses_efter AND :Oprettelse_for
    AND cd.CODE IN (:Kode)
ORDER BY 
    s.CREATION_TIME;
