...
s.CREATOR_CENTER ||'emp'|| s.CREATOR_ID
FROM PERSONS p
...
cd.CODE is not null
--AND s.state !=3
--AND cd.CODE !='studiepris'
AND cd.CODE !='træn1kr'
AND cd.CODE !='spar50'
AND cd.CODE !='start'
AND cd.CODE !='winterbag2020'
AND cd.CODE !='ptbca100'
--and cd.code like 'FW%'
--AND cd.code ilike 'stud%'
AND longtodate(s.CREATION_TIME) >= (:Oprettelses_efter)
--AND pr.name like 'Plus%'
--AND s.STATE = 2
