 ...
 CASE  PERSONTYPE  WHEN 0 THEN 'PRIVATE'  WHEN 1 THEN 'STUDENT'  WHEN 2 THEN 'STAFF'  WHEN 3 THEN 'FRIEND'  WHEN 4 THEN 'CORPORATE'  WHEN 5 THEN 'ONEMANCORPORATE'  WHEN 6 THEN 'FAMILY'  WHEN 7 THEN 'SENIOR'  WHEN 8 THEN 'GUEST' ELSE 'UNKNOWN' END AS PERSONTYPE,
         p.sex                                                                                                                   AS Gender,
     e.IDENTITY                                                      AS Pin,
     cen.SHORTNAME                                                        AS HomeClub,
     cen2.SHORTNAME                                                       AS VisitClub,
     br.NAME                                                         AS AccessPoint,
     TO_CHAR(TRUNC(longToDateTZ(att.START_TIME,'Europe/Zurich'),'HH'),'dd-MM-YYYY') AS "Visit date",
     TO_CHAR(longtodatetz(att.START_TIME,'Europe/Zurich'),'HH24:MI:SS') AS "Time"
 FROM
     PERSONS p
 JOIN
     ATTENDS att
 ...
 WHERE
     ...
     AND br.name NOT IN ('Drinks', 'Solarium')
