WITH
    ...
    (
        SELECT
            ...
            'Europe/Zurich') AS BIGINT)                                                          AS from_date,
            CAST(dateToLongTZ(TO_CHAR(CURRENT_DATE,'YYYY-MM-DD HH24:MI'), 'Europe/Zurich') AS BIGINT) AS
            to_date
    )
    ,
    ...
    (
        SELECT
            CAST(dateToLongTZ(TO_CHAR(s.d1 , 'YYYY-MM-DD HH24:MI') , 'Europe/Zurich') AS BIGINT) AS
            lower_bound,
            CAST(dateToLongTZ(TO_CHAR((s.d1 + INTERVAL '1 hour'), 'YYYY-MM-DD HH24:MI'), 'Europe/Zurich') AS
