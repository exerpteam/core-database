...
--  
 SELECT
     ...
     CASE
         WHEN (sub.end_date > date_trunc('day', longtodateC(sc.CHANGE_TIME, sub.center))) AND sc.EMPLOYEE_CENTER = 100 AND sc.EMPLOYEE_ID = 17401 THEN 'ONLINE'
         WHEN sub.end_date > date_trunc('day', longtodateC(sc.CHANGE_TIME, sub.center)) THEN 'MEMBER'
         ELSE 'PUREGYM'
 ...
 AND pr.REQ_DATE >= CURRENT_TIMESTAMP - INTERVAL '7 days'
 AND pr.state IN (2,3,4)
 AND pr.req_date >= date_trunc('day', longtodateC(sc.CHANGE_TIME, sub.center)) - INTERVAL '1 days'
 WHERE
     st.ST_TYPE = 1
 AND sc.CANCEL_TIME IS NULL
 AND sc.OLD_SUBSCRIPTION_CENTER IN ($$scope$$)
 AND sc.TYPE = 'END_DATE'
 AND sc.CHANGE_TIME >= datetolongTZ(TO_CHAR(CURRENT_TIMESTAMP - INTERVAL '5 days', 'YYYY-MM-DD HH24:MI'), 'Europe/Zurich')
 ...
 AND (
         sub.end_date <= date_trunc( 'day', longtodateC(sc.CHANGE_TIME, sub.center))
