WITH
    ...
    (
        SELECT
            ...
            datetolongC(TO_CHAR(to_date(:TODATE,'YYYY-MM-DD')+interval '1 days', 'YYYY-MM-DD HH24:MI'), c.id)-1 AS TODATE,
            c.id
            FROM centers c
            WHERE c.id in (:scope)
    )
    ...
    (
        ...
        WHERE
        su.CREATION_TIME <dateToLong(TO_CHAR(CURRENT_DATE, 'YYYY-MM-dd HH24:MI'))
/*        AND su.center = 172
AND su.id IN(235641 ,
             235822)*/
        AND NOT EXISTS
