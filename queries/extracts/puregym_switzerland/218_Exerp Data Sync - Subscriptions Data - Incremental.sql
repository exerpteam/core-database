        ...
        FROM
            (
                SELECT
                    ...
                    sp.price,
                    sp.from_date,
                    rank() over (partition BY su.center,su.id ORDER BY sp.from_date ASC) AS rnk
                FROM
                    params,
                    subscriptions su
                JOIN
                ...
                WHERE
                    su.LAST_MODIFIED >= PARAMS.FROMDATE
                AND su.LAST_MODIFIED < PARAMS.TODATE
                AND pu.id IS NULL
				and not sp.cancelled) t
        WHERE
            rnk=1
    )
SELECT DISTINCT
    ...
    TO_CHAR(COALESCE(SPP.SUBSCRIPTION_PRICE, COALESCE(SP.PRICE, SU.SUBSCRIPTION_PRICE)),
    'FM999999990.00')                                       AS "CURRENTPRICE",
    TO_CHAR(ROUND(ndp.price::NUMERIC, 2), 'FM999999990.00')       AS "NORMALPRICE",
    ndp.from_date                                                 AS "NORMALPRICEDATE",
    TO_CHAR(longtodate(su.CREATION_TIME) , 'YYYY-MM-DD')          AS "MEMBERSIGNUPDATE",
    TO_CHAR(ROUND(il.TOTAL_AMOUNT::NUMERIC, 2), 'FM999999990.00') AS "PRICEJFEE",
    CASE
    ...
    TO_CHAR(longtodateC(su.LAST_MODIFIED,100),'YYYY-MM-DD HH24:MI:SS') AS "LASTMODIFIEDDATE"
FROM
    PERSONS p
CROSS JOIN
    PARAMS
...
LEFT JOIN
    non_discount_price AS ndp
ON
    ndp.center = su.center
AND ndp.id = su.id
LEFT JOIN
...
AND CURRENT_DATE BETWEEN date_trunc('day',sc.EFFECT_DATE) AND COALESCE(date_trunc('day',longtodate
