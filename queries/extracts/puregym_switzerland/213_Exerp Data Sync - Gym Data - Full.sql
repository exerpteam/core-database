...
SELECT
    ...
    (
        CASE
            WHEN single.globalid='CORE_MONTHLY'
            THEN single.price
            ELSE NULL
        END )                                                  AS "LOWEST_CORE_PRICE_POINT",
    MIN(t2.min_prod) FILTER (WHERE t2.external_id = 'OffPeak')      AS "LOWEST_OFFPEAK_PRICE_POINT",
    MIN(t2.min_prod) FILTER (WHERE t2.external_id = 'MULTI ACCESS') AS
    "LOWEST_MULTIACCESS_PRICE_POINT",
    ...
    MIN(t2.min_prod) FILTER (WHERE t2.external_id = 'NATIONAL MULTI ACCESS') AS
    "LOWEST_NATIONAL_PRICE_POINT",
    MIN(t2.min_prod) FILTER (WHERE t2.external_id = 'NATIONAL-PREMIUM') AS
                                "LOWEST_NATIONALPREMIUM_PRICE_POINT",
    lowest_annual_core.price    AS "LOWEST_CORE_ANNUAL_PRICE_POINT",
    lowest_annual_premium.price AS "LOWEST_PREMIUM_ANNUAL_PRICE_POINT",
...
ON
    region.ID = ar1.AREA
AND region.name != 'ZZZ Closed Centres'
AND region.root_area = 1
LEFT JOIN
    products prod
ON
    prod.CENTER = cen.ID
AND prod.primary_product_group_id = 5602 --off peak
AND prod.blocked = 0
LEFT JOIN
    products single
ON
...
AND single.external_id = 'LOCAL'
AND single.blocked = 0
LEFT JOIN
    products lowest_annual_core
ON
    lowest_annual_core.CENTER = cen.ID
AND (lowest_annual_core.external_id = 'LOCAL12'
    OR  lowest_annual_core.name = 'Core 12')
AND lowest_annual_core.blocked = 0
LEFT JOIN
    products lowest_annual_premium
ON
    lowest_annual_premium.CENTER = cen.ID
AND (lowest_annual_premium.external_id = 'PREMIUM12'
    OR  lowest_annual_premium.name = 'Premium 12')
AND lowest_annual_core.blocked = 0
LEFT JOIN
    (
        SELECT
        ...
        FROM
            products pr
        WHERE
            pr.external_id IN ('OffPeak',
                               'MULTI ACCESS',
                               'PREMIUM',
                               'NATIONAL MULTI ACCESS',
                               'NATIONAL-PREMIUM')
