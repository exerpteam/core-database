        ...
        SELECT
            ...
            (
                ...
                FROM
                    ...
                    KPI_FIELDS F
                WHERE
                    F.KEY in('POSITIVEGAINONEMON','POSITIVEGAINTWELVEMON','POSITIVEGAINTWELVEMONBIND')
                    AND D.FIELD = F.ID
                    AND C.ID = D.CENTER
                    ...
                    CASE
                        ...
                        ELSE CAST(DATE_TRUNC('month', CAST(P.STARTDATE AS DATE)) + INTERVAL '1 MONTH' AS DATE)
                    END ) AS "Total New" ,
            (
                SELECT
                    -SUM(D.VALUE)
            ...
            (
                ...
                FROM
                    ...
                    KPI_FIELDS F
                WHERE
                    F.KEY in('NETGAINONEMON','NETGAINTWELVEMON','NETGAINTWELVEMONBIND')
                    AND D.FIELD = F.ID
                    AND C.ID = D.CENTER
                    AND D.FOR_DATE BETWEEN P.STARTDATE AND
                    CASE
            ...
            (
				...
				(
					...
					ON
						MC.FIELD = MF.ID
					WHERE
						MF.KEY in('POSITIVEGAINONEMON','POSITIVEGAINTWELVEMON','POSITIVEGAINTWELVEMONBIND')
            ...
            (
				...
				(
					...
					ON
						MC.FIELD = MF.ID
					WHERE
						MF.KEY in('LOSSONEMON','LOSSTWELVEMON','LOSSTWELVEMONBIND')
						AND MC.FOR_DATE BETWEEN P.STARTDATE AND
						CASE
							WHEN CAST(DATE_TRUNC('month', P.STARTDATE) AS DATE) = CAST(DATE_TRUNC('month', CAST(now() AS DATE)-1) AS DATE)
							THEN CAST(now() AS DATE)-1
        ...
        WHERE
            (
                ...
                WHERE
                    ...
                    AND D.FOR_DATE BETWEEN P.STARTDATE AND
                    CASE
                        WHEN CAST(DATE_TRUNC('month', CAST(P.STARTDATE AS DATE)) + INTERVAL '1 MONTH' AS DATE) = CAST(DATE_TRUNC('month', CAST(now() AS DATE)) + INTERVAL '1 MONTH' AS DATE)
                        THEN CAST(now() AS DATE)-1
                        ELSE CAST(DATE_TRUNC('month', CAST(P.STARTDATE AS DATE)) + INTERVAL '1 MONTH' AS DATE)
                    END ) > 0
