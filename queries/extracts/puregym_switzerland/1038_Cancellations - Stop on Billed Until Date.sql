...
LEFT JOIN cashcollectioncases mac
...
(
        SELECT
                ...
                acl2.state,
                MAX(acl2.id) AS Id
        FROM agreement_change_log acl2
        WHERE
              acl2.text is null 
...
LEFT JOIN agreement_change_log acl
        ON acl.id = acl3.id
        join puregym_switzerland.clearinghouses ch on ch.id = pag.clearinghouse
WHERE
        mac.center IS NOT NULL
        AND ch.ctype in (141,144,160,169,173,175,183,184,186,188,190,193,194) -- CreditCard
