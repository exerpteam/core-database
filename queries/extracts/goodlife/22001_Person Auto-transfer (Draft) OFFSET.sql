WITH
        ...
        (        
                SELECT
                    /*+ materialize */
                    datetolongTZ(TO_CHAR(current_date - interval '30 days','YYYY-MM-DD HH24:MI:SS'), c.time_zone)                   AS newCurrentDate,
                    datetolongTZ(TO_CHAR(current_date - interval '1 months' - interval '30 days','YYYY-MM-DD HH24:MI:SS'), c.time_zone)                   AS cutDateOneMonths,
                    datetolongTZ(TO_CHAR(current_date - interval '2 months' - interval '30 days','YYYY-MM-DD HH24:MI:SS'), c.time_zone)                   AS cutDateTwoMonths,
                    datetolongTZ(TO_CHAR(current_date - interval '3 months' - interval '30 days','YYYY-MM-DD HH24:MI:SS'), c.time_zone)                   AS cutDateThreeMonths,
                    datetolongTZ(TO_CHAR(current_date - interval '10 days','YYYY-MM-DD HH24:MI:SS'), c.time_zone)                   AS cutDateTenDays,
                    c.id AS centerid
                FROM
                    goodlife.centers c
                WHERE
...
SELECT
        finalt.Personhomeclub || 'p' || finalt.Personid AS "PERSON_ID",
...
(
        SELECT
                ...
                STRING_AGG(CAST((CASE WHEN tab1.is_included = 1 THEN SubscriptionId END) AS text), ',') AS Includedsubscriptions,
                STRING_AGG(CAST((CASE WHEN tab1.is_included = 0 THEN SubscriptionId END) AS text), ',') AS Excludesubscriptions,
                        ...
                        (
                                SELECT
                                        ...
                                        SUM(CASE
                                                ...
                                                AND ch.checkin_center != ch.person_center THEN 1
                                                ELSE 0
                                        END) Month1,
                                        SUM(CASE
                                                WHEN ch.checkin_time < params.cutDateOneMonths AND 
                                                     ch.checkin_time > params.cutDateTwoMonths AND
                                                     ch.checkin_center != ch.person_center THEN 1
                                                ELSE 0
                                        END) Month2,
                                        SUM(CASE
                                                WHEN ch.checkin_time < params.cutDateTwoMonths AND 
                                                     ch.checkin_time > params.cutDateThreeMonths AND
                                                     ch.checkin_center != ch.person_center THEN 1
                                                ELSE 0
                                        END) Month3,
                                ...
                                (
                                        ...
                                        (
                                               SELECT
                                                ...
                                                /*                        AND NOT EXISTS ( -- Members with missing agreement case
                                                                                                SELECT
                                                                                                        *
                                                                                                FROM    goodlife.cashcollectioncases ccc
                                                                                                WHERE
                                                                        ...
                                                                        AND NOT EXISTS ( -- Members with any incomplete questionnaire case
                                                                                                SELECT
                                                                                                        *
                                                                                                FROM    goodlife.questionnaire_answer qan
                                                                                                WHERE
                                                                                                        p.center = qan.center
                                                                                                        AND p.id = qan.id
                                                                                                        AND qan.completed = false
                                                                                       )
                                                                        ...
                                                                        AND NOT EXISTS ( -- Members due to change subscription
                                                                                                SELECT
                                                                                                        *
                                                                                                FROM    goodlife.subscriptions s
                                                                                                JOIN    goodlife.subscription_change sc 
                                                                        ...
                                                                        AND NOT EXISTS ( -- Members whose main membership subscription is EFT and the subscription period start date is NOT 'today'
                                                                                                SELECT
                                                                                                        *
                                                                                                FROM    goodlife.subscriptions s
                                                                                                JOIN    goodlife.subscriptiontypes st ON s.subscriptiontype_center = st.center AND s.subscriptiontype_id =  st.id
                                                                                                JOIN    goodlife.products pr ON st.center = pr.center AND st.id = pr.id
                                                                                                JOIN    goodlife.product_and_product_group_link ppl ON pr.center = ppl.product_center AND pr.id = ppl.product_id
                                                                                                WHERE
                                                                                                        p.center = s.owner_center 
                                                                                                        AND p.id = s.owner_id
                                                                       ...
                                                                       AND NOT EXISTS ( -- Members who have a Corporate (part) sponsored members where the transfer timing would cause a pro-rata/period break in the company invoice.
                                                                                                SELECT
                                                                                                        s.*
                                                                                                FROM
                                                                                                        goodlife.subscriptions s
                                                                                                JOIN    goodlife.subscriptiontypes st ON s.subscriptiontype_center = st.center AND s.subscriptiontype_id = st.id
                                                                                                JOIN    goodlife.products pr ON pr.center = st.center AND pr.id = st.id
                                                                                                WHERE
                                                                                                        p.center = s.owner_center 
                                                                                                        AND p.id = s.owner_id
                                                                                                        AND pr.globalid = 'PAP_M_ALL_CLUB_KAFP'
                                                                                                        AND s.state IN (2,4,8)
                                                                                                        AND NOT EXISTS (
                                                                                                                        SELECT 
                                                                                                                                *
...
AND NOT EXISTS
                                                                    ( -- exluding everyone except corporate
                                                                        SELECT
                                                                            ind.*
                                                                        FROM
                                                                        ...
                                                                        ind.owner_center=
                                                                            p.center
                                                                        AND ind.owner_id=p.id
                                                                        AND pl.product_group_id IN
                                                                                                    (
                                                                                                    2801,
                                                                                                    4201
                                                                                                    )
                                                        ...
                                                        -- Minimum time ACTIVE at home club => 3 months
                                                        JOIN goodlife.state_change_log scl ON scl.center = nonexcl.center AND scl.id = nonexcl.id AND scl.entry_type=1 AND scl.entry_end_time IS NULL AND scl.stateid = 1
                                                ) extra
                                                CROSS JOIN params
                                                JOIN goodlife.checkins ch ON extra.PersonCenter = ch.person_center AND extra.PersonId = ch.person_id 
                                                                            AND ch.checkin_time < params.newCurrentDate
                                        ...
                                        WHERE
                                                t1.LastActiveDate < current_date - interval '3 months'
                                ) t2
                                JOIN goodlife.checkins ch
                                        ON t2.PersonCenter = ch.person_center AND t2.PersonId = ch.person_id AND ch.checkin_result = 1
                                JOIN params ON params.centerid = ch.checkin_center     
                                WHERE
                        ...
                        JOIN goodlife.centers origin
                                ON tch.PersonCenter = origin.id
                        JOIN goodlife.centers destination
                                ON tch.CheckinCenter = destination.id
                        WHERE
                                tch.Month1 > 3
                                AND tch.Month2 > 3
                                AND tch.Month3 > 3
                                AND ((tch.VisitsPerClub * 100) / tch.TotalVisits) > 60
                ) withsub
                LEFT JOIN goodlife.subscriptions s
                        ON s.owner_center = withsub.PersonCenter AND s.owner_id = withsub.PersonId
                WHERE
                        s.state IN (2,4,8)
        ) tab1
        GROUP BY
