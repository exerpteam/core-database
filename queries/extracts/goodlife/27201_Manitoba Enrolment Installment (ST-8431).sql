...
--AND s.center IN    (SELECT id FROM centers JOIN zipcodes z ON z.country = c.country AND z.city = c.city AND z.zipcode = c.zipcode WHERE z.province = 'ON')
...
AND ((
    ...
    -- ONLY INCLUDE THOSE RENEWED TODAY
AND EXISTS
    (
        SELECT
            1
...
AND NOT EXISTS
    (
        ...
        -- When do they show up for bank holidays
        -- last sub query not exists, what happens if it was missed the first go around