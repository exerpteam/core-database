...
SELECT
    ...
    r.center || 'p' || r.id                            AS "OtherPayer",
    TO_CHAR(longtodateC(pag.creation_time, 990),'YYYY-MM-DD')                AS pag_creation_time ,
    TO_CHAR(longtodateC(scl.entry_start_time , 990),'YYYY-MM-DD')            AS relation_start_time,
...
ON
    ...
    OR
        -- the subscription is not linked to any agreement and the member does have another payer
        (
            r.center IS NOT NULL
        AND s.payment_agreement_center IS NULL
        AND r_ar.center = pag.center
        AND r_ar.id = pag.id
        AND pag.subid = r_pa.active_agr_subid) )
JOIN
