...
--	AND entry_time >= CAST((CURRENT_DATE-60-to_date('1-1-1970','MM-DD-YYYY')) AS BIGINT)*24*3600*1000
	AND trans_time 
		BETWEEN
		  (SELECT EXTRACT(EPOCH FROM CAST($$dayFrom$$ AS date)) * 1000)
			AND
		  (SELECT EXTRACT(EPOCH FROM CAST($$dayTo$$ AS date)) * 1000)
	--AND ref_center = 216 and ref_id = 1 and ref_subid = 911339 -- for testing
) 
, artEmp 
AS (
...
WHERE e.personcenter IN ('990', '100')
)
, artEmpRec
AS (
...
--SELECT * FROM artEmpRecTrans LIMIT 10
, artEmpRecTrans2
AS (
SELECT aert.*
--aert.center, aert.id, aert.subid, aert.debit_accountcenter
--, aert.debit_accountid, aert.aggregated_transaction_center
--, aert.aggregated_transaction_id
...
-- SELECT * FROM artEmpRecTrans2 LIMIT 10
, a990
AS (
...
SELECT
	...
	LONGTODATE (t.trans_time) AS TransactionDate,
	t.amount,
	t.due_date, 
	t.info,
	t.text,
