...
-- This extract pulls a list of transactions on a company's account based on parameters provided. The user can specify the Invoice ID, Company id, or both, as well as whether or not the transaction has been invoiced. This was reviewed by Exerp in ST-4579: https://clublead.atlassian.net/browse/ST-4579.
SELECT
...
SELECT
    ...
    TO_CHAR(LONGTODATEC(at.entry_time, 100),'YYYY-MM-DD HH:MI:SS') AS "entrydate",
	TO_CHAR(LONGTODATEC(at.trans_time, 100),'YYYY-MM-DD HH:MI:SS') AS "bookdate",
    (CASE
...
(CASE
	...
	,px.fullname AS GL_Employee_Name
	,e.center||'emp'||e.id AS GL_Employee_id
...
AND pr.state NOT IN (5,6,7,8,12,17,19,21,22)
...
('All' = :reference OR prs.ref = :reference)
    ...
    AND (CASE
			WHEN :Payment = 2 AND :Collected = 5 AND :Uncollected = 5
			THEN ((at.ref_type = 'ACCOUNT_TRANS' AND at.text = 'Payment into account') OR at.collected = 2)
			ELSE at.COLLECTED IN (:Collected,:Uncollected,:Payment)
		END)	
