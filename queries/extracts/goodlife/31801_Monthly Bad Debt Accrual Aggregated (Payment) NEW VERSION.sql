...
WITH
    ...
    (
       SELECT t.*, 
		ROUND(t.OPEN_AMOUNT/COALESCE((1+t.rate),1), 2) AS OPEN_REVENUE_AMOUNT,
		t.OPEN_AMOUNT - ROUND(t.OPEN_AMOUNT/COALESCE((1+t.rate),1), 2) AS OPEN_VAT_AMOUNT
	   FROM
	   (
		   SELECT
				tv.sales_center,
...
SELECT
    ...
    SUM(t.OPEN_AMOUNT)                               AS "Sum of Open Amount",
    SUM(t.OPEN_VAT_AMOUNT)                           AS "Sum of Open VAT Amount",
    SUM(t.OPEN_REVENUE_AMOUNT) 						 AS "Sum of Open Revenue Amount"
FROM
    TRANS_AGG_VIEW t
JOIN
    centers percenter
