...
WHERE
    part.state IN ('PARTICIPATION',
                   'CANCELLED')
    AND bk.STARTTIME BETWEEN datetolong(TO_CHAR(TRUNC(SYSDATE -7), 'YYYY-MM-DD HH24:MI')) AND datetolong(TO_CHAR(TRUNC(SYSDATE), 'YYYY-MM-DD HH24:MI')) and part.center in ($$scope$$)