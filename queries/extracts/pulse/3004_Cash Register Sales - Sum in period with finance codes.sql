 SELECT
     ...
     '001'    fund,
     CASE
         WHEN CHAR_LENGTH(prodExternalId)-CHAR_LENGTH(REPLACE(prodExternalId,'-','')) = 2
         THEN SUBSTR(prodExternalId, char_length(prodExternalId) - position('-' IN reverse(prodExternalId)) +2)
     END   AS "ID End",
     --    vat_rate,
         ...
         WHERE
             ...
             (
                 ...
                 WHERE
                     ...
                     AND crr.ID = cr.ID
                     AND crr.REPORTTIME > :FromDate
                     AND crr.REPORTTIME < :ToDate + 60*60*1000*24 ) ) t
 GROUP BY
     sales_center,
     sales_club,
     prodExternalId,
