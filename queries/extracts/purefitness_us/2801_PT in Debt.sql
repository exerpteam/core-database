...
FROM
    ...
    SELECT DISTINCT
               ...
               FROM
                    only_staff p
               JOIN
                    ACCOUNT_RECEIVABLES ar ON p.personcenter = ar.CUSTOMERCENTER AND p.personid = ar.CUSTOMERID AND ar.ar_type = 4
               JOIN
                    PAYMENT_ACCOUNTS pac ON pac.center = ar.center AND pac.id = ar.id
               JOIN
               ...
               JOIN
                    AR_TRANS art ON art.center = ar.center AND art.id = ar.id
          LEFT JOIN
                    CASHCOLLECTIONCASES cc ON cc.PERSONCENTER = p.personcenter AND cc.PERSONID = p.personid AND cc.MISSINGPAYMENT = true AND
                    cc.CLOSED = false
              WHERE
                    cc.id IS NOT NULL OR (
                        --pag.payment_cycle_config_id = 1401 -----PG UK
                        --AND
                        art.UNSETTLED_AMOUNT < 0 AND art.due_date < CURRENT_TIMESTAMP)
            )
            ,
            ...
            (
             SELECT
               ...
               FROM
                    eligable_members c
          LEFT JOIN
                    payment_agreements last_failed_pag ON c.pagcenter = last_failed_pag.center AND c.pagid = last_failed_pag.id AND
                    last_failed_pag.state != 4
            )
            ,
            ...
            (
             SELECT
               ...
               FROM
                    base3 b
          LEFT JOIN
                    PAYMENT_REQUESTS last_preq ON last_preq.center = b.pagcenter AND last_preq.id = b.pagid
              WHERE
                    b.rnk3 = 1
            )
            ,
            ...
            (
             SELECT
                    ...
                    rank() over (partition BY last_checkin.person_center, last_checkin.person_id ORDER BY last_checkin.CHECKIN_TIME DESC) AS rnk5
               FROM
                    base4 c
          LEFT JOIN
                    CHECKINS last_checkin ON last_checkin.person_center = c.personcenter AND last_checkin.person_id = c.personid
              WHERE
                    rnk4 = 1
            )
            ,
            ...
            (
             SELECT
               ...
               FROM
                    base5 b
          LEFT JOIN
                    AGREEMENT_CHANGE_LOG last_failed_pa_msg ON last_failed_pa_msg.state != 4 AND last_failed_pa_msg.agreement_center =
                    last_failed_pag_center AND last_failed_pa_msg.agreement_id = last_failed_pag_id AND last_failed_pa_msg.agreement_subid =
                    last_failed_pag_subid
              WHERE
                    rnk5 = 1
            )
            ,
            ...
            (
             SELECT
               ...
               FROM
                    base6 b
          LEFT JOIN
                    PAYMENT_REQUESTS last_failed_preq ON last_failed_preq.center = b.pagcenter AND last_failed_preq.id = b.pagid AND
                    last_failed_preq.state > 4
              WHERE
                    b.rnk6 = 1
            )
            ,
            ...
            (
             SELECT
               ...
               FROM
                    base7 d
          LEFT JOIN
                    AR_TRANS art ON art.center = d.acenter AND art.id = d.aid AND art.STATUS IN ('NEW',
                                                                                                 'OPEN')
              WHERE
                    rnk7 = 1
            )
   ...
   GROUP BY
            ...
            last_REQ_DATE,
            CHECKIN_TIME,
            last_failed_pa_msg_TEXT) t2
LEFT JOIN
    SUBSCRIPTIONS s ON s.OWNER_CENTER = t2.personcenter AND s.OWNER_ID = t2.personid AND s.state IN (2,
                                                                                                     4,
                                                                                                     8)
LEFT JOIN
    PRODUCTS pd ON pd.center = s.SUBSCRIPTIONTYPE_CENTER AND pd.id = s.SUBSCRIPTIONTYPE_ID
GROUP BY
    t2.personcenter,
    t2.personid,
    t2.Club,
