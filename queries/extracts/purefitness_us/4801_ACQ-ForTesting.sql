...
WITH RECURSIVE cte_current_timestamp_ms AS (
    SELECT
        EXTRACT(
            EPOCH
            FROM
                CURRENT_TIMESTAMP AT TIME ZONE GETCENTERTZ(100)
        ) * 1000 AS value
),
cte_centers_in_area AS (
    SELECT
        AREA.id,
        AREA.parent,
...
cte_all_campaigns AS (
    ...
    FROM
        ...
        CROSS JOIN cte_current_timestamp_ms
    WHERE
        STARTUP_CAMPAIGN.state = 'ACTIVE'
        AND STARTUP_CAMPAIGN.campaign_apply_for = 'NEW_SALE'
        AND STARTUP_CAMPAIGN.endtime > cte_current_timestamp_ms.value
        AND STARTUP_CAMPAIGN.plugin_codes_name IN(
            'GENERATED',
            'UNIQUE'
        )
    GROUP BY
    ...
    WHERE
        ...
        AND PRIVILEGE_RECEIVER_GROUP.endtime > cte_current_timestamp_ms.value
        AND PRIVILEGE_RECEIVER_GROUP.plugin_codes_name IN(
            'GENERATED',
            'UNIQUE'
        )
),
...
SELECT
    ...
    (
        campaign.starttime > cte_current_timestamp_ms.value
    ) AS "StartDate",
    (
        campaign.endtime > cte_current_timestamp_ms.value
    ) AS "EndDate",
    CASE
