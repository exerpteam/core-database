...
AND ( (
            pr.request_type = 1
        AND pr.req_date+1 BETWEEN CAST(DATE_TRUNC('day', now()- INTERVAL '7' DAY) AS DATE) AND CAST
    ...
    OR  (
            pr.STATE NOT IN (6,7)
        AND pr.request_type = 6
        AND pr.req_date+5 BETWEEN CAST(DATE_TRUNC('day', now()- INTERVAL '7' DAY) AS DATE) AND CAST
            (DATE_TRUNC('day', now()) AS DATE)
        AND pr.xfr_info IN ('Declined Non Generic',
                            'Not enough balance',
                            'Withdrawal amount exceeded',
