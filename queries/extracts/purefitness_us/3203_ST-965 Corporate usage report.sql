WITH
     ...
     (
         SELECT 
            CAST(datetolong(TO_CHAR(DATE (:start_date), 'YYYY-MM-DD')) AS bigint) AS start_date,
            CAST(datetolong(TO_CHAR(DATE (:end_date), 'YYYY-MM-DD')) AS bigint)+1000*60*60*24 AS end_date
     ) , 
     table1 as
     (
     SELECT
 ...
 SELECT
         ...
         to_char(longToDateC(table1.CHECKIN_TIME, c.ID), 'HH24:MI:SS') as "Visit Club Time",
         to_char(longToDateC(table1.CHECKIN_TIME, c.ID), 'YYYY-MM-DD') as "Visit Club Date",
         to_char(longToDateC(table1.CHECKOUT_TIME, c.ID), 'HH24:MI:SS') as "Checkout Time",
         to_char(longToDateC(table1.CHECKOUT_TIME, c.ID), 'YYYY-MM-DD') as "Checkout Date",
