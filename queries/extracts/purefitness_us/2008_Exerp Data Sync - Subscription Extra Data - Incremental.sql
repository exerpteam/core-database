     ...
     (
		SELECT
			...
			CAST(datetolongC(to_char(date_trunc('day',to_timestamp(getcentertime(ID), 'YYYY-MM-DD HH24:MI:SS')+interval '1' day),'YYYY-MM-DD HH24:MI'), ID) AS BIGINT) AS TODATE,
			 ...
			 CAST(datetolongC(to_char(to_timestamp(getcentertime(c.ID), 'YYYY-MM-DD HH24:MI:SS'),'YYYY-MM-DD HH24:MI'), c.ID) AS BIGINT) AS CURRENT_TIME_TS,
			 CAST(datetolongC(to_char(to_timestamp(getcentertime(c.ID), 'YYYY-MM-DD HH24:MI:SS'),'YYYY-MM-DD'), c.ID) AS BIGINT) AS CURRENT_DATE_TS
		FROM 
			centers c
     )
    ,
    ...
    np.EXTERNAL_ID                                      AS "EXTERNALID",
     ...
     (CASE
         WHEN (su.LAST_MODIFIED IS NULL AND FrzPerMem.Freeze_Last_Modified IS NULL AND sa.Boltons_Last_Created IS NULL) THEN
                 NULL
         ELSE
               TO_CHAR(longtodatetz(GREATEST(su.LAST_MODIFIED, FrzPerMem.Freeze_Last_Modified, sa.Boltons_Last_Created),params.TZFORMAT),params.DATETIMEFORMAT)
...
WHERE
    p.CENTER IN (:scope)
AND su.CREATION_TIME < params.CURRENT_DATE_TS
AND (
        su.STATE = 2
    OR  (
...
UNION ALL
     SELECT 
        ...
        NULL AS "LASTFROZENSTARTDATE",
        NULL AS "LASTFROZENDAY",
        NULL AS "ACCUMULATEDFREEZEDAYS",
        NULL AS "APPUSER",
        NULL AS "REGISTEREDIOS",
        NULL AS "REGISTEREDANDROID",
        NULL AS "SUCCESSFULDDPAYMENTS",
        NULL AS "BOLTONS",
        NULL AS "COUNTOFREFERRALSMADE",
