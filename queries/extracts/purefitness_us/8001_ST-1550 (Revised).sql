...
WITH
    ...
    (
        SELECT
            c.id,
            CAST (dateToLongTZ(TO_CHAR(CAST(s.d1 AS DATE)+1, 'YYYY-MM-dd HH24:MI'), coalesce(c.time_zone, co.defaulttimezone)) AS BIGINT) AS dt,
            s.d1                                                                    AS d
        FROM
            dates_series s
        CROSS JOIN
...
SELECT
    longtodateC(t.dt,t.CENTER)-1 AS "Date",
    CENTER                 AS "ClubId",
    C_NAME                 AS "Club",
    CASE PERSON_TYPE
        WHEN 0
    ...
    (
        ...
        WHERE
            (
                ...
                AND SCL1.BOOK_START_TIME < dates.dt 
                AND (
                    SCL1.BOOK_END_TIME IS NULL
                    OR SCL1.BOOK_END_TIME >= dates.dt )
                AND SCL1.ENTRY_START_TIME < dates.dt 
                AND SCL2.BOOK_START_TIME < dates.dt 
                AND (
                    SCL2.BOOK_END_TIME IS NULL
                    OR SCL2.BOOK_END_TIME >= dates.dt)
                AND SCL2.ENTRY_START_TIME < dates.dt 
                AND c.id IN ($$Scope$$)
     AND (
                ...
                (
                    ...
                    WHERE
                        ...
                        AND ppgl.product_center = st.center) )) t
WHERE
    rnk = 1
    AND rnk2 = 1
GROUP BY
