...
JOIN centers cen ON ss.subscription_center = cen.id
JOIN employees e ON ss.employee_center = e.center AND ss.employee_id = e.id
JOIN persons emp ON e.personcenter = emp.center AND e.personid = emp.id
...
JOIN subscriptionperiodparts spp ON spp.center = s.center AND spp.id = s.id
JOIN spp_invoicelines_link sppl ON spp.center = sppl.period_center AND spp.id = sppl.period_id AND spp.subid = sppl.period_subid
JOIN invoice_lines_mt il ON sppl.invoiceline_center = il.center AND sppl.invoiceline_id = il.id AND sppl.invoiceline_subid = il.subid
JOIN products pr2 ON il.productcenter = pr2.center AND il.productid = pr2.id
WHERE
