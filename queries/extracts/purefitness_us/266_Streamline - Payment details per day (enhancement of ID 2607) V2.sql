...
edit: ES-31581, ES-32513
WITH
    params AS MATERIALIZED
    (
        SELECT
GETSTARTOFDAY(CAST (CAST ($$FromDate$$ AS DATE) AS TEXT), C.ID) AS FromDate,
GETENDOFDAY(CAST (CAST ($$ToDate$$ AS DATE) AS TEXT), C.ID) AS ToDate
...
ON
    ...
    AND (
        je.NAME IN ('Payment', 'Debts Payment Sage Pay',
            ...
            CASE
                WHEN
                    ...
                    (
                        ...
                        WHERE
                            ( ClosestJE.NAME IN ('Payment', 'Debts Payment Sage Pay',
                                                 'Day Pass Purchase Sage Pay',
                                                 'Pure Loser Purchase Sage Pay',
                                                 --'Grit Course Purchase Sage Pay',
                                                 'Join Payment Sage Pay',
