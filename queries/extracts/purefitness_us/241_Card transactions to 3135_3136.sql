...
-- https://clublead.atlassian.net/browse/ST-9417
WITH
    params AS
    (
        SELECT
            /*+ materialize */
            CAST (dateToLongTZ(TO_CHAR(CAST($$fromDate$$ AS DATE), 'YYYY-MM-dd HH24:MI'), 'US/Eastern') AS BIGINT)                  AS FromDate,
            CAST((dateToLongTZ(TO_CHAR(CAST($$toDate$$ AS DATE), 'YYYY-MM-dd HH24:MI'), 'US/Eastern')+ 86400 * 1000)-1 AS BIGINT) AS ToDate
    )
SELECT
