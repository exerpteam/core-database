...
WITH
    ...
    (
     SELECT
            datetolongC(TO_CHAR(TO_DATE(getCenterTime(c.id), 'YYYY-MM-DD'), 'YYYY-MM-DD'), c.id) AS FROMDATE, -- Ahora toma el día actual
        datetolongC(TO_CHAR(TO_DATE(getCenterTime(c.id), 'YYYY-MM-DD'), 'YYYY-MM-DD'), c.id) AS TODATE,
            c.id,
    ...
    (
        SELECT
               distinct 
               (CASE WHEN customer = 'VIVAGYM' THEN 1 ELSE 9999 END) as CodigoEmpresa
        FROM db_version
    )
SELECT
    "ContractNo"  ,
