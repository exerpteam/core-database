...
(
        SELECT
    DATE_TRUNC('month', TO_DATE(getCenterTime(c.id), 'YYYY-MM-DD') - INTERVAL '1 month') AS fromDate,
    (DATE_TRUNC('month', TO_DATE(getCenterTime(c.id), 'YYYY-MM-DD')) - INTERVAL '1 day') AS toDate,
    
    CAST(
        dateToLongC(
            TO_CHAR(DATE_TRUNC('month', TO_DATE(getCenterTime(c.id), 'YYYY-MM-DD') - INTERVAL '1 month'), 'YYYY-MM-DD'),
            c.id
        ) AS BIGINT
    ) AS fromDateLong,
    ...
    CAST(
        dateToLongC(
            TO_CHAR(DATE_TRUNC('month', TO_DATE(getCenterTime(c.id), 'YYYY-MM-DD')), 'YYYY-MM-DD'),
            c.id
        ) - 1 AS BIGINT
    ) AS toDateLong,
