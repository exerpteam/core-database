...
(
	...
	CAST(dateToLongC(TO_CHAR(TO_DATE('2023-01-01','YYYY-MM-DD'),'YYYY-MM-DD'),C.id) AS BIGINT) AS fromDate,
                        CAST(dateToLongC(TO_CHAR(TO_DATE('2023-03-01','YYYY-MM-DD'),'YYYY-MM-DD'),c.id)-1 AS BIGINT) AS toDate,
                        c.id AS center_id
                FROM
                        CENTERS c
                WHERE
        ...
        (
                ...
                SELECT  
                        ------------------------ INVOICE MASTER ------------------------
                        CAST(extract(YEAR FROM longToDateC(settlement.latest_settlement, i.center)) AS TEXT) || 'P' AS serie,
                ...
                WHERE
                        ...
                        AND art.unsettled_amount = 0
                        AND settlement.latest_settlement between par.fromDate AND par.toDate
        ),
        v_pivot AS
        (
                SELECT
