...
(
        ...
        FROM
        (
                -- SALES LIST NO PAYMENT ACCOUNT
                SELECT  
                        'Stand Alone Transaction (Sales List)' AS Transaction_Group,
                        (CASE 
                                WHEN crt.center IS NULL THEN CAST('ELGYMIBERIAECOM' AS TEXT)
                                WHEN cr.type = 'WEB' THEN CAST('ELGYMIBERIAECOM' AS TEXT)
                                WHEN cr.type = 'POS' AND crt.crttype = 7 THEN CAST('ELGYMIBERIAPOS' AS TEXT)
                                WHEN cr.type = 'POS' AND crt.crttype = 1 THEN CAST('CASH' AS TEXT)
                        ...
                        par.toDate,
                        (CASE 
                                WHEN (i.employee_center,i.employee_id) IN ((100,8603),(100,13801)) THEN cct.transaction_id
                                ELSE NULL
                        END) AS PSP
                ...
                LEFT JOIN vivagym.cashregisters cr
        ...
        UNION ALL
        ----------------------------   CASH REGISTER TRANSACTIONS WITH NO INVOICE  --------------------------------
        SELECT
                r2.*
        FROM
        (
                SELECT 
                        'Cash Register Transaction - No Sales List' AS Transaction_Group,
                        (CASE 
                                WHEN cr.type = 'WEB' THEN CAST('ELGYMIBERIAECOM' AS TEXT)
                                WHEN cr.type = 'POS' AND crt.crttype = 7 THEN CAST('ELGYMIBERIAPOS' AS TEXT)
                                WHEN cr.type = 'POS' AND crt.crttype = 1 THEN CAST('CASH' AS TEXT)
                                WHEN cr.type = 'POS' AND crt.crttype = 12 THEN CAST('INVESTIGATE CRTTPYE = 12' AS TEXT)
                                ELSE CAST('UNKNOWN' AS TEXT)
                        END) AS Transaction_type,
                        CAST('MANUAL' AS TEXT) AS Transaction_subtype,
                        par.center_id,
                        par.center_name,
                        longToDateC(crt.transtime, crt.center) AS entry_time,
                        crt.customercenter || 'p' || crt.customerid AS Person_Id,
                        cp.external_id,
                        crt.amount,
                        'PS:' || crt.paysessionid AS Referencia,
                        crt.coment AS text,
                        CAST(NULL AS TEXT) AS Exerp_Comment,
                        CAST(NULL AS TEXT) AS VivaGym_Definition,
                        1 is_in_the_bank,
                        ...
                        par.toDate,
                        NULL AS PSP
                FROM vivagym.cashregistertransactions crt
                JOIN params par
                        ON par.center_id = crt.center
                JOIN vivagym.cashregisters cr
                        ON cr.center = crt.center
                        AND cr.id = crt.id
                LEFT JOIN vivagym.invoices i
                        ON i.paysessionid =crt.paysessionid
                LEFT JOIN vivagym.persons payer
                        ON payer.center = crt.customercenter
                        AND payer.id = crt.customerid
                LEFT JOIN vivagym.persons cp
                        ON payer.current_person_center  = cp.center
                        AND payer.current_person_id = cp.id
                WHERE
                        crt.transtime >= par.fromDateLong
                        AND crt.transtime <= par.toDateLong
                        AND i.center IS NULL
                        AND crt.amount <> 0
        ) r2
        UNION ALL
        SELECT
        ...
        (
                ...
                (
                        ...
                        WHERE
                                ...
                                AND pr.req_amount != 0
                                AND pr.clearinghouse_id IN (1,3002,3201,3601,3803,4001,4201,4402,4601,5201,7402)
                        GROUP BY
                                ar.customercenter,
                                ar.customerid,
                                pr.state,
