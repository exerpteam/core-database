...
(
        SELECT
                CAST(dateToLongC(TO_CHAR(TO_DATE(getCenterTime(c.id),'YYYY-MM-DD') - INTERVAL '1 days','YYYY-MM-DD HH24:MI'),c.id) AS BIGINT)  AS fromDate,
                CAST(dateToLongC(TO_CHAR(TO_DATE(getCenterTime(c.id),'YYYY-MM-DD'),'YYYY-MM-DD HH24:MI'),c.id) AS BIGINT) AS toDate,
                c.id AS centerId,
