...
(
        WITH params AS MATERIALIZED
                (
                        SELECT
                                                                DATE '2025-06-01' AS fromDate,
                                                                DATE '2025-06-30' AS toDate,
                        ...
                        (
                                ...
                                (
                                        SELECT
            EXTRACT(DAY FROM TO_DATE(getCenterTime(c.id), 'YYYY-MM-DD') - interval '1 month') AS current_day,
            DATE_TRUNC('month', TO_DATE(getCenterTime(c.id), 'YYYY-MM-DD') - interval '1 month') AS firstMonth, -- 01/04/2025
            DATE_TRUNC('month', TO_DATE(getCenterTime(c.id), 'YYYY-MM-DD')) - interval '1 day' AS endMonth,     -- 30/04/2025
            DATE_TRUNC('month', TO_DATE(getCenterTime(c.id), 'YYYY-MM-DD') - interval '2 month') AS firstPreviousMonth,
            DATE_TRUNC('month', TO_DATE(getCenterTime(c.id), 'YYYY-MM-DD') - interval '1 month') - interval '1 day' AS endPreviousMonth,
            c.id AS center_id,
        ...
        (
                ...
                (
                        ...
                        WHERE
                                ...
                                AND pr.req_amount != 0
                                AND pr.clearinghouse_id IN (1,3002,3201,3601)
                        GROUP BY
                                ar.customercenter,
                                ar.customerid,
                                pr.state,
