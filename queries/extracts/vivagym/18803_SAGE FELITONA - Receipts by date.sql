...
-- Extract para SAGE FELITONA FitUp por días.
WITH params AS MATERIALIZED
(
        SELECT
            datetolongC(TO_CHAR(TO_DATE(:fromDate,'YYYY-MM-DD'), 'YYYY-MM-DD'), c.id) AS FROMDATE,
            datetolongC(TO_CHAR(TO_DATE(:toDate,'YYYY-MM-DD'), 'YYYY-MM-DD'), c.id)-1 AS TODATE,
            TO_CHAR(TO_DATE(getCenterTime(c.id),'YYYY-MM-DD'), 'YYYY-MM-DD') AS INVOICE_DATE_DASH,
            TO_CHAR(TO_DATE(getCenterTime(c.id),'YYYY-MM-DD'), 'DD/MM/YYYY') AS INVOICE_DATE_SLASH,
            c.id,
    ...
    (
     SELECT
        ...
        -- Reemplazamos " y ; para evitar problemas con los archivos csv
            ...
            par.INVOICE_DATE_DASH                           AS           "FechaFactura"        ,
            ...
            CASE
WHEN prod.PTYPE = 7 AND credacc.EXTERNAL_ID = '7050030' 									OR  credacc.EXTERNAL_ID = '7050031'							
									THEN '46'
...
WHEN prod.PTYPE = 7 THEN '11'
            ...
            CONCAT(par.INVOICE_DATE_SLASH,' ', to_char(longtodatec(i.TRANS_TIME,i.CENTER),'HH24:MI'))       "Fecha"     ,
            --to_char(longtodatec(i.ENTRY_TIME,i.CENTER),'DD/MM/YYYY HH24:MI')                              "FechaEnvio",
            CONCAT(par.INVOICE_DATE_SLASH,' ', to_char(longtodatec(i.ENTRY_TIME,i.CENTER),'HH24:MI'))       "FechaEnvio",
  ...
  UNION ALL
     SELECT
        ...
        -- Reemplazamos " y ; para evitar problemas con los archivos csv
            ...
            debitacc.EXTERNAL_ID                                           "GL_DEBIT_ACCOUNT"    ,
            CASE
				WHEN prod.PTYPE = 7 AND debitacc.EXTERNAL_ID = '7050030' 									THEN '46' 
            ...
            CONCAT(par.INVOICE_DATE_DASH,' ', to_char(longtodatec(c.ENTRY_TIME,c.CENTER),'HH24:MI'))       "Fecha"        ,
            --to_char(longtodatec(c.ENTRY_TIME,c.CENTER),'DD/MM/YYYY HH24:MI')                             "FechaEnvio"   ,
            CONCAT(par.INVOICE_DATE_DASH,' ', to_char(longtodatec(c.ENTRY_TIME,c.CENTER),'HH24:MI'))       "FechaEnvio"   ,
