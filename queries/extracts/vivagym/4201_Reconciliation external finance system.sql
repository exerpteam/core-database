    ...
    (
        ...
        FROM
            (
                ...
                WHERE
		    art.center in (:scope)  
	            AND art.BOOK_DATE >= CAST(TO_CHAR(TRUNC(CAST(:FromDate AS DATE), 'DDD'), 'YYYY-MM-DD') AS DATE)
	            AND art.BOOK_DATE < CAST(TO_CHAR(TRUNC(CAST(:ToDate AS DATE) + 1 , 'DDD'), 'YYYY-MM-DD') AS DATE)
	        GROUP BY
                    art.center,
