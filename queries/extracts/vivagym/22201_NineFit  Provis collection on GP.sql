...
WHERE
    pag.clearinghouse IN (1)
    AND pag.clearinghouse_ref ~ '^(:[a-f0-9\-]{36}\|\d+|[a-f0-9]{40}|\d{16})$'
    AND pag.creation_time BETWEEN 1757973600000 AND 1758059999999
    AND art.text LIKE '%(Renovación automática)%'
    AND art.entry_time BETWEEN 1758063600000 AND 1758067200000
)
SELECT
    c.id AS center_id,
