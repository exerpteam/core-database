...
SELECT
		...
		(CASE 
			WHEN original_query.transaction_type = 'ELGYMIBERIAECOM' THEN
				'altafit com'
			WHEN original_query.transaction_type = 'ELGYMIBERIAPOS' THEN
				'altafit pos'
			ELSE
				'ERROR'
			END) AS forma_de_pago,
/*		original_query.transaction_type,*/
		SUM(original_query.total_amount) AS importe,
		1 AS clearinghouse
FROM
(
	SELECT
...
GROUP BY original_query.center_id, 
	forma_de_pago,
/*	original_query.transaction_type,*/
	clearinghouse
