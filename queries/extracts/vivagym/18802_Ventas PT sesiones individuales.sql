...
SELECT
    ...
    p.city AS Ciudad_del_socio,
    p.birthdate AS Fecha_nacimiento,
    pea.txtvalue AS Email,
    pea2.txtvalue AS telefono,
    p.center AS Club_Origen,
    
    -- Campos nuevos
    il.quantity AS Numero_unidades_compradas,
    il.product_normal_price AS Precio_por_unidad,
    il.total_amount AS Precio_pagado

FROM
    INVOICE_LINES_MT il
JOIN
    vivagym.account_trans ats
    ON il.account_trans_center = ats.center
    AND il.account_trans_id = ats.id
    AND il.account_trans_subid = ats.subid
JOIN
    centers ce ON il.account_trans_center = ce.id
JOIN
    params ON params.CenterID = il.account_trans_center
JOIN
    persons p ON il.person_center = p.center AND il.person_id = p.id
LEFT JOIN
    vivagym.person_ext_attrs pea ON p.current_person_center = pea.personcenter 
    AND p.current_person_id = pea.personid 
    AND pea.name = '_eClub_Email'
LEFT JOIN
    vivagym.person_ext_attrs pea2 ON p.current_person_center = pea2.personcenter 
    AND p.current_person_id = pea2.personid 
    AND pea2.name = '_eClub_PhoneSMS'
WHERE
    il.text = 'PT sesión individual'
