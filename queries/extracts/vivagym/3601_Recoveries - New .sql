...
JOIN 
        payment_request_specifications prs ON pr.inv_coll_center = prs.center AND pr.inv_coll_id = prs.id AND pr.inv_coll_subid = prs.subid
JOIN 
        ar_trans art ON art.payreq_spec_center = prs.center AND art.payreq_spec_id = prs.id AND art.payreq_spec_subid = prs.subid
JOIN vivagym.account_receivables ar ON art.center = ar.center AND art.id = ar.id
JOIN vivagym.centers c ON ar.customercenter = c.id AND c.country = 'ES'
JOIN
                    ART_MATCH arm
                ON
