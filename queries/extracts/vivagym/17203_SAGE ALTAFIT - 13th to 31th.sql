WITH params AS (
      SELECT
        datetolongC(TO_CHAR(DATE_TRUNC('month', TO_DATE(getCenterTime(c.id), 'YYYY-MM-DD') - INTERVAL '1 month'),'YYYY-MM-DD'), c.id) AS FROMDATE,
        datetolongC(TO_CHAR(DATE_TRUNC('month', TO_DATE(getCenterTime(c.id), 'YYYY-MM-DD')),'YYYY-MM-DD'), c.id) AS TODATE,
            c.id,
