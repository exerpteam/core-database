...
(
        ...
        WHERE
                ...
                AND mpr.globalid = 'ALUGUER_DE_CACIFO'
)
SELECT
        wt.center_name,
        wt.external_id,
...
FROM
        with_taquilla wt
LEFT JOIN
        vivagym.person_ext_attrs pea ON wt.center = pea.personcenter AND wt.id = pea.personid AND pea.name = 'NUMEROCACIFO'
WHERE
	wt.center IN (:Scope)
UNION ALL
SELECT
...
FROM
        vivagym.persons p
JOIN 
        vivagym.person_ext_attrs pea2 ON p.center = pea2.personcenter AND p.id = pea2.personid AND pea2.name = 'NUMEROCACIFO'
JOIN
