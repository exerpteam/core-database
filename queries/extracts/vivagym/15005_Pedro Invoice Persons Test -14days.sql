...
(
        SELECT
                ...
                CAST(dateToLongC(TO_CHAR(TO_DATE(getCenterTime(c.id),'YYYY-MM-DD') - interval '14 days','YYYY-MM-DD'),c.id) AS BIGINT) AS fromDate,
                CAST(dateToLongC(TO_CHAR(TO_DATE(getCenterTime(c.id),'YYYY-MM-DD'),'YYYY-MM-DD'),c.id)-14 AS BIGINT) AS toDate,
                c.id AS center_id
        FROM
                CENTERS c
        WHERE
