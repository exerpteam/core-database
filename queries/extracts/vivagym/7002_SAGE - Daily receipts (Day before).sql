WITH
    ...
    (
        SELECT
            /*+ materialize  */
            datetolongC(TO_CHAR(TO_DATE(getCenterTime(c.id), 'YYYY-MM-DD') - interval '2 day',
            'YYYY-MM-DD'), c.id) AS FROMDATE,
            (datetolongC(TO_CHAR(TO_DATE(getCenterTime(c.id), 'YYYY-MM-DD')- interval '1 day',
            'YYYY-MM-DD'), c.id)-1) AS TODATE,
            --datetolongC(TO_CHAR(TO_DATE(getCenterTime(c.id), 'YYYY-MM-DD') - interval '1 day','
            -- YYYY-MM-DD'), c.id) AS TODATE,
            --datetolongC(TO_CHAR(TO_DATE(getCenterTime(c.id), 'YYYY-MM-DD') - interval '2 day','
            -- YYYY-MM-DD'), c.id) AS FROMDATE,
            --datetolongC(getCenterTime(c.id), c.id) AS TODATE,
            c.id,
...
SELECT
    "ContractNo" ,
    sys.CodigoEmpresa AS "CodigoEmpresa",
    "IdDelegacion",
    --"IS_COMPANY",
    --"SALE_PERSON_ID",
    --"ENTRY_DATETIME",
...
FROM
    (
        SELECT
            ...
            'ES' AS                                                   "SiglaNacion" ,
            substring(payer.firstname || ' ' || payer.lastname,1,40)  "RazonSocial" ,
            substring(payer.firstname || ' ' || payer.lastname,41,40) "RazonSocial2",
            CASE
            ...
            CASE
                ...
                ELSE NULL
            END                             "CifEuropeo" ,
            substring(payer.address1,1,40)  "Domicilio" ,
            substring(payer.address1,41,40) "Domicilio2" ,
