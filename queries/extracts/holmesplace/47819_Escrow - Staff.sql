...
SELECT DISTINCT
	...
	subs.center || 'ss' || subs.id  AS "ExternalContractId", 							
	p.firstname                        AS "Name",
...
CASE p.status
WHEN 1 THEN subs.billed_until_date+1
WHEN 3 THEN subs.billed_until_date+1
ELSE '01/01/1900' END AS "StartChargingFromDate",
NULL AS "MinCancelTimeMonths",

	

CASE subt.auto_stop_on_binding_end_date 
WHEN '1' THEN '1'
WHEN '0' THEN '0'
END AS "IsAutomaticallyEnded",

---subt.autorenew_binding_count AS "AutomaticRenew",---
NULL AS "AutomaticRenew",

	'0' AS "StopChargingAfterMinPeriod",

CASE
	WHEN subt.st_type = 0 THEN '1'
	ELSE '0' END AS "IsUpfront",

CASE subt.unrestricted_freeze_allowed
WHEN '0' THEN '0'
WHEN '1' THEN '1'
END AS "FreezeAvailable",
	...
	'0' AS "IsProrata", --- dont calculate prorata for anyone---
	'1' AS "ProrataDay",
	...
	CASE prod.name
	WHEN 'Spa 12 pack Monthly 12m Free' THEN '12'
	WHEN 'Spa 12 pack Monthly 12m Free' THEN '12'
	WHEN 'Spa 12 pack Annual Free' THEN '12'
	WHEN 'Spa 6 pack Annual Free' THEN '6'
	WHEN 'Spa 6 pack Monthly 12m Free' THEN '6'
	ELSE 'Unlimited' END AS "VisitLimit", 
	...
	WHEN 'Spa 6 pack Annual Free' THEN '6'
	WHEN 'Spa 6 pack Monthly 12m Free' THEN '6'
	ELSE 'Unlimited' END AS "RemainingVisitCount",

	CASE prod.name
	WHEN 'Spa 12 pack Monthly 12m Free' THEN 'YEAR'
	WHEN 'Spa 12 pack Monthly 12m Free' THEN 'YEAR'
	WHEN 'Spa 12 pack Annual Free' THEN 'YEAR'
	WHEN 'Spa 6 pack Annual Free' THEN 'YEAR'
	WHEN 'Spa 6 pack Monthly 12m Free' THEN 'YEAR'
	ELSE 'Unlimited' END AS "VisitPeriod",   
...
END AS "IsAdditionalContract",
	...
	CASE 
WHEN subt.is_addon_subscription = 'FALSE' THEN '0'
ELSE '0' END AS "SynchronizeWithContract",
...
NULL AS "StartGeneratingTransactionsFromDate",
	...
	'0' AS "IsPaymentChosenDay", ---except for Weekly---
...
pa.IBAN                       AS  "BankAccountIban",
pa.BIC				AS "BankAccountBic",
pa.REF                        AS "BankAccountMandatoryId",
...
personCreation.txtvalue            AS personCreationDate,	
	...
	NULL AS  "Cancellation Reason",
	'LPI ' || LASTPRICEINCREASE.txtvalue AS "Comment3",
	NULL AS "Tags",
...
CASE  channelEmail.txtvalue  WHEN 'true' THEN  1  ELSE 0 END                                                                                                             AS "Allow Email",
CASE  channelSMS.txtvalue  WHEN 'true' THEN  1  ELSE 0 END                                                                                                               AS "Allow SMS",
    CASE  channelLetter.txtvalue  WHEN 'true' THEN  1  ELSE 0 END                                                                                                            AS "Allow Letter",
    CASE  channelPhone.txtvalue  WHEN 'true' THEN  1  ELSE 0 END                                                                                                             AS "Allow Phone",
    CASE  emailNewsLetter.txtvalue  WHEN 'true' THEN  1  ELSE 0 END                                                                                                          AS "AllowNewsLetter",
    CASE  thirdPartyOffers.txtvalue  WHEN 'true' THEN  1  ELSE 0 END                                                                                                         AS "AllowThirdPartyOffers",
...
NULL AS "CustomAttribute.Aggregator",
...
priceok.txtvalue AS "CustomAttribute.Price",
startpackok.txtvalue AS "CustomAttribute.StartPack",
...
comp.lastname AS "CustomAttribute.Company",
...
---pa.active AS "PaActive"---
CASE

WHEN clh.ctype IN (145,192) THEN '1'
WHEN clh.ctype IN (184) THEN '2'
WHEN clh.ctype IN (142) THEN '3'
ELSE clh.ctype
END AS "PaymentType",
...
CASE
	WHEN subt.st_type = 0 THEN subs.end_date       
	WHEN subt.st_type = 1 THEN subs.binding_end_date
	END AS "MinCancelTimeMonths", ---commitment end date instead---
NULL AS "Cancellation Reason",

source.txtvalue AS "MarketingSource",
offer.txtvalue AS "Sales Offer",
NULL AS "Guest Type",
contok.txtvalue AS "Contract Compliant",
...
prod.name  AS "PaymentPlanConvert",
prod.globalid AS "MembershipGlId",
NULL AS "PPNameGlobalIDSponsoredOK",
NULL AS "SponsoredOK",
NULL AS "Autorenew",
 ---prod.name|| ',' ||prod.globalid AS "PPName and Global ID",---
...
LEFT JOIN
     PERSON_EXT_ATTRS LASTPRICEINCREASE
 ON
     p.center=LASTPRICEINCREASE.PERSONCENTER
     AND p.id=LASTPRICEINCREASE.PERSONID
     AND LASTPRICEINCREASE.name='LASTPRICEINCREASE'
...
WHERE
	...
	AND (
	p.status IN (1,3)AND subs.state IN (2,4,8)-- active frozen created
	OR p.status IN (6,9)   -- prospect contact
	OR p.status IN (0) AND has_clipcard.OWNER_CENTER IS NOT NULL
	OR p.status IN (0) AND pay_for.payer_center IS NOT NULL
	OR p.status IN (2) AND corcomp.txtvalue IS NOT NULL ---Inactive
	OR p.status IN (2) AND corgiven1.txtvalue IS NOT NULL
	OR p.status IN (2) AND corgiven2.txtvalue IS NOT NULL
	OR p.status IN (2) AND corgiven3.txtvalue IS NOT NULL
	OR p.status IN (2) AND corgiven4.txtvalue IS NOT NULL
	OR p.status IN (2) AND corgiven5.txtvalue IS NOT NULL
	OR p.status IN (2) AND corgiven6.txtvalue IS NOT NULL
	OR p.status IN (2) AND corgiven7.txtvalue IS NOT NULL
	OR p.status IN (2) AND corgiven8.txtvalue IS NOT NULL
