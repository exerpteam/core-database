...
WITH
    ...
    (
        SELECT
            /*+ materialize */
            datetolongTZ(TO_CHAR(ADD_MONTHS(TO_DATE(getcentertime(c.id),'YYYY-MM-DD HH24:MI'),-1),'YYYY-MM-DD HH24:MI'), co.DEFAULTTIMEZONE) AS FROMDATE,
            c.id                                                                                                                             AS CLUBID
        FROM
            HP.CENTERS c
        JOIN
...
WHERE
    ...
    (
        ...
        WHERE
            ...
            AND ar.ID = prs.ID
            AND pr.REQ_DATE >= to_date('2021-05-28','YYYY-MM-DD') )
GROUP BY
    p.CENTER,
    p.ID,
    pcc.NAME,
