...
WITH PARAMS AS
        (
                SELECT
                        /*+ materialize */
                        datetolongTZ(TO_CHAR(TO_DATE('2020-03-31','YYYY-MM-DD'),'YYYY-MM-DD') || ' 00:00',co.DEFAULTTIMEZONE) AS FROMDATE,
                        datetolongTZ(TO_CHAR(TO_DATE('2020-04-01','YYYY-MM-DD'),'YYYY-MM-DD') || ' 00:00',co.DEFAULTTIMEZONE) AS TODATE,
                        c.id AS CLUBID
                FROM    
                        HP.CENTERS c
                JOIN
                        HP.COUNTRIES co ON c.COUNTRY = co.ID
                WHERE
                        co.ID = 'DE'
        )
        SELECT 
        ...
        JOIN
                ...
                ON 
                        pac.center = ar.center AND pac.ID = ar.ID
        JOIN
        ...
        JOIN
                HP.AR_TRANS art
                ON
                        art.CENTER = ar.CENTER AND art.ID = ar.ID
        WHERE
                ar.AR_TYPE = 4
                ...
                (
                        ...
                        WHERE
                                ...
                                AND ar.ID = prs.ID
                                AND pr.REQ_DATE >= to_date('2020-04-01','YYYY-MM-DD')
                )
        GROUP BY
              p.CENTER,
              p.ID,
