...
-- changed total debt to "is overdue"
WITH
    params AS
    (
        SELECT
    ...
    (--find all AR transactions and their settlements which have happened by the cutDate
        SELECT
            ...
            CASE
                WHEN ar.ar_type = 4
                THEN params.CutDate - art_paid.due_date
                ELSE CAST(extract(days FROM params.CutDate - longtodateTZ(art_paid.TRANS_TIME,
                    'Europe/Berlin')) AS INTEGER)
            END AS age_days,
