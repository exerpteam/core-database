...
WITH PARAMS AS
        (
                SELECT
                        /*+ materialize */
                        datetolongTZ(TO_CHAR(TO_DATE('2020-11-30','YYYY-MM-DD'),'YYYY-MM-DD') || ' 00:00',co.DEFAULTTIMEZONE) AS FROMDATE,
                        datetolongTZ(TO_CHAR(TO_DATE('2020-12-07','YYYY-MM-DD'),'YYYY-MM-DD') || ' 00:00',co.DEFAULTTIMEZONE) AS TODATE,
                        c.id AS CLUBID
                FROM    
                        HP.CENTERS c
                JOIN
        ...
        WHERE
                ...
                (
                        ...
                        WHERE
                                ...
                                AND ar.ID = prs.ID
                                AND pr.REQ_DATE >= to_date('2020-12-06','YYYY-MM-DD')
                )
        GROUP BY
              p.CENTER,
              p.ID,
