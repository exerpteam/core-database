...
SELECT DISTINCT
    ...
    THEN ROUND(current_date - p.LAST_ACTIVE_START_DATE)
...
LEFT JOIN
    HP.PERSONS cstaff
ON
    cstaff.CENTER = staff.CURRENT_PERSON_CENTER
    AND cstaff.id = staff.CURRENT_PERSON_ID
LEFT JOIN
...
ON
    c.id = p.CENTER
WHERE
    s.START_DATE BETWEEN date_trunc('year',CURRENT_DATE - 1 - $$offset$$) AND date_trunc('day',CURRENT_DATE - 1 - $$offset$$)
    /*AND s.START_DATE < (current_date - (:periodLength * 3) )*/
        ...
        WHERE
            ...
            AND (
                s2.CENTER,s2.ID ) NOT IN ((s.CENTER,
                                           s.id))
            AND ( (
                    scl2.BOOK_START_TIME BETWEEN dateToLong(TO_CHAR(s.START_DATE - interval '3 month','YYYY-MM-dd HH24:MI')) AND dateToLong(TO_CHAR(s.START_DATE, 'YYYY-MM-dd HH24:MI') ) -1000 )
                OR (
                    scl2.BOOK_END_TIME > dateToLong(TO_CHAR(s.START_DATE - interval '3 month','YYYY-MM-dd HH24:MI') )
