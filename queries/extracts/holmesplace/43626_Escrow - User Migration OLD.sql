...
CASE
WHEN latest_sub.product_group_id IN (1201)THEN '0'
ELSE '1'
										END AS "IsActive",
CASE
WHEN latest_sub.product_group_id IN (1201)THEN '1'
ELSE '0'
										END AS "IsGuest",
	payment_ar.balance                    AS "TodayBalance",
	...
	NULL  AS "Membership",
	latest_sub.name  AS "PaymentPlan",
	CASE 
	WHEN latest_sub.product_group_id IN (1201)THEN 'No Contract'
	ELSE 'Contract' 
	END AS "PaymentPlanType",
	NULL AS "VisitLimit",
...
END AS "PaymentType",
	...
	latest_sub.billed_until_date+1 AS "StartChargingFromDate",---Billed until plus 1 day---
	...
	latest_sub.auto_stop_on_binding_end_date AS "IsAutomaticallyEnded",
	latest_sub.is_addon_subscription AS "IsAdditionalContract",---only 1 if addonn---
	'0' AS "StopChargingAfterMinPeriod",
	latest_sub.unrestricted_freeze_allowed AS "FreezeAvailable",
	...
	CASE
        ...
        ELSE NULL
    								END    AS "PayerName",
    op.ADDRESS1 						AS "PayerAddress",
	op.zipcode 							AS "PayerPostalCode",
	op.City 							AS "PayerCity",
	CASE
        WHEN op.CENTER IS NOT NULL
...
pa.BANK_ACCNO                 AS "AccountNumber",
pa.BANK_REGNO        		  AS   "AccountRegNo",
...
personcomment.txtvalue     AS "Comment1",
	...
	ContractCom.txtvalue AS "Comment2",
	Grouponcode.txtvalue AS "Comment3",
	NUMBERPLATE.txtvalue AS "Comment4",
	NULL AS "Tags",

---tags need to list header+value separated by coma---
bs.txtvalue AS "AmountHC",
tow.txtvalue AS "AmountTow",
loyreg.txtvalue AS "LoyReg",
loyalty.txtvalue AS "Loy",
loyadd.txtvalue AS "LoyAdd",
priceinc.txtvalue AS "PriceIncr",
priceinc3.txtvalue AS "PriceIncr3%",
corcomp.txtvalue AS "CorWS",
corgiven1.txtvalue AS "CorApMay20",
corgiven2.txtvalue AS "CorNov20",
corgiven3.txtvalue AS "CorDec20",
corgiven4.txtvalue AS "CorJan21",
corgiven5.txtvalue AS "CorFeb21",
corgiven6.txtvalue AS "CorMar21",
corgiven7.txtvalue AS "CorApr21",
corgiven8.txtvalue AS "CorMay21",

---agreement fields---
	
CASE  channelEmail.txtvalue  WHEN 'true' THEN  1  ELSE 0 END                                                                                                             AS "AllowEmail",
CASE  channelSMS.txtvalue  WHEN 'true' THEN  1  ELSE 0 END                                                                                                               AS "AllowSMS",
    CASE  channelLetter.txtvalue  WHEN 'true' THEN  1  ELSE 0 END                                                                                                            AS "AllowLetter",
    CASE  channelPhone.txtvalue  WHEN 'true' THEN  1  ELSE 0 END                                                                                                             AS "AllowPhone",
...
DOI.txtvalue  AS "Doi",
OPTIN_Date.txtvalue  AS "OptinDate",
DOI_Date.txtvalue AS "DoiDate",

---customfields--
source.txtvalue AS "CustomAttribute.Source",
contdoc.txtvalue AS "CustomAttribute.ContractDocumentation",
contok.txtvalue AS "CustomAttribute.ContractCompliant",
priceok.txtvalue AS "CustomAttribute.ContractPrice",
startpackok.txtvalue AS "CustomAttribute.ContractStartPack",
offer.txtvalue AS "CustomAttribute.Offer",
locker.txtvalue AS "CustomAttribute.Lockernumber",
hesitation.txtvalue AS "CustomAttribute.Hesitations",
goal.txtvalue AS "CustomAttribute.Goals",
prefactivity.txtvalue AS "CustomAttribute.PreferredActivities",
guest.txtvalue AS "CustomAttribute.GuestType",
...
pa.BANK_ACCOUNT_HOLDER        AS    "BankAccountHolder",
...
CASE ei.IDMETHOD
WHEN 1 THEN  'BARCODE'  
WHEN 2 THEN  'MAGNETIC_CARD'  
WHEN 3 THEN  'SSN'  
WHEN 4 THEN  'RFID_CARD'  
WHEN 5 THEN  'PIN'  
WHEN 6 THEN  'ANTI DROWN'  
WHEN 7 THEN  'QRCODE'  
ELSE 'UNKNOWN' 
...
CASE
WHEN latest_sub.st_type = 0 THEN latest_sub.end_date
WHEN latest_sub.st_type = 1 THEN latest_sub.binding_end_date
END AS "BindingEnds"
...
WHERE
    ...
    AND ( 
            p.status IN (1,2,3)   --Active Temp inactive
		--OR p.status IN (0,6,9) AND cp.LAST_ACTIVE_END_DATE < 01-05-2018)--
            
        OR pay_for.payer_center IS NOT NULL --if payer or clips all status
