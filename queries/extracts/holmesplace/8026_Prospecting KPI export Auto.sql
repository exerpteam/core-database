...
WITH
    ...
    (
        SELECT
            datetolong( TO_CHAR(TRUNC(sysdate -1 -$$offset$$,'MM'),'YYYY-MM-DD') || ' 00:00')                 AS fromtime,
...
CROSS JOIN
    (
        ...
        FROM
            (
                SELECT
                    sysdate -1 -$$offset$$ start_date,
                    sysdate -1 -$$offset$$ end_date
                FROM
                    dual) CONNECT BY level <= months_between( TRUNC(add_months(sysdate -1 -$$offset$$,1),'MM'), TRUNC(start_date,'MM') ) * + 1) month_list
LEFT JOIN
    (
        SELECT
            t.CENTER                         AS CENTERID,
