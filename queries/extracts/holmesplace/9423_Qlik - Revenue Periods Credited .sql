WITH
    params AS
    (
        SELECT
            /*+ materialize */
            DECODE($$offset$$,0,0,dateToLongC(TO_CHAR(TRUNC(SYSDATE-$$offset$$), 'YYYY-MM-dd HH24:MI'),100)) AS from_time,
            dateToLongC(TO_CHAR(TRUNC(SYSDATE), 'YYYY-MM-dd HH24:MI'),100)                                   AS to_time
        FROM
            dual
    )
SELECT
    cl.CENTER || 'cred' || cl.ID || 'cnl' || cl.SUBID              SALES_LINE_ID,
    TO_CHAR(spp.FROM_DATE,'yyyy-MM-dd')                            FROM_DATE,
    TO_CHAR(spp.TO_DATE,'yyyy-MM-dd')                              TO_DATE,
    -1*ROUND(cl.TOTAL_AMOUNT, 2)                                   TOTAL_AMOUNT,
    cl.RATE                                                        VAT_RATE,
    spp.TO_DATE-spp.FROM_DATE +1                                   PERIOD_DAYS,
    'SUBSCRIPTION_PERIOD'                                          SOURCE_TYPE,
    spp.center||'ss'||spp.id||'id'||spp.SUBID                      SOURCE_ID,
    DECODE (spp.SPP_STATE, 1,'ACTIVE', 2,'CANCELLED','UNKNOWN')    STATE,
    longtodate(cn.TRANS_TIME)                                   AS BOOK_DATE
FROM
    params,SUBSCRIPTIONPERIODPARTS spp
JOIN
    SUBSCRIPTIONS s
ON
    s.center = spp.CENTER
    AND s.id = spp.id
JOIN
    SPP_INVOICELINES_LINK sppl
ON
    sppl.PERIOD_CENTER = spp.CENTER
    AND sppl.PERIOD_ID = spp.id
    AND sppl.PERIOD_SUBID = spp.SUBID
JOIN
    PERSONS p
ON
    p.center = s.OWNER_CENTER
    AND p.id = s.OWNER_ID
JOIN
    PERSONS cp
ON
    cp.center = p.CURRENT_PERSON_CENTER
    AND cp.id = p.CURRENT_PERSON_ID
JOIN
    CREDIT_NOTE_LINES cl
ON
    cl.INVOICELINE_CENTER = sppl.INVOICELINE_CENTER
    AND cl.INVOICELINE_ID = sppl.INVOICELINE_ID
    AND cl.INVOICELINE_SUBID = sppl.INVOICELINE_SUBID
JOIN
    CREDIT_NOTES cn
ON
    cn.center = cl.center
    AND cn.id = cl.id
WHERE
    spp.ENTRY_TIME between params.from_time and params.to_time
    AND spp.SPP_STATE = 2
and cp.center in ($$scope$$)