...
SELECT
	...
	SUM( (
            CASE fie.key
                ...
                THEN dat.value
                ELSE 0
            END)) AS "MTD Storm Retail revenue",
	...
	SUM( (
            CASE fie.key
                ...
                THEN dat.value
                ELSE 0
            END)) AS "MTD PILAT3S Revenue",
	SUM(
        CASE
            WHEN fie.key = 'PilatesRevenue'
                AND for_date = PARAMS.ToDate
            THEN dat.value
            ELSE NULL
        END) AS "Daily PILAT3S Revenue",
	SUM( (
            CASE fie.key
                WHEN 'PilatesClipsSold'
                THEN dat.value
                ELSE 0
            END)) AS "MTD PILAT3S Clips Sold"
...
or (
        -- Movement fields: fields that need to be sum/aggregated in the extract
        fie.key IN ('EXTRA')
         AND (( dat.for_date BETWEEN PARAMS.FromDate AND PARAMS.ToDate ) or (dat.for_date BETWEEN PARAMS.ToDate2 AND PARAMS.ToDate and PARAMS.FromDate > PARAMS.ToDate2) )))
