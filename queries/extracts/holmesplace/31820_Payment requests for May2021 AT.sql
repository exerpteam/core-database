WITH PARAMS AS
        (
                SELECT
                        /*+ materialize */
                        datetolongTZ(TO_CHAR(TO_DATE('2021-04-30','YYYY-MM-DD'),'YYYY-MM-DD') || ' 00:00',co.DEFAULTTIMEZONE) AS FROMDATE,
                        datetolongTZ(TO_CHAR(TO_DATE('2021-05-28','YYYY-MM-DD'),'YYYY-MM-DD') || ' 00:00',co.DEFAULTTIMEZONE) AS TODATE,
                        c.id AS CLUBID
                FROM    
                        HP.CENTERS c
                JOIN
        ...
        WHERE
                ...
                (
                        ...
                        WHERE
                                ...
                                AND ar.ID = prs.ID
                                AND pr.REQ_DATE >= to_date('2021-05-26','YYYY-MM-DD')
                )
        GROUP BY
              p.CENTER,
              p.ID,
