WITH
    ...
    (
        SELECT
                                                                                           :scope AS CENTER ,
            datetolong(TO_CHAR(date_trunc('day',a.currentdate), 'YYYY-MM-DD HH24:MI'))-1    AS STARTTIME ,
            datetolong(TO_CHAR(date_trunc('day',a.currentdate +1), 'YYYY-MM-DD HH24:MI'))-1 AS ENDTIME,
            datetolong(TO_CHAR(date_trunc('day',a.currentdate +2), 'YYYY-MM-DD HH24:MI'))   AS STATUSFIELDTIME,
            a.CURRENTDATE,
            'EXTRA' AS SELECTED_STATUS
            --,
            --datetolong(TO_CHAR(TRUNC(currentdate +2, 'DDD'), 'YYYY-MM-DD HH24:MI')) AS HARDCLOSETIME
        FROM
            (
                SELECT
                    cast(:date as DATE)-1 AS currentdate) a
    )
SELECT
    COUNT(*)
FROM
        ...
        SELECT
            ...
            CASE
                WHEN (P.BIRTHDATE IS NOT NULL
                        AND extract ('year' from age(P.BIRTHDATE)) < 16)
                THEN 'true'
                ELSE NULL
            END AS KIDS,
            CASE
                ...
                THEN 'EXTRA'
                WHEN (P.BIRTHDATE IS NOT NULL
                        AND TRUNC(months_between(PARAMS.currentdate,P.BIRTHDATE)/12) < 16)
                THEN 'KIDS'
            END AS STATUS
            --   COALESCE(STATUSES.Debtor, 'false') as Debtor
        FROM
            (
                SELECT DISTINCT --LC
                    SU.OWNER_CENTER,
...
SELECT
        ...
        WHERE
           ...
           AND pcl1.ENTRY_TIME <= PARAMS.ENDTIME
            /*AND (
                pcl2.id IS NULL
...
where rnk = 1
 ...
 GROUP BY
            ranking.PERSON_CENTER,
            ranking.PERSON_ID) STATUSES
        ON
            STATUSES.PERSON_CENTER = MEMBERS.OWNER_CENTER
            AND STATUSES.PERSON_ID = MEMBERS.OWNER_ID ) x,
    PARAMS
WHERE
