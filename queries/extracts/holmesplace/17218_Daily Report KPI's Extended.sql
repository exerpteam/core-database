...
-- Extra columns Coaching, EFT and CASH revenue
...
SELECT
    ...
    MAX(
        CASE
            WHEN fie.key = 'CoachingRevenue'
                AND for_date = PARAMS.ToDate
            THEN dat.value
            ELSE NULL
        END) AS "Daily Coaching Fee revenue",
    SUM( (
            CASE fie.key
                WHEN 'CoachingRevenue'
                THEN dat.value
                ELSE 0
            END)) AS "MTD Coaching Fee revenue",
    MAX(
        CASE
            WHEN fie.key = 'SubsRevenue'
                AND for_date = PARAMS.ToDate
            THEN dat.value
            ELSE NULL
        END) AS "Daily Subs EFT revenue",
    SUM( (
            CASE fie.key
                WHEN 'SubsRevenue'
                THEN dat.value
                ELSE 0
            END)) AS "MTD Subs EFT revenue",
    MAX(
        CASE
            WHEN fie.key = 'SubsRevenueCash'
                AND for_date = PARAMS.ToDate
            THEN dat.value
            ELSE NULL
        END) AS "Daily Subs Cash revenue",
    SUM( (
            CASE fie.key
                WHEN 'SubsRevenueCash'
                THEN dat.value
                ELSE 0
            END)) AS "MTD Subs Cash revenue"
FROM
    PARAMS,
    centers c
JOIN
