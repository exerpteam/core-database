...
-- https://clublead.atlassian.net/browse/ST-11041
 ...
 SELECT
     ...
     peaChannelSMS.TXTVALUE       AS "ALLOW_CHANNEL_SMS",
     peaChannelPhone.TXTVALUE       AS "ALLOW_CHANNEL_PHONE",
     peaGDPROPTIN.TXTVALUE   AS "GDPR_OPTIN",
     peaGDPROPTINDATE.TXTVALUE   AS "GDPR_OPTIN_DATE",
     peaGDPRDOUBLEOPTINdate.TXTVALUE   AS "GDPR_DOUBLE_OPTIN_DATE",
 ...
 WHERE
    ...
    --Filter out members in debt
    AND NOT EXISTS
    (SELECT 1 FROM CASHCOLLECTIONCASES cc
     WHERE cc.PERSONCENTER = p.CENTER AND cc.PERSONID = p.ID AND cc.MISSINGPAYMENT = 1 AND closed = 0)
    ...
    AND peaEmail.TXTVALUE is not null
