...
ELSE payment_ar.balance END AS "TodayBalance",
	...
	END AS "MinCancelTimeMonths(com end date)",---use formula if nec
...
---customfields find out what to put in AS maybe the code---
...
CASE
    ...
    pt_rel_p.fullname 					AS "FriendFamilyName",
...
WHERE
    ...
    AND ( 
            p.status IN (1,3)   --Active Tempinactive
		OR p.status IN (2) AND pay_for.PAYER_CENTER IS NOT NULL)
