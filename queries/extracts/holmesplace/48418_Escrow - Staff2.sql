...
-- Active Temp Inactive Prospect Contsct and Lead if clips or payer and inactive if has corona comp. Excludes Exerp staff
SELECT DISTINCT
...
ELSE payment_ar.balance END AS "TodayBalance",
	...
	NULL AS "IsAutomaticallyEnded",
	NULL AS "AutomaticRenew",
	NULL AS "StopChargingAfterMinPeriod",
	NULL AS "IsUpfront",
	NULL AS "FreezeAvailable",
	NULL AS "IsProrata",
	NULL AS "ProrataDay",
	...
	NULL AS "VisitPeriod",
	NULL AS "IsAdditionalContract",
	NULL AS "SynchronizeWithContract",
 	NULL AS "TimePeriod",
	NULL AS "ContractLength",
	NULL  AS "ContractFrequency",
	'19' AS "VatRate",
	
	NULL AS "MembershipFee",
	
	NULL AS "StartGeneratingTransactionsFromDate",
	NULL AS "ForcePaymentPlan",
	NULL AS "PaymentPlanAutoName",
	NULL AS "IsPaymentChosenDay", 
...
CASE pa.REQUESTS_SENT WHEN '0' THEN 'true' ELSE 'false' 
            END AS "BankAccountFirstPayment",
...
CASE  channelSMS.txtvalue  WHEN 'true' THEN  1  ELSE 0 END                                                                                                               AS "Allow SMS",
    ...
    CASE  emailNewsLetter.txtvalue  WHEN 'true' THEN  1  ELSE 0 END                                                                                                          AS "Allow NewsLetter",
    CASE  thirdPartyOffers.txtvalue  WHEN 'true' THEN  1  ELSE 0 END                                                                                                         AS "Allow ThirdPartyOffers",
...
--comp.lastname blank for guests--
...
NULL AS "MinCancelTimeMonths",
NULL AS "Cancellation Reason",
source.txtvalue AS "Marketing Source",
offer.txtvalue AS "Sales Offer",
guest.txtvalue AS "Guest Type",
contok.txtvalue AS "Contract Compliant",
corcomp.txtvalue AS "CustomAttribute.WebShop Voucher",   
corgiven1.txtvalue AS "CustomAttribute.Given Apr May20",  
corgiven2.txtvalue AS "CustomAttribute.Given Nov20",
corgiven3.txtvalue AS "CustomAttribute.Given Dec20",
corgiven4.txtvalue AS "CustomAttribute.Given Jan21",
corgiven5.txtvalue AS "CustomAttribute.Given Feb21",
corgiven6.txtvalue AS "CustomAttribute.Given Mar21",
corgiven7.txtvalue AS "CustomAttribute.Given Apr21",
corgiven8.txtvalue AS "CustomAttribute.Given May21",
...
LEFT JOIN  
       PERSONS cp  ---For last active end date dont need here---
        ON
...
WHERE
	...
	AND p.firstname NOT IN ('Exerp','Exerp Support', 'MK EXERP', 'Exerpsupport', 'EXERP SUPPORT')
    AND
	( p.status IN (6,9)   -- prospect contact
	OR p.status IN (1,3) AND subs.state IN (2,4,8)-- active frozen created
