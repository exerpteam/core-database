SELECT DISTINCT
	...
	s.center || 'ss' || s.id  AS "ExternalContractId", 							
	p.firstname                        AS "Name",
...
CASE p.status
WHEN 1 THEN s.billed_until_date+1
WHEN 3 THEN s.billed_until_date+1
ELSE '01/01/1900' END AS "StartChargingFromDate",
NULL AS "MinCancelTimeMonths",


'0'  AS "IsAutomaticallyEnded",
...
'Unlimited' AS "VisitLimit", 
'Unlimited' AS "RemainingVisitCount",
'Unlimited' AS "VisitPeriod",   
...
'1' AS "SynchronizeWithContract",
'MONTH' AS "TimePeriod",
'1' AS "ContractLength",
'1'  AS "ContractFrequency",
'19' AS "VatRate",
...
WHEN 3 THEN 'FAILED'  
WHEN 4 THEN 'OK'  
WHEN 5 THEN 'ENDED BY DEBITOR BANK'
WHEN 6 THEN  'ENDED BY THE CLEARING HOUSE'
WHEN 7 THEN 'ENDED BY DEBITOR'  
WHEN 8 THEN 'SHAL BE CANCELLED'  
WHEN 9 THEN 'END REQUEST SENT'  
WHEN 10 THEN  'ENDED BY CREDITOR'  
WHEN 11 THEN 'NO AGREEMENT WITH DEBITOR'  
WHEN 12 THEN 'DEPRECATED'  
WHEN 13 THEN 'NOT NEEDED' 
WHEN 14 THEN  'INCOMPLETE' 
WHEN 15 THEN  'TRANSFERRED' 
ELSE 'UNKNOWN'
END AS dd_state,
...
CASE
	WHEN subt.st_type = 0 THEN s.end_date       
	WHEN subt.st_type = 1 THEN s.binding_end_date
	ELSE s.end_date
...
NULL AS HAS_EFT_SUB,
NULL AS HAS_CASH_SUB,
NULL AS HAS_RCC_SUB,
NULL AS HAS_CLIP_CARD,
...
ON
    subt.CENTER = s.SUBSCRIPTIONTYPE_CENTER
    AND subt.id = s.SUBSCRIPTIONTYPE_ID
JOIN
    PERSONS emp
ON
