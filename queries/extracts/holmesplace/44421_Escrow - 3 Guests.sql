...
CASE
WHEN p.status IN (1,2)THEN '1'
ELSE '0'
					END AS "IsActive",---active temp---
CASE
WHEN p.status IN (0,6,9,3)THEN '1'
ELSE '0'
					END AS "IsGuest", ---lead prospect contact inactive---
...
GDPRCONSENT.txtvalue AS "MarketingConsent",
...
LEFT JOIN
     PERSON_EXT_ATTRS GDPRCONSENT
 ON
     p.center=GDPRCONSENT.PERSONCENTER
     AND p.id=OGDPRCONSENT.PERSONID
     AND GDPRCONSENT.name='GDPRCONSENT'
...
WHERE
	...
	OR p.status IN (0) AND aggregator.txtvalue IS NOT NULL
	OR p.status IN (0) AND pay_for.payer_center IS NOT NULL
	OR p.status IN (0) AND p.persontype IN (8) ---Guest
	OR p.status IN (2) AND latest_sub.owner_id IS NULL) ---Inactive no plan
	---AND pay_for.PAYER_CENTER is NULL ---maybe Include in members extract
