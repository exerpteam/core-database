...
WITH
     ...
     (
         ...
         ON
             c.id = p.CENTER
         WHERE
             p.STATUS IN (1,3)--only active and temp inactive
--dont choose status
...
subs.subscription_price AS "subsPrice",
subs.start_date AS "StartDate",
subs.end_date AS "EndDate",
subs.binding_end_date AS "Binding",
CASE subs.state
		WHEN 2 THEN 'active'
		WHEN 4 THEN 'frozen'
		WHEN 8 THEN 'created'
...
subs.billed_until_date AS "Billeduntil",

p.last_active_start_date AS "Last Start Date",
TO_CHAR(longtodate(subs.creation_time), 'DD-MM-YYYY') AS "SubsCreateDate",
...
WHERE
    subs.state IN (2,4,8)--active frozen created
	AND subs.sub_state NOT IN (8)--not cancelled
