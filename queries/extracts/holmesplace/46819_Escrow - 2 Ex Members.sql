...
---dont migrate card numbers of ex members ei.IDENTITY---
Null                  AS "MemberCardNumber",
...
NULL AS "StartGeneratingTransactionsFromDate",
	...
	---OTHER PAYER leave blank for ex members---
...
NULL  AS "PaymentType",
    ...
    ---LEAVE BLANK FOR EX MEMERS NOT PAYER BUT WHAT IF RECENTLY LEFT ---
CASE
WHEN pay_for.PAYER_CENTER IS NOT NULL THEN pa.IBAN
ELSE NULL 
END AS  "BankAccountIban",

CASE
WHEN pay_for.PAYER_CENTER IS NOT NULL THEN pa.BIC
ELSE NULL
END AS "BankAccountBic",
...
personCreation.txtvalue            AS personCreationDate,	
	...
	LASTPRICEINCREASE.txtvalue AS  "Comment4",
	NULL AS "Tags",
...
CASE  channelSMS.txtvalue  WHEN 'true' THEN  1  ELSE 0 END                                                                                                               AS "Allow SMS",
    ...
    CASE  emailNewsLetter.txtvalue  WHEN 'true' THEN  1  ELSE 0 END                                                                                                          AS "AllowN ewsLetter",
...
--comp.lastname blank for ex memers--
...
latest_sub.end_date AS "MinCancelTimeMonths",
CASE
...
latest_sub.name  AS "PaymentPlanConvert",
latest_sub.globalid AS "MembershipGlId",
...
---sponsored leave blank for ex members---

NULL AS "Sponsorship",
NULL AS "Sponsorship_amount",

---removed sponsorship and sponsorship amount code---
CASE
...
LEFT JOIN
    (
        ...
        WHERE
            subs.STATE IN (3,7)--2active 4frozen ended window
...
WHERE
    ...
    AND p.status IN (2)   --Inactive
	---AND latest_sub.state IN (3,7)---
