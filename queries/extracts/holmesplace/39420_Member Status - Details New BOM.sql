WITH
    ...
    (
        SELECT
            ...
            CAST (datetolong(TO_CHAR(DATE_TRUNC('day', d1.currentdate +2), 'YYYY-MM-DD HH24:MI')) -1 AS BIGINT) AS STATUSFIELDTIMEEXTRA,
            CURRENTDATE,
...
LEFT JOIN
    (
        SELECT
            pcl1.PERSON_CENTER,
            pcl1.PERSON_ID,
            MAX(CASE pcl1.CHANGE_ATTRIBUTE WHEN 'STATUS_DEBTOR' THEN COALESCE(pcl1.new_value,'false') END)     AS Debtor,
            MAX(CASE pcl1.CHANGE_ATTRIBUTE WHEN 'STATUS_LATE_START' THEN COALESCE(pcl1.new_value,'false') END)  AS LateStart,
            MAX(CASE pcl1.CHANGE_ATTRIBUTE WHEN 'STATUS_FROZEN' THEN COALESCE(pcl1.new_value,'false') END)  AS Frozen,
            MAX(CASE pcl1.CHANGE_ATTRIBUTE WHEN 'STATUSEXTRA2' THEN COALESCE(pcl1.new_value,'false') END)  AS Extra   
        FROM
