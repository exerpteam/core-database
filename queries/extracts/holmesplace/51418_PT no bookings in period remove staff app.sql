...
FROM
    ...
    (
        ...
        ON
            ...
            AND p.ID = e.PERSONID
			AND P.firstname NOT IN
 ('Exerp','EXERP','API','MEMBER WEB','Memberweb')
        WHERE
            p.center IN ($$center$$)
            AND e.USE_API = 1
            AND e.BLOCKED = 0
...
PERSONS per
ON
	not_booked_staff.StaffPersonCenter=per.center
	AND not_booked_staff.StaffPersonId=per.ID

LEFT JOIN
EMPLOYEES emp
ON 
	not_booked_staff.StaffPersonCenter=emp.PERSONCENTER
	AND not_booked_staff.StaffPersonId=emp.PERSONID
	AND emp.BLOCKED = 0
