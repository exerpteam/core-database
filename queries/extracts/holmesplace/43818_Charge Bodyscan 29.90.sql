...
CAST($$deduction_date$$ AS DATE) deduction_date
        FROM
            (
                SELECT DISTINCT
					...
					pea_charge_2990.txtvalue   AS CHARGE2990,
                    pea_charge_co_fee.txtvalue   AS CHARGE29,
					pea_charge_old_BS_Fee.txtvalue  AS CHARGE25,
		...
		LEFT JOIN
                    HP.PERSON_EXT_ATTRS pea_charge_2990
                ON
                    pea_charge_2990.PERSONCENTER = p.center
                    AND pea_charge_2990.PERSONID = p.id
                    AND pea_charge_2990.name = 'CHARGEBODYSCANFEE2990'
